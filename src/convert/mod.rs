...
mod tests {
    ...
    fn llama3_end_to_end_tiny_safetensors_round_trip() {
        ...
        for name in &expected_gguf_names {
            ...
            // Q8_0 — token_embd + output route through the OUTPUT branch
            // which bumps to Q6_K only if `new_type != Q8_0`; since
            // new_type IS Q8_0 for `MostlyQ8_0` ftype, the bump does not
            // fire and they stay Q8_0. Compare the variant, not a
            // positional cast (mlx_native's GgmlType is repr-less; `as u32`
            // is the variant index, which shifted when BF16 was added).
