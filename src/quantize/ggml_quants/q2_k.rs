...
/// `make_qkx2_quants` (`ggml-quants.c:737-816`). Used by the `_ref` path,
/// which always supplies a non-null `weights` slice (per-sub-block
/// `fabsf(x)`), so the optional-weights branches in the C source are
/// inlined here as required slices.
///
/// Returns `(scale, the_min)` where `the_min = -min` matches the C OUT
/// parameter semantics.
...
///
/// Q2_K's `_impl` always passes a non-null weights slice, mirroring the
/// Q4_1 port.
#[allow(clippy::too_many_arguments)]
fn make_qkx3_quants(
    n: usize,
...
/// `make_qp_quants` (`ggml-quants.c:1014-1085`). Iterative least-squares
/// for unsigned-positive quantization (called twice in Q2_K `_impl` for
...
fn make_qp_quants(n: usize, nmax: i32, x: &[f32], l: &mut [u8], quant_weights: &[f32]) -> f32 {
    ...
    for _itry in 0..5 {
        ...
        for i in 0..n {
            let w = quant_weights[i];
            let li_cur = l[i] as i32;
            let mut slx = sumlx - w * x[i] * li_cur as f32;
            let mut sl2 = suml2 - w * (li_cur * li_cur) as f32;
            if slx > 0.0 && sl2 > 0.0 {
