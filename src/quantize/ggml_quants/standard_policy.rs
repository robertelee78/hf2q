//! `StandardPolicy` — port of the peer's standard quant-target picker.
//!
...
//!    the peer's row-misalignment
//!    first-downshift table. Per ADR §"shape_fallback contract" the
//!    second-misalignment case returns `Err` instead of silently
//!    demoting to F16.
//!
//! 2. **`QsState`** — Rust mirror of the peer's `quantize_state_impl`.
//!    Carries the per-(ftype,arch,model)
...
//!    the peer's `llama_tensor_get_type_impl`. The
...
///
/// The peer's `tensor_get_category` does **substring** matches (not
...
impl TensorCategory {
    /// Classify a tensor by name. Mirrors the peer's
    /// `tensor_get_category` — note the priority order is
    ...
    /// Port of the peer's `category_is_attn_v`.
...
/// `tensor_type_fallback` — port of the peer's fallback table.
///
...
/// "second misalignment" case where the peer silently demotes to
...
pub struct HParams {
    ...
    /// canonical denominator for `use_more_bits` on ffn_down/gate/up
    /// (the peer's `init_quantize_state_counters` hardcodes
...
impl HParams {
    /// `n_gqa` — port of the peer's helper.
...
/// `QsState` — Rust mirror of the peer's `quantize_state_impl`.
///
...
/// `use_more_bits` — port of the peer's lambda of the same name.
...
/// `layer_info` — port of the peer's lambda of the same name.
///
...
/// `StandardPolicy` — picks `GgmlType` per tensor at a given `GgufFtype`.
/// Per ADR-033 Decision §"QuantPolicy trait (Decision §3 concrete)".
///
/// The body of `target_for` is a 1:1 port of the peer's
...
impl StandardPolicy {
    ...
    ///
    /// Faithful port of the peer's `llama_tensor_get_type_impl`.
    /// Mirrors:
    /// - branch order
    ...
    pub fn target_for(
        ...
        // Start from the ftype's primary (i.e. the `default_type`
        // parameter in the C source's default-type pick).
            ...
            || (qs.has_tied_embeddings && category == TensorCategory::TokenEmbd)
        {
            if let Some(t) = qs.params.output_tensor_type {
                ...
                if ftype == GgufFtype::MostlyMXFP4_MOE {
                    // C:446-448
                    ...
                    ftype,
                    GgufFtype::MostlyIQ2_XXS
                        | GgufFtype::MostlyIQ2_XS
                        | GgufFtype::MostlyIQ3_XXS
                        | GgufFtype::MostlyIQ1_S
                        | GgufFtype::MostlyIQ2_S
                        | GgufFtype::MostlyIQ2_M
                        | GgufFtype::MostlyIQ1_M
                ) {
                    // C:452-456
        ...
        } else if ftype == GgufFtype::MostlyMXFP4_MOE {
            ...
            } else if matches!(ftype, GgufFtype::MostlyIQ2_S | GgufFtype::MostlyIQ2_M) {
                // C:477-479
                new_type = GgmlType::IQ3_S;
            } else if ftype == GgufFtype::MostlyIQ3_XXS {
                // C:480-482
                new_type = GgmlType::IQ3_S;
            } else if matches!(ftype, GgufFtype::MostlyTQ1_0 | GgufFtype::MostlyTQ2_0) {
                // C:483-485
            ...
            if category.is_attn_v() {
                ...
                if qs.hparams.n_expert == 8 {
                    new_type = GgmlType::Q5_K;
                } else if matches!(ftype, GgufFtype::MostlyIQ1_S | GgufFtype::MostlyIQ1_M) {
            ...
            } else if ftype == GgufFtype::MostlyQ2_K_S && qs.hparams.n_gqa() >= 4 {
                // C:515-517
                new_type = GgmlType::Q4_K;
            } else if ftype == GgufFtype::MostlyIQ3_XXS {
                // C:518-520
            ...
            } else if matches!(ftype, GgufFtype::MostlyIQ3_XS | GgufFtype::MostlyIQ3_S)
                && qs.hparams.n_gqa() >= 4
            {
                // C:521-523
                new_type = GgmlType::Q4_K;
            } else if ftype == GgufFtype::MostlyIQ3_M {
                // C:524-526
                new_type = GgmlType::Q4_K;
            } else if ftype == GgufFtype::MostlyQ3_K_M {
                // C:527-529
            ...
            } else if ftype == GgufFtype::MostlyQ3_K_L {
                // C:530
                new_type = GgmlType::Q5_K;
            } else if matches!(ftype, GgufFtype::MostlyIQ4_NL | GgufFtype::MostlyIQ4_XS)
                && qs.hparams.n_gqa() >= 4
            {
                // C:531-533
                new_type = GgmlType::Q5_K;
            } else if matches!(ftype, GgufFtype::MostlyQ4_K_M | GgufFtype::MostlyQ5_K_M)
                && use_more_bits(qs.i_attention_wv, qs.n_attention_wv)
            {
                // C:534-535
                new_type = GgmlType::Q6_K;
            } else if ftype == GgufFtype::MostlyQ4_K_S && qs.i_attention_wv < 4 {
                // C:536
                new_type = GgmlType::Q5_K;
            }
            ...
            if qs.hparams.n_expert == 8 {
                // C:550-554
                new_type = GgmlType::Q8_0;
            } else if ftype == GgufFtype::MostlyIQ3_XS {
                // C:555-557
            ...
            if ftype == GgufFtype::MostlyIQ3_XS {
                // C:562-564
                new_type = GgmlType::IQ3_XXS;
            } else if ftype == GgufFtype::MostlyIQ3_XXS {
                // C:565-567
            ...
            } else if ftype == GgufFtype::MostlyQ2_K_S {
                // C:572-574
                if i_layer < n_layer / 8 {
                    new_type = GgmlType::Q4_K;
                }
            } else if ftype == GgufFtype::MostlyIQ3_XXS && !qs.has_imatrix {
                // C:575-577
                    ...
                    || (qs.hparams.n_expert == 8 && use_more_bits(i_layer, n_layer)))
            {
            ...
            } else if ftype == GgufFtype::MostlyQ4_K_M {
                // C:590-597
                ...
                && matches!(ftype, GgufFtype::MostlyIQ4_NL | GgufFtype::MostlyIQ4_XS)
                && !qs.has_imatrix
            {
                // C:598-600
                new_type = GgmlType::Q5_K;
            } else if ftype == GgufFtype::MostlyQ5_K_M && use_more_bits(i_layer, n_layer) {
                // C:601
                new_type = GgmlType::Q6_K;
            } else if ftype == GgufFtype::MostlyQ4_K_S
                && arch != ArchName::Falcon
                && i_layer < n_layer / 8
            {
                // C:602-604
                new_type = GgmlType::Q5_K;
            } else if matches!(ftype, GgufFtype::MostlyQ4_0 | GgufFtype::MostlyQ5_0)
                && qs.has_imatrix
                && i_layer < n_layer / 8
            {
                // C:605-611 — imatrix-only ffn_down guard.
                new_type = if ftype == GgufFtype::MostlyQ4_0 {
                    GgmlType::Q4_1
                } else {
                    GgmlType::Q5_1
                };
                ...
                if qs.hparams.n_expert == 8 {
                    ...
                    if matches!(
                        ...
                        GgufFtype::MostlyQ2_K
                            ...
                            | GgufFtype::MostlyIQ3_S
                            | GgufFtype::MostlyIQ3_M
                            | GgufFtype::MostlyIQ4_XS
                    ) {
                        new_type = GgmlType::Q5_K;
                    }
                } else {
            ...
            if matches!(
                ftype,
                GgufFtype::MostlyQ3_K_M | GgufFtype::MostlyQ3_K_L | GgufFtype::MostlyIQ3_M
            ) {
                new_type = GgmlType::Q4_K;
            } else if ftype == GgufFtype::MostlyQ4_K_M {
                new_type = GgmlType::Q5_K;
            } else if ftype == GgufFtype::MostlyQ5_K_M {
                new_type = GgmlType::Q6_K;
            }
        ...
        // `tensor_type_fallback` call made by the OUTER wrapper
        // `llama_tensor_get_type`, which we collapse into the
...
mod tests {
    ...
    fn mk_qs(ftype: GgufFtype, arch: ArchName) -> QsState {
    ...
    #[test]
    fn passthrough_f32() {
        let mut qs = mk_qs(GgufFtype::AllF32, ArchName::Llama3);
        let shape = [4096, 1];
    ...
    #[test]
    fn passthrough_f16() {
        let mut qs = mk_qs(GgufFtype::MostlyF16, ArchName::Llama3);
        let shape = [4096, 1];
        let t = mk_tensor("blk.0.attn_q.weight", &shape, ArchName::Llama3);
        let pol = StandardPolicy::new();
        let cat = TensorCategory::classify(t.name);
    ...
    #[test]
    fn passthrough_bf16() {
        let mut qs = mk_qs(GgufFtype::BF16, ArchName::Llama3);
        let shape = [4096, 1];
        let t = mk_tensor("blk.0.attn_q.weight", &shape, ArchName::Llama3);
        let pol = StandardPolicy::new();
        let cat = TensorCategory::classify(t.name);
    ...
    fn output_q5_k_m_bumps_to_q6_k() {
        ...
        // new_type != Q8_0).
        let mut qs = mk_qs(GgufFtype::MostlyQ5_K_M, ArchName::Llama3);
        let shape = [4096, 1];
    ...
    fn token_embd_q4_k_s_stays_q4_k() {
        ...
        let mut qs = mk_qs(GgufFtype::MostlyQ4_K_S, ArchName::Llama3);
    ...
    fn ffn_down_q4_k_m_first_layers_bump_q6_k() {
        ...
        let mut qs = mk_qs(GgufFtype::MostlyQ4_K_M, ArchName::Llama3);
        qs.n_ffn_down = 32;
    ...
    fn all_v1_arches_q5_k_m_attn_q_basic() {
        ...
        for arch in arches {
            let mut qs = mk_qs(GgufFtype::MostlyQ5_K_M, arch);
            let shape = [4096, 1];
