...
//! and (3) flipping `ProjectorType::QwenVisionMerger.is_supported()` /
//! `ArchProfile::QwenVisionSiglip.is_supported()` to TRUE so
...
//! | Sub-iter | Scope                                                                    |
//! |----------|--------------------------------------------------------------------------|
//! | 4c.1     | LANDED — `QwenVisionViTConfig` + `compute_vision_embeddings_gpu_qwen`    |
...
//! |          |   Flipped `ProjectorType::QwenVisionMerger.is_supported()` AND           |
//! |          |   `ArchProfile::QwenVisionSiglip.is_supported()` to `true`. Wedge-4d     |
...
pub struct QwenVisionViTConfig {
    ...
    /// for the qwen_vision_merger projector.
...
impl QwenVisionViTConfig {
    /// Extract a `QwenVisionViTConfig` from a parsed `MmprojConfig` plus
    ...
    pub fn from_mmproj(cfg: &MmprojConfig, num_position_embeddings: u32) -> Result<Self> {
        let spatial_merge_size = cfg.spatial_merge_size.ok_or_else(|| {
            anyhow!(
                "QwenVisionViTConfig: MmprojConfig.spatial_merge_size is None — \
        ...
        let out_hidden_size = cfg.projection_dim.ok_or_else(|| {
            anyhow!(
                "QwenVisionViTConfig: MmprojConfig.projection_dim is None — \
        ...
        let deepstack_indexes = cfg.deepstack_indexes.clone().ok_or_else(|| {
            anyhow!(
                "QwenVisionViTConfig: MmprojConfig.deepstack_indexes is None — \
        ...
        if num_position_embeddings == 0 {
            return Err(anyhow!(
                "QwenVisionViTConfig: num_position_embeddings = 0 — must be > 0 \
        ...
        for &idx in &deepstack_indexes {
            if idx >= cfg.num_hidden_layers {
                return Err(anyhow!(
                    "QwenVisionViTConfig: deepstack_indexes contains {} which is \
    ...
    pub fn n_image_tokens(&self, image_size: u32) -> Result<u32> {
        ...
        if stride == 0 {
            return Err(anyhow!(
                "QwenVisionViTConfig::n_image_tokens: stride (patch_size * spatial_merge_size) = 0"
            ));
        }
        if image_size % stride != 0 {
            return Err(anyhow!(
                "QwenVisionViTConfig::n_image_tokens: image_size {} not divisible by \
...
/// This is the input to [`qwen_vision_2x2_block_merge_reshape`].
///
/// # Errors
///
...
//
// Square-input wrapper retained for unit tests only. Production dispatches
// both stored-format patch matrices through `qwen_vision_stage_a_dispatch`.
#[cfg(test)]
pub(crate) fn qwen_vision_dual_conv_patch_embed_cpu(
    pixel_values: &[f32],
    weight_0: &[f32],
    weight_1: &[f32],
    bias: Option<&[f32]>,
    ...
    qwen_vision_dual_conv_patch_embed_cpu_hw(
        pixel_values,
        weight_0,
        weight_1,
        bias,
...
///
/// Same algorithm as [`qwen_vision_dual_conv_patch_embed_cpu`] but accepts
...
/// row-major.
#[cfg(test)]
pub(crate) fn qwen_vision_dual_conv_patch_embed_cpu_hw(
    pixel_values: &[f32],
    weight_0: &[f32],
    weight_1: &[f32],
    bias: Option<&[f32]>,
    ...
    .context("qwen_vision_dual_conv_patch_embed_cpu_hw: stem 0")?;
    ...
    .context("qwen_vision_dual_conv_patch_embed_cpu_hw: stem 1")?;

    if out_0.len() != out_1.len() {
        return Err(anyhow!(
            "qwen_vision_dual_conv_patch_embed_cpu_hw: stem outputs length mismatch \
...
//
// 4c.3 closure: consumed by `compute_vision_embeddings_gpu_qwen`
...
#[cfg(test)]
pub(crate) fn qwen_vision_2x2_block_merge_reshape(
    input: &[f32],
    nx: usize,
    ny: usize,
    n_embd: usize,
    ...
    if nx == 0 || ny == 0 || n_embd == 0 {
        return Err(anyhow!(
            "qwen_vision_2x2_block_merge_reshape: nx ({nx}), ny ({ny}), n_embd ({n_embd}) \
             must all be > 0"
        ));
    }
    if nx % 2 != 0 || ny % 2 != 0 {
        return Err(anyhow!(
            "qwen_vision_2x2_block_merge_reshape: nx ({nx}) and ny ({ny}) must both be \
    ...
    if input.len() != expected {
        return Err(anyhow!(
            "qwen_vision_2x2_block_merge_reshape: input.len() ({}) != ny*nx*n_embd ({})",
            input.len(),
            expected
        ));
    }
...
/// # Inputs
///
/// - `pos_embd_table`: exact mapped F32
///   `[num_position_embeddings, n_embd]` state from
///   `mmproj_weights.position_embd_weight()`. The table is square — i.e.
...
///
///   A1   im2col_2d_3ch_f32 + 2× native stored-format matrix dispatch +
///        elementwise_add + optional add_bias_row_2d_f32
///        — dual-conv patch embed.
///   A2   bilinear_resize_2d_f32 — antialiased pos-embd resize.
...
///
/// The native matrix route reduces over the K axis using backend kernels,
/// NOT the (ic, dy, dx) sequential order
...
fn qwen_vision_stage_a_dispatch(
    ...
    let ctx = "qwen_vision_stage_a_dispatch";
    ...
    let expected_pos_table = (num_position_embeddings as usize)
        .checked_mul(h)
        .and_then(|elements| elements.checked_mul(DType::F32.size_of()))
        .ok_or_else(|| anyhow!("{ctx}: position-embedding byte extent overflow"))?;
    ...
    anyhow::ensure!(
        weight_0.info.rows == h && weight_0.info.cols == k_total,
        "{ctx}: weight_0 native shape [{}, {}] != [{h}, {k_total}]",
        weight_0.info.rows,
        weight_0.info.cols
    );
    anyhow::ensure!(
        weight_1.info.rows == h && weight_1.info.cols == k_total,
        "{ctx}: weight_1 native shape [{}, {}] != [{h}, {k_total}]",
        weight_1.info.rows,
        weight_1.info.cols
    );
    if let Some(b) = bias {
        anyhow::ensure!(
            b.dtype() == DType::F32 && b.data_byte_len() == h * DType::F32.size_of(),
            "{ctx}: bias must be exact F32 [{h}], got {:?}/{} bytes",
            b.dtype(),
            b.data_byte_len()
        );
    }
    anyhow::ensure!(
        pos_embd_table.dtype() == DType::F32
            && pos_embd_table.data_byte_len() == expected_pos_table,
        "{ctx}: pos_embd_table must be exact F32 [{num_position_embeddings}, {hidden}], got {:?}/{} bytes",
        pos_embd_table.dtype(),
        pos_embd_table.data_byte_len()
    );

    // Pixels are request state and are uploaded once. Matrix/state payloads
    // remain mapped in their native stored representations.
    ...
    // dst0 = im2col @ w0.T using the exact loaded codec and physical M.
    let dst0_buf = vit_linear_gpu(
        encoder,
        registry,
        device,
        &im2col_buf,
    ...
    let patches_pre_buf = if let Some(bias_buf) = bias {
...
pub(crate) fn qwen_vision_resize_position_embeddings_bilinear(
    ...
    if num_position_embeddings == 0 || n_embd == 0 || target_n_x == 0 || target_n_y == 0 {
        return Err(anyhow!(
            "qwen_vision_resize_position_embeddings_bilinear: num_position_embeddings \
    ...
    if trained_n.saturating_mul(trained_n) != num_position_embeddings {
        return Err(anyhow!(
            "qwen_vision_resize_position_embeddings_bilinear: \
    ...
    // iter-225 Codex Phase-2c (HIGH finding in this module):
...
///
/// The projector position contract is section-contiguous y/x/y/x: section 0
/// is y, section 1 is x, and sections 2/3 duplicate y/x. It is not a
/// per-token-interleaved `[t,h,w,0]` layout. mlx-native vision mode consumes
/// sections 0/1 and ignores the duplicated trailing sections.
///
...
fn build_qwen_vision_2d_rope_positions(
    device: &MlxDevice,
    n_x: u32,
    n_y: u32,
    ...
    if n_x == 0 || n_y == 0 {
        return Err(anyhow!(
            "build_qwen_vision_2d_rope_positions: n_x ({n_x}) and n_y ({n_y}) must be > 0"
        ));
    }
    ...
    // `qwen_vision_2x2_block_merge_reshape`, patches are in 2x2-block-major
    ...
    if block_merged_order {
        if n_x % 2 != 0 || n_y % 2 != 0 {
            return Err(anyhow!(
                "build_qwen_vision_2d_rope_positions(block_merged_order=true): \
...
/// `[t=0, h, w, 0]` positions.
fn vit_qwen_vision_2d_rope_gpu(
    encoder: &mut CommandEncoder,
    registry: &mut KernelRegistry,
    device: &MlxDevice,
    qkv_buf: &MlxBuffer,
    ...
    if n_pos == 0 || n_heads == 0 || head_dim == 0 {
        return Err(anyhow!(
            "vit_qwen_vision_2d_rope_gpu: n_pos ({n_pos}), n_heads ({n_heads}), \
    ...
    if head_dim % 4 != 0 {
        ...
        return Err(anyhow!(
            "vit_qwen_vision_2d_rope_gpu: head_dim ({head_dim}) must be a multiple \
    ...
    let out = device
        ...
        .map_err(|e| anyhow!("vit_qwen_vision_2d_rope_gpu: alloc: {e}"))?;
    ...
    dispatch_rope_multi_cached(encoder, registry, device, qkv_buf, &out, positions, params)
        .map_err(|e| anyhow!("vit_qwen_vision_2d_rope_gpu: dispatch_rope_multi_cached: {e}"))?;
    Ok(out)
}
...
fn vit_qwen_vision_gelu_gpu(
    ...
    if n_elements == 0 {
        return Err(anyhow!("vit_qwen_vision_gelu_gpu: n_elements must be > 0"));
    }
    let out = device
        .alloc_buffer(
        ...
        .map_err(|e| anyhow!("vit_qwen_vision_gelu_gpu: alloc: {e}"))?;
    dispatch_gelu(encoder, registry, device.metal_device(), input, &out)
        .map_err(|e| anyhow!("vit_qwen_vision_gelu_gpu: dispatch_gelu: {e}"))?;
    Ok(out)
}
...
fn vit_qwen_vision_geglu_split_gpu(
    encoder: &mut CommandEncoder,
    registry: &mut KernelRegistry,
    device: &MlxDevice,
    gate: &MlxBuffer,
    ...
    if n_elements == 0 {
        return Err(anyhow!(
            "vit_qwen_vision_geglu_split_gpu: n_elements must be > 0"
        ));
    }
    let gelu_out = vit_qwen_vision_gelu_gpu(encoder, registry, device, gate, n_elements)?;
    encoder.memory_barrier();
    let out = device
        .alloc_buffer(
            (n_elements as usize) * 4,
        ...
        .map_err(|e| anyhow!("vit_qwen_vision_geglu_split_gpu: alloc out: {e}"))?;
    elementwise_mul(
        encoder,
        registry,
        device.metal_device(),
    ...
    .map_err(|e| anyhow!("vit_qwen_vision_geglu_split_gpu: elementwise_mul: {e}"))?;
    Ok(out)
}
...
/// `build_qwen_vision_2d_rope_positions`). `scale` is the attention
...
fn apply_qwen_vision_block_forward_gpu(
    ...
    if n_heads == 0 || hidden % n_heads != 0 {
        return Err(anyhow!(
            "apply_qwen_vision_block_forward_gpu: hidden ({hidden}) must be divisible \
             by n_heads ({n_heads})"
        ));
    }
    ...
    let ln1 = bert_layer_norm_gpu(
        ...
        block_state("ln1.bias")?,
        eps,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: ln1")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: q proj")?;
    encoder.memory_barrier();
    let q = bert_bias_add_gpu(
        ...
        device,
        &q,
        block_state("attn_q.bias")?,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: q bias")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: k proj")?;
    encoder.memory_barrier();
    let k = bert_bias_add_gpu(
        ...
        device,
        &k,
        block_state("attn_k.bias")?,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: k bias")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: v proj")?;
    encoder.memory_barrier();
    let v = bert_bias_add_gpu(
        ...
        device,
        &v,
        block_state("attn_v.bias")?,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: v bias")?;
    encoder.memory_barrier();

    // -----------------------------------------------------------------
    // Stage 3: 2D-RoPE on Q and K.
    // -----------------------------------------------------------------
    let q_rot = vit_qwen_vision_2d_rope_gpu(
        encoder, registry, device, &q, positions, n_pos, n_heads, head_dim, freq_base,
    )
    .context("apply_qwen_vision_block_forward_gpu: rope Q")?;
    encoder.memory_barrier();
    let k_rot = vit_qwen_vision_2d_rope_gpu(
        encoder, registry, device, &k, positions, n_pos, n_heads, head_dim, freq_base,
    )
    .context("apply_qwen_vision_block_forward_gpu: rope K")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: attention")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: out proj")?;
    encoder.memory_barrier();
    let attn_proj = bert_bias_add_gpu(
        ...
        device,
        &attn_proj,
        block_state("attn_out.bias")?,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: out bias")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: residual 1")?;
    encoder.memory_barrier();
    ...
    let ln2 = bert_layer_norm_gpu(
        ...
        block_state("ln2.bias")?,
        eps,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: ln2")?;
    encoder.memory_barrier();
    ...
    // `build_synth_qwen_vision_weights_with_deepstack` always writes a
    ...
    // -----------------------------------------------------------------
    let ffn_gate_w_opt = weights.block_matrix(block_idx, "ffn_gate.weight").ok();
    ...
    .context("apply_qwen_vision_block_forward_gpu: ffn_up proj")?;
    encoder.memory_barrier();
    let up = bert_bias_add_gpu(
        ...
        device,
        &up,
        block_state("ffn_up.bias")?,
        n_pos,
        intermediate,
    )
    .context("apply_qwen_vision_block_forward_gpu: ffn_up bias")?;
    encoder.memory_barrier();
    ...
    let activated = if let Some(ffn_gate_w) = ffn_gate_w_opt {
        ...
        .context("apply_qwen_vision_block_forward_gpu: ffn_gate proj")?;
        encoder.memory_barrier();
        let gate = bert_bias_add_gpu(
            ...
            device,
            &gate,
            block_state("ffn_gate.bias")?,
            n_pos,
            intermediate,
        )
        .context("apply_qwen_vision_block_forward_gpu: ffn_gate bias")?;
        encoder.memory_barrier();
        let act = vit_qwen_vision_geglu_split_gpu(
            encoder,
            registry,
            device,
            &gate,
        ...
        .context("apply_qwen_vision_block_forward_gpu: geglu split")?;
        encoder.memory_barrier();
        act
    } else {
        ...
        // GELU branch).
        let act = vit_qwen_vision_gelu_gpu(
            encoder,
            registry,
            device,
            &up,
        ...
        .context("apply_qwen_vision_block_forward_gpu: gelu")?;
        encoder.memory_barrier();
        act
    };
    ...
    .context("apply_qwen_vision_block_forward_gpu: ffn_down proj")?;
    encoder.memory_barrier();
    let down = bert_bias_add_gpu(
        ...
        device,
        &down,
        block_state("ffn_down.bias")?,
        n_pos,
        hidden,
    )
    .context("apply_qwen_vision_block_forward_gpu: ffn_down bias")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_block_forward_gpu: residual 2")?;
...
// `apply_qwen_vision_block_forward_gpu` above, they implement the full
...
//   - vit_qwen_vision_gelu_gpu (above) — FFN_GELU activation
//
...
fn apply_qwen_vision_deepstack_head_gpu(
    ...
    if merge_factor == 0 {
        return Err(anyhow!(
            "apply_qwen_vision_deepstack_head_gpu: spatial_merge_size² = 0"
        ));
    }
    if n_pos % merge_factor != 0 {
        return Err(anyhow!(
            "apply_qwen_vision_deepstack_head_gpu: n_pos ({}) must be divisible by \
    ...
    let norm_w = weights.f32_state(&ds_name("norm.weight")).map_err(|_| {
        anyhow!(
            "deepstack head missing F32 state '{}'",
            ds_name("norm.weight")
        )
    })?;
    let norm_b = weights.f32_state(&ds_name("norm.bias")).map_err(|_| {
        anyhow!(
            "deepstack head missing F32 state '{}'",
            ds_name("norm.bias")
        )
    ...
    let fc1_w = weights
        .matrix(&ds_name("fc1.weight"))
        .map_err(|_| anyhow!("deepstack head missing matrix '{}'", ds_name("fc1.weight")))?;
    let fc1_b = weights
        .f32_state(&ds_name("fc1.bias"))
        .map_err(|_| anyhow!("deepstack head missing F32 state '{}'", ds_name("fc1.bias")))?;
    let fc2_w = weights
        .matrix(&ds_name("fc2.weight"))
        .map_err(|_| anyhow!("deepstack head missing matrix '{}'", ds_name("fc2.weight")))?;
    let fc2_b = weights
        .f32_state(&ds_name("fc2.bias"))
        .map_err(|_| anyhow!("deepstack head missing F32 state '{}'", ds_name("fc2.bias")))?;
    ...
    if fc1_w.info.cols as u32 != merged_hidden {
        return Err(anyhow!(
            "apply_qwen_vision_deepstack_head_gpu: deepstack.{il}.fc1.weight native shape [{}, {}] \
             must be [fc1_out, n_embd*spatial_merge²={merged_hidden}]",
            fc1_w.info.rows,
            fc1_w.info.cols
        ));
    }
    let fc1_out = fc1_w.info.rows as u32;
    if fc2_w.info.cols as u32 != fc1_out {
        return Err(anyhow!(
            "apply_qwen_vision_deepstack_head_gpu: deepstack.{il}.fc2.weight native shape [{}, {}] \
             must be [lm_hidden, fc1_out={fc1_out}]",
            fc2_w.info.rows,
            fc2_w.info.cols
        ));
    }
    let lm_hidden = fc2_w.info.rows as u32;
    if lm_hidden != cfg.out_hidden_size {
        return Err(anyhow!(
            "apply_qwen_vision_deepstack_head_gpu: deepstack.{il}.fc2.weight output dim \
    ...
    .context("apply_qwen_vision_deepstack_head_gpu: norm")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_deepstack_head_gpu: fc1 proj")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_deepstack_head_gpu: fc1 bias")?;
    encoder.memory_barrier();
    ...
    let activated = vit_gelu_erf_f32_gpu(encoder, registry, device, &fc1_out_buf)
        .context("apply_qwen_vision_deepstack_head_gpu: gelu")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_deepstack_head_gpu: fc2 proj")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_deepstack_head_gpu: fc2 bias")?;
    Ok(head_out)
}
...
fn apply_qwen_vision_main_projector_gpu(
    encoder: &mut CommandEncoder,
    registry: &mut KernelRegistry,
    device: &MlxDevice,
    weights: &LoadedMmprojWeights,
    ...
    if merge_factor == 0 {
        return Err(anyhow!(
            "apply_qwen_vision_main_projector_gpu: spatial_merge_size² = 0"
        ));
    }
    if n_pos % merge_factor != 0 {
        return Err(anyhow!(
            "apply_qwen_vision_main_projector_gpu: n_pos ({}) must be divisible by \
             spatial_merge_size² ({})",
            n_pos,
            merge_factor
        ));
    ...
    let mm0_w = weights
        .mm_0_matrix()
        .context("apply_qwen_vision_main_projector_gpu: mm.0.weight")?;
    let mm0_b = weights
        .f32_state(super::mmproj::TENSOR_MM_0_BIAS)
        .map_err(|_| {
            anyhow!(
                "apply_qwen_vision_main_projector_gpu: missing F32 state '{}' (Qwen3-VL projector \
    ...
    let mm2_w = weights
        .mm_2_matrix()
        .context("apply_qwen_vision_main_projector_gpu: mm.2.weight")?;
    let mm2_b = weights
        .f32_state(super::mmproj::TENSOR_MM_2_BIAS)
        .map_err(|_| {
            anyhow!(
                "apply_qwen_vision_main_projector_gpu: missing F32 state '{}' (Qwen3-VL projector \
    ...
    // `[lm_hidden, mm0_out]`.
    if mm0_w.info.cols as u32 != merged_hidden {
        return Err(anyhow!(
            "apply_qwen_vision_main_projector_gpu: mm.0.weight native shape [{}, {}] must be \
             [mm0_out, n_embd*spatial_merge²={merged_hidden}]",
            mm0_w.info.rows,
            mm0_w.info.cols
        ));
    }
    let mm0_out = mm0_w.info.rows as u32;
    if mm2_w.info.cols as u32 != mm0_out {
        return Err(anyhow!(
            "apply_qwen_vision_main_projector_gpu: mm.2.weight native shape [{}, {}] must be \
             [lm_hidden, mm0_out={mm0_out}]",
            mm2_w.info.rows,
            mm2_w.info.cols
        ));
    }
    let lm_hidden = mm2_w.info.rows as u32;
    if lm_hidden != cfg.out_hidden_size {
        return Err(anyhow!(
            "apply_qwen_vision_main_projector_gpu: mm.2.weight output dim ({lm_hidden}) \
    ...
    .context("apply_qwen_vision_main_projector_gpu: mm.0 proj")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_main_projector_gpu: mm.0 bias")?;
    encoder.memory_barrier();
    ...
    let activated = vit_gelu_erf_f32_gpu(encoder, registry, device, &mm0_out_buf)
        .context("apply_qwen_vision_main_projector_gpu: gelu")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_main_projector_gpu: mm.2 proj")?;
    encoder.memory_barrier();
    ...
    .context("apply_qwen_vision_main_projector_gpu: mm.2 bias")?;
    Ok(proj_out)
}
...
///   in slot 0)
#[cfg(test)]
fn qwen_vision_concat_augmented_embed_cpu(
    chunks: &[Vec<f32>],
    ...
    if chunks.is_empty() {
        return Err(anyhow!(
            "qwen_vision_concat_augmented_embed_cpu: chunks must contain at least the \
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        if chunk.len() != expected_chunk_len {
            return Err(anyhow!(
                "qwen_vision_concat_augmented_embed_cpu: chunk[{i}] length {} != \
...
///
/// `is_supported()` on `ProjectorType::QwenVisionMerger` is `true` as of
...
///   projector).
/// - `cfg`: shape contract from `QwenVisionViTConfig::from_mmproj`.
...
pub fn compute_vision_embeddings_gpu_qwen(
    ...
    if inputs.len() != 1 {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: per-image entry point \
    ...
    let (pixel_values, pixel_w, pixel_h): (&[f32], u32, u32) = match input {
        ...
        VisionInput::Gemma4v(_) => {
            return Err(anyhow!(
                "compute_vision_embeddings_gpu_qwen: Qwen3-VL preprocessing \
    ...
    if stride == 0 {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: patch_size ({}) * \
    ...
    if pixel_w == 0 || pixel_h == 0 {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: pixel grid ({}x{}) has \
             zero dimension",
            pixel_w,
            pixel_h
        ));
    ...
    if pixel_w % stride != 0 || pixel_h % stride != 0 {
        ...
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: per-image pixel grid \
    ...
    if pixel_values.len() != expected_pixels {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: pixel_values.len() ({}) != \
    ...
    if n_x_pre == 0 || n_y_pre == 0 {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: post-conv grid \
    ...
    // -----------------------------------------------------------------
    // Stage A — request pixels plus native mapped matrix/state payloads.
    // -----------------------------------------------------------------

    let patch_embd = mmproj_weights
        .patch_embd_matrix()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: {e}"))?;
    let patch_embd_1 = mmproj_weights
        .patch_embd_matrix_1()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: dual-stem patch matrix: {e}"))?;
    let patch_bias = mmproj_weights
        .f32_state(super::mmproj::TENSOR_PATCH_EMBD_BIAS)
        .ok();

    let pos_embd = mmproj_weights
    ...
    let executor = GraphExecutor::new(
        MlxDevice::new().map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: device: {e}"))?,
    );
    let mut prelude_session = executor
        .begin()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: begin: {e}"))?;
    ...
    let input_gpu = qwen_vision_stage_a_dispatch(
    ...
    .context("compute_vision_embeddings_gpu_qwen: Stage A (GPU, single session)")?;
    ...
    let positions = build_qwen_vision_2d_rope_positions(
        device, n_x_merged, n_y_merged,
        true, // block-merged order — patches are 2x2-block-major
    )
    .context("compute_vision_embeddings_gpu_qwen: build rope positions")?;
    ...
    if let (Ok(pre_ln_w), Ok(pre_ln_b)) = (
        mmproj_weights.f32_state("v.pre_ln.weight"),
        mmproj_weights.f32_state("v.pre_ln.bias"),
    ) {
        ...
        .context("compute_vision_embeddings_gpu_qwen: pre-LN")?;
        prelude_session.encoder_mut().memory_barrier();
    }
    prelude_session
        .finish()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: prelude finish: {e}"))?;

    // DeepStack head outputs collected across the per-block loop, in
    // ascending block-index order (matches `cfg.deepstack_indexes`
    // which is sorted ascending — see QwenVisionViTConfig::from_mmproj).
    // Per the peer, the
    ...
    for block_idx in 0..(cfg.n_layer as usize) {
        let mut block_session = executor.begin().map_err(|e| {
            anyhow!("compute_vision_embeddings_gpu_qwen: block {block_idx} begin: {e}")
        })?;
        let block_out = apply_qwen_vision_block_forward_gpu(
        ...
        .with_context(|| format!("compute_vision_embeddings_gpu_qwen: block {block_idx}"))?;
        ...
        if deepstack_set.contains(&(block_idx as u32)) {
            let head_out = apply_qwen_vision_deepstack_head_gpu(
                block_session.encoder_mut(),
                &mut registry,
                device,
                mmproj_weights,
            ...
            .with_context(|| {
                format!("compute_vision_embeddings_gpu_qwen: deepstack head at block {block_idx}")
            })?;
            block_session.encoder_mut().memory_barrier();
        ...
        block_session.finish().map_err(|e| {
            anyhow!("compute_vision_embeddings_gpu_qwen: block {block_idx} finish: {e}")
        })?;
    ...
    if deepstack_outputs.len() != cfg.deepstack_indexes.len() {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: deepstack_outputs.len() ({}) != \
    ...
    let mut final_session = executor
        .begin()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: projector begin: {e}"))?;
    let post_ln_w = mmproj_weights
        .post_ln_weight()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: post_ln.weight: {e}"))?;
    let post_ln_b = mmproj_weights
        .f32_state(super::mmproj::TENSOR_POST_LN_BIAS)
        .map_err(|_| {
            anyhow!(
                "compute_vision_embeddings_gpu_qwen: missing F32 state '{}' (Qwen3-VL ViT \
    ...
    .context("compute_vision_embeddings_gpu_qwen: post-LN")?;
    ...
    let main_out = apply_qwen_vision_main_projector_gpu(
        final_session.encoder_mut(),
        &mut registry,
        device,
        mmproj_weights,
    ...
    .context("compute_vision_embeddings_gpu_qwen: main projector")?;
    ...
    // CPU memcpy. Eliminates the `qwen_vision_concat_augmented_embed_cpu`
    // call entirely.
    // -----------------------------------------------------------------
    let merge_factor = (cfg.spatial_merge_size as usize).pow(2);
    if merge_factor == 0 || n_pos_merged % merge_factor != 0 {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: n_pos_merged ({}) not divisible \
    ...
    let augmented_buf = device
        ...
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: alloc augmented buf: {e}"))?;
    ...
    .with_context(|| {
        format!(
            "compute_vision_embeddings_gpu_qwen: K5 feature_concat (main) — \
    ...
    for (i, head_out) in deepstack_outputs.iter().enumerate() {
        ...
        .with_context(|| {
            format!(
                "compute_vision_embeddings_gpu_qwen: K5 feature_concat (deepstack {i}) — \
    ...
    final_session
        .finish()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: finish: {e}"))?;

    let augmented_slice: &[f32] = augmented_buf
        .as_slice::<f32>()
        .map_err(|e| anyhow!("compute_vision_embeddings_gpu_qwen: augmented readback: {e}"))?;
    if augmented_slice.len() < expected_augmented_len {
        return Err(anyhow!(
            "compute_vision_embeddings_gpu_qwen: augmented embed len {} < \
...
mod tests {
    ...
    /// shape conventions from the Qwen vision mmproj fixtures /
    /// `mmproj.rs::tests::qwen_vision_cfg`. We don't import that helper
    ...
    fn synth_qwen_vision_mmproj_cfg(
        num_layers: u32,
    ...
    #[test]
    fn qwen_vision_vit_config_from_mmproj_round_trip() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        // Qwen3-VL-2B-style fixture: 24 layers, DeepStack at 5/11/17.
        let cfg = synth_qwen_vision_mmproj_cfg(24, Some(vec![5, 11, 17]), Some(2), Some(2048));
        let vit = QwenVisionViTConfig::from_mmproj(&cfg, 2304)
    ...
    fn qwen38_real_mmproj_rectangular_forward_is_finite() {
        ...
        assert_eq!(cfg.projector, ProjectorType::QwenVisionMerger);
        let weights =
        ...
        let n_pos = weights
            ...
            .shape()[0] as u32;
        let vit = QwenVisionViTConfig::from_mmproj(&cfg, n_pos).expect("Qwen vision config");
        ...
        let output = compute_vision_embeddings_gpu_qwen(&inputs, &weights, &vit, &cfg)
    ...
    #[test]
    fn qwen_vision_vit_config_fails_on_missing_deepstack_indexes() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        // deepstack_indexes = None means the writer omitted
        // `clip.vision.is_deepstack_layers`. Refuse to fall back.
        let cfg = synth_qwen_vision_mmproj_cfg(
            24,
            None, // ← the missing field under test
            Some(2),
            Some(2048),
        );
        let err = QwenVisionViTConfig::from_mmproj(&cfg, 2304)
    ...
    #[test]
    fn qwen_vision_vit_config_fails_on_missing_spatial_merge_size() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let cfg = synth_qwen_vision_mmproj_cfg(
            24,
            Some(vec![5, 11, 17]),
    ...
    #[test]
    fn qwen_vision_vit_config_fails_on_missing_projection_dim() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let cfg = synth_qwen_vision_mmproj_cfg(
            24,
            Some(vec![5, 11, 17]),
    ...
    #[test]
    fn qwen_vision_vit_config_fails_on_zero_num_position_embeddings() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let cfg = synth_qwen_vision_mmproj_cfg(24, Some(vec![5, 11, 17]), Some(2), Some(2048));
        let err = QwenVisionViTConfig::from_mmproj(&cfg, 0)
            .expect_err("from_mmproj must reject num_position_embeddings=0");
        assert!(format!("{err}").contains("num_position_embeddings"));
    }

    #[test]
    fn qwen_vision_vit_config_fails_on_out_of_range_deepstack_index() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let cfg = synth_qwen_vision_mmproj_cfg(24, Some(vec![5, 11, 24]), Some(2), Some(2048));
        let err = QwenVisionViTConfig::from_mmproj(&cfg, 2304)
    ...
    #[test]
    fn qwen_vision_vit_config_accepts_empty_deepstack_indexes() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let cfg = synth_qwen_vision_mmproj_cfg(24, Some(vec![]), Some(2), Some(2048));
        let vit = QwenVisionViTConfig::from_mmproj(&cfg, 2304)
    ...
    #[test]
    fn compute_vision_embeddings_gpu_qwen_returns_scaffold_error() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        // 1-input call (see `qwen_vision_compute_returns_err_with_4c3_marker`
        // below).
        let cfg = synth_qwen_vision_mmproj_cfg(24, Some(vec![5, 11, 17]), Some(2), Some(2048));
        let vit_cfg = QwenVisionViTConfig::from_mmproj(&cfg, 2304).unwrap();
        let weights = make_empty_loaded_mmproj_weights();
        let result = compute_vision_embeddings_gpu_qwen(&[], &weights, &vit_cfg, &cfg);
        let err = result
            .expect_err("compute_vision_embeddings_gpu_qwen must reject an empty input slice");
        let msg = format!("{err}");
    ...
    /// `qwen_vision_dual_conv_patch_embed_cpu` adds the two stems
    /// element-wise, as the peer does.
    #[test]
    fn qwen_vision_dual_conv_patch_embedding_synthetic() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let dual = qwen_vision_dual_conv_patch_embed_cpu(
            &pixel_values,
            &weight_0,
            &weight_1,
            None,
    ...
    /// pass-through.
    #[test]
    fn qwen_vision_position_embedding_no_resize_when_trained_size_matches() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let resized = qwen_vision_resize_position_embeddings_bilinear(
    ...
    ///   → val = 2*0.5 + 4*0.5 = 3.0.
    #[test]
    fn qwen_vision_position_embedding_bilinear_interpolation_when_resize() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let trained_n: u32 = 2;
        let n_embd: u32 = 1;
    ...
    #[test]
    fn qwen_vision_position_embedding_antialias_downsample_diverges_from_plain_bilinear() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let trained_n: u32 = 4;
        let n_embd: u32 = 1;
        let num_pos = trained_n * trained_n; // 16
    ...
    /// outer axis), drives `QwenVisionViTConfig::from_mmproj` with the
    ...
    #[test]
    fn qwen_vision_dispatch_real_num_position_embeddings_extracted() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let mmproj_cfg =
            synth_qwen_vision_mmproj_cfg(24, Some(vec![5, 11, 17]), Some(2), Some(2048));
        let vit_cfg =
            QwenVisionViTConfig::from_mmproj(&mmproj_cfg, extracted_count).expect("from_mmproj");
        ...
        assert_ne!(
            ...
            "regression: 4c.1 sentinel `num_position_embeddings = 1` must NOT \
             reach QwenVisionViTConfig — 4c.2 closes that drift trap"
        );
    }
    ...
    #[test]
    fn qwen_vision_compute_reaches_patch_embed_after_4c3() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let vit_cfg = QwenVisionViTConfig::from_mmproj(&mmproj_cfg_small, 4).unwrap();
        ...
        let weights = make_empty_loaded_mmproj_weights();
        let result =
            compute_vision_embeddings_gpu_qwen(&inputs, &weights, &vit_cfg, &mmproj_cfg_small);
    ...
    /// [`qwen_vision_2x2_block_merge_reshape`] performs (the reshape is
    ...
    /// order: 0,1,4,5.
    #[test]
    fn qwen_vision_2x2_block_merge_reshape_known_pattern() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let nx = 4;
        let ny = 4;
        let n_embd = 1;
        let input: Vec<f32> = (0..(nx * ny * n_embd) as u32).map(|i| i as f32).collect();
        let out = qwen_vision_2x2_block_merge_reshape(&input, nx, ny, n_embd)
    ...
    fn build_synth_qwen_vision_weights(
        ...
        build_synth_qwen_vision_weights_with_deepstack(
            device,
            n_layers,
            hidden,
            intermediate,
            ...
            patch_size,
            &[],
            32, // out_hidden_size: matches synth_qwen_vision_block_cfg projection_dim
        )
    }

    /// Variant of `build_synth_qwen_vision_weights` that additionally adds
    ...
    #[allow(clippy::too_many_arguments)]
    fn build_synth_qwen_vision_weights_with_deepstack(
        device: mlx_native::MlxDevice,
        n_layers: u32,
        hidden: u32,
        intermediate: u32,
        ...
        // `synth_qwen_vision_block_cfg` so merge_factor is 4 here.
    ...
    fn synth_qwen_vision_block_cfg(n_layers: u32) -> (QwenVisionViTConfig, MmprojConfig) {
        let mut cfg = synth_qwen_vision_mmproj_cfg(n_layers, Some(vec![]), Some(2), Some(32));
        cfg.image_size = 128;
        cfg.patch_size = 16;
        ...
        let vit_cfg = QwenVisionViTConfig::from_mmproj(&cfg, 64)
            .expect("synth_qwen_vision_block_cfg: from_mmproj");
        (vit_cfg, cfg)
    }
    ...
    #[allow(clippy::too_many_arguments, dead_code)]
    fn build_synth_qwen_vision_weights_split_block_vs_head(
        device: mlx_native::MlxDevice,
        n_layers: u32,
        hidden: u32,
        intermediate: u32,
        ...
        //
        // Actually: build_synth_qwen_vision_weights_with_deepstack uses one
    ...
    /// Test #1 — `qwen_vision_per_block_forward_synthetic_2_blocks`
    ...
    ///
    /// Direct unit test of `apply_qwen_vision_block_forward_gpu`:
    ...
    ///     `apply_qwen_vision_block_forward_gpu` Stage 6 (replace
    ...
    /// residuals intact.)
    #[test]
    fn qwen_vision_per_block_forward_synthetic_2_blocks() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::GraphExecutor;
        use std::collections::HashMap;
        ...
        // Build a QwenVisionViTConfig matching this fixture. spatial_merge=2
        ...
        let vit_cfg = QwenVisionViTConfig::from_mmproj(&mmproj_cfg, 16).expect("from_mmproj");
        ...
        .expect("apply_qwen_vision_block_forward_gpu must succeed");
        ...
        assert!(
            ...
            "RESIDUAL-PRESENCE PIN: residual chain must propagate the \
             ...
             apply_qwen_vision_block_forward_gpu (Stage 6 OR Stage 9) → \
    ...
    /// Test #2 — `qwen_vision_2d_rope_vision_mode_consumed`.
    ///
    ...
    #[test]
    fn qwen_vision_2d_rope_vision_mode_consumed() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
    ...
    /// Test #3 — `qwen_vision_attention_is_bidirectional_no_causal_mask`.
    ///
    ...
    #[test]
    fn qwen_vision_attention_is_bidirectional_no_causal_mask() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::{DType, GraphExecutor};
    ...
    /// Test #4 — `qwen_vision_mlp_uses_gelu_not_silu`.
    ///
    /// Direct test of the FFN_GELU path via `vit_qwen_vision_geglu_split_gpu`.
    ...
    #[test]
    fn qwen_vision_mlp_uses_gelu_not_silu() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::GraphExecutor;
        ...
        let activated = vit_qwen_vision_geglu_split_gpu(
            session.encoder_mut(),
            &mut registry,
            device,
            &gate,
    ...
    /// Test #5 — `qwen_vision_per_block_residual_present`
    ...
    #[test]
    fn qwen_vision_per_block_residual_present() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::GraphExecutor;
        use std::collections::HashMap;
        ...
        let positions = build_qwen_vision_2d_rope_positions(device_borrow, 8, 4, true)
            .expect("build positions");
        let head_dim = (n_embd / n_head) as f32;
        let scale = 1.0_f32 / head_dim.sqrt();
        ...
        for block_idx in 0..2usize {
            hidden = apply_qwen_vision_block_forward_gpu(
                session.encoder_mut(),
                &mut registry,
                device_borrow,
                &weights,
            ...
            .expect("apply_qwen_vision_block_forward_gpu");
        ...
        assert!(
            ...
            "RESIDUAL-PRESENCE PIN (2-block chain): output max_abs={got_max_abs:.4} \
             ...
             either residual in apply_qwen_vision_block_forward_gpu → output \
    ...
    /// Test #6 — `qwen_vision_compute_returns_ok_augmented_for_2_block_synthetic`
    ///   (4c.4 — replaces 4c.3's `qwen_vision_compute_returns_ok_for_2_block_synthetic`
    ///    that asserted the OLD `[n_pos_merged, n_embd]` shape).
    ///
    /// End-to-end smoke test: the public dispatch entry-point
    /// `compute_vision_embeddings_gpu_qwen` returns `Ok` for a
    ...
    #[test]
    fn qwen_vision_compute_returns_ok_augmented_for_2_block_synthetic() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        let (mut vit_cfg, mmproj_cfg) = synth_qwen_vision_block_cfg(2);
        ...
        let weights = build_synth_qwen_vision_weights_with_deepstack(
            device,
            vit_cfg.n_layer,
            vit_cfg.n_embd,
        ...
        let result = compute_vision_embeddings_gpu_qwen(&inputs, &weights, &vit_cfg, &mmproj_cfg)
        ...
        // counterpart.
        assert!(
            ProjectorType::QwenVisionMerger.is_supported(),
            "4c.5 LANDED: ProjectorType::QwenVisionMerger.is_supported() must \
    ...
    /// Test 4c.4-A — `qwen_vision_deepstack_head_layernorm_then_mlp_synthetic`.
    ///
    /// Direct unit test of `apply_qwen_vision_deepstack_head_gpu` with a
    ...
    /// `qwen_vision_mlp_uses_gelu_not_silu` test uses).
    #[test]
    fn qwen_vision_deepstack_head_layernorm_then_mlp_synthetic() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::{DType, GraphExecutor};
        use std::collections::HashMap;
        ...
        // Build a QwenVisionViTConfig with these shapes.
        let mmproj_cfg = MmprojConfig {
            image_size: 32,
            patch_size: 16,
            num_patches_side: 2,
        ...
        let vit_cfg = QwenVisionViTConfig::from_mmproj(&mmproj_cfg, 4)
            .expect("from_mmproj for deepstack head test");

        // Synthesize an input buffer `[n_image_tokens=1, merged_hidden=32]`
        // = [1, 2, 3, ..., 32] linearly. The `apply_qwen_vision_deepstack_head_gpu`
        ...
        // pool. (Same pattern as `compute_vision_embeddings_gpu_qwen`.)
        let executor =
            GraphExecutor::new(mlx_native::MlxDevice::new().expect("MlxDevice for executor"));
        let mut session = executor.begin().expect("begin");
        let mut registry = mlx_native::KernelRegistry::new();
        ...
        .expect("apply_qwen_vision_deepstack_head_gpu must succeed");
        session.finish().expect("finish");
    ...
    /// Test 4c.4-B — `qwen_vision_spatial_merger_2x2_concat_along_channel`.
    ///
    ...
    #[test]
    fn qwen_vision_spatial_merger_2x2_concat_along_channel() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::GraphExecutor;
    ...
    /// Test 4c.4-C — `qwen_vision_main_projector_2layer_gelu_synthetic`.
    ///
    /// Direct unit test of `apply_qwen_vision_main_projector_gpu` with a
    ...
    #[test]
    fn qwen_vision_main_projector_2layer_gelu_synthetic() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        use mlx_native::{DType, GraphExecutor};
        use std::collections::HashMap;
        ...
        let proj_out = apply_qwen_vision_main_projector_gpu(
            session.encoder_mut(),
            &mut registry,
            device_borrow,
            &weights,
        ...
        .expect("apply_qwen_vision_main_projector_gpu must succeed");
        session.finish().expect("finish");
    ...
    /// Test 4c.4-D — `qwen_vision_deepstack_head_taps_correct_block_index`.
    ///
    ...
    ///     `(1 + n_layer) * lm_hidden`.
    #[test]
    fn qwen_vision_deepstack_head_taps_correct_block_index() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        // 3-block model with deepstack flagged at index 2 (the last
        // block ONLY, not all blocks).
        let mut mmproj_cfg = synth_qwen_vision_mmproj_cfg(
            3,
        ...
        let vit_cfg = QwenVisionViTConfig::from_mmproj(&mmproj_cfg, 64).expect("from_mmproj");
    ...
    // structural invariant in the Qwen vision projector loop:
    //
    //   for block_idx in 0..(cfg.n_layer as usize) {
    //       let block_out = apply_qwen_vision_block_forward_gpu(...);
    //       if deepstack_set.contains(&(block_idx as u32)) {
    ...
    /// Test 4c.4-E — `qwen_vision_augmented_embed_shape_matches_lm_split_contract`.
    ///
    ...
    /// embedding.
    #[test]
    fn qwen_vision_augmented_embed_shape_matches_lm_split_contract() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        // 3-block fixture with all 3 blocks flagged.
        let mut mmproj_cfg =
            synth_qwen_vision_mmproj_cfg(3, Some(vec![0, 1, 2]), Some(2), Some(32));
        mmproj_cfg.image_size = 128;
        mmproj_cfg.patch_size = 16;
        mmproj_cfg.num_patches_side = 8;
        mmproj_cfg.hidden_size = 32;
    ...
    ///
    /// Pin `qwen_vision_concat_augmented_embed_cpu` produces the exact
    ...
    #[test]
    fn qwen_vision_cpu_concat_augmented_embed_byte_exact() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let chunks: Vec<Vec<f32>> = (0..3)
            ...
            .collect();
        let out = qwen_vision_concat_augmented_embed_cpu(&chunks, n_image_tokens, lm_hidden)
        ...
        let err = qwen_vision_concat_augmented_embed_cpu(&[], 1, 4).unwrap_err();
        assert!(format!("{err}").contains("at least the base"));

        // Length mismatch → Err.
        let bad = vec![vec![1.0f32; 8], vec![1.0f32; 7]];
        let err = qwen_vision_concat_augmented_embed_cpu(&bad, 2, 4).unwrap_err();
    ...
    //   1. `QwenVisionMerger.is_supported() == true`
    //   2. dispatch path reaches `compute_vision_embeddings_gpu_qwen`
    //      when the GGUF declares Qwen3-VL.
    // -----------------------------------------------------------------

    #[test]
    fn qwen_vision_merger_is_supported_after_wedge_4c5() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        assert!(
            ProjectorType::QwenVisionMerger.is_supported(),
            "Wedge-4c.5 LANDED: ProjectorType::QwenVisionMerger.is_supported() must return true"
        );
        assert!(
            super::super::mmproj::ArchProfile::QwenVisionSiglip.is_supported(),
            "Wedge-4c.5 LANDED: ArchProfile::QwenVisionSiglip.is_supported() must return true \
    ...
    #[test]
    fn qwen_vision_dispatch_routes_to_compute_vision_embeddings_gpu_qwen() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        // Drive `compute_vision_embeddings_gpu_dispatch` with a
        // QwenVisionSiglip arch + a synthetic QwenVisionSiglip-marked
        // mmproj_weights tensor map and confirm the call reaches
        // `compute_vision_embeddings_gpu_qwen` (rather than the
        // Gemma/SigLIP path). The dispatch entry-point re-derives
        // `QwenVisionViTConfig` from `mmproj_cfg` (so the vit_cfg returned
        // by `synth_qwen_vision_block_cfg` is informational only — what
        ...
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        let (vit_cfg, mmproj_cfg) = synth_qwen_vision_block_cfg(2);
        // Empty deepstack_indexes (default in synth_qwen_vision_block_cfg)
        ...
        let result = compute_vision_embeddings_gpu_dispatch(
            &inputs,
            super::super::mmproj::ArchProfile::QwenVisionSiglip,
            &weights,
            &mmproj_cfg,
        ...
        match result {
            ...
            Err(e) => {
                ...
                let msg = format!("{e:#}");
                assert!(
                    msg.contains("qwen3vl") || msg.contains("QwenVisionSiglip"),
                    "dispatch must route QwenVisionSiglip to the qwen3vl module; \
    ...
    #[test]
    fn qwen_vision_pos_embed_resize_to_rectangular_target_iter225() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
    ...
    #[test]
    fn qwen_vision_compute_accepts_rectangular_input_iter225() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        let (mut vit_cfg, mut mmproj_cfg) = synth_qwen_vision_block_cfg(2);
        vit_cfg.deepstack_indexes = vec![0];
        ...
        // (synth_qwen_vision_block_cfg uses num_position_embeddings=64).
    ...
    #[test]
    fn qwen_vision_compute_rejects_misaligned_rectangular_input_iter225() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        let (vit_cfg, mut mmproj_cfg) = synth_qwen_vision_block_cfg(1);
        let weights = build_synth_qwen_vision_weights_with_deepstack(
            device,
            vit_cfg.n_layer,
            vit_cfg.n_embd,
        ...
        let err = compute_vision_embeddings_gpu_qwen(&inputs, &weights, &vit_cfg, &mmproj_cfg)
    ...
    #[test]
    fn qwen_vision_compute_backward_compat_square_input_iter225() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        let device = mlx_native::MlxDevice::new().expect("MlxDevice");
        let (mut vit_cfg, mmproj_cfg) = synth_qwen_vision_block_cfg(2);
        vit_cfg.deepstack_indexes = vec![0];
    ...
    #[test]
    fn qwen_vision_compute_square_byte_deterministic_iter225() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        let weights_1 = build_synth_qwen_vision_weights_with_deepstack(
            device_1,
            vit_cfg.n_layer,
            vit_cfg.n_embd,
            vit_cfg.intermediate_size,
        ...
        let weights_2 = build_synth_qwen_vision_weights_with_deepstack(
            device_2,
            vit_cfg.n_layer,
            vit_cfg.n_embd,
            vit_cfg.intermediate_size,
        ...
        let r1 = compute_vision_embeddings_gpu_qwen(&inputs_1, &weights_1, &vit_cfg, &mmproj_cfg)
            .expect("first call");
        let r2 = compute_vision_embeddings_gpu_qwen(&inputs_2, &weights_2, &vit_cfg, &mmproj_cfg)
            .expect("second call");
        assert_eq!(r1.len(), 1);
        assert_eq!(r2.len(), 1);
    ...
    /// Pins that the seam's `qwen_vision_image_grids` plumbing
    ...
    /// `dispatch_qwen_vision_seam_split` directly via a small CPU mirror.
    #[test]
    fn qwen_vision_phase2_image_grid_n_tokens_invariant_iter225() {
        let _gpu = crate::inference::hf2q_gpu_test_lock();
        ...
        // `qwen_vision_image_grids` parameter (Phase-2 contract).
        assert_ne!(
    ...
    fn override_tensor_with_seeded_sin(
        ...
        // synth-test pattern (see `fill_f32` at build_synth_qwen_vision_*).
        let s: &mut [f32] =
            unsafe { std::slice::from_raw_parts_mut(buf.contents_ptr() as *mut f32, n) };
    ...
    /// augmented-embed output of `compute_vision_embeddings_gpu_qwen`
    ...
    ///
    /// Fixture: `synth_qwen_vision_block_cfg(2)` → 2 ViT layers, hidden=32,
    /// patch=16, image=128, deepstack=[0]. Weights are first
    /// constant-filled by `build_synth_qwen_vision_weights_with_deepstack`
    ...
    fn adr021_iter1a_e2e_byte_pinned_baseline_2026_05_07() {
        ...
        //                         live fixture.
        //   mlx-native 0.13.0  — native dispatch-plan kernels change only
        //                         floating reduction order. Two clean runs
        //                         reproduced this pin, while the independent
        //                         Stage-A CPU oracle stayed within 1.2e-6.
        //                         fnv1a64=0x2fd99b93_3f5d0432
        const EXPECTED_FNV1A64: u64 = 0x2fd9_9b93_3f5d_0432;
    ...
    fn adr021_iter5b_ac3_substitute_three_shapes_2026_05_07() {
        ...
        let run_at_shape = |pixel_h: u32, pixel_w: u32| -> (Vec<f32>, u64) {
            ...
            let result =
                compute_vision_embeddings_gpu_qwen(&inputs, &weights, &vit_cfg, &mmproj_cfg)
    ...
    ///
    /// Compares the GPU Stage A pipeline (`qwen_vision_stage_a_gpu_to_cpu`,
    ...
    fn adr021_iter5c_ac6_perf_stage_a_gpu_vs_cpu_2026_05_07() {
        ...
        const TRAINED_N: usize = 16; // 16² = 256 trained pos embd entries
        ...
        let cpu_oracle = || -> Vec<f32> {
            let patches_pre = qwen_vision_dual_conv_patch_embed_cpu_hw(
                &pixels,
                &weight_0,
                &weight_1,
                Some(&bias),
            ...
            let pos_resized = qwen_vision_resize_position_embeddings_bilinear(
                &pos_embd,
            ...
            qwen_vision_2x2_block_merge_reshape(&summed, nps_x, nps_y, HIDDEN as usize)
        ...
        let gpu_pipeline = || -> Vec<f32> {
            ...
            let mut w0_buf = device
                .alloc_buffer(
                    weight_0.len() * DType::F32.size_of(),
                    DType::F32,
                    vec![HIDDEN as usize, k_total],
                )
                .expect("weight 0 buffer");
            w0_buf
                .as_mut_slice::<f32>()
                .expect("weight 0 view")
                .copy_from_slice(&weight_0);
            let w0 = MlxQWeight::from_test_buffer(
                w0_buf,
            ...
            let mut w1_buf = device
                .alloc_buffer(
                    weight_1.len() * DType::F32.size_of(),
                    DType::F32,
                    vec![HIDDEN as usize, k_total],
                )
                .expect("weight 1 buffer");
            w1_buf
                .as_mut_slice::<f32>()
                .expect("weight 1 view")
                .copy_from_slice(&weight_1);
            let w1 = MlxQWeight::from_test_buffer(
                w1_buf,
                mlx_native::GgmlType::F32,
                HIDDEN as usize,
                k_total,
            ...
            let mut pos_buf = device
                .alloc_buffer(
                    pos_embd.len() * DType::F32.size_of(),
                    DType::F32,
                    vec![TRAINED_N, TRAINED_N, HIDDEN as usize],
                )
                .expect("position buffer");
            pos_buf
                .as_mut_slice::<f32>()
                .expect("position view")
                .copy_from_slice(&pos_embd);
            let merged_buf = qwen_vision_stage_a_dispatch(
                session.encoder_mut(),
                &mut registry,
                device,
                &pixels,
