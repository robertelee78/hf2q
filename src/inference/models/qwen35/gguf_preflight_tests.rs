...
fn matrix_rejects_known_non_native_role_combinations() {
    for (role, kind) in [
        ...
        (TensorRole::FfnGateUp, GgmlType::I16),
    ] {
...
#[test]
fn every_native_ggml_matmul_codec_is_admitted_for_dense_and_expert_ffn_roles() {
    for kind in [
        GgmlType::Q2_K,
        GgmlType::Q3_K,
        GgmlType::Q4_0,
        ...
        for role in [
            ...
            admit_storage_for_role("canary.weight", role, TensorStorage::Parsed(kind))
                .unwrap_or_else(|error| panic!("{role:?} must admit {kind:?}: {error:#}"));
        }
    }
}

#[test]
fn every_native_scalar_expert_codec_is_admitted_without_substitution() {
    for kind in [GgmlType::F32, GgmlType::F16, GgmlType::BF16] {
        admit_storage_for_role(
            "blk.0.ffn_gate_exps.weight",
            TensorRole::ExpertStack,
            TensorStorage::Parsed(kind),
        )
        .unwrap_or_else(|error| panic!("scalar expert stack must admit {kind:?}: {error:#}"));
    }
}

#[test]
fn scalar_and_mixed_scalar_expert_triplets_reach_exact_capability_checks() {
    const EXPERTS: usize = 2;
    const WIDTH: usize = 256;
    let routing = GgmlRoutingPolicy::default();
    let stack = |name: &str, ggml_type: GgmlType| TensorInfo {
        name: name.into(),
        shape: vec![EXPERTS, WIDTH, WIDTH],
        ggml_type,
        offset: 0,
        byte_len: EXPERTS * checked_tensor_bytes_for_profile(ggml_type, WIDTH, WIDTH),
    };
    for (gate_type, up_type, down_type) in [
        (GgmlType::F32, GgmlType::F32, GgmlType::F32),
        (GgmlType::F16, GgmlType::BF16, GgmlType::F32),
    ] {
        let gate = stack("blk.0.ffn_gate_exps.weight", gate_type);
        let up = stack("blk.0.ffn_up_exps.weight", up_type);
        let down = stack("blk.0.ffn_down_exps.weight", down_type);
        ...
        .unwrap_or_else(|error| {
            panic!(
                "scalar expert triplet {gate_type:?}/{up_type:?}/{down_type:?} rejected: {error:#}"
            )
        });
    }
}
...
#[test]
fn mixed_expert_projection_codecs_are_checked_independently() {
    const EXPERTS: usize = 2;
    const WIDTH: usize = 256;
    let routing = GgmlRoutingPolicy::default();
    let stack = |name: &str, ggml_type: GgmlType| TensorInfo {
    ...
    let gate = stack("blk.0.ffn_gate_exps.weight", GgmlType::Q2_K);
    let up = stack("blk.0.ffn_up_exps.weight", GgmlType::Q5_K);
    let down = stack("blk.0.ffn_down_exps.weight", GgmlType::Q6_K);
    ...
    .expect("mixed expert codecs must reach their own exact capability checks");
}

#[test]
fn legacy_block_embeddings_are_admitted_only_on_native_gather_routes() {
    for kind in [GgmlType::Q4_0, GgmlType::Q5_0] {
        admit_storage_for_role(
            "token_embd.weight",
            TensorRole::Embedding,
            TensorStorage::Parsed(kind),
        )
        .unwrap_or_else(|error| panic!("{kind:?} embedding must use its native gather: {error}"));
    }
}

#[test]
fn f32_matrix_is_counted_while_elementwise_state_is_not() {
    let mut receipt = Qwen35GgufPreflightReceipt::default();
    receipt
        .record_tensor(
            TensorRole::F32State,
            TensorStorage::Parsed(GgmlType::F32),
            16,
        )
        .unwrap();
    receipt
        .record_tensor(
            TensorRole::F32Matrix,
            TensorStorage::Parsed(GgmlType::F32),
            64,
        )
        .unwrap();
    assert_eq!(receipt.required_tensor_count, 2);
    assert_eq!(receipt.matrix_tensor_count, 1);
    assert_eq!(receipt.matrix_bytes, 64);
}
