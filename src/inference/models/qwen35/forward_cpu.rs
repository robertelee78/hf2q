    ...
    pub fn forward_cpu(&self, tokens: &[u32], positions: &[[i32; 4]]) -> Result<Vec<f32>> {
        ...
        for layer in &self.layers {
            ...
            let ffn_out = match &layer.ffn() {
                ...
                // Native scalar/quantized variants retain Metal buffers rather
                // than a CPU F32 shadow. CPU-only execution is intentionally
                // limited to explicit reference fixtures.
                Qwen35FfnWeights::DenseNative(_) | Qwen35FfnWeights::DenseQ(_) => {
                    return Err(anyhow!(
                        "forward_cpu does not support native dense FFN buffers; \
                         use forward_gpu instead"
                    ));
                }
