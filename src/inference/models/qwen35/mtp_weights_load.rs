...
use super::gpu_full_attn::FullAttnQGateWeightsGpu;
...
use super::weight_loader::{
    ...
    load_moe_ffn_quantized, load_native_projection, map_f32_state_with_residency,
    qwen35_moe_expert_type_supported, validate_native_projection_info,
    validate_native_row_projection_info, DenseFfnStorage,
};
use super::Qwen35Config;
use crate::serve::forward_mlx_shared::MlxQWeight;

fn ensure_shared_head_is_resolved_gguf_tensor(
    ...
    let [rows, cols] = info.shape.as_slice() else {
        bail!(
            "qwen35 MTP loader: resolved shared head '{}' must be rank 2, got {:?}",
            info.name,
            info.shape
        );
    };
    ensure!(
        supplied.affine.is_none()
            && supplied.info.ggml_dtype == info.ggml_type
            && supplied.info.rows == *rows
            && supplied.info.cols == *cols,
        "qwen35 MTP loader: supplied shared output head metadata does not match resolved GGUF tensor '{}'",
        info.name
    );
    let resolved = crate::serve::forward_mlx_shared::map_native_gguf_tensor_view(mapped, info)?;
    ensure!(
        supplied.buffer.contents_ptr() == resolved.contents_ptr()
            && supplied.buffer.byte_offset() == resolved.byte_offset()
            && supplied.buffer.data_byte_len() == info.byte_len
            && supplied.buffer.data_byte_len() == resolved.data_byte_len(),
        "qwen35 MTP loader: supplied shared output head is not the exact mapped byte range of resolved GGUF tensor '{}'",
        info.name
    );
    Ok(())
}
...
pub fn load_mtp_weights_if_present_with_shared_head(
    ...
    let enorm = load_norm_gpu(gguf, mapped, &format!("{nextn}.enorm.weight"), h, device)?;
    let hnorm = load_norm_gpu(gguf, mapped, &format!("{nextn}.hnorm.weight"), h, device)?;
    ...
    let shared_head_norm = load_norm_gpu(
        gguf,
        mapped,
        ...
        .mtp_use_dedicated_embeddings
    {
        ...
        if let Some(main) = main_output_head {
            let info = gguf.tensor_info(main_lm).ok_or_else(|| {
                anyhow::anyhow!(
                    "qwen35 MTP loader: supplied shared head has no resolved GGUF tensor {main_lm}"
                )
            })?;
            ensure_shared_head_is_resolved_gguf_tensor(mapped, info, main)?;
            (
                main.buffer.clone(),
...
pub(crate) fn validate_mtp_tensor_topology(gguf: &GgufFile, cfg: &Qwen35Config) -> Result<()> {
    ...
    let head_rows = if cfg.mtp_use_dedicated_embeddings {
        let info = gguf
            .tensor_info(&head)
            .ok_or_else(|| anyhow::anyhow!("required MTP tensor `{head}` is missing"))?;
        ensure!(
            info.shape.len() == 2,
        ...
        info.shape[0]
            ...
            gguf.tensor_info(&head).is_none(),
            "qwen35 MTP loader: mtp_use_dedicated_embeddings=False but `{head}` is present; refusing a silent shared-head replacement"
        );
    ...
    if has_dense {
        ...
        let shared_gate_name = format!("{p}.ffn_gate_inp_shexp.weight");
        let shared_gate = gguf.tensor_info(&shared_gate_name).ok_or_else(|| {
            anyhow::anyhow!("required MTP tensor `{shared_gate_name}` is missing")
        })?;
        validate_native_row_projection_info(&shared_gate_name, shared_gate, h)?;
...
fn load_mtp_attn(
    ...
    let d = cfg.head_dim as usize;
    let attn_norm = load_norm_gpu(gguf, mapped, &format!("{p}.attn_norm.weight"), h, device)?;
    let post_attn_norm = load_norm_gpu(
        gguf,
        mapped,
        &format!("{p}.post_attention_norm.weight"),
        h,
    ...
    Ok(MtpFullAttnWeightsGpu {
        ...
        wv,
        wv_ggml_type,
        attn_q_norm: load_norm_gpu(gguf, mapped, &format!("{p}.attn_q_norm.weight"), d, device)?,
        attn_k_norm: load_norm_gpu(gguf, mapped, &format!("{p}.attn_k_norm.weight"), d, device)?,
        wo,
        wo_ggml_type,
    })
}
...
fn load_norm_gpu(
    ...
    let buffer = map_f32_state_with_residency(gguf, mapped, name, device)
        .with_context(|| name.to_string())?;
    ensure!(
        buffer.element_count() == len,
        "{name} length {} != {len}",
