...
use super::profile::{KernelTypeProfile, TokenProfile, merge_profiles};
use crate::debug::{INVESTIGATION_ENV, dumps};
...
use crate::serve::forward_mlx_shared::{
    RmsNormPerHeadArgs, dispatch_qmatmul, dispatch_rms_norm_unit_perhead,
};
...
impl MlxModelWeights {
    ...
    pub(crate) fn forward_decode_impl(
        ...
        // declared head representation, with no synthesized dense oracle.
        let gpu_top1: u32 = {
            let idx: &[u32] = self
                .activations
                .argmax_index
...
mod g4_cfa3_tests {
    ...
    fn mk_tiny_g4_model(
        ...
        let activations = MlxActivationBuffers {
            ...
            argmax_params: alloc_placeholder_u32(device),
