...
impl MlxModelWeights {
    ...
    /// Run the full Gemma 4 model in tree-verify mode (ADR-037/ADR-038 Step 4).
    ///
    /// Processes all `tree_tokens` in a single batched forward pass, collecting
    /// hidden states at `hidden_collector.target_layer_ids()` for EAGLE-3,
    /// and returning `[tree_seq_len, vocab_size]` F32 logits.
    pub fn forward_tree_verify_gpu(
        &self,
        tree_tokens: &[u32],
        tree_mask: &[f32],
        tree_positions_flat: &[u32],
        prefix_len: usize,
        kv_tree_caches: &mut Vec<(MlxBuffer, MlxBuffer)>,
        hidden_collector: &mut crate::inference::spec_decode::eagle3::multi_layer_hidden::Eagle3HiddenCollector,
        freq_factors_buf: &MlxBuffer,
    ) -> Result<Vec<f32>> {
        use anyhow::ensure;
        use crate::serve::config::LayerType;
        use super::gpu_full_attn::{
            Gemma4TreeVerifyLayerShape, Gemma4TreeVerifyFullLayerShapeQ,
            gemma4_tree_verify_full_layer_q, nkv_capacity_divisor,
        };

        if tree_tokens.is_empty() {
            return Err(anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: tree_tokens must be non-empty"));
        }
        let tree_seq_len = tree_tokens.len();
        let mask_stride = prefix_len
            .checked_add(tree_seq_len)
            .ok_or_else(|| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: prefix_len + tree_seq_len overflow"))?;
        ensure!(
            tree_mask.len() == tree_seq_len * mask_stride,
            "MlxModelWeights::forward_tree_verify_gpu: tree_mask len {} != tree_seq_len({}) * mask_stride({})",
            tree_mask.len(), tree_seq_len, mask_stride
        );
        ensure!(
            tree_positions_flat.len() == tree_seq_len,
            "MlxModelWeights::forward_tree_verify_gpu: tree_positions len {} != tree_seq_len {}",
            tree_positions_flat.len(), tree_seq_len
        );
        ensure!(
            kv_tree_caches.len() == self.layers.len(),
            "MlxModelWeights::forward_tree_verify_gpu: kv_tree_caches len {} != num_layers {}",
            kv_tree_caches.len(), self.layers.len()
        );
        ensure!(
            hidden_collector.seq_len() == tree_seq_len,
            "MlxModelWeights::forward_tree_verify_gpu: collector seq_len {} != tree_seq_len {}",
            hidden_collector.seq_len(), tree_seq_len
        );
        ensure!(
            hidden_collector.hidden_size() == self.hidden_size,
            "MlxModelWeights::forward_tree_verify_gpu: collector hidden_size {} != model {}",
            hidden_collector.hidden_size(), self.hidden_size
        );
        ...
        let device = MlxDevice::new()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: MlxDevice::new: {e}"))?;
        ...
        let hidden_bytes = tree_seq_len * hs * std::mem::size_of::<f32>();
        let mut hidden = device
            .alloc_buffer(hidden_bytes, mlx_native::DType::F32, vec![tree_seq_len, hs])
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: alloc hidden: {e}"))?;

        let token_ids_bytes = tree_seq_len * std::mem::size_of::<u32>();
        let mut token_ids_buf = device
            .alloc_buffer(token_ids_bytes, mlx_native::DType::U32, vec![tree_seq_len])
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: alloc token_ids: {e}"))?;
        token_ids_buf
            .as_mut_slice::<u32>()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: token_ids slice: {e}"))?
            .copy_from_slice(tree_tokens);

        let mut embed_enc = device
            .command_encoder()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: embed enc: {e}"))?;
        mlx_native::ops::elementwise::embedding_gather_scale_batch_f32(
            &mut embed_enc, &mut registry, device.metal_device(),
            &self.embed_weight,
            &token_ids_buf,
            &hidden,
            hs,
        ...
        .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: embed_tokens: {e}"))?;
        embed_enc.commit_and_wait()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: embed commit: {e}"))?;

        let mask_bytes = tree_mask.len() * std::mem::size_of::<f32>();
        let mut tree_mask_buf = device
            .alloc_buffer(mask_bytes, mlx_native::DType::F32, vec![tree_seq_len, mask_stride])
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: alloc tree_mask: {e}"))?;
        tree_mask_buf
            .as_mut_slice::<f32>()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: tree_mask slice: {e}"))?
            .copy_from_slice(tree_mask);

        let pos_bytes = tree_positions_flat.len() * std::mem::size_of::<u32>();
        let mut tree_pos_buf = device
            .alloc_buffer(pos_bytes, mlx_native::DType::U32, vec![tree_seq_len])
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: alloc tree_pos: {e}"))?;
        tree_pos_buf
            .as_mut_slice::<u32>()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: tree_pos slice: {e}"))?
            .copy_from_slice(tree_positions_flat);

        for (layer_idx, layer_weights) in self.layers.iter().enumerate() {
            let is_full = layer_weights.layer_type == LayerType::Full;
            let head_dim = layer_weights.head_dim as u32;
            let num_kv_heads = layer_weights.num_kv_heads as u32;
            let rope_theta = if is_full {
                self.rope_theta_global
            } else {
                self.rope_theta_sliding
            };
            let ff_opt = if is_full { Some(freq_factors_buf) } else { None };
            ...
            let shape = Gemma4TreeVerifyFullLayerShapeQ {
                attn: Gemma4TreeVerifyLayerShape {
                    ...
                    kv_capacity: kv_tree_caches[layer_idx].0
                        .byte_len()
                        .checked_div(nkv_capacity_divisor(num_kv_heads as usize, head_dim as usize))
                        .unwrap_or(0) as u32,
                    mask_stride: mask_stride as u32,
                    rms_norm_eps,
                    rope_theta,
                    freq_factors_present: is_full,
                },
            ...
            let enc = device
                .command_encoder()
                .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: enc L{layer_idx}: {e}"))?;
            let (k_cache, v_cache) = &mut kv_tree_caches[layer_idx];
            hidden = gemma4_tree_verify_full_layer_q(
                enc, &device, &mut registry,
                &hidden, &tree_mask_buf, &tree_pos_buf,
                k_cache, v_cache,
                layer_weights, ff_opt,
                shape,
            )
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: layer {layer_idx}: {e}"))?;

            if let Some(capture_idx) = hidden_collector.capture_index_for(layer_idx) {
                let slab: Vec<f32> = hidden
                    .as_slice::<f32>()
                    .map_err(|e| anyhow::anyhow!("download capture L{layer_idx}: {e}"))?
                    .to_vec();
                hidden_collector
                    .write_layer_slab(capture_idx, &slab)
                    .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: write capture L{layer_idx}: {e}"))?;
            }
        }

        ensure!(
            hidden_collector.is_complete(),
            "MlxModelWeights::forward_tree_verify_gpu: hidden collector incomplete after {} layers",
            self.layers.len()
        );

        let mut head_enc = device
            .command_encoder()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: head enc: {e}"))?;

        let normed_bytes = tree_seq_len * hs * std::mem::size_of::<f32>();
        let final_normed = device
            .alloc_buffer(normed_bytes, mlx_native::DType::F32, vec![tree_seq_len, hs])
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: alloc final_normed: {e}"))?;
        let mut rms_params_buf = device
            .alloc_buffer(8, mlx_native::DType::F32, vec![2])
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: alloc rms_params: {e}"))?;
        {
            let s = rms_params_buf
                .as_mut_slice::<f32>()
                .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: rms_params slice: {e}"))?;
            s[0] = rms_norm_eps;
            s[1] = hs as f32;
        }
        mlx_native::ops::rms_norm::dispatch_rms_norm(
            &mut head_enc, &mut registry, device.metal_device(),
            &hidden,
            &self.final_norm,
            &final_normed,
            &rms_params_buf,
        ...
        .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: final_norm: {e}"))?;

        head_enc.memory_barrier();

        let lm_head_buf = match &self.lm_head_q6k {
            Some(q6k) => &q6k.buffer,
            None => return Err(anyhow::anyhow!(
                "MlxModelWeights::forward_tree_verify_gpu: lm_head_q6k not loaded (required for tree-verify)"
            )),
        };
        let logits_buf = crate::inference::models::qwen35::gpu_full_attn::apply_linear_projection_f32(
            &mut head_enc, &mut registry, &device,
            &final_normed, lm_head_buf,
            tree_seq_len as u32, hs as u32, vocab_size as u32,
        )
        .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: lm_head: {e}"))?;

        head_enc.commit_and_wait()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: head commit: {e}"))?;

        logits_buf
            .as_slice::<f32>()
            .map_err(|e| anyhow::anyhow!("MlxModelWeights::forward_tree_verify_gpu: logits as_slice: {e}"))
            .map(|s| s.to_vec())
    }
