...
impl MlxModelWeights {
    ...
    pub fn load_from_gguf(
        ...
        if let Ok(ref mut w) = result {
            w.verify_native_matrix_storage()?;
...
fn alloc_activation_buffers(
    ...
    // Argmax's shader ABI takes one u32 vocabulary length, including dtype
    // metadata: current mlx-native validates the buffer before dispatch.
