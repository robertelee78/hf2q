...
fn cmd_generate_qwen35(args: cli::GenerateArgs, gguf: mlx_native::gguf::GgufFile) -> Result<()> {
    ...
    // ADR-013 coherence fix (2026-05-01): the HF `tokenizer.json` shipped
    // alongside abliterated/apex GGUFs declares an `added_tokens` vocabulary
    // that exceeds the GGUF's actual `tokenizer.ggml.tokens` count and the
    // model's `token_embd.weight` row count. Concretely the apex35B-A3B GGUF
    // ships 248,044 rows of `token_embd` but the HF tokenizer can emit IDs up
    // through 248,319 (e.g. `<|im_start|>`=248045, `<|im_end|>`=248046,
    // `<think>`=248068). `weight_loader.rs:699-707` pads the trailing rows
    // with ZEROS, so any chat-template-rendered prompt that contains one of
    // those special markers gets an all-zero embedding at that position,
    // decapitating the residual stream and producing deterministic prompt-
    // repetition gibberish ("How to make bread?\nHow to noob bread?\n...").
    //
    // llama.cpp avoids this by tokenizing the literal text via the GGUF's
    // own vocab — `<|im_start|>` then splits to the byte-level chars
    // `<`, `|`, `im`, `_start`, `|`, `>` — all of which are in-range. We
    // mirror that here: walk the encoding, find any token whose id is
    // outside the model's physical vocab, and re-encode the underlying
    // substring character-by-character so the BPE never matches the
    // out-of-range special token. This keeps tokenization byte-equivalent
    // to llama.cpp on apex/dwq fixtures whose HF tokenizer overshoots the
    // emitted vocab, without forking the tokenizer crate.
    let physical_vocab = model.cfg.vocab_size;
    let oob_count = prompt_tokens.iter().filter(|&&t| t >= physical_vocab).count();
    if oob_count > 0 {
        let offsets = encoding.get_offsets();
        let original_bytes = prompt_text.as_bytes();
        let mut fixed: Vec<u32> = Vec::with_capacity(prompt_tokens.len() + oob_count * 8);
        for (i, &tok_id) in prompt_tokens.iter().enumerate() {
            if tok_id < physical_vocab {
                fixed.push(tok_id);
                continue;
            }
            let (start, end) = offsets[i];
            // Slice the original prompt text via UTF-8 byte offsets reported
            // by the HF tokenizer. Encode each Unicode scalar individually so
            // the BPE merges never reconstruct the special token.
            let oob_text =
                std::str::from_utf8(&original_bytes[start..end]).map_err(|e| {
                    anyhow::anyhow!(
                        "OOB-token offset {start}..{end} not UTF-8 boundary: {e}"
                    )
                })?;
            for ch in oob_text.chars() {
                let mut buf = [0u8; 4];
                let ch_str: &str = ch.encode_utf8(&mut buf);
                let ch_enc = tokenizer
                    .encode(ch_str, false)
                    .map_err(|e| anyhow::anyhow!("char-fallback encode {ch:?}: {e}"))?;
                for &cid in ch_enc.get_ids() {
                    if cid >= physical_vocab {
                        // Should be impossible (single-char tokenization
                        // never matches an added_token whose content is a
                        // multi-char marker), but be defensive: drop rather
                        // than poison the residual stream.
                        tracing::warn!(
                            "char-fallback produced still-OOB token {cid} \
                             for char {ch:?}; dropping"
                        );
                        continue;
                    }
                    fixed.push(cid);
        ...
        tracing::warn!(
            "Replaced {} out-of-vocab token(s) (id >= {}) with char-level \
             fallback (HF tokenizer.json vocab {} > GGUF token_embd rows {}). \
             Prompt {} -> {} tokens.",
            oob_count,
            physical_vocab,
