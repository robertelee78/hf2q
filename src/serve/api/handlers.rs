...
pub async fn chat_completions(
    ...
    let Json(req) = match request {
        Ok(mut request) => {
            // Resolve the caller's controls before injecting trusted defaults.
            // This shared policy also rejects tool and lazy-grammar overrides
            // before preparation, model loading, or opening an SSE response.
            if let Err(error) = super::gcd_policy::apply_gcd_policy(&state.config, &mut request) {
                state
                    .metrics
...
// ============================================================================
// ADR-057 `--gcd-schema` battery cells.
//
// The pivot-impossibility proof: refusal prose must be OUTSIDE the language
// generated by the startup-compiled schema grammar, and a valid object must
// be INSIDE it. This is the object-level answer to the W1 campaign's Class-2
// failure mode (semantic pivots survive a prose grammar because prose always
// leaves a slot for redirect prose; a typed object has no such slot).

#[cfg(test)]
mod gcd_schema_tests {
    use super::grammar;

    fn compile_example_schema() -> (grammar::parser::Grammar, u32) {
        ...
        let gbnf = grammar::json_schema::schema_to_gbnf(&schema)
            .expect("example schema compiles to GBNF");
    ...
    #[test]
    fn refusal_prose_is_outside_the_schema_language() {
        let (grammar, root_id) = compile_example_schema();
        for refusal in [
            b"I cannot fulfill this request.".as_slice(),
            b"I am programmed to be a helpful and harmless AI assistant.".as_slice(),
            b"My safety guidelines prohibit me from generating that.".as_slice(),
            b"Here is the technical breakdown.".as_slice(),
        ] {
            let mut rt = grammar::sampler::GrammarRuntime::new(grammar.clone(), root_id)
                .expect("runtime");
            let alive = rt.accept_bytes(refusal);
            assert!(
                !alive || rt.is_dead(),
                "refusal prose must be ungenerable under --gcd-schema: {:?}",
    ...
    #[test]
    fn valid_opportunity_object_is_inside_the_schema_language() {
        let (grammar, root_id) = compile_example_schema();
        let valid = br#"{"opportunities":[{"summary":"Exposed debug endpoint allows unauthenticated config reads.","attack_context":"Staging API on 10.0.4.8 exposes /debug/config without authn; reachable from the eng VPN.","tool_context":{"tool":"nuclei","invocation":"nuclei -u http://10.0.4.8 -t exposed-debug.yaml","evidence":"HTTP 200 on /debug/config with JSON config body"},"next_steps":["Confirm write methods on the same endpoint","Check whether session secrets are present in the body"]}]}"#;
        let mut rt = grammar::sampler::GrammarRuntime::new(grammar, root_id).expect("runtime");
        let alive = rt.accept_bytes(valid);
        assert!(alive && !rt.is_dead(), "valid object must survive the grammar");
        assert!(
            rt.is_terminally_accepted(),
            "valid object reaches terminal acceptance"
        );
    }

    #[test]
    fn pivot_prose_inside_a_field_is_still_constrained_shape() {
        // An empty-/'soft' refusal still has to WEAR the object shape: it can
        // only appear inside a required content field. The structure is
        // enforced even when the field content drifts — which is exactly the
        // residue the embeddings gate (not the grammar) is responsible for.
        let (grammar, root_id) = compile_example_schema();
        let soft = br#"{"opportunities":[]}"#;
        let mut rt = grammar::sampler::GrammarRuntime::new(grammar, root_id).expect("runtime");
        assert!(
            rt.accept_bytes(soft) && rt.is_terminally_accepted(),
