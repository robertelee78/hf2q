...
impl ArtifactCatalogCoordinator {
    ...
    pub fn resolve(&self, candidate_id: &str) -> Result<HubGgufArtifact, CatalogError> {
        let now = Instant::now();
        let mut state = self.state.lock().map_err(|_| CatalogError::State)?;
        let candidate = state
            .candidates
...
mod tests {
    ...
    fn only_server_selectable_candidates_receive_opaque_ids() {
        ...
        assert_eq!(
            coordinator.resolve(selected).unwrap().sha256,
            "b".repeat(64)
        );
    ...
    fn earlier_candidate_remains_bound_to_its_immutable_catalog_revision() {
        ...
        let selected = coordinator.resolve(first_id).unwrap();
