...
impl Deepseek4Session {
    ...
    pub(super) fn recover_after_cancellation(&mut self) -> Result<()> {
        ...
        self.restore_committed_anchor(recovery_index, "cancellation rollback")?;
    ...
    fn restore_committed_anchor(&mut self, index: usize, cause: &'static str) -> Result<()> {
        let (tokens, token_count) = {
            let anchor = self
                .anchor_store
                .committed(index)
                .context("DeepSeek-V4 committed anchor disappeared before restore")?;
            (anchor.prompt_tokens.to_vec(), anchor.prompt_tokens.len())
        };
        let restore = {
            let anchor = self
                .anchor_store
        ...
        if let Err(error) = restore {
            return self.fail_closed_restore(error, cause);
        }
        ...
        let prune = match anchor_store_policy::prune_after_restore(
            ...
            Ok(prune) => prune,
            Err(error) => return self.fail_closed_restore(error, cause),
        };
        if self.cache.position() != self.committed_tokens.len() {
        ...
        anchor_store_policy::record_restore_hit(token_count, prune);
        Ok(())
    }
    ...
    fn fail_closed_restore(
        ...
        anchor_store_policy::record_restore_miss();
        let clear = anchor_store_policy::clear_all(
            &mut self.anchor_store,
            &self.anchor_budget,
            "failed-restore",
...
impl Deepseek4LoadedModel {
    ...
    fn begin_resumable_cold_prefill(
        ...
        anchor_store_policy::record_restore_miss();
        self.reset_live_cache_and_clear_anchors("cold-resumable-prefill")?;
    ...
    fn begin_resumable_cached_prefill(
        ...
        let (cached_tokens, cache_action) = match reuse {
            ...
            PrefixReuse::RecoveryAnchor { count, index } => {
                self.restore_committed_anchor(index, "cached resumable restore")?;
                (
                    count,
                    if cache_grew {
    ...
    fn prefill_suffix(
        ...
        let (cached_tokens, cache_action) = match reuse {
            PrefixReuse::Live(count) => (count, if cache_grew { "grow-live" } else { "live" }),
            PrefixReuse::RecoveryAnchor { count, index } => {
                self.restore_committed_anchor(index, "prompt-boundary restore")?;
                (
                    count,
                    if cache_grew {
                        "grow-recovery-anchor"
            ...
            PrefixReuse::Reset => {
                anchor_store_policy::record_restore_miss();
                self.reset_live_cache_and_clear_anchors("cold-prefill")?;
...
mod tests {
    ...
    use super::{
        ...
        select_prefix_reuse_from_store, should_retain_decode_scratch,
    ...
    fn deepseek_anchor_lineage_rewind_rejects_old_descendants() {
        ...
        session
            .restore_committed_anchor(index, "lineage regression")
            .unwrap();
    ...
    fn pending_anchor_is_affinity_invisible_and_cancellation_preserves_committed_set() {
        ...
        session.recover_after_cancellation().unwrap();
        assert_eq!(
    ...
    fn late_restore_failure_hard_resets_and_clears_the_whole_store() {
        ...
        let error = session
            .restore_committed_anchor(0, "injected late-layer restore")
            .unwrap_err();
    ...
    fn cold_reset_and_poison_both_invalidate_anchor_authority() {
        ...
        poisoned.recover_after_cancellation().unwrap();
