        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            ...
            // ================================================================
            {
                ...
                if is_sliding {
                    ...
                    // ADR-011 Phase 2 Wave 4: Gemma 4's 5 global layers
                    // (head_dim=512) continue on the bf16-auto-dispatched
                    // `s.sdpa` path rather than flash_attn_prefill_d512.
                    //
                    // Why: the D=512 port (flash_attn_prefill_d512.metal,
                    // mirrors llama.cpp's kernel_flash_attn_ext_impl with
                    // NSG=8 + threadgroup-half O accumulator) passes its
                    // unit tests at atol=5e-3, rtol=2e-2 vs sdpa_reference_f32
                    // but the half-precision O accumulation drift compounds
                    // across Gemma 4's 5 global layers enough to flip token
                    // argmaxes in sourdough_gate.sh (verified via git bisect:
                    // commit 99847da introduced "managing your starter" →
                    // "the starter" divergence at byte 127). sdpa_bf16 keeps
                    // its O accumulator in f32 registers, avoiding that
                    // drift. Sliding layers (D=256) use flash_attn_prefill
                    // and DO stay byte-identical, so the kernel design is
                    // sound — the drift is specific to D=512's threadgroup-
                    // half-O path.
                    //
                    // Follow-up (logged): investigate a D=512 variant with
                    // f32 O (requires tile-geometry adjustments to fit the
                    // 32 KB threadgroup-memory budget) or otherwise bound
                    // the O precision loss.
