...
//!
//! ---------------------------------------------------------------------------
//! hf2q vendor patch — ADR-005 1bNEW.20.FIX (2026-04-11)
//!
//! This file is a vendored copy of candle-nn-0.10.2/src/ops.rs with a single
//! surgical bug fix applied to `Sdpa::forward`. The upstream crate (both
//! released 0.10.2 on crates.io and HEAD on github.com/huggingface/candle as
//! of 2026-04-11) passes `layout.start_offset()` — an **element count** —
//! directly into `candle_metal_kernels::call_sdpa_*` at nine sites (three
//! per dispatch path: q/k/v, across `call_sdpa_vector_2pass`,
//! `call_sdpa_vector`, `call_sdpa_full`). Those kernel entry points in turn
//! forward the value through `set_params!((buffer, offset), ...)` to
//! `metal::ComputeCommandEncoder::set_buffer(index, buffer, offset)`, which
//! calls Metal's `setBuffer:offset:atIndex:` — where the offset is
//! interpreted as **bytes**.
//!
//! Every other Metal op in candle does the conversion correctly, e.g.
//! `candle-nn/src/ops.rs:154, 419, 611, 613, 856, 858, 860` (rms_norm,
//! layer_norm, rotary_emb, etc.), and `candle-core/src/metal_backend/mod.rs:
//! 1060, 1062, 1209, 1210, 1702, 1705, 1783` (matmul, conv, binary). The SDPA
//! dispatch in candle-nn is the lone exception, and the bug is latent across
//! all candle history back to the introduction of `ops.rs` in cb02b38
//! (2025-03-26) because every other candle consumer feeds SDPA fresh
//! contiguous activations with `start_offset == 0`.
//!
//! hf2q exposes the bug through ADR-005 1bNEW.20, which ports llama.cpp's
//! in-place KV cache append pattern. Once `current_len > sliding_window` on a
//! Gemma-4 sliding-attention layer, the narrow view returned from
//! `KvCache::append_in_place` has `start_offset = visible_start * head_dim`
//! (in elements). candle-nn then passes that element count as a byte offset
//! to Metal, the sdpa_vector kernel binds q/k/v from the wrong memory
//! location (for f32, 1/4 of the intended offset), and the model produces
//! plausible-looking but semantically incoherent tokens from that point
//! onward. Reproduces on a 20k-token Gemma-4 generate at exactly the
//! sliding-window boundary (~token 1024).
//!
//! Fix: multiply `q_l.start_offset()`, `k_l.start_offset()`,
//! `v_l.start_offset()` by `q.dtype().size_in_bytes()` at all nine sites
//! inside `Sdpa::forward`, matching the rest of candle's Metal op
//! conventions. `q`, `k`, and `v` are asserted to share the same dtype
//! immediately above each site, so a single `dtype_size` binding per branch
//! covers all three offsets.
//!
//! Upstream-bug audit (2026-04-11):
//!   - crates.io `candle-nn-0.10.2`: buggy at lines 1114/1117/1121 (2pass),
//!     1139/1142/1146 (vector), 1203/1207/1211 (full).
//!   - github.com/huggingface/candle HEAD (200 commits deep): same identical
//!     lines, still buggy.
//!   - No open PR or issue touches these lines.
//!
//! Follow-up tracked in ADR-005 1bNEW.20.FIX: upstream PR to huggingface/candle
//! proposing the same fix so the vendored patch can be dropped when candle
//! next releases.
//! ---------------------------------------------------------------------------
...
impl candle::CustomOp3 for Sdpa {
    ...
    fn metal_fwd(
        ...
        // hf2q vendor patch — ADR-005 1bNEW.20.FIX.
        // q/k/v share dtype (asserted above). `start_offset()` on Layout
        // is in **elements**; Metal's `setBuffer:offset:atIndex:` wants
        // **bytes**. Every candle-nn/candle-core Metal op multiplies by
        // `size_in_bytes()` at dispatch time except the three SDPA
        // dispatch paths below. This binding is consumed at every
        // `start_offset() * dtype_size` site inside the `supports_sdpa_*`
        // branches. See the file header for the full bug analysis.
        let dtype_size = q.dtype().size_in_bytes();
        ...
        if supports_sdpa_vector {
            ...
            if k_seq >= TWO_PASS_K_THRESHOLD {
                ...
                candle_metal_kernels::call_sdpa_vector_2pass(
                    ...
                    &encoder,
                    q.device().kernels(),
                    // hf2q vendor patch: element-count → byte-count.
                    q_l.start_offset() * dtype_size,
                    q_l.dims(),
                    q.buffer(),
                    k_l.start_offset() * dtype_size,
                    k_l.dims(),
                    k_l.stride(),
                    k.buffer(),
                    v_l.start_offset() * dtype_size,
                    v_l.stride(),
                    v.buffer(),
                    &output,
                    &intermediate,
