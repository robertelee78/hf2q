...
- **Verdict.** Feasible at "Phase B-dense.1-class" scope (~1,060 LOC additive — Codex-revised total per §10.7; was ~930 pre-audit), 7 numbered implementation iters (1→7) plus iter-0 dossier and iter-2.5 ownership refactor per §10.9 = 9 total phases; ~2-3 weeks. **iter-1 (LcpRegistry standalone) LANDED at commit `9beb906`** (358 LOC src + 369 LOC tests; 13 unit tests). **iter-2 (engine probe + metrics, partial-prefill OFF) LANDED at commit `d17163b`**. **iter-2.5 (Strategy A ownership refactor) LANDED at commit `70fca89`** (R-C4 sourdough PASS). **iter-3 (LCP partial-prefill resume — env-gated default-OFF) IMPLEMENTATION LANDED at commit `a20606a`** — 623 LOC across 5 files; new `forward_prefill_with_soft_tokens_resume` entry point with `restored_lcp: Option<usize>` param; `LcpRegistry::take_prefix` consuming variant; engine wires probe→take_prefix→install→prefill on `HF2Q_KV_LCP_RESUME=1` + `HF2Q_USE_DENSE=1`; default OFF preserves byte-identical behavior (R-C4 sourdough still PASS). **iter-3 PENDING:** K<N partial-prefix byte-identity falsifier test (§10.4 corrected) + Codex Phase-2b audit (§4.3 mandate) before env-flip to default-ON. iters 4 / 5 / 6 / 7 PENDING.
...
| 5 | **R-C4 LCP variant gate** — extend sourdough harness with K ∈ {0, N/4, N/2, 3N/4, N-1} | ~120 tests | H1 byte-identity at 5 fractions. Falsifier: any K yields decode-stream divergence vs full-prefill baseline (KILL CRITERION — falsifies Phase E.a entirely) | ⏸ PENDING |
