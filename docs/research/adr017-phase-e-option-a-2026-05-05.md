...
| 2 | `engine.rs` Request plumbing + worker handler thread-through (LCP path STILL gated; full-equality `PromptCache` still wins on K=N) | ~155 src + ~30 tests | Integration: real `/v1/chat/completions` 2-turn; second turn's `cached_tokens` reflects LCP value at server side **but partial-resume path is OFF**. Falsifier: `cached_tokens=0` despite shared prefix | ⏸ PENDING |
