...
- **Verdict.** Feasible at "Phase B-dense.1-class" scope. iter-1 `9beb906`, iter-2 `d17163b`, iter-2.5 `70fca89`, iter-3 `a20606a` + audits `1006ab0` + `22e456a`, iter-3.5 `7ffb563`, iter-3.5c + iter-5 `31d04b3`, iter-3.5d + iter-6 `04e6955`. **iter-7 (sliding-window prefill-wrap guard correctness + Qwen3.5 hybrid deferral marker) LANDED** — long-prompt byte-identity verified: server A (HF2Q_KV_LCP_RESUME=1, Q+P both > sliding_window=1024 tokens) produced 46 bytes byte-identical to server B (control); engine guard skipped store on both turns (`lcp_lookups_total=2`, `lcp_detected_total=0`). Iter-7 surfaced + fixed a **real latent bug** in `forward_prefill.rs:1881` snapshot path: per-head memcpy used `seq_len*hd*elem` but iter-3.5d snap_cap = sw → dst slice overrun when `seq_len > sw`. Fix: gate snapshot creation on `!any_sliding || seq_len <= sw` (matches engine guard at `engine.rs:4516`); set `snap_cap = sw.max(seq_len + max_decode_tokens)` so pure-dense long prompts fit. Conservative pass criteria all hold; dossier targets miss on small prompts where request overhead dominates. **iter-4 (env-flip default-ON) gated on final Codex Phase-2b audit only.** iter-3.5c restriction (`prompt_len ≤ sliding_window` for sliding-layer models) lifts at iter-3.6 via mid-prefill snapshot.
...
| 6 | **R-P7 multi-turn-chat speedup bench + /cfa fan-out shared-prefix bench** | ~80 tests | H2/H3 speedup: 5-turn chat with 95 % shared prefix; turn 2+ TTFT ≤ 0.10 × turn 1 TTFT. **Plus**: /cfa-shape fan-out fixture (4 workers, shared `[SYSTEM][QUEEN_SPEC]` prefix, diverging suffixes) — aggregate prefill ≤ 1.25× single-agent (matches the synthetic R-P6 ship-gate but for the natural workload shape). Falsifier on either: TTFT ratio worse than spec | ✅ LANDED at `04e6955` (with iter-3.5d snapshot capacity headroom fix). **R-P7**: turn 2+ ratio 0.40-0.42× turn 0 (CONSERVATIVE pass < 1.0× holds; dossier 0.10× misses on small prompts where request overhead dominates). **/cfa fan-out**: workers 1-3 cached at 0.23× cold; aggregate 1.71× single-worker (CONSERVATIVE pass < 2.0× holds; dossier 1.25× misses for same reason). Iter-3.5d surfaced + fixed: snapshot per-layer capacity for global layers was sized to current prompt only — multi-turn prompts grew past it → engine probe per-layer cap-check failed → resume engaged but fell through to fresh prefill. Fix: size snapshot buffer with capacity = `sliding_window` (matching sliding layers), strided per-head memcpy when dimensions differ. |
| 7 | Sliding-window LCP > sw edge cases + Qwen3.5 hybrid deferral marker | ~340 LOC tests + ~50 LOC src fix + doc | Edge: prompt with prompt_len=2000+ tokens against sliding_window=1024; engine + forward_prefill guards both skip; decoded bytes byte-identical to control fresh prefill. Falsifier: any divergence at LCP > sw boundary OR `lcp_detected_total > 0` (would mean store happened despite ring-wrap corruption). | ✅ LANDED. Test `iter7_prefill_wrap_guard_long_prompt_byte_identity` PASS on Gemma 4 26B-DWQ: server A `lcp_lookups_total Δ=2`, `lcp_detected_total Δ=0`; A vs B 46 bytes byte-identical. **iter-7 surfaced + fixed a real latent bug**: `forward_prefill.rs:1881` per-head memcpy assumed `seq_len ≤ snap_cap = sw`, but pure-dense (global-only) models have engine_guard `prefill_safe = !has_sliding_layer || ...` ⇒ pure-dense prompts with `seq_len > sw` reached the per-head copy with `copy_len_per_head > snap_cap*hd*elem` ⇒ dst overrun panic. **Fix in same iter-7**: (a) gate snapshot on `!any_sliding_layer || seq_len <= sw` matching engine guard; (b) `snap_cap = sw.max(seq_len + max_decode_tokens)` to fit long pure-dense prompts. Test `iter7_qwen35_hybrid_lcp_deferred_marker` documents the structural Qwen3.5 deferral (lcp_registry field on GemmaLoadedModel only at engine.rs:1102; Qwen35 worker arm returns Err at engine.rs:3152). 2804/0/13 bin tests + iter-3 + iter-5 (5/5) + iter-6 (R-P7 + /cfa fan-out) all PASS post-fix. |
| 8 | Streaming-path byte-identity falsifier (Codex audit recommendation closure) | ~250 LOC tests | `iter8_streaming_path_byte_identity` mirrors iter-3 falsifier with `stream=true`. Server A streaming resume engages via `engine.rs:6158` probe + `engine.rs:6182` resume gate + `engine.rs:6238` install. Server B streaming control fresh-prefills. Falsifier: streamed bytes diverge OR `lcp_detected_total < 1` (probe didn't engage). | ✅ LANDED. Test PASS on Gemma 4 26B-DWQ: server A 144 bytes streamed (resume engaged, `lcp_detected_total=1`); server B 144 bytes streamed (control); BYTE-IDENTICAL. Confirms streaming + non-streaming paths converge (matches iter-3's 144 bytes). Closes Codex Phase-2b audit recommendation re streaming store path coverage. |

**Promotion criterion** (env-gate flip from default-OFF to default-ON): all of iter-3 (R-C4 LCP byte-identity at K<N) ✅, iter-5 (R-C4 LCP 5-fraction sweep) ✅, iter-6 (R-P7 speedup + /cfa fan-out) ✅, iter-7 (sliding-wrap byte-identity + latent overrun fix) ✅, **iter-8 (streaming-path byte-identity) ✅** — all PASS as of 2026-05-05. **Codex Phase-2b read-only audit on iter-7 verdict: APPROVED-WITH-CAVEATS** (1 LOW: marker test as-is is optional and not promotion-blocking; matches design intent — see [`adr017-phase-e-a-iter7-codex-audit-2026-05-05.txt`](adr017-phase-e-a-iter7-codex-audit-2026-05-05.txt)). All 7 specific audit questions answered positively. **iter-8 closes Codex's audit recommendation re streaming store path coverage**: streaming-resume path produces byte-identical 144 bytes to streaming-control on Gemma 4 26B-DWQ (matches iter-3 non-streaming 144 bytes — confirms streaming + non-streaming converge). Final gate before iter-4 default-ON: **operator soak under `HF2Q_KV_LCP_RESUME=1`** (recommended ≥ 2 weeks before promotion).
