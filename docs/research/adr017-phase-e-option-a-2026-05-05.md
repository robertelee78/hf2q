...
- **Verdict.** Feasible at "Phase B-dense.1-class" scope. iter-1 `9beb906`, iter-2 `d17163b`, iter-2.5 `70fca89`, iter-3 `a20606a` + audits `1006ab0` + `22e456a`, iter-3.5 `7ffb563`, iter-3.5c + iter-5 `31d04b3`. **iter-3.5d (snapshot capacity headroom) + iter-6 (R-P7 multi-turn + /cfa fan-out speedup benches) LANDED at `04e6955`** — multi-turn turn 2+ at 0.40-0.42× turn 0; /cfa fan-out workers 1-3 at 0.23× cold; aggregate 1.71× single-worker. iter-3.5d surfaced+fixed snapshot-too-small bug (per-layer capacity was sized for current prompt; multi-turn prompts grew past it; resume engaged but fell through to fresh prefill). Conservative pass criteria all hold; dossier targets miss on small prompts where request overhead dominates. iters 4 / 7 PENDING. iter-4 (env-flip default-ON) gated on iter-7 + final Codex review. iter-3.5c restriction (`prompt_len ≤ sliding_window`) lifts at iter-3.6 via mid-prefill snapshot.
