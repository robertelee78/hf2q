...
- **Verdict.** Feasible at "Phase B-dense.1-class" scope (~930 LOC additive), 7 iters, ~2-3 weeks. iter-3 (the env-gated wholesale-reset modification) is the **highest-risk** iter and **requires** Codex Phase-2b audit per project memory `feedback_codex_review_catches_unified_memory_races`. Recommend env-gated rollout `HF2Q_KV_LCP_RESUME=1` default-OFF first; promote to default once R-C4 LCP variant + multi-turn-chat speedup gates pass at every fixture.
...
- **LOC estimate.** ~930 LOC (forward_prefill ~80, engine.rs ~150, kv_restore_gemma ~50, new `lcp_registry.rs` ~250, tests ~400) vs Phase B-dense.1's ~1500 LOC and Phase D scaffolding's 1722 LOC — closer to Phase B than Phase D.
...
| # | Iter | LOC | Falsifiable measurement |
|---|------|-----|--------------------------|
| 1 | **Read-only audit (this dossier)** | 0 src | This document; verifies Q1-Q7 acceptance + git status clean |
| 2 | `lcp_registry.rs` + `tokens` prefix-lookup unit tests | ~250 src + ~100 tests | Unit: registry returns correct K for token-prefix-equal sequences, rejects on fingerprint/tenant/params mismatch. Falsifier: any unit test fails |
| 3 | **HIGH-RISK** — `forward_prefill.rs` conditional reset under env-gate `HF2Q_KV_LCP_RESUME=1` (default OFF) + same-process round-trip test | ~80 src + ~50 tests | Same-process: prime cache with prompt P, then issue prompt P with LCP=full; baseline-vs-resumed decoded bytes byte-identical. Falsifier: any byte mismatch. **Codex Phase-2b audit REQUIRED here** (mirror `feedback_codex_review_catches_unified_memory_races` precedent) |
| 4 | `engine.rs` Request plumbing + worker handler thread-through | ~150 src + ~30 tests | Integration: real `/v1/chat/completions` 2-turn; second turn's `cached_tokens` reflects LCP value. Falsifier: `cached_tokens=0` despite shared prefix |
| 5 | **R-C4 LCP variant gate** — extend sourdough harness with K ∈ {0, N/4, N/2, 3N/4, N-1} | ~120 tests | H1 byte-identity at 5 fractions. Falsifier: any K yields decode-stream divergence vs full-prefill baseline (KILL CRITERION — falsifies Phase E.a entirely) |
| 6 | **R-P7 multi-turn-chat speedup bench** | ~80 tests | H2/H3 speedup: 5-turn chat with 95 % shared prefix; turn 2+ TTFT ≤ 0.10 × turn 1 TTFT. Falsifier: turn-2 TTFT > 0.10 × turn-1 TTFT after warmup (no win → not worth shipping) |
| 7 | Sliding-window LCP > sw edge cases + Qwen3.5 hybrid deferral note | ~50 tests + doc | Edge: prompt with LCP=4096 against sliding_window=1024; decoded bytes byte-identical to full prefill. Falsifier: any divergence at LCP > sw boundary |

**Promotion criterion** (env-gate flip from default-OFF to default-ON): all of iter-5 (R-C4 LCP), iter-6 (R-P7 speedup), iter-7 (sliding-wrap) PASS for at least 2 weeks of operator use under HF2Q_KV_LCP_RESUME=1.
