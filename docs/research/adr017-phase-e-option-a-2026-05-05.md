...
- **Verdict.** Feasible at "Phase B-dense.1-class" scope (~1,060 LOC additive — Codex-revised total per §10.7; was ~930 pre-audit), 7 numbered implementation iters (1→7) plus iter-0 dossier and iter-2.5 ownership refactor per §10.9 = 9 total phases; ~2-3 weeks. **iter-1 (LcpRegistry standalone) LANDED at `9beb906`**. **iter-2 (engine probe + metrics, partial-prefill OFF) LANDED at `d17163b`**. **iter-2.5 (Strategy A ownership refactor) LANDED at `70fca89`**. **iter-3 (LCP partial-prefill resume — env-gated default-OFF) LANDED across `a20606a` + `1006ab0` + `22e456a`** (Codex Phase-2b audit verdict APPROVED-WITH-CAVEATS over two rounds). **iter-3.5 (dtype invariant + end-of-prefill snapshot — lifts sliding-ring wrap restriction) LANDED at `7ffb563`**: DenseKvBuffers gains a `dtype` field populated at every alloc site + checked at probe sites (closes Codex round-1 LOW #4); MlxModelWeights gains `dense_kvs_snapshot_for_lcp` populated at end-of-prefill before decode mutates the live buffers — long-conversation prompts (`prompt_len + decode > sliding_window`) are now cacheable, the iter-3 v1 wrap guard is GONE. **R-C4 sourdough byte-identical PASS at default flags + iter-3 K<N falsifier PASS** post-snapshot. iters 4 / 5 / 6 / 7 PENDING. iter-4 (env-flip to default-ON) gated on iter-5 (R-C4-LCP 5-fraction sweep) + iter-6 (R-P7 multi-turn-chat speedup + /cfa fan-out bench).
