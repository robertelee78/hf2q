...
**Promotion criterion** (env-gate flip from default-OFF to default-ON): all of iter-3 (R-C4 LCP byte-identity at K<N) ✅, iter-5 (R-C4 LCP 5-fraction sweep) ✅, iter-6 (R-P7 speedup + /cfa fan-out) ✅, iter-7 (sliding-wrap byte-identity + latent overrun fix) ✅ — all PASS as of 2026-05-05. **Codex Phase-2b read-only audit on iter-7 verdict: APPROVED-WITH-CAVEATS** (1 LOW: marker test as-is is optional and not promotion-blocking; matches design intent — see [`adr017-phase-e-a-iter7-codex-audit-2026-05-05.txt`](adr017-phase-e-a-iter7-codex-audit-2026-05-05.txt)). All 7 specific audit questions answered positively (snapshot predicate sound; snap_cap formula sufficient; copy bounds proven; test non-vacuous; env-gate idempotent; no new concurrency risk; iter-3.6 forward-compatible). Final gate before iter-4 default-ON: **operator soak under `HF2Q_KV_LCP_RESUME=1`** (recommended ≥ 2 weeks before promotion; Codex audit recommends focus on both non-streaming and streaming store paths).
