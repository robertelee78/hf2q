...
**Effect on sourdough gate.** With ADR-012's auto-inject in place, every newly-converted qwen35 / qwen35moe GGUF carries the verbatim 7764-byte vendor Qwen3 ChatML template; the Gemma4 fallback never engages for these arches; sourdough's chat-template-side noise is eliminated. Existing GGUFs on disk are unaffected — they need re-conversion to pick up the metadata (tracked separately as task #6, NOT this CFA's scope per spec). For the on-disk set, operator can supply `--chat-template-file <vendor.jinja>` to bypass the missing GGUF metadata until re-conversion.

**Verification.** No new code in this ADR-013 worktree; the fix lives entirely in `src/backends/{chat_templates.rs,gguf.rs}` + `src/backends/mod.rs` + the new `src/backends/chat_templates/qwen3-chatml.jinja` fixture. `cargo test --release --bin hf2q backends::gguf` 71/71 PASS (incl. iter211 provenance regression suite); `cargo test --release --bin hf2q backends::chat_templates` 3/3 PASS.
