...
| `docs/ADR-040-continuous-batching-reopen.md` | ~190 | ~5 | this §6.1.27 closure + Status-line update + §6 sequencing table row + §6.1.24 followups update |
...
- **iter-C2d-cont-kernel-iter-LCP**: slot-aware LCP / chunked-prefill snapshot codec. The persisted snapshot format (kv_persist) is sized per-request today; a slot-aware variant either (a) extends the persisted format with a slot_id discriminator, or (b) carries a separate per-slot LCP registry. Either path is a multi-iter sub-arc. iter-1 DISABLES LCP / chunked-prefill in slot-aware mode (fast-path is fresh monolithic prefill) so the slot-aware path is correct without LCP; iter-LCP re-enables the LCP fast-path for slot-aware mode.
- **iter-C2d-cont-kernel-iter-G**: slot-aware greedy fast-path via `forward_gpu_greedy(.., slot_id)` (the GPU-side argmax kernel that saves the ~250µs per-step CPU download of vocab-size F32 logits). iter-1 uses `forward_gpu_last_logits + greedy_argmax_last_token` for greedy mode (cheaper LOC delta); iter-G ports the existing `forward_gpu_greedy` dispatch.
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~200 | ~10 | this §6.1.28 closure + Status-line update + §6 sequencing table row + §6.1.27 commit-hash patch + deferrals matrix update. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~150 | ~10 | this §6.1.29 closure + Status-line update + §6 sequencing table row + §6.1.28 commit-hash patch. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~200 | ~10 | this §6.1.30 closure + Status-line update + §6 sequencing table row + §6.1.29 commit-hash patch + deferrals matrix update. |
| `docs/ADR-040-continuous-batching-reopen.md` (§6.1.24 followups) | ~5 | ~5 | `iter-C2d-cont-kernel` bullet updated to record iter-2/3/4 SHIPPED state + narrow surviving sub-deferrals to iter-LCP + iter-G |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~200 | ~2 | this §6.1.31 closure + Status-line update + §6 sequencing table row |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~210 | ~1 | this §6.1.32 closure + Status-line update + §6.1.31 followups marked SHIPPED for iter-2A |
...
- **iter-B4c-kernel-iter-2C**: legacy 4-bit native `flash_attn_vec_tq` slot routing.  `HF2Q_TQ_CODEBOOK_BITS=4` opt-in surface (127-byte sourdough ceiling — not shippable as default).  Gated on iter-2A-cont landing first (HB-encoded path is the dominant production surface).
- **iter-B4c-kernel-iter-2D**: dense F32 KV path slot routing (`HF2Q_USE_DENSE=1`).  LCP-eligible regime — the LCP partial-prefix resume in `engine.rs::generate_once_with_soft_tokens` (~line 6700-6843) consumes this regime; iter-2D ports the slot-aware LCP path.  Orthogonal to TQ-active paths; gated on iter-A3b-2 `DenseKvBuffers` multi-seq lift (currently typed-clamped per A3b iter-1).
...
- **iter-B4c-kernel-iter-2-embed**: `forward_embed_last` slot-aware port (~60 LOC; single-call wrapper around `forward_prefill(.., max_decode_tokens=0, ..)`).  Same shape as iter-2-decode but no decode loop.  Pinned in the iter-B4c-kernel-iter-4 (Embed worker-arm) deferral cite at iter-1's §6.1.31 closure.
- **iter-B4c-kernel-iter-2-batched**: `forward_prefill_batched` slot-aware port — orthogonal to per-request scope (the batched variant is gated on the `HF2Q_SERVE_BATCHED` env, off-by-default).
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~150 | ~5 | this §6.1.33 closure + Status-line update + §6.1.21 follow-ups SHIPPED-half marker + §6.1.32 iter-2B "spawn-prerequisite unblocked" marker. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~190 | ~3 | this §6.1.34 closure + Status-line update + §6.1.32/§6.1.33 followups marked SHIPPED for iter-2B. |
...
- **iter-B4c-kernel-iter-2B-xlen** (NEW from this iter): BF16 xlen K/V buffer slot routing through `dispatch_kv_cache_copy_seq_bf16_to_bf16_head_major` callers. `HF2Q_DFLASH_XLEN_SDPA=1` opt-in surface (ADR-030 iter-96). Structurally analogous to the F16 K slot routing this iter shipped, but on a separate kernel-write path. Pinned by the iter-2B hybrid branch's xlen-gate typed `CapabilityUnsupported` + H103 source-grep.
- **iter-B4c-kernel-iter-2A-cont** (UNCHANGED from §6.1.32 + §6.1.33): HB-encoded prefill body slot routing — the `HF2Q_HYBRID_KV=0` opt-out surface. Structurally analogous to iter-2B but for `MultiSeqHbKvBuffers` instead of `MultiSeqHybridKvBuffers`. Gated on Path A consideration: iter-2B's slot-view mount + delegate-to-sibling pattern is directly portable.
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~180 | ~10 | this §6.1.35 closure + Status-line update + §6 sequencing table row + §6.1.31 followups iter-3 SHIPPED annotation + commit-hash patch for iter-2B (`1676fcd1`). |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~190 | ~5 | this §6.1.36 closure + Status-line update + §6 sequencing table row + §6.1.35 followups iter-4 SHIPPED annotation + commit-hash patch for iter-3 (`0c63bfe9`). |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~200 | ~5 | this §6.1.37 closure + Status-line update + §6 sequencing table row + iter-4/iter-5 follow-up sweeps. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~250 | ~5 | this §6.1.38 closure + Status-line update + §6 sequencing table row + §6.1.32/§6.1.34/§6.1.35/§6.1.37 followups marked SHIPPED for iter-2-decode-A. |
...
- **iter-B4c-kernel-iter-2-decode-B**: HB-encoded `HF2Q_HYBRID_KV=0` opt-out decode-side slot routing.  Surfaced from the new fn body's HB-encoded branch (typed `CapabilityUnsupported` naming this iter).  Mirror of iter-2A-cont prefill scope.  Same primitive (slice_view mount + delegate-to-sibling) but for `MultiSeqHbKvBuffers` instead of `MultiSeqHybridKvBuffers`.
- **iter-B4c-kernel-iter-2-decode-C**: orchestrator-side full sampler / grammar / tool-call / stop-strings / logprobs / reasoning-text surface.  Surfaced from the 3 orchestrator decode-loop entry-sampling-clamps (typed `CapabilityUnsupported` when `T>0 || grammar.is_some() || !stop_strings.is_empty() || logprobs`).  The load-bearing surface for non-greedy chat completion at SlotId(N>0): logit_bias, grammar masking, sampler_pure call, stop-string scanning, logprobs accumulation, reasoning-marker splitter, tool-call splitter, response-format enforcement.  ~600 LOC of mechanical extraction-and-replication of `generate_once`'s slow path at engine.rs:7752-7820.
- **iter-B4c-kernel-iter-2-decode-D**: dense F32 (`HF2Q_USE_DENSE=1`) + legacy 4-bit (`HF2Q_TQ_CODEBOOK_BITS=4`) decode-side slot routing.  Mirror of iter-2C/2D prefill scopes.  Opt-in pre-default surfaces.
- **iter-B4c-kernel-iter-2-decode-A-xlen**: BF16 xlen K/V decode-side slot routing (`HF2Q_DFLASH_XLEN_SDPA=1` opt-in).  Mirror of iter-2B-xlen prefill scope.
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~145 | ~2 | this §6.1.39 closure + Status-line update + §6 sequencing table row. |
...
- **iter-B4c-kernel-iter-2-decode-C-stream-tool-call**: streaming tool-call body emission via Wave 3 W-B3 `ToolCallStreamEmitter` (~200 LOC of stateful incremental JSON-arguments parsing).  Surfaced from the GenerateStream-arm entry-check when a registered `ToolCallSplitter` is present AND `grammar_kind ∈ {ToolCallBodyAuto, ToolCallBodyRequired}`.  Mirror of the Wave 3 W-B3 `tool_call_emitter.advance()` / `.finalize()` plumbing at engine.rs:11277+; deferred so iter-2-decode-C's scope remains structurally bounded.
- **iter-B4c-kernel-iter-2-decode-B** (UNCHANGED): HB-encoded `HF2Q_HYBRID_KV=0` opt-out decode-side slot routing.
- **iter-B4c-kernel-iter-2-decode-D** (UNCHANGED): dense F32 + legacy 4-bit decode-side slot routing.
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~135 | ~5 | this §6.1.40 closure + status-line append + §6 sequencing-row update (A2b-cont SHIPPED) + §6.1.23 followups marker update |
...
- **iter-A2b-cont-test-helpers**: bring the `gpu_delta_net.rs` `#[cfg(test)]` `n_seqs: 1` literals (in `prepare_ssm_conv_buffers` + the dispatch-parity unit tests under `mod tests`) under the same `FORWARD_DISPATCH_N_SEQS` discipline. The 6 test-site `build_delta_net_layer*` call sites now correctly pass `SlotId(0)` per A2b-cont, but the lower-level `n_seqs: 1` struct-field literals at `gpu_delta_net.rs:789`, `:5579`, `:5986`, `:6002` are inside test fixtures that allocate their own single-seq buffers — touching them would be a cosmetic centralization without a behaviour delta. Deferred to a follow-up cleanup iter; pinned by H137's grep against PRODUCTION-side literals.
- **iter-A2b-cont-forward-gpu-greedy**: `forward_gpu_greedy` is the B4d-scope greedy decode entry that hard-codes `SlotId(0)` at every internal dispatch site (FA at :5293 + :5612, DN at :5374 + :5722). Multi-slot routing for that path lands as **iter-B4d-kernel** alongside the FA-side lift; iter-A2b-cont keeps the SlotId(0) discipline at the DN call sites verbatim to mirror the existing FA contract.
...
- **iter-A3b-3** — `MlxKvCache` full multi-seq lift (legacy 4-bit path).  STILL DEFERRED — no multi-seq sibling shipped yet; H16 typed-clamp pin retained.  Scope: lift `k_packed`/`k_norms`/`v_packed`/`v_norms` shapes + replace `seq_len: usize` + `write_pos: usize` with `Vec<u32>` cursors.  Reachable only via legacy 4-bit GGUF path (off-default since ADR-007 TQ 8-bit).  Low priority — not gated on iter-B4c (the production-default + dev-debug paths are now both lifted).
...
- ✅ MlxKvCache (iter-A3b-3) STILL typed-clamped — not shipped this iter (per brief constraint).
...
- **iter-A3b-3** (MlxKvCache full lift): UNCHANGED — still typed-clamped per H16; low priority since the legacy 4-bit path is off-default since ADR-007.
