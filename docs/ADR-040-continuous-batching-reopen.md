...
| `docs/ADR-040-continuous-batching-reopen.md` | ~250 | ~3 | this §6.1.46 closure + Status-line update + §6.1.32/§6.1.38/§6.1.45 followups marked SHIPPED for iter-2C + iter-2D + iter-2-decode-D. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~210 | ~5 | this §6.1.47 closure + Status-line update + §6.1.34/§6.1.38/§6.1.46 followups marked SHIPPED for iter-2B-xlen + iter-2-decode-A-xlen + §6 sequencing table row. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~150 | ~4 | this §6.1.48 closure + Status-line update + §6 sequencing table row + §6.1.39 + §6.1.47 followups marked SHIPPED for iter-2-decode-C-stream-tool-call. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~145 | ~4 | this §6.1.49 closure + Status-line update + §6 sequencing table row + §6.1.32 followups marked SHIPPED for iter-2-embed + iter-2-batched. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~165 | ~3 | this §6.1.50 closure + Status-line update + §6.1.27 + §6.1.46 followups marked SHIPPED for iter-LCP + iter-G + iter-2D-lcp. |
...
| `docs/ADR-040-continuous-batching-reopen.md` | ~150 | ~5 | this §6.1.51 closure + status-line append + §6.1.40 sub-deferral marker SHIPPED + §6.1.44 sub-deferral markers SHIPPED. |
...
- **iter-C2e-cont (gated on iter-228a)**: Qwen3-VL worker hot path lift onto the persistent multi-seq KV cache.  Today, `Qwen3VlTextLoadedModel::provision_multi_seq_kv_for_slot_aware` populates `self.slot_aware_max_slots = Some(max_slots)` at spawn time as a witness scalar; iter-C2e-cont replaces this with a real persistent KV cache scaffold + lifts each of the four worker arms (Generate / GenerateStream / Embed / GenerateWithSoftTokens) through it.  Pinned by H220's `iter-C2e-cont per ADR-040 §6.1.52` clamp labels at each of the four Qwen3-VL worker arms.  **Upstream blocker**: iter-228a must land first to replace the 501 sentinel arms (`qwen3vl_text_forward_pending_err` at `engine.rs:5697-5703`, `:5697`, `:6035-6037`, etc.) with a real Qwen3-VL forward path before any persistent KV cache can be allocated.
...
**Surviving sub-deferrals after §6.1.54** (each gated on EXTERNAL signals per dossier §6 + §7):

- **`iter-A4-cont-moe-validation`** — empirical Qwen3.6-A3B A/B at N=1,2,4,8 concurrent against the MoE-routing trap finding (dossier §1.6 #3). Gated on real-hardware bench (multi-week on H100 or M-series).
- **`iter-A4-cont-acceptance-telemetry`** — production telemetry pipeline for `spec_decode.accepted_tokens_per_step` per slot for empirical inflection-point measurement on hf2q's actual workload. Gated on operator telemetry infrastructure (`/metrics` schema extension + scrape pipeline).
- **`iter-A4-cont-inflection-bench`** — D3-style AC-4 throughput bench extended to plot acceptance rate × concurrent_count for hf2q's Qwen35/Qwen3.6 model + EAGLE-3 drafter combos. Gated on iter-A4 iter-1 landing (this iter) + a workload-representative prompt corpus.
- **`iter-A4-cont-drafter-dispatcher`** — kernel-level dispatcher routing `MultiSeqDrafterKvCache` per-slot byte offsets through the existing EAGLE-3 `tree_attention` kernel (mirror of B4c-kernel iter-{2A,2B,2C,2D} pattern applied to the drafter K/V buffers). Gated on iter-A4-cont-inflection-bench confirming hf2q's workload-specific inflection point falls inside the safe zone (≤4 concurrent).
...
**Surviving typed deferrals after §6.1.54**:
- `iter-C2e-cont` (Qwen3VL worker hot path lift) — gated on iter-228a 501 sentinel removal in a separate ADR
- `iter-A4-cont-moe-validation` — gated on real-hardware Qwen3.6-A3B A/B bench
- `iter-A4-cont-acceptance-telemetry` — gated on operator telemetry infrastructure
- `iter-A4-cont-inflection-bench` — gated on iter-A4-cont-acceptance-telemetry data
- `iter-A4-cont-drafter-dispatcher` — gated on iter-A4-cont-inflection-bench confirming safe-zone fit

**ALL FIVE surviving deferrals are gated on EXTERNAL signals** (separate ADR landing or empirical measurement or hardware bench), not on missing hf2q work. ADR-040 remains functionally COMPLETE as of §6.1.54.
