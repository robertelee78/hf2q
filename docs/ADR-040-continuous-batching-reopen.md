...
- **iter-B4c-kernel**: Gemma 4 worker hot path FULL lift onto the per-layer `MultiSeqHbKvBuffers` (provisioned by C2c at spawn time per §6.1.21).  **iter-1 SHIPPED 2026-05-30** (Generate-arm scaffold lift — `generate_gemma4_once_slot_aware` + `MultiSeqHbKvBuffers::reset_for_slot` + sibling `MultiSeqHybridKvBuffers::reset_for_slot` per §6.1.31; kernel-forward step itself typed-deferred as `iter-B4c-kernel-iter-2`).  **iter-2A SHIPPED 2026-05-30** (model-level slot-aware fn — `forward_prefill_with_soft_tokens_slot_aware` + 4-way dispatch fork per §6.1.32).  **iter-2B SHIPPED 2026-05-30** (production-default hybrid F16-K + TQ-HB-V slot routing via slice_view mount + delegate-to-sibling per §6.1.34).  **iter-3 SHIPPED 2026-05-30** (GenerateStream-arm slot-aware orchestrator port — `generate_stream_gemma4_once_slot_aware` per §6.1.35).  Remaining sub-iters: **iter-B4c-kernel-iter-2-decode** (decode-loop body wrapping `forward_decode` at slot_id — load-bearing for both Generate + GenerateStream arms post-iter-2B/iter-3); **iter-B4c-kernel-iter-2A-cont** (HB-encoded prefill slot routing — HF2Q_HYBRID_KV=0 opt-out path); **iter-B4c-kernel-iter-2C/2D** (legacy 4-bit + dense F32 dispatch-fork branches); **iter-B4c-kernel-iter-2B-xlen** (xlen BF16 K/V slot routing); **iter-B4c-kernel-iter-4** (Embed orchestrator port); **iter-B4c-kernel-iter-5** (GenerateWithSoftTokens orchestrator port).  Parallel to iter-C2d-cont-kernel for the Qwen35 surface (now TERMINAL post-iter-4 §6.1.30); both unblock Phase E1 production cutover for their respective architectures.
...
- **iter-B4c-kernel-iter-2-decode** (UNCHANGED from §6.1.31 + §6.1.32 + §6.1.33): decode-loop body wrapping `forward_decode` calls. The orchestrator's hypothetical-Ok branch still surfaces typed `CapabilityUnsupported` here — iter-2B's prefill side returns the first decode token but the decode-loop body is the next sub-deferral. `forward_decode` in `gemma4/forward_gpu.rs:310` has NO `slot_id` parameter today; its lazy-alloc block at lines 396-466 mirrors the prefill alloc sites and must thread `slot_id` + consult the persistent multi-seq scaffold. Gated on the same slot-view mount + delegate-to-sibling primitive iter-2B established.
...
- **iter-B4c-kernel-iter-2D** (unchanged from §6.1.32): dense F32 dispatch-fork branch (HF2Q_USE_DENSE=1 LCP-eligible regime).
- **iter-B4c-kernel-iter-2B-xlen** (unchanged from §6.1.34): xlen BF16 K/V slot routing (HF2Q_DFLASH_XLEN_SDPA=1 opt-in surface).
- **iter-B4c-kernel-iter-5**: GenerateWithSoftTokens worker-arm slot-aware orchestrator port via `generate_gemma4_once_with_soft_tokens_slot_aware` (mirror of Qwen35 iter-C2d-cont-kernel iter-4 §6.1.30).  Vision-aware path — SOLE SURVIVING Gemma 4 worker-arm port post-iter-4.
- **iter-B4c-kernel-iter-LCP / iter-G** (unchanged from §6.1.31): orthogonal slot-aware LCP + greedy fast-path optimizations.
