...
- **iter-B4c-kernel-iter-2-embed** (UNCHANGED from §6.1.32): `forward_embed_last` slot-aware port.
- **iter-B4c-kernel-iter-2-batched** (UNCHANGED from §6.1.32): `forward_prefill_batched` slot-aware port.
- **iter-B4c-kernel-iter-2D-lcp** (NEW carve-out): LCP partial-prefix resume slot-aware port for the dense F32 regime.  The iter-2D landing this iter passes `restored_lcp = None` to the sibling; threading slot-aware LCP through the `restored_lcp=Some(k)` branch is a separate sub-deferral (mirror of the iter-2B no-LCP discipline at §6.1.34).
