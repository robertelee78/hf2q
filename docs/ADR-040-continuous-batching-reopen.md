...
- **Status**: 🔴 **REOPENED 2026-06-23** (was "STRUCTURAL CLOSURE"). The Phase A–E arc shipped the full multi-seq KV substrate, the scheduler scaffold, and the per-arch worker-arm lifts (historical log below) — but the **load-bearing goal of the ADR, an aggregate-throughput gain from concurrency, was never achieved and is architecturally unreachable as built.** Empirical bench 2026-06-23 (Gemma-4 Ara `Q5_K_M`, M5 Max, `--scheduler inflight-batched --max-slots 4`): 4 concurrent decodes = **8.1 s** vs 4 sequential = **6.9 s** → **0.85× — a regression, not a speedup.** Root cause, triangulated from a codex code-trace + adversarially-verified web research (see §0): the SlotAware path runs **N independent `batch=1` forward passes that time-slice one GPU**, never a fused `batch=N` decode, so model weights are re-streamed once per slot and the bandwidth-bound weight read is never amortized. The §1.4 / §3.1 "zero new Metal kernels / SeparateSlots reuses every existing kernel" assumption is the defect: SeparateSlots delivers KV *coexistence*, not batched *compute*. **The fix is specified in §0 (Correction & Phase F).** Production default stays `EngineMode::SerialFifo` — existing users see no change; the reopening is purely additive.
...
**F3 — Weight-amortizing dense decode GEMM at small batch (m=2..8)** · *mlx-native* · **load-bearing**
...
- ADR-041: `docs/ADR-041-qwen3vl-text-lm-engine-seam.md`
