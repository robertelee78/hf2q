...
- **Status**: ✅ CLOSED — Design 2026-05-23, iter-1 SHIPPED 2026-05-23 (commit 1a1d6a26), 25 sequencing-table iters SHIPPED 2026-05-23..2026-05-29 culminating in **Phase E1 closure ceremony 2026-05-29** (commit `b928ffaf`; §6.1.26), plus **iter-C2d-cont-kernel iter-1 SHIPPED 2026-05-30** (commit `19b38df9`; §6.1.27 — Qwen35 worker hot path Generate-arm lift onto persistent multi-seq HybridKvCache), plus **iter-C2d-cont-kernel iter-2 SHIPPED 2026-05-30** (commit hash recorded in §6.1.28 at commit time — Qwen35 worker hot path GenerateStream-arm lift onto persistent multi-seq HybridKvCache).  Decision per §3.6 + §3.7: **KEEP [`EngineMode::SerialFifo`] as production default** — the reopen trigger (ADR-005 carve-out: ≥8 concurrent users sustained 7 days OR a customer asks explicitly) is NOT MET today.  [`EngineMode::SlotAware { max_slots: 4 }`] has **spawn + per-arch multi-seq KV provisioning wired end-to-end** behind `--scheduler inflight_batched` + `HF2Q_SCHEDULER=inflight_batched` (per §6.1.9 C4); **SlotId(0) is byte-equivalent** to SerialFifo; **for Qwen35 Generate + GenerateStream arms, SlotId(N>0) routes through the persistent multi-seq cache** (iter-C2d-cont-kernel iter-1 §6.1.27 + iter-2 §6.1.28); **for all other arms (Qwen35 Embed/GenerateWithSoftTokens, all Gemma 4 arms, all linear-attn forward dispatch) SlotId(N>0) returns typed `MultiSeqError::CapabilityUnsupported`** (HTTP 501 with operator-grep'able label) until the remaining kernel-level lifts (`iter-C2d-cont-kernel-iter-{3,4,LCP,G}` for Qwen35, `iter-B4c-kernel` for Gemma 4, `iter-A2b-cont` for linear-attn dispatch, plus the 4 KV-variant follow-ups `iter-A2c` / `iter-A3c` / `iter-A3b-2` / `iter-A3b-3`) land.  These TYPED DEFERRALS are pinned by Display labels + source-grep tests, ready to fire when the reopen trigger lands and the kernel lifts complete.  Reopens the [ADR-005 Decision #1 carve-out](ADR-005-inference-server.md) and CLOSES the activation; ADR-005 line 1097 update lands at iter-E2 (downstream-ADR closure-block cross-link sweep, deferred to operator).
- **Date**: 2026-05-23
...
- **iter-C2d-cont-kernel**: Qwen35 worker hot path FULL lift onto the persistent multi-seq cache.  **iter-1 SHIPPED 2026-05-29** (Generate-arm lift only — `generate_qwen35_once_slot_aware` + `HybridKvCache::reset_for_slot` per §6.1.27).  iter-{2,3,4} (GenerateStream / Embed / GenerateWithSoftTokens arm lifts) + iter-LCP (slot-aware LCP / chunked-prefill snapshot codec) + iter-G (slot-aware greedy fast-path via `forward_gpu_greedy`) are typed sub-deferrals pinned by H57 + the worker-arm clamps' iter-N labels.  Originally specified co-changes: (1) refactor `generate_qwen35_once*` + `embed_qwen35` to accept `Option<&mut HybridKvCache>` + `SlotId`; (2) sub-region LCP snapshot/restore for the persistent cache (the persistent cache is sized to `cfg.max_position_embeddings` while `restore_from` expects per-request sizing — a `slot_id` + range-aware variant is needed); (3) per-slot rollback on EOS / max_tokens.  iter-1 closes (1) for the Generate arm via the `take()`+restore borrow pattern and closes (3) via `restore_partial` + the new `reset_for_slot(slot_id)` primitive (NOT `rollback_la_to` — that's spec-decode-only, gated on `ensure_la_capture`); iter-LCP closes (2) for the remaining LCP/chunked-prefill path.
...
| C | 2d-cont-kernel iter-{3,4,LCP,G} | **TYPED DEFERRED** | §6.1.28 | iter-3 (Embed arm) + iter-4 (GenerateWithSoftTokens arm) + iter-LCP (slot-aware LCP snapshot codec) + iter-G (slot-aware forward_gpu_greedy fast-path); each pinned by H57 + H62 + the worker-arm clamps' iter-N labels |
...
| `iter-C2d-cont-kernel-iter-{3,4,LCP,G}` | Qwen35 worker hot path remaining-arms lift onto persistent multi-seq cache (`Qwen35LoadedModel.persistent_kv_cache`) — iter-1 SHIPPED 2026-05-29 (Generate arm + `reset_for_slot` + `restore_partial`-based prompt-cache lift) per §6.1.27; iter-2 SHIPPED 2026-05-30 (GenerateStream arm + same shape via `generate_stream_qwen35_once_extended_slot_aware`) per §6.1.28; iter-{3,4} = Embed / GenerateWithSoftTokens arms; iter-LCP = slot-aware LCP / chunked-prefill snapshot codec; iter-G = slot-aware forward_gpu_greedy fast-path | H57 + H62 + worker-arm clamps' `iter-C2d-cont-kernel-iter-{3,4} per ADR-040 §6.1.27 / §6.1.28` label cites | AC-4 D3 hard-gate fires for Qwen35 against a non-{Generate,GenerateStream}-arm request mix (current iter-1 + iter-2 already unblock AC-4 for the non-streaming + streaming chat/completion mix) |
...
### 6.1.28 Iter-C2d-cont-kernel iter-2 closure — Qwen35 worker hot path GenerateStream-arm lift onto persistent multi-seq HybridKvCache (2026-05-30, commit hash recorded in §6.1.28 at commit time)
