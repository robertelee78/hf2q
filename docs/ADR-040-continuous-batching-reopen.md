...
- **iter-B4c-kernel-iter-2B**: HybridKvBuffers slot routing — PRODUCTION DEFAULT path (`HF2Q_HYBRID_KV=1` since ADR-029 iter-13 per H10 falsification at §6.1.11).  Mirrors iter-2A-cont's structure for the F16-K + TQ-HB-V regime.  Gated upstream on a parallel iter-C2c-cont commit provisioning the `MultiSeqHybridKvBuffers` sibling scaffold on `GemmaLoadedModel` (C2c §6.1.21 currently provisions ONLY `MultiSeqHbKvBuffers`).  Pinned by the iter-2A dispatch fork's typed `CapabilityUnsupported` label + H87 source-grep.
