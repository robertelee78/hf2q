...
**Remaining work after §6.1.55** (operator-runtime measurements, NOT hf2q dev work):
1. **Real-hardware MoE A/B bench** — operator runs `HF2Q_A4_MOE_AB_VALIDATION_E2E=1` against a production Qwen3.6-A3B GGUF on M5 Max or H100; produces the inflection-point measurement.
2. **iter-228a Qwen3-VL real-forward-path** — separate ADR's responsibility; lifts the 501 sentinel through `qwen3vl_text_forward_pending_err()`.  Once it lands, the iter-C2e-cont worker hot path's witness take/restore discipline is the production cache get-then-put pattern.
3. **`/metrics` schema extension** — operator infrastructure work; plugs into the no-op `emit_acceptance_metric` seam.
4. **Inflection-bench acceptance-rate measurement body** — gated on (3) telemetry shipping + (1) dispatcher-kernel landing.

**ADR-040 architectural state post-§6.1.55** (delta from §6.1.54):

| Phase | State |
|---|---|
| A4 — drafter multi-seq KV | **All 5 surviving deferrals SHIPPED structurally §6.1.55**; remaining work is operator-runtime measurement |
| C2e — Qwen3-VL SlotAware activation | **iter-C2e-cont SHIPPED structurally §6.1.55** (witness take/restore + sentinel delegation); real-forward-path lift on iter-228a's responsibility |
| ADR-040 implementation | **CLOSED** — no surviving hf2q-side structural work remains |
