...
- **iter-B4c-kernel**: Gemma 4 worker hot path FULL lift onto the per-layer `MultiSeqHbKvBuffers` (provisioned by C2c at spawn time per §6.1.21).  **iter-1 SHIPPED 2026-05-30** (Generate-arm scaffold lift — `generate_gemma4_once_slot_aware` + `MultiSeqHbKvBuffers::reset_for_slot` + sibling `MultiSeqHybridKvBuffers::reset_for_slot` per §6.1.31; kernel-forward step itself typed-deferred as `iter-B4c-kernel-iter-2`).  Remaining sub-iters: **iter-B4c-kernel-iter-2** (the kernel-forward step — refactor 3 inline alloc sites at `forward_prefill.rs:843-882`, `forward_prefill_batched.rs:443-475`, `forward_gpu.rs:443-459` through `alloc_hb_kv_for_layer(.., n_seqs=max_slots)`; thread `slot_id: SlotId` through `forward_prefill_with_kv_cache` / `forward_embed_last` / `forward_prefill_with_soft_tokens`; wire per-slot `MlxBuffer::slice_view(byte_offset, n_elements)` at the `dispatch_hadamard_quantize_kv_hb_*` callers — same primitive Qwen35 B4a-cont uses per §6.1.20); **iter-B4c-kernel-iter-3** (GenerateStream orchestrator port); **iter-B4c-kernel-iter-4** (Embed orchestrator port); **iter-B4c-kernel-iter-5** (GenerateWithSoftTokens orchestrator port).  Parallel to iter-C2d-cont-kernel for the Qwen35 surface (now TERMINAL post-iter-4 §6.1.30); both unblock Phase E1 production cutover for their respective architectures.
...
- **iter-B4c-kernel-iter-3**: GenerateStream worker-arm slot-aware orchestrator port via `generate_stream_gemma4_once_slot_aware` (mirror of Qwen35 iter-C2d-cont-kernel iter-2 §6.1.28).  Pinned by H83 + the GenerateStream clamp's `iter-B4c-kernel-iter-3 per ADR-040 §6.1.31` cite.  Same shape as iter-1 (take+restore persistent scaffold, thread slot_id, reset_for_slot entry+exit) but with the streaming event channel as the result surface.  Gated upstream by iter-B4c-kernel-iter-2 (the kernel-forward step must land first to make the orchestrator load-bearing).
...
- **iter-B4c-kernel-iter-5**: GenerateWithSoftTokens worker-arm slot-aware orchestrator port via `generate_gemma4_once_with_soft_tokens_slot_aware` (mirror of Qwen35 iter-C2d-cont-kernel iter-4 §6.1.30).  Pinned by the soft-token clamp's `iter-B4c-kernel-iter-5 per ADR-040 §6.1.31` cite.  Vision-aware path.
...
- **iter-B4c-kernel-iter-2B** — **SPAWN-PREREQUISITE UNBLOCKED 2026-05-30 (see §6.1.33)**: HybridKvBuffers slot routing — PRODUCTION DEFAULT path (`HF2Q_HYBRID_KV=1` since ADR-029 iter-13 per H10 falsification at §6.1.11).  Mirrors iter-2A-cont's structure for the F16-K + TQ-HB-V regime.  **Spawn prerequisite SHIPPED**: iter-C2c-cont (§6.1.33) provisions the `MultiSeqHybridKvBuffers` sibling scaffold on `GemmaLoadedModel.multi_seq_kv_hybrid` IFF `INVESTIGATION_ENV.hybrid_kv == true` (default), so the kernel-dispatch fork now has a destination buffer to consume. iter-2B remaining work: thread `slot_id`-aware `MlxBuffer::slice_view` at `slot_id.0 * nkv * cap * hd * 2`-byte offset through `forward_prefill.rs`'s `INVESTIGATION_ENV.hybrid_kv` branch (line 2562) + the `dispatch_kv_cache_copy_batch_f32_to_f16` callers + the F16-K read-path SDPA consumers.  Pinned by the iter-2A dispatch fork's typed `CapabilityUnsupported` label + H87 source-grep + H91-H96 (§6.1.33).
...
- **iter-B4c-kernel-iter-2-decode**: decode-loop body wrapping `forward_decode` calls.  `forward_decode` in `src/inference/models/gemma4/forward_gpu.rs:310` has NO `slot_id` parameter today; its lazy-alloc block at lines 396-466 mirrors the prefill alloc sites and must thread `slot_id` + consult the persistent multi-seq scaffold.  Gated on iter-2A-cont/2B kernel-dispatch refactor landing the prefill side first.  Pinned by the orchestrator's hypothetical-Ok branch typed `CapabilityUnsupported` label + H87 source-grep.
...
- **iter-B4c-kernel-iter-2B — STRUCTURALLY UNBLOCKED**: HybridKvBuffers slot routing through `forward_prefill.rs::forward_prefill_with_soft_tokens_slot_aware`'s `INVESTIGATION_ENV.hybrid_kv` branch (line 2562). Now has a destination: `GemmaLoadedModel.multi_seq_kv_hybrid` carries the per-layer `MultiSeqHybridKvBuffers` scaffold provisioned by this iter. iter-2B remaining work: (a) extend the orchestrator `engine::generate_gemma4_once_slot_aware` signature with `Option<&mut Vec<MultiSeqHybridKvBuffers>>` (or split into hybrid + HB orchestrators); (b) extend the model fn `forward_prefill_with_soft_tokens_slot_aware` signature with the hybrid parameter; (c) thread `slice_view(slot_id.0 * nkv * cap * hd * 2, nkv * cap * hd * 2)` through the existing alloc site at `forward_prefill.rs:843-891` (the hybrid_kv branch) + the `dispatch_kv_cache_copy_batch_f32_to_f16` callers + the F16-K read-path SDPA consumers; (d) parallel worker-arm take-and-restore borrow pattern at `engine.rs::worker_run` Gemma 4 Generate arm: extract BOTH scaffolds (or branch on `INVESTIGATION_ENV.hybrid_kv` at the worker layer). Estimated ~400 LOC across forward_prefill.rs + engine.rs (forward path + orchestrator + worker-arm extraction).
...
- **iter-B4c-kernel-iter-2C / iter-2D / iter-2-decode / iter-2-embed / iter-2-batched / iter-3 / iter-4 / iter-5 / iter-LCP / iter-G** (UNCHANGED from §6.1.31 + §6.1.32): all remain named sub-deferrals.
