...
| `docs/ADR-040-continuous-batching-reopen.md` | ~125 | ~5 | this §6.1.44 closure + status-line append + §6 Phase B sequencing row update + §6.1.20 deferrals matrix row update + §6.1.26 deferrals matrix row update + §6.1.40 followup marker SHIPPED. |
...
- **iter-B4d-multi-seq-stress**: an integration test that runs `SpecDecode::new_with_eos_set_and_slot(.., SlotId(1))` end-to-end (loading a real Qwen35 MoE GGUF + MTP weights) is gated on the SlotAware runtime activation arc (currently `EngineMode::SlotAware` for spec-decode is gated on Phase A4).  H169 establishes the GPU-content side of the contract at the tiny fixture level; full-model stress is operator-runnable but not part of this iter.
- **iter-B4d-test-helpers**: bring the `qwen35::spec_decode::tests` fixture-level callsites (e.g., `run_rejects_missing_mtp_before_gpu_alloc`) under the slot-aware builders.  Currently they exercise `SpecDecode::run(&model, &[1], 1)` which defaults to `SlotId(0)` via the existing call-graph; cosmetic cleanup, no behaviour delta.
