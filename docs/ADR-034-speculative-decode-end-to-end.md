...
| **A — Qwen 3.5/3.6 35B-A3B MoE-MTP** | ✅ **WORKING (8/8 quants)** | Coherent "**Paris**." haiku at temp=0 on canonical Q4_K_M GGUF (SHA byte-identical to canonical). 8/8 quants pass: Q4_0/Q4_K_S/Q4_K_M/Q5_K_S/Q5_K_M/Q6_K/Q8_0/IQ4_NL — all produce identical "**Paris**." with 85.7-100% MTP acceptance. Determinism PASS 3/3 byte-identical. K=1 dispatch via `qwen35 spec` path. Fix at HEAD: (1) enum refactor `MtpFfnWeightsGpu::{Dense, Moe}` in `mtp.rs` + detection/dispatch in `mtp_weights_load.rs::load_mtp_ffn` (afbf5684); (2) extended `load_moe_ffn_quantized` supported() allowlist with IQ4_NL + Q5_1 (66f2008f) — the kernels were already ported in ADR-022 Phase 1 but the load gate was stale. | P1 parity harness (G2 gate); P6 perf gate |
