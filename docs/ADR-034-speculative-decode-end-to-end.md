...
| **B — Qwen 3.6 DFlash** | ✅ **WIRED + COHERENT, 0.53× production winner — perf needs task #89** | Task #78 Steps 3a-4 SHIPPED at HEAD `e10946cf` (2026-05-21). Architecture: option (b) — `DFlashTarget` trait with blanket impl on `MlxModelWeights` + `Qwen35DFlashTarget<'a>` wrapper bundling `&mut Qwen35Model` + `&mut HybridKvCache` + `Option<DFlashCaptureSession>`. New `forward_gpu_with_hidden_dflash` uses `LayerActivations` post-process (with new `target_layer_filter` field — Step 3c.A.4 — gating download to drafter layers only, ~10× memory reduction). New public APIs `embed_tokens_gpu` + `per_position_argmax_from_normed_hidden` on `Qwen35Model`. New `dispatch_qwen35_dflash_generate` orchestrator (Option A xlen verify only — Option C re-prefill rejected because DeltaNet state reset is multi-sec/round). Wired via `HF2Q_SPEC_DFLASH=1` opt-in. Codex /cfa caught Critical (ensure_la_capture ordering), High (cache-capacity arithmetic), Major (panic→Result), Minor x4 — all closed. Bench at HEAD `e10946cf` on Qwen 3.6 27B Q8_0 + 128 tok + temp=0: DFlash BS=2 = **16.1 t/s** (best); BS=4 = 12.3; BS=8 = 8.7; BS=16 (drafter default) = 3.8. Production MTP K=1 BATCHED MH = **30.2 t/s** @ 92.4% accept. DFlash is **0.53× production winner**. Coherence: greedy-byte-identity proven; DFlash actually produces a real haiku where the base path degenerates on the same prompt (a side-effect of the drafter's larger lookahead → less stuck in local minima — observed empirically, not a perf signal). | **Task #89** (forward_gpu_batched_decode for seq_len[2,8]) is the structural prerequisite to close the gap. K+1 batched verify currently uses BF16 prefill_resume kernel which is slower per-token than F32 flash_attn_vec single-token decode. Same bottleneck blocks K=N MTP on MoE. Multi-week scope per cont. 38 memory entries. |
