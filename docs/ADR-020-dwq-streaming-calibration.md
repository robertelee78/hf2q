...
| 11h-e3 | 1/2 | **`switch_mlp` per-token expert dispatch** on GpuTape — for each token, run its top-K experts' MLP and weight by scores.  This is the SCATTER-GATHER pattern (per-token expert weight slicing → per-expert matmul → per-token scatter back).  Production kernel exists in mlx-native (`moe_dispatch.rs`); needs autograd-aware variant for training.  Investigation 2026-05-08: this is genuinely multi-iter scope — `gather_mm` with `[n_experts, output, input]` weight tensor, per-token weight selection, autograd-aware scatter. | DEFERRED (multi-iter) |
