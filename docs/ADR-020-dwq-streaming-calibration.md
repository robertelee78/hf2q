...
| 11h-c2 | 1/2 | **`gated_delta_update` forward composition on GpuTape** — per-token recurrent chain: `alpha = exp(-g[t])` (uses 11h-c1) → `state_dec = scalar_mul(alpha, state)` → `delta = v[t] - matmul(state_dec, k[t])` → `state = state_dec + scalar_mul(beta[t], outer(delta, k[t]))` → `output[t] = matmul(state, q[t])`.  May need `OpKind::OuterProduct` (= matmul of column × row vectors).  Naive O(N) sequential composition; fast-kernel-equivalent forward but slow per-step. Backward auto-derives from chain rule. | NEXT |
