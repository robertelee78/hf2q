...
| 15b | 2 | Tiled + simdgroup-MMA variant of qmm_affine_t — port mlx's `affine_qmm_t` template (BM=BK=BN=32, WM=WN=2, 128 threads/TG, 4 simdgroups, `mlx::steel::BlockMMA` backend).  Target: 5-10× speedup over iter-15's per-element kernel.  Validate via release benchmark vs hf2q's existing GGUF Q4_0 mm_id kernel as a peer baseline. | NEXT |
