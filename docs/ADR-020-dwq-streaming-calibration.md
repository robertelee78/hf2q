...
| 11h-c | 1/2 | **`gated_delta_update` forward** on GpuTape — recurrent linear-attention state update.  Production GPU kernels exist in mlx-native (`gated_delta_net*.metal`, 6 kernels); this sub-iter adds GpuTape `OpKind::GatedDeltaUpdate` + tape-side wrappers.  Forward only; backward (research-grade vjp derivation) is iter-11h-d. | |
