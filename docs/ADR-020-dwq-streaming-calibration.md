...
| **8b.1** | 1 | Wrap GPU matmul into a `GpuTape` + `GpuTensor` autograd structure (analogous to CPU oracle's `Tape`/`Tensor`); enable composition for multi-op forward chains. | **NEXT** |
