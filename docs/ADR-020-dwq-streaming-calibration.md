...
| 11c | 1 | Full transformer layer on GpuTape — `pre_norm → attention(.) → +residual → post_norm → ffn(.) → +residual` chain.  Tests: backward routes gradients to all 7 quantizable Linears (4 attn + 3 FFN). | |
| 11d | 1 | Multi-layer stack + embedding + lm_head + final RMSNorm.  Loop the iter-11c layer N times.  Embedding lookup is a tape op `OpKind::Embedding` (forward = gather; backward = scatter-add).  lm_head = matmul. | |
| 11e | 1 | Qwen3-0.6B-base weight loader integration — load GGUF dequant'd tensors into the GpuTape model's leaf tensors.  Reuses hf2q's existing GGUF loader; adds an `into_gpu_tape_leaves` adapter. | |
| 11f | 1 | Calibration data pipeline — tokenize sample text from `/opt/llama.cpp/tests` or HuggingFace WikiText fixture; produce `[batch, seq_len]` token ID buffers; embed via the iter-11d embedding op. | |
| 11g | 1 | End-to-end run on Qwen3-0.6B-base + sensitivity ranking + spot-check vs `crate::calibrate::sensitivity::compute_layer_sensitivity` (variance-magnitude).  Pass if `lm_head` and `output_norm` rank in the top half of both rankings (these are well-known sensitive layers). | |
...
| 13d | 2 | **Multi-step DWQ loop on real GGUF tensor** — loads `blk.0.attn_qkv.weight` (52,422,400 f32 elements, 1.6M groups @ group_size=32, 4-bit) from a Qwen 3.6 27B Q4_0 GGUF via mlx-native's `GgufFile::load_tensor_f32`, runs the affine init kernel, perturbs scales+biases by 2.0×, and trains 100 Adam steps.  Test gated `#[ignore]` (multi-GB GGUF on disk).  **Measured (15s runtime on M5 Max)**: init reconstruction MSE 9.38e-7 (per-group min/max heuristic near-optimal for real Q4_0 distributions); 2× perturbation start loss 2.44e-4 (260× init MSE → non-trivial); 100-step convergence to 1.47e-6 (165× reduction, ratio 0.006, far past the 5× floor); all 1.6M scales + 1.6M biases stay finite.  Validates real-tensor magnitude regime, per-group init on real distributions, KL gradient stability over 100+ steps with non-synthetic activations, peak GPU memory bounded via shared-tape + per-step `tape.reset()` (no `MlxDevice::new()` churn). Sibling tooling: `src/bin/dump_gguf_blk0.rs` diagnostic. | **DONE** (hf2q `cfa/adr020-iter10/claude` `pending`) |
| 13e | 2 | Replace iter-13d's reconstruction MSE with KL-div on a real-tensor logit distribution: pick a real activation fixture (random Gaussian X with stddev matched to typical layer-input distribution; later: real activations from a calibration corpus), compute teacher logits = X @ W_real, student logits = X @ qdq(W_real, s, b), KL with T=2.0.  Closes the conceptual gap to iter-13c's synthetic KL on real-tensor scale.  Pre-step toward iter-14's compute_dwq_targets. | NEXT |
