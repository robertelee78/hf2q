...
| 11h-e3 | 1/2 | **`switch_mlp` per-token expert dispatch** on GpuTape — for each token, run its top-K experts' MLP and weight by scores.  This is the SCATTER-GATHER pattern.  Production kernel exists in mlx-native (`moe_dispatch.rs`); needs autograd-aware variant for training. | |
