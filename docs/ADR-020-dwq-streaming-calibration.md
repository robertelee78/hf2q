...
| **10c** | 1 | **4-layer Qwen35 attention block on GpuTape** — real `RMSNorm → Q/K/V/O projections → SDPA(Q@K^T/√d → softmax → @V) → out_proj` chain consuming qdq'd weights from iter-10a as the W_low leaves.  All ops already on tape (matmul, rms_norm, softmax, scalar fold-into-weight).  Per-projection sensitivity scoring via the iter-9 estimate_sensitivities hook. | **NEXT** |
