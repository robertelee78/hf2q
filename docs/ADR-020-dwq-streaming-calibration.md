...
| 13d | 2 | **Multi-step DWQ loop on real GGUF tensor** — wire iter-11e's GGUF tensor adapter to iter-13c's training loop on a single Linear from Qwen3-0.6B-base.  Validates: real-tensor scales/biases initialization, KL gradient stability over 100+ Adam steps with non-synthetic logit distribution, peak GPU memory bounded.  Pre-step toward iter-14's full compute_dwq_targets stream-to-disk integration. | NEXT |
