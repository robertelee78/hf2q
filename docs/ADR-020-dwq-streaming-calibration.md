...
| **13b** | 2 | **Differentiable qdq + DWQ training loop** — replaces parameterless Q4_0/Q8_0 round-trip with explicit per-group `scales` + `biases` learnable params; tape-side `qdq_affine` with backward routing gradients to scales/biases (param weight is FROZEN).  Training loop: forward through qdq'd model → KL-div vs teacher (with temperature scale 1/T per mlx-lm) → backward → AdamOptimizer.step() → repeat.  Tests: small synthetic 2-Linear model converges KL-div loss to teacher logits via Adam over learnable scales+biases. | **NEXT** |
