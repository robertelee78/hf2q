...
| 11h-e2 | 1/2 | **MoE routing forward composition** on GpuTape.  Compose `gate(x) = matmul(x, W_gate)` → `softmax` → callsite computes `top_k_indices` (non-tape, via `top_k_f32` kernel or `argpartition`) → `take_along_axis_topk` (gather scores) → optionally renorm via `1/sum` (need divide op or compose row_sum + reciprocal).  Returns top-K (scores, indices) for downstream `switch_mlp` dispatch. | NEXT |
