...
| **8d** | 1 | GPU activations: {softmax, log_softmax, relu} + finite-diff parity vs CPU oracle.  mlx-native already has `softmax` kernel; relu via composed elementwise; log_softmax composed from softmax + log.  Backward formulas need per-row reductions for softmax-family. | **NEXT** |
