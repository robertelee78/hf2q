...
| **11c** | 1 | Full transformer layer on GpuTape — `pre_norm → attention(.) → +residual → post_norm → ffn(.) → +residual` chain.  Tests: backward routes gradients to all 7 quantizable Linears (4 attn + 3 FFN). | **NEXT** |
