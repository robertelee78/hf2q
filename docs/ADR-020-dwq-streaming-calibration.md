...
| 11h-c3 | 1/2 | **`gated_delta_step` forward composition on GpuTape** — per-token recurrent chain using rms_norm_gated (11h-a), conv1d_depthwise_causal (11h-b), exp (11h-c1), outer_product (11h-c2) + existing matmul + scalar_mul + sub + mul.  Math (single-step, scalar gating): `state_dec = scalar_mul(g, state)` → `kv_mem = matmul(state_dec, k_T)` → `delta = scalar_mul(beta, sub(v, kv_mem))` → `state_out = state_dec + outer(delta, k)` → `y = matmul(state_out, q_T)`.  Backward auto-derives from chain rule.  Tape-side FD falsifier on q/k/v/state/g/beta. | NEXT |
