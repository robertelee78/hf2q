...
| 13c | 2 | qdq_affine → reshape → matmul → KL-div training-loop completion: tape `view` op (zero-copy shape change; backward = identity through view), KL-div loss with temperature scale 1/T per mlx-lm; replace iter-13b's reconstruction MSE with end-to-end logit KL on a synthetic 2-Linear teacher/student MLP.  Closes the conceptual gap to mlx-lm `dwq_quantize`. | NEXT |
