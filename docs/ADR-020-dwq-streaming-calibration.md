...
| 17b | 2 | **Real-model DWQ e2e** — replace synthetic teacher in iter-17 with `GgufTeacherProvider` (iter-14b — landed `498c3e6`).  Concrete chain: (a) load `qwen3.6-27b-mtp-q4_0.gguf` via `GgufTeacherProvider`; (b) extract one real Linear's W (e.g. `blk.0.attn_qkv.weight`) via `mlx_native::GgufFile::load_tensor_f32` (iter-13e pattern); (c) tokenize a small fixed calibration corpus deterministically (host-side; no external tokenizer shellout); (d) `compute_dwq_targets` → top-K targets in tempdir; (e) `drop(teacher)` per `dwq.py:386-387` defensive pattern; (f) Adam-train per-group scales+biases via existing `dwq_loop` chain (iter-13b/c/d/e); (g) save trained `MlxAffineLinear` via iter-16b writer; (h) reload via iter-16 loader; (i) run iter-15 `qmm_affine_t_f32` infer on the same X used pre-train; (j) assert post-DWQ logit distribution properties hold (finite + non-degenerate top-1 + KL-vs-Q4-baseline within plausible band).  **Per §11 peer comparison is offline**: capture llama.cpp / mlx-lm reference numbers once via the user manually, then hardcode as static reference constants in the test — no runtime shellout to peer CLIs from inside the test (matches iter-19c pattern of citing canonical numbers from peer publications). | NEXT — unblocked by 14b |
