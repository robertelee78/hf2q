...
| **11g** | 1 | End-to-end run on Qwen3-0.6B-base + sensitivity ranking + spot-check vs `crate::calibrate::sensitivity::compute_layer_sensitivity` (variance-magnitude).  Pass if `lm_head` and `output_norm` rank in the top half of both rankings (these are well-known sensitive layers). | **NEXT** |
