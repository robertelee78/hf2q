...
| 15c | 2 | **Simdgroup-MMA `qmm_affine_t_f32_simd` kernel** — Apple GPU hardware `simdgroup_matrix<float, 8, 8>` MMA for the inner reduction.  **15c-1 (this iter, mlx-native `feb1e0e`)**: simplest-correct port — single-simdgroup-per-TG (32 threads), BM=BN=8, BK=32, output 8×8 tile per TG.  Inner reduction: 4 sub-tiles of 8x8 along BK; per sub-tile `simdgroup_load(ma)` + `simdgroup_load(mb, transpose=true)` + `simdgroup_multiply_accumulate`.  Bounds-checked write-back: full-tile path direct `simdgroup_store`, partial-tile path stages 8x8 to TG mem + element-wise copy.  3 new tests PASS (byte-parity vs per-element on 32×64×128, unaligned M=13 N=21 partial-tile, group_size validation).  qmm_affine module: 8 → **11 PASS** (no regressions).  **Bench (M5 Max, 64×4096×4096 g=32 20-iter avg)**: per-element 899 GFLOPS, tiled (15b) 2053 GFLOPS (2.28× over baseline), simd-MMA (15c-1) **2423 GFLOPS (2.69× over baseline, 1.18× over 15b)**.  **Honest gap**: §8.3 AC #4 5× perf target not yet met — single-simdgroup layout uses only 32 of the ~128+ threads available per Apple GPU core's simdgroup pool.  **15c-2 NEXT**: 4-simdgroup-per-TG variant with 32×32 output tile mirroring GGML reference structure (`quantized_matmul_mm.metal` pattern, `mc[8]` accumulators per simdgroup); expected 4-6 TFLOPS.  **15c-3**: full mlx `affine_qmm_t<BM=BK=BN=32, WM=WN=2, mlx::steel::BlockMMA>` template port; expected 6-7 TFLOPS — clears AC #4. | **DONE 15c-1** (mlx-native `feb1e0e`); 15c-2 NEXT |
