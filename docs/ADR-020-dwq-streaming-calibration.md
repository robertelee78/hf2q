...
| **10d** | 1 | **Per-batch streaming pattern** — drop per-batch grads + force GPU sync between batches (Metal commit_and_wait equivalent of `del grads + mx.eval(grad_accum)`).  Peak memory bound to single-batch grads + grad_accum.  Test: streaming-vs-bulk parity at byte-identity. | **NEXT** |
