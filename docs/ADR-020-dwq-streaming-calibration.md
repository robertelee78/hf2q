...
| 11a | 1 | Multi-head SDPA on GpuTape — slice + concat tape ops, head-loop composition wraps the iter-10c single-head pieces (`matmul`, `transpose`, `softmax`).  Test: n_heads=1 byte-identical to iter-10c; n_heads=4 against per-head-loop CPU oracle. | |
| 11b | 1 | SwiGLU FFN on GpuTape — `silu(x @ W_gate) · (x @ W_up) → @ W_down`.  Adds `OpKind::SiLU` (forward `silu(x) = x · sigmoid(x)` + backward `silu'(x) = sigmoid(x) · (1 + x · (1 − sigmoid(x)))`).  Tests: GPU↔CPU oracle parity + finite-diff falsifier. | |
