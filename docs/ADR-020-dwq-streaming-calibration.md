...
| 11h-f | 1/2 | **Full Qwen3.5MoE forward on GpuTape integration** — composes 11h-a..e + existing OpKinds into `Qwen35Model::forward_on_tape`.  Once landed, unblocks iter-12d (Track 2 production driver SOTA training) AND iter-12b-2 (real-model GPU FD scorer via direct gradient-Taylor instead of FD). | |
