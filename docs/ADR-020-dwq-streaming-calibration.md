...
| 11h-b2 | 1/2 | **GpuTape `OpKind::Conv1dDepthwiseCausal` wire-up** in `hf2q/src/calibrate/autograd_gpu_tape.rs`.  Single-parent OpKind variant carrying `kernel_w` as a co-parent (or stored alongside).  Forward via `dispatch_conv1d_depthwise_causal_forward_f32`; backward routes to `dx_f32` + `dw_f32` dispatches.  Tape-side FD falsifier on a small fixture composed with matmul + silu. | NEXT |
