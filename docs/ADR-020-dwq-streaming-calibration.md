...
| 12b | 1 | Re-back Track 1 sensitivity scorer with native gradient-Taylor (replace `src/calibrate/sensitivity.rs:68` `score = sqrt(variance) * log2(1 + max_magnitude)` with iter-9's `estimate_sensitivities` formula `(∇_W KL · (W_low − W_high)).sum() / params_M`).  Bump `SENSITIVITY_ALGORITHM_VERSION` to `2.0.gradient-alignment`.  Verify CLI `dynamic-quant-4-6` produces correct ranking via integration test. | NEXT |
