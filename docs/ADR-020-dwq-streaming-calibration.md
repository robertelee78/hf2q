...
| 11h-b | 1/2 | **Conv1d (depthwise) forward + backward** on GpuTape — required by `GatedDeltaNet`'s `self.conv1d` step (qwen3_5.py:105-112: `groups=conv_dim` makes it depthwise).  mlx-native already has `ssm_conv.metal` (production forward); needs vjp + GpuTape `OpKind::Conv1dDepthwise` wiring. | NEXT |
