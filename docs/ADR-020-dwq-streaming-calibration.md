...
| **11b** | 1 | SwiGLU FFN on GpuTape — `silu(x @ W_gate) · (x @ W_up) → @ W_down`.  Adds `OpKind::SiLU` (forward `silu(x) = x · sigmoid(x)` + backward `silu'(x) = sigmoid(x) · (1 + x · (1 − sigmoid(x)))`).  Tests: GPU↔CPU oracle parity + finite-diff falsifier. | **NEXT** |
