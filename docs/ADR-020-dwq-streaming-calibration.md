...
| 19b | — | Real-model mlx-lm KL parity test — external comparison harness.  Steps: (1) run `mlx_lm.dwq` on `jenerallee78/Qwen3.6-35B-A3B-Abliterix-EGA-abliterated` to produce mlx-format DWQ output; (2) run `mlx_lm.kld` against an 8-bit ref to capture the canonical target KL; (3) run hf2q's port on the same model + same calibration corpus; (4) compare per-token KL within 5%.  **Gated on**: iter-14b (real GgufTeacherProvider — exposes batched logits from `forward_prefill_batched` instead of just argmax token) + iter-11h (full multi-layer Qwen3 forward on GpuTape — currently iter-11h is itself gated on Qwen3-0.6B-base download for sensitivity-rank spot-check; for iter-19b we need the bigger 35B-A3B forward path).  External step: requires user authorization to run mlx-lm against the on-disk apex Q5_K_M reference + write the DWQ output to disk. | NEXT (gated on 14b + 11h) |
