...
| 14 | 2 | Port `compute_dwq_targets` stream-to-disk (`/opt/mlx-lm/mlx_lm/quant/dwq.py:29-66`) — teacher logits precomputed once on full forward, top-1024 take_along_axis sparsification, write to safetensors per-batch under `target_dir/{train,valid}/{i:010d}.safetensors`.  Teacher-drop sequencing: teacher model freed after target precompute. | **NEXT** |
