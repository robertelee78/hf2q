...
| 12b-2 | 1 | Real-model GPU FD scorer: capture `y_T` via `GgufTeacherProvider` (iter-14b), capture per-Linear input activations via `activation_capture_real.rs`, swap one Linear's `W` in-place in `Qwen35Model` and re-run `forward_gpu` → measured `y_low`, then `y_high`.  Returns per-Linear sensitivity `BTreeMap<String, f32>`.  Ignored real-model test on Qwen3.6-27B Q4_0. | NEXT |
...
| 15c-3 | 2 | Full mlx `affine_qmm_t<BM=BK=BN=32, WM=WN=2, mlx::steel::BlockMMA>` template port.  Optional — only if small-M case (M<128) is profile-confirmed bottleneck after 15c-2 lands in production. | OPTIONAL |
