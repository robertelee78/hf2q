...
| 19b (live) | — | **Live mlx-lm DWQ run on cached 35B BF16 — IN PROGRESS** as of 2026-05-07.  Working dir: `scripts/dwq_kl_parity/abliterix_with_chat_template/` (weight symlinks + tokenizer fixes from `00_prep_working_dir.sh`).  Two harness fixes required for the abliterated model: (1) inject `chat_template` from cached `Qwen/Qwen3.6-27B` (maintainer stripped); (2) remap `model_type "qwen3_5_moe_text"` → `"qwen3_5_moe"` (mlx-lm 0.31.2 MODEL_REMAPPING gap).  First run OOM'd at batch=4 in first training step on 128 GB unified memory; restarted batch=2 + `--grad-checkpoint`.  **Captured pre-training number**: initial validation loss = **0.0664** at T=2.0 (per-token KL between BF16 teacher and fresh RTN-Q4 student, BEFORE DWQ training).  **Consistent with smcleod's published RTN-Q4 baseline of 0.07418 on the same model class** (within 11%; differences from sample seed / corpus shard).  Training run continues; final validation loss + post-mlx_lm.kld measurement will land when complete. | **PRE-TRAINING KL CAPTURED 0.0664; training in progress** (PID 36650, batch=2) |
...
| 19b (half-2) | — | hf2q-side parity: run our port on the same model + same calibration corpus + same seed → measure per-token KL via the same vendored kld.py → compare against half-1's measured target.  Gated on iter-14b (real GgufTeacherProvider — exposes batched logits from `forward_prefill_batched` instead of just argmax token) + iter-11h (full multi-layer Qwen3.5MoE forward on GpuTape including hybrid linear+full attention layer types). | NEXT (gated on 14b + 11h) |
