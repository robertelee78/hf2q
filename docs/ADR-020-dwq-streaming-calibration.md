...
| 10 | 1 | Wire estimate_sensitivities into a real 4-layer Qwen35 attention block (RMSNorm + Q/K/V/O projections); add per-batch streaming with `del grads + mx.eval(grad_accum)` rhythm; add real qdq primitive (CPU oracle + GPU). Split into 10a/10b/10c/10d (see §8.2). | IN PROGRESS |
...
| **10b** | 1 | **GPU RMSNorm backward kernel** + GpuTape `OpKind::RmsNorm` op.  Forward already exists in mlx-native (`rms_norm.metal`); backward is the symmetric 2-phase: per-row `Σ_j x[j] · y[j] · w[j] / D` for the residual, then `dx = (dy · w − residual · x) · rms_inv`.  Tests: GPU↔CPU oracle parity on the analytical formula. | **NEXT** |
| 10c | 1 | **4-layer Qwen35 attention block on GpuTape** — real `RMSNorm → Q/K/V/O projections → SDPA → out_proj` chain consuming qdq'd weights from iter-10a as the W_low leaves.  Per-projection sensitivity scoring via the iter-9 estimate_sensitivities hook. | |
| 10d | 1 | **Per-batch streaming pattern** — drop per-batch grads + force GPU sync between batches (Metal commit_and_wait equivalent of `del grads + mx.eval(grad_accum)`).  Peak memory bound to single-batch grads + grad_accum.  Test: streaming-vs-bulk parity at byte-identity. | |
