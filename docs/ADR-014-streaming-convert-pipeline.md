...
| P6 — Imatrix calibrator (pure-Rust port) | 🟡 algorithm + legacy I/O + GGUF I/O write-side landed; GGUF read-side + cross-validation gate pending | `511d35c` (algorithm core), `2577d89` (legacy .imatrix I/O), `<iter-3-commit>` (P6 iter-3 — GGUF format write per llama.cpp PR #9400 / commit 90083283 / 2025-07-19) | `ImatrixCollector` with dense (`GGML_OP_MUL_MAT`) + MoE (`GGML_OP_MUL_MAT_ID`) accumulators; `Stats { values, counts }` invariant; `finalise()` produces per-column importance vectors. Legacy `.imatrix` save/load byte-for-byte matches `imatrix.cpp::save_imatrix_legacy` + `load_imatrix_legacy`. **GGUF format write** matches the schema landed in llama.cpp PR #9400 (commit `90083283`, 2025-07-19): GGUF v3 magic + 4 metadata KVs (`general.type`, `imatrix.datasets`, `imatrix.chunk_count`, `imatrix.chunk_size`) + 2 F32 tensors per stat entry (`<name>.in_sum2` 2D + `<name>.counts` 2D), 32-byte aligned data section. 21 unit + round-trip tests (15 legacy + 6 GGUF). Pending: GGUF-format **read** (`load_imatrix_gguf`) for compatibility with modern llama.cpp imatrix files; cross-validation gate against `llama-imatrix` (lands at P7 close when Calibrator trait wires forward-pass). |
