...
| P7 — `Calibrator` × `OutputFormat` orthogonal split | 🟡 trait + None impl landed; Imatrix/Dwq impls + k-quant codebook + Layout A migration pending | `33081aa` (Calibrator trait + NoneCalibrator + CalibrationData enum) | `Calibrator` trait (Send + Sync + object-safe) + `CalibrationData` enum (None / Imatrix / ImatrixWithStats / Dwq) + `CalibrationCorpus` + `NoneCalibrator` truly no-op impl + 5 typed `CalibrationError` variants. 7 unit tests. Pending: ImatrixCalibrator + DwqCalibrator impls (P7 iter-2-bis); k-quant codebook port (P7 iter-3); Layout A path migration (P7 iter-4). |
