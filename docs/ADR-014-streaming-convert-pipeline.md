...
| P11 — Re-emit ADR-012's four DWQ GGUFs under streaming pipeline + measured gate close | ⏳ pending hardware | — | Depends on every prior phase. Closure AC.  **P11-prereq fully closed (commits `4349fb2`, `3ce8674`, `49f1c0c`)**: codec-direct fast-path + `DwqKQuantizer` + `cmd_convert` wiring; `--quant dwq46/48/68/28` now emits Q4_K_M-family GGUFs. P11 itself (re-emit ADR-012's 4 DWQ artefacts + measured gate close) still pending hardware. |
...
**Iter C landed (commit `49f1c0c`, 2026-04-27)**: P11-prereq fully closed.
