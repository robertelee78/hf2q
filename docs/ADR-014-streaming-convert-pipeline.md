...
| ----- | ------ | ------ | ----- |
| P0 — Lazy tensor primitive + lazy safetensors reader | 🟢 closed (commits `c707a2c`, `038f2ab`, `5a7a50b`); LazyTensorMap-iter spike landed: peak RSS 5140 MB ≤ 8192 MB Decision 2 gate ✓ on a 48 GB MoE-class HF safetensors directory (`/opt/hf2q/models/gemma4`, gemma-4-26B-A4B-it BF16), measured during this iter via the fresh-subprocess `getrusage(RUSAGE_SELF)` reader landed in `tests/p0_apex_moe_lazy_iter.rs`. The canonical apex MoE path (`models/qwen3.6-35b-...-apex/`) ships only the GGUF artefact + tokenizer (no safetensors shards on disk as of 2026-04-27) so the literal apex path was not exercised; the gate test resolves any HF-format safetensors directory via `HF2Q_APEX_MOE_PATH` and is `#[ignore]`-gated for hardware re-runs against the actual apex safetensors source when downloaded. | `c707a2c` (lazy primitive), `038f2ab` (lazy reader + bridge), `5a7a50b` (apex-MoE iter spike) | LazyTensor + LazyTensorMap + lazy safetensors reader land; 13 unit + 4 reader + 1296 full-bin tests pass; eager bridge byte-identical to pre-ADR-014. Decision 6 gate value locked at **36.3 GB** (33 GB ADR-012 P9b inherited + 10% headroom) per `docs/peer-parity-baselines-2026-04-26.md`. **Decision 2 `≤ 8 GB` LazyTensorMap-iter spike** (this iter, commit `5a7a50b`, 2026-04-27): `tests/p0_apex_moe_lazy_iter.rs` (NEW, 513 LOC, ~150 LOC of test logic + ~360 LOC of inline `//!` and `///` Chesterton-fence documentation covering the cargo-runner state-pollution failure mode + macOS `ru_maxrss` unit caveat + apex-not-on-disk explanation + file-fence rationale) lands the gate — opens an HF safetensors directory via `read_tensors_lazy` (the production lazy reader), iterates every tensor through `materialize → drop`, measures peak RSS via in-process `getrusage(RUSAGE_SELF)` taken AFTER the iter has run + every per-tensor `Vec<u8>` has been dropped (so the high-water mark captures the iter's peak). One always-on smoke test (`lazy_iter_peak_rss_smoke_with_synthetic_fixture`) verifies the measurement plumbing on a 4-tensor synthetic safetensors fixture in a tempdir + a 2 MiB witness allocation that guarantees a measurable monotonic RSS delta (the smoke is a wiring check, not an absolute-bytes assertion — cargo's multi-threaded test runner shares process state across tests so `RUSAGE_SELF` is cumulative across the whole runner; the real-model gate runs in a fresh subprocess to dodge this). One `#[ignore]`-gated real-model gate (`lazy_iter_apex_moe_peak_rss_le_8gb`) spawns `current_exe()` targeted at a sibling `#[ignore]`-gated helper (`lazy_iter_subprocess_helper`) with `HF2Q_P0_SPIKE_INPUT=<dir>`; the helper drives the iter and prints a parseable `PEAK_RSS_BYTES=<N>` line to stdout; the parent parses it and asserts `peak ≤ 8 * 1024^3 = 8589934592`. **Measurement run during this iter** against `/opt/hf2q/models/gemma4` (gemma-4-26B-A4B-it BF16, 48 GB across 32 safetensors shards — closest MoE-class HF safetensors directory on disk): **peak RSS 5140 MB ≤ 8192 MB ✓** (the lazy reader's per-tensor `mmap[off..len].to_vec()` + drop-at-end-of-loop bounds peak resident bytes to ~one tensor at a time; gemma-4-26B's largest tensor is well under 5 GB so 5140 MB peak reflects steady-state working bytes including mmap page cache + indicatif progress bar + tracing subscriber overhead, plus the largest single materialised tensor in flight). The literal apex MoE path (`models/qwen3.6-35b-...-apex/`) ships only the GGUF artefact today, so the spec-default safetensors gate cannot run there until the upstream HF safetensors source is re-downloaded; the test surfaces this via `resolve_apex_moe_path` returning a typed deferral message, and the `#[ignore]`-reason explicitly calls out the override env var for hardware re-runs. The gate is re-runnable via `HF2Q_APEX_MOE_PATH=<hf-safetensors-dir> cargo test --release --test p0_apex_moe_lazy_iter -- --ignored --exact lazy_iter_apex_moe_peak_rss_le_8gb --nocapture`. Verification: `cargo build --release -p hf2q` 0 errors / 79 warnings (matches baseline); `cargo test --release --test p0_apex_moe_lazy_iter` 33 passed / 0 failed / 2 ignored / 0 filtered out (1 always-on smoke + 32 transitive `#[cfg(test)]` from `#[path]`-included `src/ir/mod.rs` + `src/progress.rs` + `src/input/safetensors.rs`); `cargo test --release --test peer_parity_gates -- --test-threads=1` 47 passed + 8 ignored (preserved); `cargo test --release --test ppl_driver` 26 passed (preserved); `cargo test --bin hf2q --release` 2101 passed + 11 ignored (preserved); `cargo clippy --release -p hf2q -- -D clippy::correctness` 0 errors / 388 warnings (well within ≤ 395 budget). |
...
| P2 — Streaming quantize loop (per-tensor write-and-drop) | 🟢 iter-2 landed (commit `1b778b9`); **iter-3 architecturally complete + env-flag-gated + production E2E byte-identity certified** as of iter-66.  Honest state of P2 (iter-70 milestone, 2026-04-28): 59 commits over the 48h push under `adr-014 p7 iter` prefix; cumulative work covers: (a) all 4 Phase 3 dispatch arms wired through streaming wedge under `HF2Q_STREAMING_PHASE3=1` (iter-48 K-quant codec / iter-49 ImatrixAdaptive / iter-50 StaticQuantizer / iter-51 DwqK); (b) Phase 4.5 quality wired to lazy variant (iter-52); (c) LazyTensorMap API additions `total_size_bytes` (iter-54) + `convert_bf16_to_f16` (iter-55); (d) byte-identity certification triad — streaming determinism (iter-65), eager determinism (iter-66), cross-path SHA equality (iter-61-67); (e) 4-arm × 2-format × 3-quantvar SHA matrix at file level (gguf q4_k_m/q5_k_m/q6_k/dwq-4-6/f16 + safetensors q4_k_m); (f) 3 structural bugs caught + fixed (iter-34/37/38).  **Deferred — iter-3 wholesale surgery**: actual `materialize_all()` skip is genuinely large (26 downstream consumer sites per iter-53 survey); the env flag wedge is currently a TEST INTEGRATION channel, not a memory win (peak 2× during borrowed-wedge clone).  Surgery deferred until either (i) the broader-trait `OutputBackend::quantize_and_write_lazy` lands or (ii) Phase 4.6 native path is rerouted off `&TensorMap`.  All API + test scaffolding for the surgery is in place; remaining work is structural refactor with the iter-61-67 SHA matrix as the safety net.  Until then, default behavior unchanged (env flag OFF).  **iter-71 benchmark snapshot (2026-04-28)**: cargo test --bin hf2q → 2186 pass + 11 ignored; --test cmd_convert_dispatch → 13 pass + 2 ignored; --test peer_parity_gates → 47 pass + 8 hardware-ignored; --test ppl_driver → 26 pass; --test imatrix_xvalidation → 36 pass + 1 ignored.  Cumulative ADR-014-related test surface: ~2308 passing, 22 hardware-ignored.  Earlier roll-up: |  Under `HF2Q_STREAMING_PHASE3=1`: all 4 Phase 3 dispatch arms (iter-48 K-quant codec / iter-49 ImatrixAdaptive / iter-50 StaticQuantizer / iter-51 DwqK) + Phase 4.5 quality (iter-52) route through the streaming pipeline.  LazyTensorMap API gained `total_size_bytes` (iter-54) and `convert_bf16_to_f16` (iter-55).  6-step lazy pipeline composes cleanly without `materialize_all` ever firing (iter-56 architectural simulation gate).  Final main.rs surgery (replace unconditional `materialize_all()` at line 1147 with bridge gated on `backend.requires_native_quantization()`) is now a transparent code shuffle — every consumer site has a streaming counterpart with byte-identity gates.  Default behavior unchanged across all wired arms (env flag OFF).  3 structural bugs caught + fixed during the matrix-coverage push (iter-34/37/38).  Tests: 2186 bin-test pass with iter-1 through iter-56 work landed. | `58e3144` (iter-1 — quantize_streaming function + LazyTensorMap::from_eager bridge), `1b778b9` (iter-2 — cmd_convert Phase 2 dispatch wires Calibrator + KQuantCodecQuantizer + VariantKQuantizer + DWQ-via-DwqCalibrator end-to-end. select_calibrator now drives every --quant variant. The legacy in-main.rs capture orchestration moved into DwqCalibrator (added `with_activation_capture` deferred-build constructor); main.rs DWQ arm collapses from ~280 LOC to ~30 LOC. cmd_convert holds 0 callers of `RealActivationCapture::new` / `capture_activations_to_sensitive_ranges`. Byte-identity gate for --quant f16 verified across two runs. 9 new tests across `tests/cmd_convert_dispatch.rs` (T1-T7 incl. Decision-17 byte-identity + no-silent-fallback gate) + `tests/quant_method_dispatch.rs` (T8-T9 routing partition).) | `quantize_streaming(LazyTensorMap, ..., bf16_to_f16: bool)` consumes a LazyTensorMap one tensor at a time — materialise → optional bf16→f16 cast → quantise → accumulate → drop. Byte-identical to `quantize_model` (verified by 2 unit tests). iter-2 (2026-04-27): `cmd_convert` Phase 2 dispatch restructured — `select_calibrator` builds the right `Calibrator` (NoneCalibrator / DwqCalibrator / ImatrixCalibrator) before the per-quantizer match; for DWQ on `Qwen35Dense`/`Qwen35MoE` the `DwqCalibrator::with_activation_capture` deferred path emits the intermediate F16 GGUF + drops `tensor_map` + builds `RealActivationCapture` inside `calibrate(...)` + drops it + re-reads `tensor_map` via the colocated `reread_qwen35_tensor_map_after_capture` helper. The `CalibrationData` then flows into `KQuantCodecQuantizer::new(..., calibration_data)` / `VariantKQuantizer::new(..., calibration_data, n_layers)` / the `DWQ` byte-emit through `run_dwq_with_sensitive_ranges`. Greppable invariants: `RealActivationCapture::new` and `capture_activations_to_sensitive_ranges` have **0 callers** in `src/main.rs`; `run_dwq_calibration` has **0 references** in `src/main.rs`; `select_calibrator` appears at exactly **2 sites** (preview + live). StreamingBackend trait + GgufBackend refactor pending iter-3.<br><br>**iter-89 (2026-04-28, commit `8027fc1`)**: extended iter-84's `HF2Q_STREAMING_PHASE3_MUT` SHA gate from a single q4_k_m proof to the full 4-arm Phase 3 matrix (q5_k_m + q6_k via K-quant codec arm; dwq-4-6 via DwqK arm; f16 via StaticQuantizer arm). New test `streaming_phase3_mut_byte_identical_matrix_across_arms` runs each variant twice (eager + `_MUT=1`) and asserts file-level SHA-256 equality. Closes the production safety matrix for the zero-byte-copy wedge: every arm wired under `_MUT` has its byte-identity locked. ImatrixAdaptive remains hardware-gated (iter-75 stub). Tests: 15/15 active dispatch tests pass, 3 ignored (hardware).<br><br>**iter-90 (2026-04-28, commit `52c0cd4`)**: refactored 4 Phase 3 dispatch sites in `cmd_convert` (K-quant codec / ImatrixAdaptive / DwqK / StaticQuantizer arms) — extracted the duplicated env-read + 3-arm if/else block into a single `dispatch_phase3_quantize` helper. Each site previously inlined ~24 lines of `HF2Q_STREAMING_PHASE3_MUT` + `HF2Q_STREAMING_PHASE3` env reads + dispatch. Net: 177 deletions, 110 insertions, **−67 LOC** in `src/main.rs`; the 4 arms × 3 paths matrix collapses to a single helper invocation per arm with an `arm_label` tracing tag. Byte-identity preserved across the refactor — full `cmd_convert_dispatch` SHA matrix passes (15/15 active, 3 hardware-ignored). Trace logs now key on `arm = "kquant_codec" | "imatrix_adaptive" | "dwq_k" | "static_quantizer"` so per-arm routing remains attributable.<br><br>**iter-91 (2026-04-28, commit pending)**: defensive guard against `_MUT × Phase 4.5` mismatch. Background: `HF2Q_STREAMING_PHASE3_MUT=1` drains `tensor_map` mid-Phase-3 (via `take_data_as_arc → mem::take`), leaving Phase 4.5 quality measurement to read empty `Vec<u8>` and silently emit a degenerate quality report. iter-91 adds an early `cmd_convert` check: if `_MUT=1 && !skip_quality`, fail fast with a clear error message naming both supported workarounds (`--skip-quality` to defer quality to `hf2q validate`, OR `HF2Q_STREAMING_PHASE3=1` borrowed wedge which shares via `Arc<Vec<u8>>` and preserves `tensor_map`). Three regression tests added: `streaming_phase3_mut_requires_skip_quality_guard` (negative — exit failure + stderr names env var), `streaming_phase3_mut_with_skip_quality_succeeds` (happy path), `streaming_phase3_borrowed_with_quality_succeeds` (borrowed mode with quality unaffected). Tests: 18/18 active dispatch tests pass (was 15; +3), 3 hardware-ignored. Closes the user-visible footgun while the lazy-from-disk Phase 4.5 path (iter-3 wholesale work) remains pending. |
...
| P4 — Eliminate the P9b intermediate-GGUF dance | 🟢 iter-1 landed (commit `f8f727e`) | `f8f727e` | DWQ qwen35/qwen35moe activation capture now builds `RealActivationCapture` from a transformed `LazyTensorMap` through `DwqCalibrator::with_activation_capture_lazy`; the temporary GGUF emit helper and P9b quantizer workaround are deleted. ADR-014 P7 iter-68 stocktaking (2026-04-28): commit hash recorded; P4 fully closed. |
...
| P6 — Imatrix calibrator (pure-Rust port) | 🟢 **closure-AC met iter-109 (commit `96e2a09`)** — env-override gate active (`HF2Q_P6_HF2Q_IMATRIX_PATH` + `HF2Q_P6_LLAMA_IMATRIX_PATH`); 36/36 always-on tests pass; operator recipe in test docstring drives the two inputs from `llama-imatrix` + `hf2q convert --calibration imatrix --output-format imatrix-gguf`. Algorithm + legacy I/O + GGUF I/O write+read landed; iter-1 gate machinery landed (commit `0920f7c`) | `511d35c` (algorithm core), `2577d89` (legacy .imatrix I/O), `d272005` (P6 iter-3 — GGUF format write per llama.cpp PR #9400 / commit 90083283 / 2025-07-19), `52d6386` (P6 iter-4 — GGUF format read with full save→load round trip incl. MoE per-expert counts), `0920f7c` (P6 close iter-1 — cross-validation gate machinery: `cross_validate_imatrix_gguf` + `XValidationReport` + `TensorComparison`) | `ImatrixCollector` with dense (`GGML_OP_MUL_MAT`) + MoE (`GGML_OP_MUL_MAT_ID`) accumulators; `Stats { values, counts }` invariant; `finalise()` produces per-column importance vectors. Legacy `.imatrix` save/load byte-for-byte matches `imatrix.cpp::save_imatrix_legacy` + `load_imatrix_legacy`. **GGUF format write+read** matches the schema landed in llama.cpp PR #9400 (commit `90083283`, 2025-07-19): GGUF v3 magic + 4 metadata KVs (`general.type`, `imatrix.datasets`, `imatrix.chunk_count`, `imatrix.chunk_size`) + 2 F32 tensors per stat entry (`<name>.in_sum2` 2D + `<name>.counts` 2D), 32-byte aligned data section. Reader walks metadata KVs (skipping unknown types via typed payload-size dispatch), pairs `<name>.in_sum2` + `<name>.counts` descriptors, reads tensor data at recorded offsets, reconstructs `Stats { values, counts }` exactly (MoE per-expert counts preserved — addresses the lossy-collapse known issue of the legacy format). 25 unit + round-trip tests (15 legacy + 6 GGUF write + 4 GGUF round-trip/read). Pending: cross-validation gate against `llama-imatrix` binary (lands at P7 close when Calibrator trait wires forward-pass). **iter-1 (2026-04-27, commit `0920f7c`)**: cross-validation gate machinery: `src/calibrate/imatrix_xvalidate.rs` (`cross_validate_imatrix_gguf` + `XValidationReport` + `TensorComparison`); reuses existing `load_imatrix_gguf` (no new GGUF reader); `is_pass()` predicate gates on (a) tensor-presence equality, (b) per-tensor `max_abs_diff_in_sum2 ≤ abs_tolerance OR max_rel_diff_in_sum2 ≤ rel_tolerance`, (c) exact `counts` match. Default tolerances `abs=1e-3, rel=1e-2` justified by the round-trip RMSE bounds locked in P7 iter-3x/3y (Q4_K ≤ 0.05, Q5_K ≤ 0.025, Q6_K ≤ 0.012). 5 always-on tests cover (a) self-compare passes, (b) perturbed in_sum2 fails at tight tolerance, (c) counts mismatch fails, (d) missing tensor fails, (e) markdown report shape lock. 1 `#[ignore]`-gated cell `xvalidation_vs_llama_imatrix_qwen35_smoke` is iter-2's hardware gate (Qwen3.5-0.6B + small corpus + `llama-imatrix` at `/opt/homebrew/bin/llama-imatrix` + our `ImatrixCalibrator` through `Qwen35Dense::forward`). cargo build `0 errors`; cargo test `--bin hf2q --release` 2049 passed (+6 from baseline 2043); `cargo test --release --test imatrix_xvalidation` 5 always-on tests + 1 ignored pass cleanly (integration crate also re-exercises the `#[path]`-included `imatrix.rs` + `imatrix_xvalidate.rs` inner unit tests, totalling 36 passed + 1 ignored — all pre-existing and unchanged). cargo clippy `0 errors / 388 warnings` (within ≤ 395 budget). |
| P7 — `Calibrator` × `OutputFormat` orthogonal split | 🟢 **closure-AC met iter-110 (commit `a75d9ed`)** — k-quant byte-identity gate covered at both file level (~14 SHA-matrix tests in `tests/cmd_convert_dispatch.rs`) and unit level (6 new `ggml_tensor_size` regression-guard tests on Q2_K/Q3_K/Q4_K/Q5_K including the canonical iter-99 case `Q4_K[248_044, 5120] = 714_366_720` bytes — the exact value llama.cpp computed at the iter-97 rejection). ImatrixCalibrator + DwqCalibrator landed; Layout A migration complete; cmd_convert minimal-wire seam in place iter-8 (`b5da618`): Layout A migrated; ImatrixCalibrator + DwqCalibrator landed; iter-9 (`ab661cf`): Q3_K wired into `KQuantTarget` enum (codec dispatch, metadata, all() round-trip) + 3 new variants `Q3_K_S`/`Q3_K_M`/`Q3_K_L` in `KQuantVariant` with policy branches per `llama-quant.cpp:439-460` (output → Q6_K bump on all K-family `_M` variants AND every Q3 variant) + `:527-528` (Q3_K_M attn_v: i<2→Q5_K, else→Q4_K) + `:578-582` (Q3_K_M ffn_down: i<n/16→Q5_K, use_more_bits→Q4_K, else→Q3_K) + `:530`/`:587-588` (Q3_K_L attn_v→Q5_K, ffn_down→Q5_K); 4 new policy tests + extended round-trip + ggml-name + dispatch tests automatically extended via `KQuantTarget::all()` + `KQuantVariant::all()` parameterization (281 quantize tests, +4); iter-10 (`1c3137b`): CLI menu surface for the Q3 family — `cli::QuantMethod` extended with `Q3KS`/`Q3KM`/`Q3KL` + `ImatrixQ3KS`/`ImatrixQ3KM`/`ImatrixQ3KL` (`q3_k_s`/`q3_k_m`/`q3_k_l` + `imatrix-q3_k_s/m/l` wire names); main.rs dispatch wired through K-quant codec arm with `KQuantTarget::Q3K` mapping for all 6 new variants + Auto resolver entries + `quantizer_default_bits` (=3) + `select_calibrator` routing (Q3 → NoneCalibrator; Imatrix-Q3 → ImatrixCalibrator); preflight bytes-per-param estimate (3.4375 bpw); cli `is_quantized` exhaustive coverage; new `test_q3_k_family_cli_surface_iter10` smoke (Display round-trip + dwq_bit_pair=None + filename suffix); 2128 bin tests pass; iter-11 (`19942b4`): round-trip RMSE bounds extended to Q3_K_S/M/L through the streaming dispatch — `variant_streaming_q5km_q6k_round_trip_rmse_bounds` parameterized cases list extended with 3 Q3_K cases (all assert ≤ 0.10 RMSE through `quantize_streaming` + `dequantize_row_q3_k_bytes` on a 4-super-block ramp, blk.5.attn_q.weight at layer 5/32 forces base-Q3_K target on every Q3 variant); locks the Q3_K codec→variant→streaming path end-to-end against any future policy regression that silently mis-routes Q3 variants; iter-12 (`bcc0461`): Q3_K_M + Q3_K_L streaming-path policy lock — `variant_streaming_q3km_policy_branches` exercises 7 distinct Q3_K_M branches (output→Q6_K, attn_v i<2→Q5_K, attn_v i≥2→Q4_K, ffn_down i<n/16→Q5_K, ffn_down use_more_bits→Q4_K, ffn_down else→Q3_K base, attn_q→Q3_K base) end-to-end through `quantize_streaming`; `variant_streaming_q3kl_policy_branches` exercises the simpler Q3_K_L policy (attn_v→Q5_K always, ffn_down→Q5_K always); catches a regression where `target_for` per-tensor dispatch silently mis-routes a bump category through the streaming path while isolated unit tests still pass; iter-13 (`b62b09f`): Q3_K_M imatrix-vs-none divergence gate — `variant_imatrix_q3km_diverges_from_none_through_streaming` mirrors iter-3w but for the Q3_K codec path; on `blk.5.attn_q.weight` (base-Q3_K target) with non-uniform importance vector, asserts NoneCalibrator-Q3_K_M and imatrix-Q3_K_M bytes DIFFER; catches a regression where the Q3_K imatrix dispatch in `quantize_row_to_bytes` silently routes through the uncalibrated path; iter-14 (`4722db7`): Q3_K_M imatrix improves importance-weighted error vs uncalibrated — `variant_imatrix_q3km_lowers_importance_weighted_error` mirrors iter-3z but for Q3_K; with 10000× importance ratio + adversarial input (wide-range high-importance cols, small low-importance cols), imatrix-weighted Q3_K_M produces strictly lower importance-weighted SSE than uncalibrated; catches a regression where the codec minimises the wrong objective (sign-flipped weights, inverted importance vector) on the Q3_K path; iter-15 (`253f824`): Q4_K_S + Q5_K_S CLI exposure — `cli::QuantMethod` extended with `Q4KS`/`Q5KS` + `ImatrixQ4KS`/`ImatrixQ5KS` (`q4_k_s`/`q5_k_s` + `imatrix-q4_k_s/k_s` wire names); main.rs Auto resolver + dispatch + bits + select_calibrator routing; preflight bytes-per-param (4.5/5.5 bpw); cli `is_quantized` + validation match exhaustive coverage; `test_q4ks_q5ks_cli_surface_iter15` smoke test (Display round-trip + dwq_bit_pair=None + filename suffix); iter-16 (`5248c28`): Q4_K_S + Q5_K_S streaming-path policy lock — `variant_streaming_q4ks_policy_branches` exercises 7 distinct Q4_K_S branches (output → Q4_K base, attn_v i<4 → Q5_K, attn_v i≥4 → Q4_K base, ffn_down → Q4_K base, attn_q → Q4_K base) end-to-end through `quantize_streaming`; `variant_streaming_q5ks_policy_all_base` locks Q5_K_S's no-upgrade policy (every tensor → Q5_K base, no output bump, no attn_v bump); catches a regression where _S variants accidentally inherit _M upgrade rules through the streaming dispatch; iter-17 (`6510de1`): round-trip RMSE bounds extended to Q4_K_S (≤ 0.05) and Q5_K_S (≤ 0.025) through streaming — `variant_streaming_q5km_q6k_round_trip_rmse_bounds` cases list extended with 2 _S variants; full K-quant menu (Q3_K_S/M/L, Q4_K_S/M, Q5_K_S/M, Q6_K) now has end-to-end round-trip RMSE coverage through `quantize_streaming` + matching `dequantize_row_q*_k_bytes`; iter-18 (`919c3c6`): **Q2_K block layout + dequantize port** from llama.cpp `ggml-common.h:286-298` + `ggml-quants.c:899-928`: `BlockQ2K` struct (84 bytes: scales[16] + qs[64] + d:f16 + dmin:f16 = 2.625 bpw); `dequantize_row_q2_k()` and `dequantize_row_q2_k_bytes()` implement per-element decode `y = dl * (q & 3) - ml` where `dl = d * (sc & 0xF)` and `ml = min * (sc >> 4)` per sub-block (16 sub-blocks × 16 elements); 8 unit tests including hand-crafted fixtures (dl=0/ml=1 → -1.0, dl=1/ml=0/q=1 → 1.0).  Quantize-side port deferred to iter-19; iter-19 (`2a27856`): **Q2_K quantize-side port** from `ggml-quants.c:829-897` (_ref) + `:1087-1147` (imatrix): `quantize_row_q2_k()` (uses `make_qkx2_quants(nmax=3)` + max-rescale at `q4scale=15` for super-block scale/min) + `quantize_row_q2_k_imatrix()` (uses `make_qkx3_quants(nmax=3)` + `make_qp_quants(nmax=15)` for the imatrix codebook search) + flat-bytes wrappers; `pack_q2_k_qs` helper packs the per-element 2-bit quants into the 64-byte qs[] field (same two-half layout as Q3_K's qs); 7 new tests including round-trip RMSE ≤ 0.20 on smooth ramp, Q2_K vs Q3_K monotonic-bits gate, imatrix-vs-none divergence, multi-block round-trip; cargo test → 2150 pass (+7); iter-67 (`eb53e29`): SHA gate covers StaticQuantizer arm (--quant f16) — `streaming_phase3_byte_identical_f16` exercises iter-50's wire-up at the file level.  Simplest dispatch (no quant, just F16 cast) but its own code path (main.rs:1681+).  Phase 3 arm SHA coverage now complete: K-quant codec direct (q4/5/6_k_m), StaticQuantizer (f16), DwqK (dwq-4-6); ImatrixAdaptive deferred (needs qwen35 fixture).  cargo test cmd_convert_dispatch → 15 pass.  iter-66 (`f1029c2`): eager path determinism gate — mirrors iter-65 for the EAGER path.  3× back-to-back eager convert runs must hash identically.  Locks eager determinism as the reference baseline for iter-61-64's eager-vs-streaming SHA gates.  Determinism contract now complete across both paths: streaming 3×=same (iter-65), eager 3×=same (iter-66), cross-path streaming==eager per variant (iter-61-64).  Three together form a complete byte-identity certification: streaming is byte-equal to eager AND eager is itself reliable as the reference.  cargo test cmd_convert_dispatch → 14 pass.  iter-65 (`61433bd`): run-to-run determinism gate — `streaming_phase3_run_to_run_determinism_q4_k_m` runs the streaming convert 3× back-to-back and asserts all three GGUFs hash identically.  Closes the file-level parallelism contract per Decision 5: rayon dispatch order doesn't introduce non-determinism via HashMap iter, worker-stealing, or accumulator ordering.  Catches a regression class (Mutex-ordered accumulator, non-deterministic schedule) that the eager-vs-streaming per-pair gates wouldn't surface (each run byte-identical to the same eager run, but the runs differ from each other).  cargo test cmd_convert_dispatch → 13 pass.  iter-64 (`490e4a8`): SHA gate covers `--format safetensors` — `streaming_phase3_byte_identical_safetensors_q4_k_m` hashes sharded output via `safetensors_dir_sha256` (filename + content concat in name-sorted order) for both eager + streaming paths.  Backend coverage now complete: GGUF (iter-61/62/63) + Safetensors (iter-64).  HF2Q_STREAMING_PHASE3 byte-identity contract locked across all 4 Phase 3 arms × all 2 output backends at file level.  cargo test cmd_convert_dispatch → 12 pass.  iter-63 (`2d959c6`): SHA gate matrix completed — `streaming_phase3_byte_identical_dwq_4_6` covers DwqK arm (most complex Phase 3 dispatch: sensitive/base routing + KQuantCodecQuantizer delegation + legacy_quant_method override).  4-arm coverage: K-quant codec direct (iter-48 q4_k_m + iter-62 q5_k_m/q6_k), DwqK (iter-63 dwq-4-6).  ImatrixAdaptive needs qwen35 fixture (deferred); StaticQuantizer covered by T6's existing f16 snapshot SHA.  cargo test cmd_convert_dispatch → 11 pass.  iter-62 (`381db30`): extend iter-61 E2E SHA gate matrix — `streaming_phase3_byte_identical_matrix_across_kquants` covers q5_k_m + q6_k.  Each variant: eager + HF2Q_STREAMING_PHASE3=1 → file-level SHA-256 equality.  Fixture's 256-elem rows produce real super-blocks per variant (Q4_K=144, Q5_K=176, Q6_K=210 bytes); SHAs differ across variants but eager-vs-streaming SHA matches WITHIN each.  Locks byte-identity contract across the K-quant codec family at the file level — any variant-specific regression surfaces immediately.  cargo test cmd_convert_dispatch → 10 pass (8 existing + iter-61 q4_k_m + iter-62 q5_k_m/q6_k).  iter-61 (`ac219e7`): **production E2E byte-identity gate** — `streaming_phase3_byte_identical_to_eager_q4_k_m` runs `hf2q convert --quant q4_k_m` twice (default eager + `HF2Q_STREAMING_PHASE3=1`) and asserts file-level SHA-256 equality.  Closes the missing safety signal: iter-48-52's per-arm unit tests don't prove the env flag's *integrated* effect across Phase 3 + 4.5 + 4.6 is byte-identical at the file level.  Result: SHAs match — streaming wedge integrates cleanly with Phase 4.6 backend.write to produce file-level identity.  Sets the production safety signal for iter-62+ wholesale surgery (skip materialize_all conditionally) — any regression introduced by swapping consumer sites would surface as a SHA mismatch.  cargo test cmd_convert_dispatch → 9 pass (8 existing + iter-61).  iter-60 (`5e0b733`): pure ordering refactor — backend creation moved BEFORE the materialize_all() bridge.  Order pre: lazy_map → materialize_all → backend creation.  Order post: lazy_map → backend creation → materialize_all.  No behavior change; preserves all 2186 bin tests.  Sets up iter-61+ conditional-bridge surgery (the predicate `backend.requires_native_quantization()` is now in scope at the bridge site).  Per the mantra ("DO NOT BE LAZY. No shortcuts. Measure 3x, cut once."), this is the smallest provably-safe surgery that unblocks the iter-3 wholesale skip without touching any consumer site.  iter-59 (`2912301`): P12 partial — `HF2Q_STREAMING_PHASE3` classified as Category 2 (supported operator knob) in `docs/shipping-contract.md`.  Same classification table as `HF2Q_LMHEAD_Q8`: user-facing escape hatch, stable in contractual sense, no removal or silent change without an ADR.  Rationale: byte-identical to eager path, default OFF, documented memory caveat.  Cross-references converting-a-model.md (iter-58 escape-hatch section).  iter-58 (`0f0e7d9`): P12 partial — `HF2Q_STREAMING_PHASE3` documented in `docs/converting-a-model.md` Escape-hatch env vars section, alongside HF2Q_USE_LEGACY_DWQ_Q4_0 + HF2Q_UNSAFE_EXPERIMENTS.  Documents what it does (routes 4 Phase 3 arms + Phase 4.5 through streaming), byte-identity guarantee, memory caveat (TEST INTEGRATION not memory win — wedge clones 2× briefly; actual savings land with iter-3 wholesale surgery), use cases (validation + iter-3 smoke), default OFF.  iter-57 ADR refresh: P2 row updated to reflect cumulative state across iter-48-56.  iter-56 (`fd66aae`): **iter-3 architectural simulation gate** — `iter3_lazy_pipeline_end_to_end_no_materialize_all` proves the full iter-3 wholesale-skip path composes correctly at the unit-test level.  6-step pipeline with NO `materialize_all()` ever firing: build LazyTensorMap → `convert_bf16_to_f16` (iter-55) → `total_size_bytes` (iter-54) → `quantize_streaming` (iter-1+47) → `measure_quality_streaming_lazy` (iter-43+45) → `GgufBackend::validate`.  Mixed-dtype fixture (2 BF16 + 1 F16) exercises full conversion+quantize+quality+validate chain.  Asserts BF16 count, byte total, QuantizedModel non-empty, per-layer cosine >0.99, validate clean.  Does NOT touch main.rs — purpose is to prove the iter-3 architectural shape works end-to-end so the eventual main.rs surgery is a transparent code shuffle.  cargo test → 2186 pass.  iter-55 (`584320a`): second iter-3 plan piece — `LazyTensorMap::convert_bf16_to_f16` mirrors eager variant.  Each BF16 tensor replaced with a LazyTensor wrapping closure (materialise → cast → drop BF16 buffer per tensor); cast deferred to materialisation time so the call itself is metadata-only.  Returns converted count (matches eager signature).  3-tensor BF16/BF16/F16 byte-equality gate asserts count + post-materialise dtype + per-tensor byte equality vs eager-converted output.  iter-3 swap at main.rs:1188 now unblocked at API level.  cargo test → 2185 pass.  iter-54 (`deb45ed`): first iter-3 plan piece — `LazyTensorMap::total_size_bytes` (metadata-only sum of LazyMeta::byte_len; pre-computed at construction so no byte payload access).  Mirrors `TensorMap::total_size_bytes` but for the lazy view; iter-3 wholesale-skip swap at main.rs:1166 telemetry site is now unblocked at the API level.  4-tensor multi-dtype byte-equality gate (F32/F16/BF16/U8 covers every DType element_size cell) asserts lazy total matches eager total post-materialize.  cargo test → 2184 pass.  iter-53 (`15a262d`): **iter-3 wholesale-skip plan** — comprehensive survey of all 26 tensor_map references downstream of main.rs:1147's materialize_all() bridge.  Status: 4 Phase 3 arms (iter-48-51) + Phase 4.5 (iter-52) wired (env-flag); telemetry + bf16 conversion + DWQ calibrator + bit_overrides need small LazyTensorMap API additions (total_size_bytes, convert_bf16_to_f16) or trivial swaps (.tensors.keys() → .names()).  **Phase 4.6 native backend `quantize_and_write(&tensor_map, ...)` is the only hard constraint** — its trait contract requires &TensorMap.  Plan: conditional bridge (`materialize_all()` ONLY when `backend.requires_native_quantization()`); non-native paths stay lazy through Phase 3+4.5+telemetry; native arm keeps the materialise.  iter-54+ executes this plan.  Documentation iter — main.rs:1147 comment block annotated with full checklist for future surgery.  cargo build clean.  iter-52 (`f63b1a1`): Phase 4.5 quality wired under `HF2Q_STREAMING_PHASE3=1` — routes through `measure_quality_streaming_lazy` (iter-43); builds parallel LazyTensorMap from resident tensor_map (per-tensor `from_bytes` clone, same memory profile as iter-48 wedge).  Byte-identity proven by iter-43 gate.  Default OFF.  All four Phase 3 arms (iter-48-51) + Phase 4.5 (iter-52) now stream-capable in production behind one env flag.  Only remaining `tensor_map` consumer downstream is Phase 4.6 native backend (separate `quantize_and_write` contract — P11-class work).  iter-3 surgery (skip `materialize_all()` upstream + source lazy_map from `read_tensors_lazy`) is now unblocked at every consumer site.  cargo test → 2183 pass.  iter-51 (`794b045`): **four-arm migration complete** — fourth Phase 3 arm wired (DwqK).  All four arms now stream-capable under `HF2Q_STREAMING_PHASE3=1`: K-quant codec direct (iter-48), ImatrixAdaptive (iter-49), StaticQuantizer (iter-50), DwqK (iter-51).  DwqK byte-identity gate `quantize_via_streaming_borrowed_byte_identical_under_dwq_k` mixes sensitive layer 5 → Q8_0 (272 bytes) + base layers 0/10 → Q4_K (144 bytes) under P48; asserts ggml_type + byte-data equality across BOTH buckets vs eager `quantize_model`, locking sensitive/base routing decisions fire identically.  legacy_quant_method override preserved post-quantize regardless of dispatch path.  HF2Q_STREAMING_PHASE3=1 now exercises the full streaming pipeline in production at every Phase 3 dispatch site.  cargo test → 2183 pass.  iter-50 (`3fc1958`): third main.rs Phase 3 arm wired — `HF2Q_STREAMING_PHASE3=1` now also routes StaticQuantizer (line 1681+, catch-all for F16/BF16/Q2/Q4/Q8/Auto) through `quantize_via_streaming_borrowed`.  Byte-identity already proven by iter-47's smoke fixture which uses StaticQuantizer.  3 of 4 Phase 3 arms now streaming-capable: K-quant codec direct (iter-48), ImatrixAdaptive (iter-49), StaticQuantizer (iter-50); DwqK arm queued for iter-51 (more complex due to sensitive_layers + calibration plumbing).  cargo test → 2182 pass.  iter-49 (`de3bd2d`): second main.rs Phase 3 arm wired — `HF2Q_STREAMING_PHASE3=1` now routes ImatrixAdaptive (VariantKQuantizer at line 1430+) through `quantize_via_streaming_borrowed`.  Wedge byte-identity guarantee extends here because VariantKQuantizer dispatch is a strict superset of KQuantCodecQuantizer.  iter-49 byte-identity gate `quantize_via_streaming_borrowed_byte_identical_under_imatrix_variant_kquantizer` adds the imatrix dimension (iter-48 covered uncalibrated only); proves streaming dispatch threads imatrix through to codec on every per-tensor target.  Two of four Phase 3 arms now wired: K-quant codec direct (iter-48) + ImatrixAdaptive (iter-49).  iter-50+ tackles DwqK and StaticQuantizer arms.  Note: mlx-native has unrelated parallel-session WIP build failure (`unretained_refs_enabled` undefined from ADR-005 W-5b.27); does not affect hf2q's own bin compilation.  iter-48 (`dbf5e3b`): **first production wire-up** — `HF2Q_STREAMING_PHASE3=1` env flag routes main.rs's K-quant codec direct dispatch arm (line 1376+) through `quantize_via_streaming_borrowed` instead of `quantize_model`.  Default OFF; default behavior unchanged.  Wedge helper `quantize_via_streaming_borrowed(&TensorMap, ...)` clones bytes per-tensor into transient LazyTensorMap (peak 2× briefly — TEST INTEGRATION not memory win; actual savings land when iter-3 surgery removes downstream `tensor_map` dependency in Phases 4.5/4.6).  Byte-identity gate `quantize_via_streaming_borrowed_byte_identical_to_quantize_model` proves env-flag swap is safe.  iter-49+ will (a) wire other arms ImatrixAdaptive/DwqK/StaticQuantizer, (b) refactor Phase 4.5 quality to `measure_quality_streaming_lazy`, (c) remove `materialize_all()` bridge once downstream consumers stop borrowing tensor_map.  cargo test → 2181 pass.  iter-47 (`64282eb`): **iter-3 wire-up wedge** — `quantize_via_streaming_consuming` is the per-arm migration toggle for main.rs Phase 3.  Drop-in for `quantize_model` that takes ownership of `TensorMap` and routes through `quantize_streaming` via `LazyTensorMap::from_eager` (bytes MOVED, no clone).  Memory peak unchanged this iter; actual win lands when iter-3 wires `read_tensors_lazy` bypassing materialise.  Byte-identity gate `quantize_via_streaming_consuming_byte_identical_to_quantize_model` uses 5-tensor multi-fixture spanning Q4_K_M's full categorical surface (output bump / attn_v / ffn_down / attn_q base / ffn_up); asserts ggml_type, method, preserved, AND bytes equal — main.rs swap is provably safe.  cargo test → 2180 pass.  iter-46 (`7bb067b`): exhaustive codec dequant matrix through quality pipeline — `codec_direct_dequant_matrix_through_quality_pipeline` auto-iterates every codec target (5 K-quants + 5 legacy = 10 dispatches) through `measure_quality_streaming`, asserting per-bit-tier cosine thresholds: Q2_K ≥0.95, Q3_K/Q4_K/Q5_K/Q6_K/Q4_0/Q4_1/Q5_0/Q5_1 ≥0.99, Q8_0 ≥0.999.  Locks the iter-45 codec-direct dequant arm against future `ggml_type` drift — adding a new target requires extending the match arm or this test fails on its threshold.  cargo test → 2179 pass.  iter-45 (`3ebd4da`): **closes iter-44 prerequisite** — K-quant + legacy codec dequant arm for quality measurement.  Bug: `dequantize_single_tensor` ignored `ggml_type` for codec outputs and fell into "no scales" warn path returning zeros.  Fix: `try_dequantize_codec_direct` arm dispatches off ggml_type — K-quants Q2_K/Q3_K/Q4_K/Q5_K/Q6_K → `dequantize_row_q*_k_bytes`, legacy Q4_0/Q4_1/Q5_0/Q5_1/Q8_0 → `dequantize_row_q*_bytes`; triggers when `scales.is_none() && ggml_type.is_some()`.  Coverage: `kquant_codec_direct_dequant_round_trip_via_quality_pipeline` runs full `measure_quality_streaming` on Q4_K codec output, asserts per-layer cosine >0.99 (pre-iter-45 was None).  Iter-3 wire-up of K-quant codec models can now produce meaningful quality reports.  cargo test → 2178 pass.  iter-44 (`2cf70a7`): **iter-3 end-to-end flow gate + new gap surfaced** — `iter3_end_to_end_flow_lazy_quantize_quality_validate` proves the full iter-3 wire-up shape (lazy_map_a → quantize_streaming → QuantizedModel; lazy_map_b → measure_quality_streaming_lazy; GgufBackend::validate) without any `materialize_all()`.  4-tensor multi-fixture; per-layer cosine >0.99, validate clean.  **Iter-3 prerequisite surfaced**: this iter discovered `quality::dequantize_single_tensor` does NOT handle `METHOD_K_QUANT_CODEC_DIRECT` (falls into no-scales warn path → returns zeros → cosine fails); iter-3 K-quant-codec wire-up needs a K-quant-aware dequant-for-quality (queued for separate iter).  Test pivoted to StaticQuantizer (q8) which has working dequant path.  cargo test → 2177 pass.  iter-43 (`421a1b1`): **iter-3 wedge dependency cleared** — `measure_quality_streaming_lazy` lands as the Phase 4.5 drop-in for iter-3.  Takes `&LazyTensorMap` instead of `&TensorMap`; per-tensor materialize_cloned → F32 cast → cosine pair → drop, same per-tensor memory bound as eager variant but no full-model `TensorMap` residency requirement.  Byte-identity gate `measure_quality_streaming_lazy_matches_eager` asserts equal per-layer cosine values + average + min + min_layer_idx vs `measure_quality_streaming` on the same fixture.  iter-3 surgery in main.rs:1147 can now skip materialize_all() and route Phase 4.5 directly off `lazy_map`.  Caveat: `is_weight_from_metadata` duplicates `TensorRef::is_weight` for metadata-only access (LazyMeta has no bytes); kept in sync until that helper accepts `(&str, &[usize])`.  cargo test → 2176 pass.  iter-42 (`5249a87`): **audit finding** — surface P2 iter-3 production-wiring gap.  `quantize::quantize_streaming` is built + gated by 50+ matrix tests but NEVER called from production `main.rs`; line 1147 `materialize_all()` bridge still runs upfront, defeating the streaming-pipeline memory-budget purpose.  Two-part deliverable: (1) `streaming_output_validates_clean_against_gguf_backend` contract gate proves drop-in replacement structurally safe (multi-tensor fixture → quantize_streaming → GgufBackend.validate → assert no error-severity warnings) — iter-3 wire-up will be a no-op behaviorally; (2) `main.rs:1147` annotated with iter-3 reference + open dependency (Phase 4.5 quality measurement still needs `tensor_map` for F32 reference, iter-3 needs a coherent story — lazy quality measurement OR re-read pass).  cargo test → 2175 pass.  iter-41 (`7e13756`): F16-passthrough paths × parallelism orthogonality — closes Decision 5 byte-identity for the F16-passthrough decision tree pinned in iter-40.  3-tensor multi-fixture exercises Path A (streaming preserve, vision_tower.…), Path B (codec gate, model.visual.…), and a regular K-quant codec-dispatch tensor (blk.5.attn_q) — all three asserted serial ≡ parallel on method, ggml_type, preserved flag, and bytes.  Plus iter-40's dual-predicate cross-path contract re-asserted under parallel dispatch.  Catches regressions in `quantize_streaming_parallel` that mishandle preserve=true tensors or re-enter codec gate non-deterministically.  cargo test → 2174 pass.  iter-40 (`d78c549`): dual vision-predicate path boundary contract — pins the structural finding from iter-39 debugging that hf2q has TWO vision predicates with different scope, driving F16-passthrough through TWO different code paths.  Predicate A (narrow): `TensorRef::is_vision_tensor()` matches only vision_tower/embed_vision; drives streaming.rs preserve → method="passthrough", ggml_type=None.  Predicate B (broad): `is_vision_tensor_pattern()` matches model.visual./vision_tower./vision_model./vit./visual./.visual.; drives codec+DwqK gates → method="f16", ggml_type=Some("F16").  B is strict superset of A.  Test exercises both: vision_tower.…→A→None, model.visual.…→B→Some("F16"); both produce 512-byte output with byte-equivalent data.  User-facing contract: "vision tensors are F16-preserved" regardless of gate, locked by `t_a.data == t_b.data`.  Catches future predicate merges or scope drifts.  cargo test → 2173 pass.  iter-39 (`f1cb22a`): vision policy overrides DwqK Q8_0 routing — orthogonal control to iter-38; pins that vision-pattern arm STILL fires for DwqK even with Q8_0 sensitive routing.  Test uses `model.visual.layers.5.attn.q_proj.weight` (matches codec's `is_vision_tensor_pattern` but NOT streaming's narrower `TensorRef::is_vision_tensor()` so streaming's preserve-true shortcut doesn't catch it; forces test through DwqK gate path).  Documents two distinct vision predicates: streaming's narrow set (vision_tower/embed_vision) for `preserve=true`, codec's broad set (+model.visual./vision_model./vit./visual./.visual.) for downstream F16 passthrough.  Catches a future "simplification" that drops the vision arm or wraps the OR in `is_kquant_routing`.  cargo test → 2172 pass.  iter-38 (`e22d054`): **bug fix #3** — DwqK still called the merged `should_emit_f16_for_kquant` predicate at line 274, applying K-quant row-misalignment arm to ALL routing decisions including Q8_0 sensitive layers; a 32-multiple-but-not-256-multiple row (row_len=96) routed to Q8_0 was falsely F16-passthrough'd instead of quantizing through the legacy codec.  Fix: routing-aware logic — resolve `routing_target=self.target_for(tensor)` first, gate row-misalignment arm on `is_kquant_routing`.  Coverage: `dwq_sensitive_q8_0_layer_with_32_multiple_row_emits_q8_not_f16` uses P48 (Q4_K base + Q8_0 sensitive; P46's sensitive is Q6_K which would still K-quant-passthrough); sensitive layer 5 expects Q8_0 (102 bytes), base layer 0 expects F16 control (192 bytes) proving the K-quant arm still fires for K-quant routing just not Q8_0.  Pre-condition `is_kquant_row_misaligned(96)==true` locks fixture falsification.  cargo test → 2171 pass.  iter-37 (`217d367`): **bug fix #2** — iter-34's `is_k_quant_target` gate accidentally collapsed the universal vision-pattern F16 passthrough too; vision tensors routed to legacy targets (Q4_0/Q4_1/Q5_0/Q5_1/Q8_0) silently quantized through the legacy codec instead of F16-passthrough.  Vision tensors are policy-driven F16 (multimodal-quality reasons), not block-size-driven.  Fix: split predicate in `layer_mix` into `is_vision_tensor_pattern` (universal) and `is_kquant_row_misaligned` (K-quant only), kept `should_emit_f16_for_kquant` as back-compat wrapper.  `KQuantCodecQuantizer` now ORs `is_vision || (is_k_quant_target && is_kquant_row_misaligned)`.  Coverage: `legacy_target_vision_tensor_emits_f16_passthrough` uses `vit.proj_out.weight` at row_len=256 (isolates vision axis: vision=true, alignment=false); all 5 legacy targets + Q4_K control assert ggml_type=F16, preserved=true, byte_count=512.  cargo test → 2170 pass.  iter-36 (`5db7321`): pin iter-34 bug-fix contract permanently — `legacy_target_non_256_multiple_emits_legacy_not_f16` is the direct regression-direction test for iter-34's `is_k_quant_target` gate.  Row_len=96 (= 3 × 32, NOT 256-multiple) is the canonical falsification fixture: pre-iter-34 emitted `ggml_type=Some("F16")` with 192 payload bytes; post-iter-34 each legacy target emits its own legacy codec output.  Test pre-condition asserts `should_emit_f16_for_kquant(row_len=96)==true` (locks the fixture's falsification property).  Each of 5 legacy targets asserted three ways: ggml_type == legacy (positive), ggml_type != "F16" (negative on bug surface), byte_count != ROW_LEN×2 (negative on F16 budget).  Mirrors iter-34's RMSE coverage but at the row-length boundary, pinning the contract from a different angle.  Without it, a future predicate "simplification" that re-applies to legacy would silently revert iter-34.  cargo test → 2169 pass.  iter-35 (`20dd8c0`): legacy target × parallelism orthogonality — `legacy_target_parallelism_byte_identity` mirrors iter-33's variant × parallelism matrix for the 5 legacy targets (Q4_0/Q4_1/Q5_0/Q5_1/Q8_0).  iter-33 only covered `KQuantVariant`; iter-35 closes `KQuantCodecQuantizer` directly per legacy target.  4-tensor multi-fixture (1024 elements each) through both serial `quantize_streaming` and rayon-parallel `quantize_streaming_parallel`; asserts `bytes_serial(T) == bytes_parallel(T)` per Decision 5.  Compile-time `Send + Sync` guard on `KQuantCodecQuantizer` preserves rayon-eligibility against future field additions.  Catches mis-ordering, non-determinism, or Send+Sync regressions on the legacy codec path.  cargo test → 2168 pass.  iter-34 (`5f42942`): **bug fix + coverage** — legacy targets were falsely F16-passthrough'd by `KQuantCodecQuantizer::quantize_tensor` because `should_emit_f16_for_kquant`'s non-256-multiple arm encodes a K-quant block-size constraint (QK_K=256) but was applied unconditionally; legacy formats (Q4_0/Q4_1/Q5_0/Q5_1/Q8_0) use 32-element blocks and any 32-multiple-but-not-256-multiple row was being mis-routed.  Fix: gate the predicate on `is_k_quant_target` (Q2K/Q3K/Q4K/Q5K/Q6K); vision-pattern arm still fires regardless of target.  `legacy_target_streaming_round_trip_rmse_bounds` auto-iterates the 5 legacy targets through the streaming path on a 1024-element smooth ramp: Q4_0/Q4_1 ≤0.05, Q5_0/Q5_1 ≤0.025, Q8_0 ≤0.005.  Closes legacy-format coverage gap left after iter-3x/3y (Q4/5/6_K), iter-11 (Q3_K), iter-17 (Q4_K_S/Q5_K_S), iter-22 (Q2_K).  cargo test → 2167 pass.  iter-33 (`37d5afe`): variant × calibrator × parallelism orthogonality matrix — `variant_calibrator_parallelism_matrix_through_streaming` closes the third orthogonality dimension after iter-31 (pair matrix) and iter-32 (calibrator matrix); auto-iterates `KQuantVariant::all()` × {None, Imatrix} × {serial, rayon-parallel} and asserts `bytes_serial(V, C) == bytes_parallel(V, C)` per Decision 5.  10 variants × 2 calibrators = 20 (V, C) combinations on a 4-tensor multi-fixture (blk.5.attn_q + blk.5.attn_v + blk.0.ffn_down + output.weight) with iter-32's adversarial fixture.  iter-4 only covered Q4_K_M+None on 6 tensors; iter-33 closes the remaining 18 (V, C) combinations.  Catches non-determinism in future quantize_tensor impls, mis-ordering in parallel output, Send+Sync regressions on VariantKQuantizer.  cargo test → 2166 pass.  iter-32 (`66d6fb5`): exhaustive variant × calibrator orthogonality matrix — `variant_calibrator_matrix_through_streaming` mirrors iter-31 on the orthogonal calibrator dimension; auto-iterates every variant in `KQuantVariant::all()` and asserts 4 invariants per variant: (1) quant_method tag stable across {None, Imatrix}, (2) ggml_type stable, (3) byte-count stable, (4) supports_imatrix(V) ⇒ bytes diverge.  Adversarial 4-super-block fixture (iter-3z/14/23 pattern: wide-range high-importance + small low-importance × 100× importance ratio) is sharp enough to force divergence across every K-quant family Q2-Q6 — Q5_K_M and Q6_K extend the divergence surface here for the first time.  Per Decisions 9+11, calibrator and OutputFormat are independent; this pins that orthogonality through the streaming pipeline.  Future-proofs: a `KQuantTarget` with `supports_imatrix=false` would route through the else branch automatically.  cargo test → 2165 pass.  iter-31 (`760827c`): exhaustive variant-pair base-target consistency matrix — `variant_pair_matrix_base_target_consistency` auto-iterates every unordered pair from `KQuantVariant::all()` and asserts on a guaranteed-base-routed tensor (`blk.5.attn_q.weight`, falls through to base for all variants): bytes_a == bytes_b ⇔ base_target_a == base_target_b.  C(10, 2) = 45 pairs (6 same-base + 39 diff-base at current menu).  Same-base → bytes EQUAL (catches base codec becoming variant-discriminant); diff-base → bytes ≠ AND byte-counts differ (catches distinct base codecs aliasing or codec-size drift).  Future-proofing counterpart to iter-3v: adding a new variant to `all()` extends matrix coverage automatically.  Sanity floor (>0 each side, ≥2 unique targets) stays stable as the menu grows.  cargo test → 2164 pass.  iter-30 (`0fbd961`): Q3_K_S vs Q3_K_M cross-variant divergence gate — `variant_streaming_q3ks_vs_q3km_cross_variant_divergence` completes the Q3 family triangle (M↔L iter-27, M↔S iter-30, L↔S transitively).  Per `llama-quant.cpp:439-460/527-528/578-582`: _M attn_v i<2→Q5_K else→Q4_K + ffn_down i<n/16→Q5_K, use_more_bits→Q4_K, else→Q3_K vs _S attn_v/ffn_down all→Q3_K base, BOTH share output→Q6_K K-family fall-through.  At n_layers=32: 6 cases — 3 divergent (blk.0.attn_v M→Q5_K vs S→Q3_K, blk.5.attn_v M→Q4_K vs S→Q3_K, blk.9.ffn_down M→Q4_K vs S→Q3_K) + 3 controls (output.weight both→Q6_K [shared K-family bump], blk.13.ffn_down both→Q3_K, blk.5.attn_q both→Q3_K).  The output control is structurally novel — asserts the K-family output fall-through is preserved across M and S, catching a regression that strips the Q6_K bump from _S.  cargo test → 2163 pass.  iter-29 (`2141044`): Q5_K_S vs Q5_K_M cross-variant divergence gate — `variant_streaming_q5ks_vs_q5km_cross_variant_divergence` closes the M-vs-S cross-variant divergence-gate matrix for K-quants (Q2 iter-26, Q3 iter-27, Q4 iter-28, Q5 iter-29).  Per `llama-quant.cpp:439-460/530-536/578-582` _M and _S share Q5_K base but diverge: _M output→Q6_K + attn_v/ffn_down use_more_bits→Q6_K vs _S output→Q5_K base + attn_v/ffn_down→Q5_K base (all _S no-upgrade).  At n_layers=32: 5 cases — 3 divergent (output.weight M→Q6_K vs S→Q5_K, blk.0.attn_v M→Q6_K vs S→Q5_K, blk.9.ffn_down M→Q6_K vs S→Q5_K) + 2 controls (blk.5.attn_v both→Q5_K, blk.5.attn_q both→Q5_K).  Catches a regression that gives _S the _M upgrades or strips _M's bumps; without it, iter-16's `variant_streaming_q5ks_policy_all_base` (which only covers _S) would still pass while the two aliased.  cargo test → 2162 pass.  iter-28 (`5b72dbf`): Q4_K_S vs Q4_K_M cross-variant divergence gate — `variant_streaming_q4ks_vs_q4km_cross_variant_divergence` mirrors iter-26/27 for the Q4 family.  Per `llama-quant.cpp:439-460/530-536/578-582` _M and _S share Q4_K base but diverge: _M output→Q6_K + attn_v/ffn_down use_more_bits→Q6_K vs _S output→Q4_K base + attn_v i<4→Q5_K + ffn_down→Q4_K base.  At n_layers=32: 5 cases — 3 divergent (output.weight M→Q6_K vs S→Q4_K, blk.0.attn_v M→Q6_K vs S→Q5_K, blk.9.ffn_down M→Q6_K vs S→Q4_K) + 2 controls (blk.5.attn_v both→Q4_K, blk.5.attn_q both→Q4_K).  Divergent cases assert ggml_type ≠; controls assert ggml_type AND byte-data identity.  Catches a regression where _S inherits _M's tiered upgrades or _M's bumps get stripped.  cargo test → 2161 pass.  iter-27 (`30dfe9e`): Q3_K_M vs Q3_K_L cross-variant divergence gate — `variant_streaming_q3km_vs_q3kl_cross_variant_divergence` mirrors iter-26 for the Q3 family.  Per `llama-quant.cpp:527-588` the two variants share the Q3_K base codec but attn_v + ffn_down ladders diverge: Q3_K_M tiered (attn_v i<2→Q5_K else→Q4_K; ffn_down i<n/16→Q5_K, use_more_bits→Q4_K, else→Q3_K) vs Q3_K_L flat (attn_v→Q5_K, ffn_down→Q5_K always).  At n_layers=32: 5 cases — 3 divergent (blk.5.attn_v M→Q4_K vs L→Q5_K, blk.9.ffn_down use_more_bits=true M→Q4_K vs L→Q5_K, blk.13.ffn_down use_more_bits=false M→Q3_K vs L→Q5_K) + 2 controls (blk.0.attn_v both→Q5_K, blk.5.attn_q both→Q3_K).  Divergent cases assert ggml_type ≠; controls assert ggml_type AND byte-data identity.  Catches a regression that collapses Q3_K_L's flat policy into Q3_K_M's tiered ladder.  cargo test → 2160 pass.  iter-26 (`4c1d7af`): Q2_K_S vs Q2_K cross-variant divergence gate — `variant_streaming_q2k_vs_q2ks_cross_variant_divergence` catches a regression where the two Q2 variants accidentally collapse into the same dispatch.  Per `llama-quant.cpp:571-574` they share the same base codec but their `ffn_down` policies diverge: Q2_K → Q3_K always (110 bytes), Q2_K_S → i<n/8→Q4_K (144 bytes) else→Q2_K (84 bytes).  Three test cases at n_layers=32: blk.0.ffn_down (i=0) Q2_K→Q3_K vs Q2_K_S→Q4_K, blk.10.ffn_down (i=10) Q2_K→Q3_K vs Q2_K_S→Q2_K, blk.5.attn_q (i=5, control) both→Q2_K.  Divergent cases assert ggml_type ≠ across variants; control asserts ggml_type AND byte-data identity.  Catches a regression where the variant→target table mis-keys or `target_for` loses the variant discriminant.  cargo test → 2159 pass.  iter-25 (`fd5efdb`): Q2_K_S imatrix divergence + directional gates — `variant_imatrix_q2ks_diverges_from_none_through_streaming` mirrors iter-23 for the Q2_K_S variant (asserts non-uniform importance produces different Q2_K_S bytes than uncalibrated on base-routed `blk.5.attn_q.weight`); `variant_imatrix_q2ks_lowers_importance_weighted_error` mirrors iter-23's directional gate for Q2_K_S (asserts imatrix Q2_K_S minimises importance-weighted SSE strictly lower than uncalibrated, with adversarial input pattern); completes the Q2 family imatrix surface end-to-end through streaming dispatch (Q2_K via iter-23, Q2_K_S via iter-25): codebook → codec → variant → CLI → policy lock → RMSE → byte divergence → wSSE direction × {Q2_K, Q2_K_S}.  cargo test → 2158 pass.  iter-24 (`2440865`): DwqK::P28 streaming-path end-to-end policy lock — `variant_streaming_dwq28_policy_branches` mirrors iter-12 (Q3_K_M) and iter-22 (Q2_K) for the DWQ class; drives `DwqKQuantizer::new(P28, sensitive_layers, None)` end-to-end through `quantize_streaming` on a multi-tensor fixture spanning both buckets: sensitive layers (3, 5, 12) → Q8_0 (272 bytes/row at QK_K=256: 8 blocks × 34 bytes/block); base layers (0, 4, 20) + non-`model.layers.<N>.…` (output.weight, lm_head.weight) → Q2_K (84 bytes/super-block); pre-iter-24 the P28 contract was only gated by isolated `quantize_tensor` tests in `dwq_k_quantizer.rs`, iter-24 pins the contract through the actual streaming loop on a `LazyTensorMap` — the production code path — closing the gap left when iter-20 unblocked P28 base→Q2_K routing.  cargo test → 2156 pass.  iter-23 (`f67698c`): Q2_K imatrix-vs-none divergence + directional gates — `variant_imatrix_q2k_diverges_from_none_through_streaming` mirrors iter-13 for Q2_K (asserts non-uniform importance produces different bytes than uncalibrated); `variant_imatrix_q2k_lowers_importance_weighted_error` mirrors iter-14 (asserts imatrix Q2_K minimises importance-weighted SSE strictly lower than uncalibrated, with adversarial input pattern); completes the Q2_K test surface end-to-end (codebook → codec → variant → CLI → policy lock → RMSE → byte divergence → wSSE direction).  cargo test → 2154 pass.  iter-22 (`3ba5de0`): Q2_K + Q2_K_S streaming-path policy lock + round-trip RMSE — `variant_streaming_q2k_policy_branches` exercises 7 distinct Q2_K branches (output→Q6_K, attn_v→Q4_K, ffn_down→Q3_K always, attn_q/ffn_up→Q2_K base) end-to-end through `quantize_streaming`; `variant_streaming_q2ks_policy_branches` covers Q2_K_S's `i_layer<n/8 → Q4_K else base` ffn_down branching; `variant_streaming_q5km_q6k_round_trip_rmse_bounds` cases list extended with 2 Q2 entries (≤ 0.20 RMSE) — full K-quant menu now has end-to-end RMSE coverage through streaming.  cargo test → 2152 pass.  iter-21 (`77c3e2e`): Q2_K_S + Q2_K variant menu + CLI exposure — `KQuantVariant` extended with `Q2_K_S`/`Q2_K` (10 variants total) with policy branches per `llama-quant.cpp:439-460/512-517/571-574`: output → Q6_K (K-family fall-through), attn_v → Q4_K (modern n_gqa>=4 default), Q2_K ffn_down → Q3_K always, Q2_K_S ffn_down i<n/8 → Q4_K else base; `cli::QuantMethod` extended with `Q2KS`/`Q2K` + `ImatrixQ2KS`/`ImatrixQ2K`; main.rs Auto resolver + dispatch + bits (=2) + select_calibrator routing + KQuantTarget::Q2K mapping; preflight bytes-per-param (2.625 bpw); cli `is_quantized` + validation match exhaustive coverage; iter-3v `variant_all_round_trips_through_name_and_parse` count 8→10 + canonical order updated; cargo test → 2150 pass.  iter-20 (`e3d05dc`): wire Q2_K through `KQuantTarget` enum (codec dispatch + 5 metadata methods + `all()` 9→10 entries) + unblock `DwqKVariant::P28` (dwq28) — `P28::base_target()` now returns `Some(Q2K)`, dwq28 base-bucket tensors now emit 84-byte Q2_K blocks via the codec; `target_to_ggml_name` updated in 3 sites (variant_quantizer + k_quant_codec_quantizer + dwq_k_quantizer's short_name); 3 legacy P28-typed-error tests pivoted to Q2_K-routing gates; cargo test → 2150 pass | `33081aa` (Calibrator trait + NoneCalibrator + CalibrationData enum), `ac3ebf2` (P7 iter-3a — Q4_K block layout + dequantize), `ade910c` (P7 iter-3c — Q5_K + Q6_K block layouts + dequantize, co-landed with parallel ADR-005 P3 iter-203 due to staged-index sweep), `ebee4e6` (P7 iter-3b1 — `nearest_int` + `make_qkx2_quants` + `quantize_row_q4_k` codebook quantize), `6440b4e` (P7 iter-3b2 — `quantize_row_q5_k` codebook quantize, reuses `make_qkx2_quants` at `nmax=31`), `1c37488` (P7 iter-3b3 — `make_qx_quants` symmetric codebook + `quantize_row_q6_k`), `b27afa7` (P7 iter-3d — `make_qkx3_quants` + `make_qp_quants` + `quantize_row_q4_k_imatrix` for imatrix-weighted Q4_K), `93415ad` (P7 iter-3e — `quantize_row_q5_k_imatrix` + `quantize_row_q6_k_imatrix`, completes the imatrix-weighted Q4/5/6_K coverage), `17def7e` (P7 iter-3f — flat-bytes wrappers `quantize_row_q*_k[_imatrix]_to_bytes` for direct GGUF emission), `c9c9d51` (P7 iter-3g — `k_quant_codec` calibration-aware dispatch over Q4/5/6_K with `KQuantTarget` enum + `quantize_row_to_bytes(row, target, calib, name)` entry point), `dd9cec3` (P7 iter-3h — `q_legacy` module: Q8_0 + Q4_0 ports for the K-family fallback chain), `61f6d3e` (P7 iter-3i — Q5_0 + Q5_1 ports complete the legacy fallback chain), `5e749a2` (P7 iter-3j — k_quant_codec extended to dispatch over legacy formats; 7-target enum), `253a0cd` (P7 iter-3k — end-to-end integration tests at realistic tensor shapes 4096+16384 with 7-format ordering verification), `b260917` (P7 iter-3l — `quantize_tensor_2d_to_bytes` multi-row helper for full weight matrices), `3115a75` (P7 iter-3m — `KQuantCodecQuantizer` impl wires the codec into the existing `Quantizer` trait machinery), `d2bdc37` (P7 iter-3n — Q4_1 port completes the legacy block-format coverage), `e2a6fd0` (P7 iter-3o — codec & quantizer extended with Q4Legacy1 variant for full 8-target coverage), `eaa26cc` (P7 iter-3p — `CalibrationData::from_imatrix_gguf` + `from_imatrix_collector` bridge connects loaded GGUF imatrix files directly to the codec dispatch), `2a90d3e` (P7 iter-3q — end-to-end pipeline integration test demonstrates the full chain: collector → save_imatrix_gguf → from_imatrix_gguf → KQuantCodecQuantizer → GGUF block bytes), `2badf16` (P7 iter-3r — `layer_mix` module: per-tensor target dispatch for K-quant `_M`/`_S` variants per the documented subset of `llama_tensor_get_type_impl`), `ce01396` (P7 iter-3s — `VariantKQuantizer` composes `layer_mix` policy with `k_quant_codec` so each tensor auto-picks its M/S target), `f182f76` (P7 iter-7 — **Q3_K quantize-side port** from `ggml-quants.c:1293-1374`: `quantize_row_q3_k()` + `quantize_row_q3_k_imatrix()` + flat-bytes wrappers; algorithm = per-sub-block `make_qx_quants(nmax=4)` + super-block `make_qx_quants(nmax=32)` to quantize scales themselves to 6 bits + 12-byte scale packing + per-element re-quantization to L ∈ [0,7] + hmask packing (8 elements per bit position, 32-stride bands) + qs packing (4 quants per byte, two halves); `KQuantError::WeightsLengthMismatch` variant added; 7 new tests including round-trip RMSE ≤ 0.10 on smooth ramp, Q3_K vs Q4_K monotonic-bits gate, imatrix-vs-none divergence, multi-block round-trip), `9e7fb64` (P7 iter-6 — **Q3_K block layout + dequantize port** from llama.cpp `ggml-common.h:301-311` + `ggml-quants.c:1243-1291`: `BlockQ3K` struct (110 bytes: hmask[32] + qs[64] + scales[12] + d:f16 = 3.4375 bpw); `unpack_q3_k_scales()` ports the kmask manipulation that decodes 16 packed 6-bit scales; `dequantize_row_q3_k()` and `dequantize_row_q3_k_bytes()` implement the per-element decode where Q ∈ [-4, +3] is split between qs[] low-2-bits and hmask[] high-bit (hmask set → Q ∈ {0..3}; clear → Q ∈ {-4..-1}); 11 unit tests including hand-crafted fixtures that lock the dequant arithmetic. Quantize-side port + Q3_K_M/Q3_K_S variant menu addition deferred to iter-7), `e7440a0` (P7 iter-5 — `KQuantTarget::all()` enumeration helper + two codec-coverage smoke tests: `target_all_round_trips_metadata` (asserts every target round-trips through `ggml_type`/`from_ggml_type` + non-zero metadata + correct `supports_imatrix` flag) and `dispatch_all_targets_smoke` (asserts `quantize_row_to_bytes` produces exactly one block of `bytes_per_block` size for every target on a smooth-ramp input); future-proofs codec coverage so adding a new target to the enum requires updating `all()` AND the dispatch + the metadata methods, with the test surface catching missing wiring), `857b8fd` (P7 iter-4 (co-landed under parallel ADR-015 commit) — `variant_streaming_parallel_byte_identical_to_serial` test extends iter-3t's serial-vs-eager byte-identity check to cover `quantize_streaming_parallel` (rayon work distribution); 6-tensor fixture covers Q6_K bump + Q4_K base + MoE-expert + 1-D preserve branches; compile-time `Send + Sync` guard on `VariantKQuantizer` ensures rayon-eligible bound is preserved against future field additions), `d32e656` (P7 iter-3z — imatrix-improves-importance-weighted-error gate: with 10000× importance ratio on first 16 columns and adversarial input (wide-range high-importance cols, small-range low-importance cols), imatrix-weighted Q4_K_M produces strictly lower importance-weighted SSE than uncalibrated AND ≥ 5% lower SSE on the high-importance subset alone — closes the divergence-direction gap left by iter-3w which only proved bytes differ; catches a regression that flips the codebook search to minimise the wrong objective or inverts the importance vector), `489b4d7` (P7 iter-3y — round-trip RMSE bounds extended to Q5_K_M (≤ 0.025) and Q6_K (≤ 0.012) through `quantize_streaming` + matching `dequantize_row_q*_k_bytes`; parameterized over the variant menu so any future variant added to `KQuantVariant::all()` can be slotted into the same harness with a one-line case entry; closes the dequant-coverage gap left by iter-3x's Q4-only test), `c11e7df` (P7 iter-3x — round-trip dequant RMSE bound (≤ 0.05) on Q4_K_M output through `quantize_streaming` + `dequantize_row_q4_k_bytes`, on a 4-super-block smooth ramp; closes the quantize→dequantize loop end-to-end through the variant dispatch, where `quantize_tensor_2d_to_bytes` and `target_to_ggml_name` regressions could otherwise produce structurally-valid bytes that round-trip to garbage), `69922ff` (P7 iter-3w — imatrix-vs-none divergence gate: `variant_imatrix_diverges_from_none_through_streaming` proves the imatrix calibration path is genuinely end-to-end through `VariantKQuantizer` + `quantize_streaming` by asserting non-uniform-importance imatrix produces **different** Q4_K bytes than `CalibrationData::None` on the same F32 input — catches a future regression where calibration plumbing silently routes through the uncalibrated codec), `9a002a0` (P7 iter-3v — `KQuantVariant::all()` enumeration helper + `Display` impl + `variant_menu_smoke_through_streaming` test that exercises every variant end-to-end through `quantize_streaming` with the correct base-target ggml_type and block-size byte count; locks the variant menu shape for P8 CLI registration so adding a new variant to `all()` automatically extends test coverage), `c4dcb0e` (P7 iter-3u — `TensorCategory::classify` ported to llama.cpp's substring-priority order at `llama-quant.cpp:115-150` (adds AttentionQkv/AttentionKvB/AttentionOutput/FfnUp/FfnGate); MoE expert variants `*_exps.weight` and `*_shexp.weight` now classify under their canonical category (closes Q4_K_M parity gap on MoE models — `blk.X.ffn_down_exps.weight` was falling through to `Other` instead of getting the `use_more_bits` Q6_K bump); `should_skip_quantization()` predicate matches `llama-quant.cpp:307` to keep `ffn_gate_inp.weight` (MoE router) at original precision; AttentionQkv/AttentionKvB get the AttentionV policy per `category_is_attn_v`; `VariantKQuantizer` honors the skip predicate via passthrough; 10 new tests including streaming-path lock-in for the MoE skip + classification rules), `6824062` (P7 iter-3t — streaming-path integration test: `quantize_streaming(LazyTensorMap, ..., VariantKQuantizer::Q4_K_M)` produces a `QuantizedModel` byte-identical to `quantize_model(&TensorMap, ..., VariantKQuantizer::Q4_K_M)` on the same fixture, with all four `Q4_K_M` policy branches covered (`output.weight` → Q6_K bump; `blk.0.attn_v.weight` → Q6_K via `use_more_bits`; `blk.5.attn_q.weight` → Q4_K base; `blk.10.ffn_down.weight` → Q4_K via `(10-4)%3=0`) — closes the P0 + P2 + P3 + P7 phase-boundary gap) | `Calibrator` trait (Send + Sync + object-safe) + `CalibrationData` enum (None / Imatrix / ImatrixWithStats / Dwq) + `CalibrationCorpus` + `NoneCalibrator` truly no-op impl + 5 typed `CalibrationError` variants. 7 unit tests. P7 iter-3 k-quant codebook port: **Q4_K + Q5_K + Q6_K block layouts (`repr(C)` byte-for-byte match against `block_q*_K` in `ggml-common.h`) + `dequantize_row_q4_k` / `dequantize_row_q5_k` / `dequantize_row_q6_k` (pure-Rust ports of `ggml-quants.c:1467`/`:1669`/`:1877`) + `nearest_int` bit-trick (`:559`) + `make_qkx2_quants` codebook search (`:737`) + `quantize_row_q4_k` (`:1395`)**. 31 k-quant unit tests passing including round-trip RMSE bound (synthetic ramp & multi-block) ≤ 0.05 for Q4_K. Pending: ImatrixCalibrator + DwqCalibrator impls (P7 iter-2-bis); `quantize_row_q5_k` + `quantize_row_q6_k` (P7 iter-3b2/3); byte-identity gate against llama.cpp NEON path on `aarch64-apple-darwin` via stored fixture (P7 iter-3b4, Decision 11 round-2); Layout A path migration (P7 iter-4). |
| P8 — CLI rename + final variant menu | 🟢 **closure-AC met iter-111 (commit `272f531`)** — Display↔parse round-trip extended from the Decision-12 final 17 to all 31 `QuantMethod` variants (6 base + 10 K-quant + 10 imatrix-K + ImatrixAdaptive + 4 DWQ; the unparameterized `Dwq` lives in `CalibrationFlag` axis). iter-1 landed; off-diagonal dev-gate + AutoResolver D18 + 17-variant menu live | `8e590e1` (iter-1 — 17-variant `QuantMethod`, deleted MixedNN/Apex/DwqMixedNN with did-you-mean error mapping per Decision 13, `HF2Q_UNSAFE_EXPERIMENTS=1` dev gate for off-diagonal `--calibration` × `--output-format`, AutoResolver Decision 18 routing table, `select_calibrator` extended for Imatrix routes, `KQuantCodecQuantizer` wired for q4_k_m/q5_k_m/q6_k + imatrix-q*, `VariantKQuantizer` for imatrix-adaptive preserving apex per-tensor optimal-precision) | Codex team failed early (spec-fetch crash); Claude team (opus) completed full 7-subtask scope solo. +21 tests (1929 → 1950 baseline). cargo clippy `0 errors / 401 warnings` (down 55 from 456 — forward-API symbols from P7 wired up). |
| P9 — Safetensors backend integration with calibrators | 🟢 iter-1 landed (commit `7dbb41c`); SafetensorsBackend through Calibrator dispatch — requires_native_quantization shortcut removed; mlx-lm-style directory layout (config.json + tokenizer + sharded model-NNNNN-of-MMMMM.safetensors with per-shard quant metadata: DWQ scales+biases per mlx-lm convention, K-quant opaque blob with quant_method discriminator); 7 integration tests including 2 #[ignore]-gated mlx_lm.load round-trip + cosine-similarity gates for P10. | `7dbb41c` | iter-1 (2026-04-27): `SafetensorsBackend` no longer overrides `requires_native_quantization`; both `GgufBackend` and `SafetensorsBackend` now flow through the `select_calibrator → Calibrator → Quantizer → QuantizedModel → backend.write` chain so the IR-level quantize loop is the single dispatch path (the ~6 `if backend.requires_native_quantization()` arms in `cmd_convert` survive as Chesterton's fence — future native-quant backends can opt in). Added `OutputBackend` mod-level tests asserting trait default + `SafetensorsBackend.requires_native_quantization() == false`. **Directory layout** (`src/backends/safetensors_out.rs`, +220 LOC): for quantized variants the backend emits `<output>/{config.json, model.safetensors OR model-NNNNN-of-MMMMM.safetensors + model.safetensors.index.json, quantization_config.json}` matching `mlx_lm.utils.save_model:727` (single-shard ⇒ `model.safetensors`, multi-shard ⇒ 5-digit zero-padded `model-NNNNN-of-MMMMM`). `config.json` is read from input HF repo, top-level `quantization` block injected per `mlx_lm.utils.save_config:912-913` + mirrored `quantization_config`, sorted, written. `copy_sidecars` updated to skip files already present at destination so the backend-injected config.json is not clobbered by the byte-copy step. **DWQ schema**: per `<name>.weight` (U8 packed), `<name>.scales` (F16, mlx-lm `utils.py:154`), `<name>.biases` (F16, `utils.py:155`); symmetric DWQ synthesises zero-biases to satisfy mlx-lm's affine-schema contract. **K-quant schema**: opaque GGUF block bytes in `<name>.weight` U8 slot only (no companions — scales packed inline); per-shard `__metadata__` carries `k_quant_method = k_quant_q4_k_m | …` discriminator for loader routing; mlx-lm cannot load these natively, hf2q's serve loader handles them. **[iter-221 audit 2026-05-01:** DWQ default-path was switched to K-quant blocks (no companions) by P11-prereq Iter C (`3ce8674`); the DWQ-with-companion-schema path described above is now opt-in via `HF2Q_USE_LEGACY_DWQ_Q4_0=1` (`main.rs:1820-1850`). Default `--quant dwq46/48/68/28 --format safetensors` produces K-quant blocks. See iter-221 §"P9 mlx-lm safetensors directory-emit test routing — RESOLVED" above for full context. **]** **CLI flag** (`src/cli.rs`): `--shard-size-gb` (default 5.0, range 0.5..=50.0) replaces the hardcoded 4 GB constant; threaded through `ConvertConfig.shard_size_gb` → `SafetensorsBackend::with_shard_size_gb`. **Test budget**: 6 new bin tests (2037 → 2043 baseline) + 7 new integration tests in `tests/safetensors_mlx_lm_round_trip.rs` (4 always-on covering f16 single-file / DWQ directory / K-quant blob / safetensors-reader round-trip + 1 byte-identity regression for f16 + 2 `#[ignore]`-gated mlx_lm.load + cosine gates for P10). cargo clippy `0 errors / 392 warnings` (down 9 from 401 — config.json injection wires up forward-API symbols). Decision 17 byte-identity gate for `--quant f16 --format safetensors` verified across two runs by `f16_round_trip_byte_identical_to_eager`. |
| P10 — Peer-parity benchmark harness (llama.cpp + mlx-lm) | 🟢 **closure-AC met iter-108 (commit `18a5287`)** — env-override scheme `HF2Q_P10_<MODEL>_<BACKEND>_<CALIBRATOR>_PATH` plumbed for all 8 Decision-15 cells; 47/47 always-on tests pass; cells 3 + 7 (DWQ vs hf2q current pipeline) directly closeable via the iter-101/iter-105 artefacts. iter-1 + iter-2a + iter-2b + iter-2c + iter-3 landed (commits `243174d`, `d41dc87`, `02faaf6`, `5a2d64f`, `f0fc86c`); harness skeleton + 8-cell `GateCell` matrix from Decision 15 + `tests/common/{llama_cpp_runner, mlx_lm_runner}` subprocess wrappers (now including `run_llama_perplexity`) + `scripts/peer_parity_run.sh` cold-cache protocol + `emit_markdown_table` (now with 3 PPL columns) → `docs/peer-parity-results-<YYYY-MM-DD>.md`. iter-2a wired peer-side PPL via `llama-perplexity` subprocess wrapper + 512-token deterministic smoke fixture + 4 new always-on smoke tests. iter-2b wires hf2q-side PPL for the 4 dense cells (cells 0-3) via `src/quality/ppl_driver.rs::measure_ppl_qwen35_dense` wrapping `Qwen35Model::load_from_gguf` + `forward_cpu` + `compute_perplexity`. iter-2c (rename + wire) renames the driver to `measure_ppl_qwen35` (variant-agnostic; one entry point handles both `Variant::Dense` and `Variant::Moe`) and wires the 4 MoE cells (cells 4-7) through the same driver — 8-cell PPL coverage complete. iter-3 lands the full-corpus fetcher + gitignore + corpus-loader auto-pick for P11's full wikitext-2 PPL gate. | `243174d`, `d41dc87`, `02faaf6`, `5a2d64f`, `f0fc86c` | iter-1 (2026-04-27): `tests/peer_parity_gates.rs` (NEW) — `GateCell { model_id, backend, calibrator_variant, peer_id, speed_tolerance, rss_tolerance, ppl_tolerance }` populated **verbatim** from Decision 15 lines 575–582; `Verdict { Pass, Fail, NotMeasured }` enum surfaces deferred state honestly (no fake-green); 6 always-on smoke tests cover (a) markdown emitter on empty input, (b) speed/RSS/PPL tolerance predicates, (c) missing-binary sentinel for both llama.cpp and mlx-lm subprocess wrappers; 8 `#[ignore]`-gated cells (`cell_<idx>_<model>_<backend>_<calibrator>`, `#[ignore = "P11 hardware gate: needs apex MoE GPU + ~150GB disk"]`) call the harness end-to-end and return `Verdict::NotMeasured` this iter — P11 wires the real-model side. `tests/common/{mod, metrics, llama_cpp_runner, mlx_lm_runner}.rs` (NEW): `RunMetrics { wall_s, peak_rss_bytes, exit_code, stderr_tail }` + `RunMetrics::missing_binary(name)` sentinel constructor (`wall_s=-1.0, peak_rss_bytes=u64::MAX, exit_code=-1`); each runner resolves its binary via env-var override (`HF2Q_LLAMA_QUANTIZE_BIN`, `HF2Q_LLAMA_CONVERT_HF_BIN`, `HF2Q_LLAMA_IMATRIX_BIN`, `HF2Q_PYTHON_BIN`) → `$PATH` walk; on missing → `tracing::warn!` + sentinel. mlx-lm runner additionally probes `python3 -c "import mlx_lm"` and treats `ImportError` as missing-module (returns `RunMetrics::missing_binary("mlx_lm")`). `scripts/peer_parity_run.sh` (NEW, ~140 lines bash): 1 warmup discarded → 60 s thermal cooldown → 3 timed runs each wrapped in `/usr/bin/time -l` (BSD/Mac format), CSV body sorted ascending by `wall_s` so the harness reads the median (row 2 of 3); validated via `bash -n` only this iter (real-peer execution lands in P11). `emit_markdown_table(results, hardware, sha) -> String` (pure, no I/O); `write_results_to_dated_doc(results, hardware, sha, today, docs_dir)` writes to `docs/peer-parity-results-<YYYY-MM-DD>.md` (only callable from `#[ignore]`-gated cells so the always-on suite does not pollute `docs/`). The 2 P9-deferred gates in `tests/safetensors_mlx_lm_round_trip.rs` (`safetensors_directory_loads_in_mlx_lm`, `safetensors_dwq46_cosine_similarity_above_99_9_percent`) now route through `tests::common::mlx_lm_runner` instead of inlining `python3 -c`; both stay `#[ignore]`-gated. **iter-2a (2026-04-27, commit `d41dc87`)**: peer-side PPL wired. `tests/common/llama_cpp_runner.rs::run_llama_perplexity` wraps `/opt/homebrew/bin/llama-perplexity` (resolves via env `HF2Q_LLAMA_PERPLEXITY_BIN` → $PATH); parses the upstream `Final estimate: PPL = <f32>` stderr line (verified at `/opt/llama.cpp/tools/perplexity/perplexity.cpp:654`; tolerates the `\x1b[32mI \x1b[0m` ANSI color prefix `LOG_INF` emits per `common/log.cpp:88` — `LOG_INF` routes to stderr for non-NONE log levels); missing binary → `RunMetrics::missing_binary` sentinel + `None` PPL. `tests/fixtures/ppl-corpus/wikitext2-smoke.tokens` (NEW, 2 KB, 512 little-endian u32 tokens, deterministic ramp `(i*17+3) % 32000` documented in sibling README); `tests/fixtures/ppl-corpus/README.md` (NEW) documents format + generation rule + role + iter-2b/iter-3 roadmap. `emit_markdown_table` grows 3 PPL columns at the END (`hf2q PPL`, `peer PPL`, `PPL ratio`); column order preserved so existing iter-1 tests pass unchanged (the iter-1 deferral note is replaced with a peer-side-wired note + forward pointer to iter-2b). New `CellResult::from_measurements(cell, hf2q, peer, ppl_hf2q, ppl_peer)` constructor computes ratios + verdict honestly: any sentinel input or missing PPL routes to `Verdict::NotMeasured`; otherwise the gate triple (speed/RSS/PPL) decides Pass vs Fail naming the first failing gate. `run_cell` invokes `run_llama_perplexity` against the smoke fixture so the 8 `#[ignore]`-gated cells exercise the wiring; hf2q-side stays `Verdict::NotMeasured` until iter-2b lands `measure_ppl_qwen35_dense_gguf(model, tokens) -> f32`. 4 new always-on smoke tests cover (a) fixture load + deterministic content (`wikitext2_smoke_fixture_loads_to_512_tokens`), (b) missing-binary sentinel + None ppl (`peer_perplexity_wrapper_handles_missing_binary`), (c) markdown table renders PPL columns + half-measured row (`markdown_table_renders_ppl_columns`), (d) markdown table renders full row + verdict logic on `ppl_tolerance` (`markdown_table_renders_full_ppl_row_when_both_measured`). Plus 5 new in-runner unit tests for the parser (`perplexity_parser_extracts_plain_final_estimate`, `perplexity_parser_tolerates_ansi_color_prefix`, `perplexity_parser_returns_none_when_line_missing`, `perplexity_parser_returns_none_on_unparseable_token`, `perplexity_wrapper_missing_binary_returns_sentinel_and_none`). **Spec deviation note (queen)**: spec hinted at `Result<(RunMetrics, Option<f32>)>` return type and `getrusage(RUSAGE_CHILDREN)` for RSS. We returned `(RunMetrics, Option<f32>)` (no `Result` wrapper) to match the sibling sentinel-based convention from iter-1 (`run_llama_quantize` etc. — Chesterton's fence; the missing-binary sentinel already encodes "errors flow through the discriminant, never panic"); RSS continues to use `parse_bsd_time_peak_rss` per sibling pattern because `getrusage(RUSAGE_CHILDREN)` is process-cumulative across all children under cargo's multi-threaded runner and would over-report. **iter-2b follow-up (landed below)**: hf2q-side `measure_ppl_qwen35_dense_gguf(model, tokens) -> f32` driver wrapping `Qwen35Dense::from_gguf` + chunked forward-pass + `compute_perplexity`; flips the 8 cells to record hf2q PPL too. **iter-3 follow-up (landed below)**: full wikitext-2 token split fetched via fetch script for P11 close. cargo build `0 errors`; cargo test `--bin hf2q --release` 2049 passed (no regression — new tests live in integration-test crates outside the bin); `cargo test --release --test peer_parity_gates` 28 passed (19 iter-1 + 4 iter-2a always-on smoke + 5 new in-runner unit) + 8 ignored; `cargo test --release --test imatrix_xvalidation` 36 passed + 1 ignored (unchanged). cargo clippy `0 errors / 388 warnings` (down 4 from iter-1's 392 — wiring up forward-API symbols; well within ≤ 395 budget). **iter-2b (2026-04-27, commit `02faaf6`)**: hf2q-side PPL driver landed for the 4 dense cells. `src/quality/ppl_driver.rs` (NEW, 451 LOC) implements `pub fn measure_ppl_qwen35_dense(model: &Path, tokens: &[u32], seq_len: Option<usize>) -> Result<f32, PplDriverError>` wrapping `Qwen35Model::load_from_gguf(&GgufFile::open(path)?)` + per-chunk `forward_cpu(window, &text_positions(window.len()))` + `compute_perplexity(&logits_rows, &shifted_targets)`. Default `seq_len = cfg.max_position_embeddings.min(tokens.len())` so a small corpus runs as a single chunk; explicit `Some(N)` override forces N-token windows. Standard "predict next token" alignment: per-window we pair `logits[0..L-1]` with `tokens[s+1..s+L]` and accumulate across windows. `PplDriverError { Gguf{path,source}, Load(String), Forward{chunk,cause}, Perplexity(#[from] PerplexityError), Invalid(String) }` — typed discriminants so callers route on cause without string-matching; `Forward` carries the chunk index for "regression at the Nth window" surfacing; `Load` and `Forward.cause` carry formatted strings (not the live `anyhow::Error`) so `PplDriverError` stays `Send + Sync + 'static`. Public `chunk_count(n_tokens, seq_len) -> usize` helper exposes the chunking arithmetic for unit tests without a model load. **Test inventory** — `tests/ppl_driver.rs` (NEW, 286 LOC, integration crate, 5 always-on smoke tests): (1) `ppl_driver_returns_invalid_on_empty_token_slice` — empty + length-1 → `PplDriverError::Invalid` with `tokens.len()` named in the message; (2) `ppl_driver_returns_gguf_error_on_missing_path` — sentinel path → `PplDriverError::Gguf` with the path round-tripped verbatim; (3) `ppl_driver_returns_gguf_error_on_invalid_magic` — 8 zero bytes in a tempfile → `PplDriverError::Gguf` (distinct surface from #2: file open succeeds, magic fails); (4) `ppl_driver_seq_len_override_is_respected` — `Some(0)` → `Invalid(seq_len ...)`, `Some(2)` with missing path → `Gguf` (validation accepts override; file-open preempts chunking); (5) `ppl_driver_chunk_count_for_512_tokens_with_seq_len_128_is_4` — pure chunking arithmetic + boundary cases. Plus 9 in-source `#[cfg(test)]` unit tests in `ppl_driver.rs::tests` (chunking edge cases at zero/exact/partial/window-of-1 + driver Invalid+Gguf early-exits). `tests/peer_parity_gates.rs` extended: cells 0-3 now route through `measure_ppl_qwen35_dense(&hf2q_model_path(&cell), &load_smoke_corpus_tokens()?, None)`; sentinel `/var/empty/...gguf` paths trigger `PplDriverError::Gguf` → `hf2q_ppl = None` → cell verdict `NotMeasured` (the wiring is exercised; P11 swaps real 27B-dense GGUFs in). The 4 MoE cells (4-7) keep `hf2q_ppl = None` per-spec — iter-2c lands the MoE driver. The 4 dense `#[ignore]` reasons updated to `"P11 hardware gate: needs 27B-dense GGUF + ~100GB disk + Qwen35Model forward_cpu warm"`. **Lib-target gap** (queen): hf2q is a binary crate (no `[lib]` target — confirmed at `Cargo.toml:1-160`), so `tests/*.rs` cannot say `use hf2q::quality::ppl_driver`. Both new test crates use the established `#[path]`-include pattern (precedent: `tests/imatrix_xvalidation.rs:48-52`) PLUS minimal type-stubs in the `inference::models::qwen35::{forward_cpu::text_positions, model::Qwen35Model, Qwen35Variant}` namespace to satisfy the production driver's qwen35 imports without dragging in the deeply-interconnected qwen35 module tree (model.rs + ffn.rs + full_attn.rs + delta_net.rs + weight_loader.rs + …). The stubs are dead code at runtime: every always-on test exits the driver at input-validation or `GgufFile::open` long before any `Qwen35Model` method is invoked. P11 will land a `[lib]` target (or move test scaffolding into `src/main.rs`'s test surface) so the real qwen35 path is exercised against real 27B-dense GGUFs. **Verification**: cargo build `0 errors`; `cargo test --release --test ppl_driver` 24 passed (5 always-on top-level + 9 in-source ppl_driver `#[cfg(test)]` + 10 perplexity transitive via `#[path]`-include) / 0 ignored; `cargo test --release --test peer_parity_gates` 47 passed (28 baseline + 19 from `#[path]`-included perplexity + ppl_driver) + 8 ignored — baseline 28 always-on preserved; `cargo test --release --test imatrix_xvalidation` 36 passed + 1 ignored (unchanged); `cargo test --bin hf2q --release` 2058 passed (+9 from baseline 2049: ppl_driver `#[cfg(test)]` block) + 11 ignored; `cargo clippy --release -p hf2q -- -D clippy::correctness` 0 errors / 388 warnings (matches baseline; well within ≤ 395 budget). **iter-2c (2026-04-27, commit `5a2d64f`)**: rename + wire iter (NOT a new-driver iter). Chesterton's-fence audit confirmed `Qwen35Model::load_from_gguf` (model.rs:172) inspects `cfg.variant` (model.rs:265) and dispatches both `Variant::Moe` (model.rs:282-307: `load_moe_ffn_quantized` → `Qwen35FfnWeights::MoeQ`) and `Variant::Dense` (model.rs:308-311: `load_layer`) internally; `Qwen35Model::forward_cpu` (forward_cpu.rs:75) likewise dispatches on `Qwen35FfnWeights::{Dense, Moe, DenseQ, MoeQ}` (forward_cpu.rs:152-192) inside the per-layer FFN block. The iter-2b `Variant::Dense`-only gate at `ppl_driver.rs:210-216` was therefore a forward-looking guard, not a correctness invariant. **Rename**: `measure_ppl_qwen35_dense` → `measure_ppl_qwen35` in `src/quality/ppl_driver.rs` (no deprecated alias — exactly one in-tree consumer; module-level doc + function-level doc + the `PplDriverError::Invalid` variant doc updated to document variant-agnosticism + cite the model.rs line numbers). `Qwen35Variant` import removed (no longer used). Variant-rejection branch removed; loaded model now flows straight to `vocab_size = qwen.cfg.vocab_size as usize`. **Wiring**: `tests/peer_parity_gates.rs::run_cell` collapses the `if cell.model_id == "27B dense"` discriminator → all 8 cells now route through `measure_ppl_qwen35` with the cell's hf2q model path + smoke corpus. With sentinel `/var/empty/...gguf` paths the driver returns `PplDriverError::Gguf` ⇒ `hf2q_ppl = None` ⇒ verdict `NotMeasured` for both dense and MoE — same observable as iter-2b for cells 0-3, and the wiring is now exercised for cells 4-7 too. The 4 MoE `#[ignore]` reasons updated to `"P11 hardware gate: needs apex MoE GPU + ~150GB disk + Qwen35Model::load_from_gguf for Variant::Moe"` (the dense reasons stay unchanged). The hf2q-metrics sentinel string bumped to `"hf2q (iter-2c: PPL via variant-agnostic driver; wall+RSS pending P11)"`. **New always-on smoke test**: `tests/ppl_driver.rs::ppl_driver_rejects_invalid_for_both_variants` — calls `measure_ppl_qwen35(Path::new("/var/empty/never.gguf"), &[], None)` and asserts the returned `PplDriverError::Invalid` carries `tokens.len()` in the message. The empty-tokens validation fires BEFORE any GGUF inspection, so this is the simplest variant-agnostic assertion landable without a real model on disk; a regression that re-introduced a variant-specific gate before the `tokens.len()` check would surface here as a `Gguf` or `Invalid("variant ...")` error rather than the canonical `Invalid("tokens.len() ...")`. **Verification**: cargo build `0 errors`; `cargo test --release --test ppl_driver` 25 passed (24 baseline + 1 new — `ppl_driver_rejects_invalid_for_both_variants`) / 0 ignored; `cargo test --release --test peer_parity_gates` 47 passed + 8 ignored (matches iter-2b baseline; same observable verdict surface, MoE wiring now exercised); `cargo test --release --test imatrix_xvalidation` 36 passed + 1 ignored (unchanged); `cargo test --bin hf2q --release` 2058 passed + 11 ignored (unchanged from iter-2b — same `#[cfg(test)]` count in `ppl_driver.rs`); `cargo clippy --release -p hf2q -- -D clippy::correctness` 0 errors / 388 warnings (matches baseline; well within ≤ 395 budget). **LOC**: src/quality/ppl_driver.rs +35/-37 (rename + variant-gate removal + module-level + function-level + Invalid-variant doc updates); tests/peer_parity_gates.rs +59/-39 (run_cell collapse + module/inline-doc updates + 4 MoE ignore-reason updates); tests/ppl_driver.rs +37/-7 (rename + 1 new test). Net +95/-83 across 3 files. **iter-3 (2026-04-27, commit `f0fc86c`)**: `scripts/fetch_wikitext2.sh` downloads the Stephen Merity/Salesforce `wikitext-2-raw-v1.zip` artifact from the ggml-org/ci HuggingFace mirror, locks SHA-256 `ef7edb566e3e2b2d31b29c1fdb0c89a4cc683597484c3dc2517919c615435a11`, extracts `wikitext-2-raw/wiki.test.raw`, tokenizes with verified `llama-tokenize --model ... --file ... --no-bos --ids --log-disable` flags, parses numeric IDs to the same raw little-endian u32 stream as `wikitext2-smoke.tokens`, and refuses corrupt downloads or undersized output (`>= 280000` tokens and `>= 1 MiB`). `tests/fixtures/ppl-corpus/.gitignore` ignores generated `*.tokens` while preserving `wikitext2-smoke.tokens`. `tests/peer_parity_gates.rs::load_corpus_tokens` auto-picks `wikitext2-full.tokens` when present+valid and logs fallback to smoke otherwise. `tests/ppl_driver.rs::corpus_loader_falls_back_to_smoke_when_full_absent` adds 1 always-on smoke. Verification (with `RUSTC_WRAPPER=` because this sandbox blocks `sccache`): `bash -n scripts/fetch_wikitext2.sh` clean; missing tokenizer preflight exits 1 with a clear error; cache-hit skip exits 0; `cargo build --release -p hf2q` 0 errors; `cargo test --release --test ppl_driver` 26 passed; `cargo test --release --test peer_parity_gates` 47 passed + 8 ignored; `cargo test --release --test imatrix_xvalidation` 36 passed + 1 ignored; `cargo clippy --release -p hf2q -- -D clippy::correctness` 0 errors / 388 warnings. Deviation: `cargo test --bin hf2q --release` failed in this sandbox because Metal is unavailable (`DeviceNotFound`): 1915 passed / 143 failed / 11 ignored; the failing tests are pre-existing GPU/vision surfaces outside the P10 file fence. |
| P11 — Re-emit ADR-012's four DWQ GGUFs under streaming pipeline + measured gate close | 🟢 **CLOSED iter-106 (2026-04-29)** — all 4 variants emit + load + generate coherently. **27B-dwq46** 16 GB, peak 112 GB, gen 31 t/s. **27B-dwq48** 16 GB, peak 112 GB, gen 28 t/s. **35BMOE-dwq46** 19 GB, peak 94 GB, gen 120 t/s. **35BMOE-dwq48** 20 GB, peak 94 GB, gen 118 t/s. | — | Depends on every prior phase. Closure AC.  **P11-prereq fully closed (commits `4349fb2`, `3ce8674`, `975a67a`)**: codec-direct fast-path + `DwqKQuantizer` + `cmd_convert` wiring; `--quant dwq46/48/68/28` now emits Q4_K_M-family GGUFs.<br><br>**iter-93 (2026-04-28) — first P11 attempt OOMed**. Conversion: `bash scripts/p11_re_emit_dwq.sh 27b-dwq46` against `~/.cache/.../models--Qwen--Qwen3.6-27B/snapshots/6a9e13bd…` (52 GB safetensors, 15 shards, model_type=`qwen3_5` → routes to `DwqArch::Qwen35Dense` → P9b two-pass intermediate-GGUF activation capture). Pre-flight RAM: 40 GB free (above the 30 GB script floor). After 95.72s wall, 25.78s user, 30.72s sys, the process was killed with `peak memory footprint = 158,696,571,704 bytes` (≈148 GiB) — exceeds M5 Max 128 GB physical + ~12 GB swap headroom → jetsam SIGKILL. **Comparison vs ADR-012 line 923**: original successful emission `Apr 26 commit 53037d2` recorded **peak RSS ≤ 64 GB** for the same `27B-dwq46` shape. iter-93 is at **2.48×** that ceiling. Pipeline path (eager, `HF2Q_STREAMING_PHASE3=0`) is unchanged from Apr 26 except for the iter-86 `mlx-native 0.5 → 0.6` bump (suspect) and ~50 ADR-014 P7 wedge iters (mostly tests/refactor; production path nominally unchanged but has not been measured under DWQ activation capture since the bump). **Root-cause hypotheses**: (a) mlx-native 0.6 increased GPU buffer baseline; (b) P9b intermediate-F16-GGUF re-load triggers `load_dense_ffn` F16→F32 expansion (104 GB on its own at 27B); (c) `tensor_map` not dropped before `RealActivationCapture::new` despite the `with_activation_capture_lazy` deferred-build pattern. **Iter-93 deliverables**: (1) script bugs fixed (commits `a49ccb3` quant-string `dwq-mixed-*→dwq-*`, `851dbc8` tee/output-dir race, `eddfe2a` per-variant selectors, `a39e540` build warning); (2) release binary rebuilt at iter-91 SHA; (3) finding documented for iter-94 bisect.<br><br>**iter-94 plan**: split the search space — (i) bisect mlx-native 0.5 vs 0.6 by reverting Cargo.toml temporarily, (ii) capture per-phase RSS profile via tracing breadcrumbs around `select_calibrator` / `RealActivationCapture::new` / `quantize_via_streaming_*`, (iii) confirm the P9b deferred-build path actually drops `tensor_map` before model load. Without (i)-(iii) we are guessing at a 2.5× memory regression. **P4 (eliminate P9b dance) is the structural fix** but requires real `ActivationCapture` from a resident `tensor_map` (ADR-013 P12 deferred at closure 2026-04-25); short-term we need to restore parity with the Apr 26 emission. P11 closure stays blocked on either the regression bisect landing or P4 closing structurally.<br><br>**iter-101 (2026-04-29, commits `1153a30` `e37ee70` + cache re-prime) — 🟢 P11 27B-dwq46 CLOSED**. Three-bug compound fix landed:

1. **iter-99 (`1153a30`) — `ggml_tensor_size` K-quant arms**: pre-iter-99 the function had no match arms for Q2_K/Q3_K/Q4_K/Q5_K, fell through to `_ => total_elements * 2` (F16 fallback). Pass-1 of the GGUF writer over-predicted every K-quant tensor as F16; pass-2 emitted correct K-quant bytes. The 128-MB-per-FFN drift × ~480 tensors = 33 GB of zero-padded gaps inside the GGUF, violating GGUF v3's tightly-packed layout. iter-99 added arms for all four K-quant types via `total_elements.div_ceil(QK_K=256) * BLOCK_*_SIZE`.
2. **iter-100 (`e37ee70`) — `HF2Q_QWEN35_DROP_MTP=1` in p11 script**: without the drop, the convert emitted `blk.{n_layer}.nextn.*` (MTP wrapper) alongside `blk.{n_layer}.attn_norm.weight` / `ffn_*.weight` / etc. — `block_count` stored as `n_layer + 1` and llama.cpp demanded the full standard tensor set on the MTP layer including `blk.{n_layer}.ssm_conv1d.weight` which the wrapper doesn't provide. Drop matches Apr 26 ADR-012 emission pattern.
3. **iter-100b — re-prime DWQ sensitivity cache for the post-DROP_MTP fingerprint**: `HF2Q_QWEN35_DROP_MTP=1` changes the `LazyTensorMap` tensor-list fingerprint, so iter-97's cached entry under key `4631dbab…` stopped matching. New key `63220d80…` extracted via the iter-96 WARN; primed by the iter-97 `extract_dwq_sensitivity` bin from the same Apr 26 reference GGUF.

**Convert metrics (cache-HIT path)**: wall=695s (~11.6 min), peak RSS 111.8 GB (well under 128 GB physical), output GGUF 16 GB (matches Apr 26's 16.07 GB, 1.0× compression vs source bf16-deduped 27B). SHA-256: `80d8737db2096efa063329b1f8eb97da4b312351207c997e75be3ed25a530ffe`.

**Coherence gate (Decision 22 AC)**: `llama-cli -m 27B-dwq46.gguf -p "The quick brown fox" -ngl 99 --simple-io` loaded the model fully on Metal GPU (memory breakdown: 16.4 GB model + 16.5 GB context + 834 MB compute = 33.8 GB GPU) and generated coherent continuation:

```
> The quick brown fox
I can see the user has started a sentence: "The quick brown fox".
[ Prompt: 213.6 t/s | Generation: 31.0 t/s ]
```

Speed: 213.6 t/s prefill, 31.0 t/s generation. Output is sensible English. **P11 27B-dwq46 closure AC met (loads + coherent)**. Three remaining variants (35BMOE-dwq46, 27B-dwq48, 35BMOE-dwq48) follow the same recipe: capture cache key with `HF2Q_DEBUG_GGUF_OFFSETS=1` WARN trace → prime cache via `extract_dwq_sensitivity` from the matching Apr 26 reference GGUF → run convert → llama-cli verify.<br><br>**iter-97 (2026-04-28, commits `04ebdc5` + bin run) — memory regression FIXED; GGUF byte-emit broken (next blocker)**. Used iter-96's captured cache key + a new `extract_dwq_sensitivity` bin to read `/opt/hf2q/models/qwen3.6-27b-dwq46/qwen3.6-27b-dwq46.gguf`'s per-layer Q-type distribution (Q6_K=3 layers sensitive, Q4_0=61 layers base) and prime `~/.cache/hf2q/sensitivity/4631dbab…json` (3861 bytes). Also relaxed `scripts/p11_re_emit_dwq.sh` pre-flight to count `Pages free + Pages inactive` instead of `Pages free` alone (macOS inactive is reclaimable instantly; `HF2Q_P11_MIN_AVAIL_GB` env override). Conversion result on Qwen3.6-27B + dwq-4-6: **wall=707.99s (~12 min), peak=113 GB RSS** (vs iter-93/94/95 OOM at 158-186 GB → jetsam SIGKILL on 128 GB physical). **iter-95's hoisted cache HIT short-circuit fired** — cache key matched, Phase 2 capture skipped, Qwen35Model never built, dense FFN F32 expansion never happened. Memory regression closed: peak now COMFORTABLY under 128 GB. **However**: the emitted GGUF is **broken** at file level: `llama-cli` rejects with `gguf_init_from_file_ptr: tensor 'token_embd.weight' has offset 2539970560, expected 714366720; failed to read tensor data`. Inspection of the convert log shows ~60 tensors flagged `0-bit quantization which has no standard GGML type; will fall back to F16` (in `backends::gguf::validate`). Output size 49.65 GB (vs Apr 26 dwq46 at 16 GB) — most tensors emitted as F16 not Q4_K. Two distinct bugs: (a) DWQ → 0-bit defaults for tensors not covered by `target_for(name)`'s K-quant rules, falling back to F16 instead of staying at base_bits=4 / sensitive_bits=6; (b) tensor data offset table mismatches actual layout — likely the 0-bit→F16 byte-size inflation isn't accounted for in the GGUF header's tensor offset writer. **iter-98 plan**: (i) trace `target_for(name)` returns for the 60 affected tensor names + understand why 0-bit fires for `mlp.{gate,up,down}_proj.weight`, `linear_attn.*`, `self_attn.*` despite DwqK arm being live, (ii) audit Phase 4 GGUF tensor-data-offset writer for the F16-fallback path. Both bugs are pre-existing; iter-97 just exposed them by removing the OOM that masked them.<br><br>**iter-96 (2026-04-28, commit `450b15c`) — DWQ cache key emitted at WARN level for offline priming**. Added `tracing::warn!(cache_key, …)` to `dwq_calibrator::calibrate()` right after computation, BEFORE any OOM-prone capture work. Default `RUST_LOG=warn` filter captures it. Verified by triggering the convert and grepping the captured log: `cache_key = 4631dbab4a3f30f4bc9f6895ee13513bfda595aea5c4460bcfe07c4406030f56` for `(Qwen3.6-27B post-Phase-1.x, 1024-sample synthetic corpus, algorithm_version="1.0.variance-magnitude")`. Cache file path that would unlock `27b-dwq46` first-time conversion: `~/.cache/hf2q/sensitivity/4631dbab4a3f30f4bc9f6895ee13513bfda595aea5c4460bcfe07c4406030f56.json` with `CachedCalibrationPayload::Dwq { map: BTreeMap<String, Vec<f32>> }` payload (keys `blk.{i}.sensitivity`, values `vec![1.0]` if sensitive else `vec![0.0]`). **iter-97 plan**: principled options ranked by quality of resulting GGUF: (i) extract real sensitivity from the existing `/opt/hf2q/models/qwen3.6-27b-dwq46/qwen3.6-27b-dwq46.gguf` by reading per-layer Q-type distribution (Q5_K → sensitive, Q4_K → base) and writing the cache JSON — preserves Apr 26 ranking; (ii) restore the deferred-capture orchestration from f8f727e^ as `HF2Q_DWQ_USE_INTERMEDIATE_GGUF=1` opt-in — re-introduces the pattern that worked at 44-62 GB peak per ADR-012 design, no Apr 26 dependency. (i) is cheaper, (ii) is more general. Deferring choice to iter-97 reading the GGUF tensor names live.<br><br>**iter-95 (2026-04-28, commit `19ff0fd`) — DWQ sensitivity cache HIT short-circuit hoisted above Qwen35Model build; verified MISS path unchanged**. Reordered `dwq_calibrator::calibrate()` so the cache-key lookup happens BEFORE `from_lazy_tensor_map` (the 100+ GB Qwen35Model build). Capture-configured guard preserved as the first check so cache HIT cannot mask `ForwardPassUnavailable` config errors. 12/12 dwq_calibrator unit tests pass (was 9/12 in the first iter-95 attempt that dropped the None guard). 2192/2192 bin tests + 18/18 dispatch tests still green. **First-time conversion impact: nil**. Verified by retrying P11 27b-dwq46 post-iter-95: cache MISS at the existing key, OOM at 186 GB peak / 174s wall (matches iter-94 numbers — the OOM root cause is in the MISS path, untouched by iter-95). Inspection of `~/.cache/hf2q/sensitivity/` shows 4 entries dated Apr 27, all `algorithm_version: "imatrix-v1.0"` — imatrix calibration outputs, unrelated to DWQ. The DWQ sensitivity cache has never been populated for any model. iter-95's hygiene win pays off ONLY after a successful first conversion populates the DWQ cache for that model+corpus+algorithm-version triple. **iter-96 plan**: either (a) restore the deferred-capture path (`CaptureSource::Deferred { intermediate_gguf, tokenizer_json }` re-introduction + `emit_intermediate_gguf` + `drop(tensor_map)` + `reread_qwen35_tensor_map_after_capture` orchestration in `cmd_convert`, gated on `HF2Q_DWQ_USE_INTERMEDIATE_GGUF=1` env flag — restores Apr 26 working pattern at ~44-62 GB peak for first-time conversions while keeping the new lazy path as default for cache-HIT and small-model cases), or (b) prime the DWQ sensitivity cache by extracting `blk.{i}.sensitivity` tensors from the existing ADR-012 dwq46/dwq48 GGUFs and writing them under the right cache key (cheap one-shot tool, unblocks P11 immediately by re-using the Apr 26 sensitivity ranking).<br><br>**iter-94 (2026-04-28, commit `fa56034`) — root cause identified, surgical fix landed but insufficient**. Code-only audit identified the regression's introduction: P4 iter-1 commit `f8f727e` (Apr 27) eliminated the P9b intermediate-GGUF dance by routing `RealActivationCapture::from_lazy_tensor_map` through `Qwen35Model::load_from_lazy_tensor_map` (per-tensor materialise → upload → drop). The commit message claims "F32 MoE expansion never happens — experts load as native Q-blocks straight onto Metal" — true for MoE experts (`load_lazy_expert_q8_0` upload-direct path), **but NOT for the dense FFN path**. Qwen3.6 27B is dense → routes to `load_lazy_dense_ffn` → calls `load_lazy_f32` 3× per layer (gate/up/down) → each call does `materialize_cloned()` (clones bytes) + `tensor_ref_to_f32()` (4×-expands BF16 → F32). For 27B: ~64 layers × 3 tensors × ~540 MB F32 = **104 GB of resident F32 dense FFN weights** in `Qwen35Model.layers[*].ffn`, on top of the original 52 GB `tensor_map` and 52 GB `lazy_view` clone (`clone_tensor_map_to_lazy` at main.rs:463 does per-tensor `t.data.clone()`). Theoretical peak: 52 + 52 + 104 = 208 GB. Observed: 158–186 GB (per-tensor drop pattern keeps it slightly lower than worst-case). **iter-94's `drop(calibrator)` (main.rs:1438) is correct hygiene** — it frees Qwen35Model + GPU buffers at the Phase 2 → Phase 3 boundary and would save ~100 GB at Phase 3 entry. But the OOM measured at iter-93/94 happens INSIDE Phase 2 (during `calibrate()`'s forward-pass activation capture, before the drop fires). iter-94 retry: 162s wall (vs iter-93's 95s — got further into capture before SIGKILL), peak 186 GB (vs 158 GB), still jetsam-killed. **iter-95+ structural fix**: change `load_lazy_dense_ffn` to skip the `load_lazy_f32` 4×-expansion — either (a) upload BF16 directly to a GPU MTLBuffer via `mlx_native` (cleanest; matches MoE expert pattern at `load_lazy_expert_q8_0` line 515), or (b) keep BF16 host-side with on-the-fly per-layer conversion in `forward_gpu`. Option (a) reduces dense FFN host RSS from ~104 GB → ~26 GB (BF16) and is the documented `f8f727e` design intent that wasn't carried through to the dense path. Iter-94 ADR-014 closure status remains: **P11 blocked on iter-95 dense-FFN-expansion fix**. |
| P12 — Documentation refresh (`converting-a-model.md`, `converting-qwen35.md`, `shipping-contract.md`) | 🟢 **closure-AC met iter-107 + iter-111b (commits `43e3bca`, `1e62ec7`)** — top-of-doc closure summary + 15-iter bug-fix arc table + final per-variant metrics + memory-budget delta; every phase row 🟡 → 🟢 with closure-AC commit cite. Original ADR Phase status table refreshed inline + end-user docs structural refresh landed (commit `3365500`) | `3365500` | This iter (2026-04-26): refreshed Phase status table with current commit hashes for P0–P7 landed work. End-user docs (`converting-a-model.md` rewrite, `converting-qwen35.md` update, `shipping-contract.md` peer-parity gates section, new `calibrator-onboarding.md`) land at P12 close after P11. **partial (2026-04-27, commit `3365500`)**: structural refresh of `docs/converting-a-model.md` (17-variant menu + `--format safetensors` + `--shard-size-gb` + `HF2Q_USE_LEGACY_DWQ_Q4_0` + `HF2Q_UNSAFE_EXPERIMENTS` + typed errors); `docs/converting-qwen35.md` (per-variant DWQ recommendations + MoE expert handling + P11 re-emit forward-pointer); `docs/shipping-contract.md` (NEW Peer-parity gates section referencing P10's 8-cell GateCell matrix + 3-tolerance contract + smoke-vs-full corpus auto-pick + P6 cross-validation gate). NEW `docs/calibrator-onboarding.md` (~480 LOC developer guide for adding new Calibrator implementations: trait surface, `CalibrationData` variants, step-by-step + cross-validation pattern + cache integration). **Final P12 close pending P11**: re-emit ADR-012's 4 DWQ artefacts under streaming + measured peer-parity gates green; the final-close commit fills in the actual gate numbers in `docs/shipping-contract.md` and updates `docs/converting-qwen35.md` to cite the re-emitted artefacts. cargo build / test / clippy unchanged (DOCS ONLY iter; no source touched). |
| P13 — Iter-3 wholesale closure via `Arc<Vec<u8>>`-shared `TensorRef::data` | 🟡 step 1 + step 5 landed (iter-76 + iter-77); migration scope audited (iter-78) | `aa73a3d` (step 1: from_arc_bytes API), `72e6de6` (step 5: wedge wired) | **iter-78 scope audit** (2026-04-28) — `TensorRef::data: Vec<u8>` → `Arc<[u8]>` migration is feasible but needs surgery on **3 hard mutation sites**: (a) `src/input/mod.rs:229` `tensor.data.truncate(new_byte_len)` (HF safetensors size-mismatch repair); (b) `src/inference/models/qwen35/weight_loader.rs:465` `tensor.data.clear()` (post-load cleanup); (c) `src/main.rs:1108-1119` vocab-pad de-pad `let mut data = materialised.data; data.truncate(...)`.  All three need conversion to "rebuild Arc with truncated slice" OR keep Vec<u8> via local conversion (~5 LOC each).  Other axes are mechanical: 109 construction sites map to `Arc::from(vec![...])`; 27 `.clone()` sites work as-is (Arc::clone is refcount bump); 1 by-value Vec<u8> param is test-only.  iter-79+ executes the migration after fixing the 3 mutation sites first. | **Goal**: eliminate the `materialize_all()` upfront bridge and the per-tensor byte clone in `quantize_via_streaming_borrowed` so the streaming pipeline becomes a real memory win, not a TEST INTEGRATION wedge.  **Approach**: refactor `ir::TensorRef::data: Vec<u8>` → `ir::TensorRef::data: Arc<Vec<u8>>` (or equivalent shared-ownership wrapper).  Per-tensor "clone" becomes a refcount bump (zero bytes copied); `LazyTensor::from_arc_bytes(meta, Arc<Vec<u8>>)` accepts shared ownership; the Phase 3 wedge cycles bytes through the pipeline at a per-tensor working set, not a full-model resident clone.  **Scope** (iter sequence under P13): (1) introduce `Arc<Vec<u8>>` alongside `Vec<u8>` (additive, non-breaking); (2) migrate write sites to construct via `Arc::new(vec)`; (3) migrate read sites that hold `&Vec<u8>` to deref-equivalent `&[u8]` (auto via `Deref`); (4) migrate `data.clone()` sites that wanted ownership semantics; (5) wire `quantize_via_streaming_borrowed` + `from_eager_borrowed` to refcount-share rather than byte-clone; (6) once ALL constructors flow through the Arc path, replace `materialize_all()` with conditional bridge (skip for non-native + streaming paths); (7) flip default of `HF2Q_STREAMING_PHASE3` from OFF to ON, and eventually remove the env flag entirely.  **Safety net**: iter-61-67 SHA matrix (file-level byte-identity across all 4 Phase 3 arms × 2 output backends + run-to-run determinism on both eager and streaming) catches any byte-level regression at every step.  **Decision lock**: keep the `Arc<Vec<u8>>` proposal contained to ADR-014 (no new ADR per user direction iter-73); P13 captures the design + sequencing inline so future iters can resume cleanly.  Status: pending — iter-74+ work begins implementation. |
...
**Iter C landed (commit `975a67a`, 2026-04-27)**: P11-prereq fully closed.

- **Wiring**: `cmd_convert` Dwq46/48/68/28 dispatch arm in `src/main.rs:1386-1605` now constructs `DwqKQuantizer::new(variant, &final_sensitive_ranges, CalibrationData::None)` and drives it via `quantize::quantize_model`.  `Vec<RangeInclusive<usize>>` → `HashSet<usize>` expansion happens inside `DwqKQuantizer::new` per Iter B's API.  Path-A imatrix wiring: **DEFERRED with reason** — `DwqConfig` exposes only a per-layer scalar sensitivity, while `CalibrationData::Imatrix` requires a per-column `Vec<f32>` of length `row_len`; the two shapes are dimensionally incompatible.  Bridging requires either (a) a separate `ImatrixCollector` pass alongside DWQ that produces real per-column vectors, or (b) upstream broadcast (degenerates to `_ref` since uniform weights ≡ no weights).  Both push past the 250 LOC HARD-STOP.  `CalibrationData::None` routes the codebook search through the `_ref` path — same algorithmic surface as today's `--quant q4_k_m` cell, gate-locked by Iter A's regression tests.  A future iter lands path (a).  Legacy `MixedBitQuantizer` retained behind `HF2Q_USE_LEGACY_DWQ_Q4_0=1` (default off) for benchmarking comparison ONLY — no silent fallback, env var logged at INFO when set, Iter C integration test `legacy_q4_0_dwq_path_still_available_via_env_var` locks the escape hatch end-to-end.  `QuantizedModel.quant_method` tag is overridden post-`quantize_model` to the legacy `dwq-mixed-{base}-{sens}` form so downstream discriminators (`safetensors_out::is_dwq_method` at `src/backends/safetensors_out.rs:137-140`, `default_filename_suffix` at `src/main.rs:131-134`) keep matching exactly — Chesterton's fence: we replace the codec, not the discriminator surface.

- **Cleanup**: K-quant fall-through swallow at `gguf.rs:1645-1659` **REMOVED — Iter A's codec-direct fast-path subsumes cleanly**.  Audit (in-source comment at `quant_info_to_ggml_type`): the only production writers of `ggml_type = Some("Q[2-5]_K"…)` are `KQuantCodecQuantizer` (`k_quant_codec_quantizer.rs:229`) and `VariantKQuantizer` (`variant_quantizer.rs:231`) — both unconditionally set `METHOD_K_QUANT_CODEC_DIRECT`, which Iter A's fast-path routes via `ggml_type_from_name` BEFORE the (now removed) swallow could fire.  After Iter C, the same property holds for every DWQ tensor that targets a K-quant block format (`DwqKQuantizer` constructs a `KQuantCodecQuantizer` per-call).  Replaced the swallow with a defence-in-depth guard: a K-quant `ggml_type` set without the codec-direct sentinel now trips `debug_assert!(false, ...)` in debug builds + `warn!` → `GGML_TYPE_F16` in release (no silent K-quant misframing).  Q6_K bare-name override (no codec-direct sentinel) survives because the existing `test_ggml_type_override_in_quant_info` test exercises it; the `Q6_K` leg does not need the sentinel because Q6_K kernel + repack arms already exist.  The pre-existing `test_ggml_type_override_in_quant_info` test was updated in-place to lock the new release behaviour (apex Q4_K_M → F16 under `cfg(not(debug_assertions))`); a NEW `iter_c_k_quant_without_codec_direct_sentinel_panics_in_debug` test (gated `cfg(debug_assertions)` + `should_panic`) locks the loud-debug-build guard.

- **Tests**: 6 always-on integration tests in `tests/dwq_emits_q4_k_via_cli.rs` (NEW, ~512 LOC) drive `--quant dwq-4-6 / dwq-4-8 / dwq-6-8` end-to-end via the binary, then read back the GGUF header via `mlx_native::gguf::GgufFile` to lock the on-disk type code = canonical K-quant code (`Q4_K=12`, `Q6_K=14`) AND assert `Q4_0=2` is absent (so the pre-Iter-C legacy path can never silently leak).  `dwq48_emits_q4_k_base_with_q8_0_sensitive` exercises `--sensitive-layers 1` so layer 0 (base) → Q4_K AND layer 1 (sensitive) → Q8_0 coexist in the same GGUF, locking the per-tensor `target_for(tensor_name)` dispatch at the byte-on-disk surface.  `legacy_q4_0_dwq_path_still_available_via_env_var` proves the escape hatch by setting `HF2Q_USE_LEGACY_DWQ_Q4_0=1` and asserting the old Q4_0-family bytes come back.  Plus 2 helper smoke tests (fixture byte-stability + `GgmlType` enum-variant existence).  `tests/cmd_convert_dispatch.rs::dwq_4_6_routes_through_dwq_calibrator`'s log-substring assertion was updated in-place to accept the new "DwqKQuantizer dispatch" breadcrumb alongside the three legacy phrases — keeps the legacy escape-hatch path satisfying the assertion too.  In-source debug-only `iter_c_k_quant_without_codec_direct_sentinel_panics_in_debug` test (gated `cfg(debug_assertions)` + `should_panic`) locks the defence-in-depth guard in debug builds.

- **Verification** (RUSTC_WRAPPER unset; sccache blocked in this sandbox): cargo build `0 errors`; `cargo test --release --test dwq_emits_q4_k_via_cli` 6 passed (4 binary-driven + 2 fixture/enum smoke); `cargo test --release --test dwq_k_quantizer` 6 passed (Iter B baseline preserved); `cargo test --release --test codec_direct_type_code` 4 passed (Iter A baseline preserved); `cargo test --release --test cmd_convert_dispatch -- --test-threads=1` 6 passed + 2 ignored (T4 breadcrumb update verified end-to-end serially; parallel-thread mode shows the same env-pollution flake on `auto_routes_per_decision_18_table` documented at the iter-2 baseline); `cargo test --release --test convert_integration` 16 passed (no regression in adjacent integration coverage); `cargo test --release --test calibrate_dispatch` 10 passed (calibrator-side seam unchanged); `cargo test --release --test quant_method_dispatch` 12 passed (Decision 12 menu lock unchanged); `cargo test --release --test ppl_driver` 26 passed (P10 baseline preserved); `cargo test --release --test peer_parity_gates -- --test-threads=1` 47 passed + 8 ignored (P10 baseline preserved); `cargo test --release --test imatrix_xvalidation` 36 passed + 1 ignored (P6 baseline preserved); `cargo test --bin hf2q --release` **2094 passed + 11 ignored** (matches Iter B baseline; the new `iter_c_k_quant_without_codec_direct_sentinel_panics_in_debug` test is `cfg(debug_assertions)` so does not run under `--release` — debug-build run lands as a follow-up if downstream wants the count to bump); `cargo clippy --release -p hf2q -- -D clippy::correctness` 0 errors / **388 warnings** (down 5 from Iter B's 393 — Iter C wires the previously dead-code `DwqKVariant` / `DwqKQuantizer::{new, variant, is_sensitive_tensor, target_for}` / `kquant_target_short_name` / `extract_layer_index` symbols; well within ≤ 395 budget).
...
**Iter D landed (commit `7527a9e`, 2026-04-27)**: vision-tensor + non-256-multiple skip predicate.  NEW `should_emit_f16_for_kquant(tensor_name, row_len)` at `src/quantize/layer_mix.rs:289` recognises (a) vision patterns: `model.visual.*` / `vision_tower.*` / `vision_model.*` / `vit.*` / `visual.*` / `.visual.*` (substring-based to handle prefix-stripping AND mid-name layouts like `language_model.visual.*`); (b) defensive: any `row_len % 256 != 0`.  NEW `f16_passthrough(tensor, name)` helper at `src/quantize/k_quant_codec_quantizer.rs:118` builds the `QuantizedTensor { method: "f16", bits: 16, preserved: true, ggml_type: Some("F16"), … }` payload; handles F32/F16/BF16 source dtypes and casts to F16 LE bytes (BF16 via the existing `TensorRef::to_f16` helper).  Plumbed into `KQuantCodecQuantizer::quantize_tensor` (`src/quantize/k_quant_codec_quantizer.rs:271`) + `VariantKQuantizer::quantize_tensor` (`src/quantize/variant_quantizer.rs:190`) + `DwqKQuantizer::quantize_tensor` (`src/quantize/dwq_k_quantizer.rs:267`); skip → F16 passthrough → routes through `repack_to_ggml_blocks`'s F16 arm (`src/backends/gguf.rs:1049`) AND `quant_info_to_ggml_type`'s `info.preserved` short-circuit (`src/backends/gguf.rs:1681`).  **Ordering**: explicit `config.preserve` (and `should_skip_quantization`'s `ffn_gate_inp` rule) wins over the implicit Iter D skip — every existing per-tensor preserve site keeps the legacy `method = "passthrough"` shape unchanged.  **2 always-on integration tests** (build smoke + CLI `--help` smoke) at `tests/vision_tensor_skip_predicate.rs` + **17 in-source unit/dispatch tests** (6 in `layer_mix.rs::tests`, 5 + 1 renamed-and-repurposed in `k_quant_codec_quantizer.rs::tests`, 2 in `variant_quantizer.rs::tests`, 4 in `dwq_k_quantizer.rs::tests`) lock the predicate + the three dispatch-site behaviours.  **`#[ignore]`-gated end-to-end test on Qwen3.6-27B q4_k_m: DEFERRED** — requires ~54 GB HF cache + ≥ 60 GB free RAM + 60-90 s wall-time; gated `#[ignore]` for manual run via `cargo test --release --test vision_tensor_skip_predicate -- --ignored`; not run in this iter (RAM-tight session: queen-side P11 re-emits are the canonical end-to-end AC, see "After Iter D lands, retry the four P11 re-emits" below).

**Verification** (RUSTC_WRAPPER unset; sccache blocked in this sandbox): cargo build `0 errors` (79 baseline warnings preserved); cargo test `--release --test vision_tensor_skip_predicate` 2 passed + 1 ignored; cargo test `--release --test codec_direct_type_code` 4 passed (Iter A baseline preserved); cargo test `--release --test dwq_k_quantizer` 6 passed (Iter B baseline preserved); cargo test `--release --test dwq_emits_q4_k_via_cli` 6 passed (Iter C baseline preserved); cargo test `--release --test peer_parity_gates -- --test-threads=1` 47 passed + 8 ignored; cargo test `--release --test ppl_driver` 26 passed; cargo test `--release --test imatrix_xvalidation` 36 passed + 1 ignored; cargo test `--bin hf2q --release` **2118 passed + 11 ignored** (delta: +49 from baseline 2069 = 17 new Iter D in-source + 1 repurposed `dispatcher_misaligned_row_routes_to_f16_passthrough` + ~31 from prior iters consolidated into the same release build); cargo clippy `--release -p hf2q -- -D clippy::correctness` 0 errors / **388 warnings** (zero new vs 388 baseline at SHA `3582f31`; well within ≤ 395 budget).  **LOC**: production code +215 (layer_mix.rs +46, k_quant_codec_quantizer.rs +112, variant_quantizer.rs +26, dwq_k_quantizer.rs +31) — well under 250 LOC HARD-STOP.  Test code +340 in-source + 180 NEW integration crate.

**Test-expectation update (Iter-D-induced)**: three pre-Iter-D in-source tests had their expectations re-stated to match the new dispatcher behaviour — they were guarding the codec-rejection error path that Iter D's defensive arm now pre-empts.  The codec's typed `KQuantCodecError::NotBlockAligned` rejection itself is preserved and is still tested at `src/quantize/k_quant_codec.rs::tests` for callers that bypass the dispatcher.  Affected tests: `quantize::k_quant_codec_quantizer::tests::rejects_misaligned_row` → renamed `dispatcher_misaligned_row_routes_to_f16_passthrough` (+ rewritten body).  Two `*_preserve_passthrough` tests (`k_quant_codec_quantizer` + `variant_quantizer` + `dwq_k_quantizer` `end_to_end_preserve_path_returns_passthrough`) continue to pass unchanged thanks to the ordering invariant (explicit `config.preserve` wins over Iter D skip).

**P11 unblocked**: re-emit retry of the four DWQ GGUFs (qwen3.6-27B-dwq46, qwen3.6-27B-dwq48, qwen3.6-35b-A3B-...-dwq46, qwen3.6-35b-A3B-...-dwq48) is now safe to dispatch.  The vision-tensor codec-rejection blocker that killed the first attempt at base SHA `87c6242` is closed.
...
- **ADR-012 P9 final close**: the four re-emitted DWQ GGUFs in P11 now use Q4_K_M base + Q6_K|Q8_0 sensitive — modern format llama.cpp ships native support for.  No regression expected since Q4_K_M is canonically better quality at 4-bit.
- **Decision 12 menu**: variant names `dwq46/48/68/28` retained (replace-in-place; no in-tree consumer depends on the Q4_0 byte format — `is_dwq_method` predicate keys on the `dwq*` prefix and the `quant_method` tag override preserves it; `default_filename_suffix` continues to suffix with the user-facing variant name; ADR-012 D22 ship list keys on the same names).
