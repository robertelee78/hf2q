...
| P7 — `Calibrator` × `OutputFormat` orthogonal split | 🟡 trait + None impl + k-quant dequantize landed; Imatrix/Dwq impls + k-quant quantize codebook + Layout A migration pending | `33081aa` (Calibrator trait + NoneCalibrator + CalibrationData enum), `ac3ebf2` (P7 iter-3a — Q4_K block layout + dequantize), `ade910c` (P7 iter-3c — Q5_K + Q6_K block layouts + dequantize, co-landed with parallel ADR-005 P3 iter-203 due to staged-index sweep) | `Calibrator` trait (Send + Sync + object-safe) + `CalibrationData` enum (None / Imatrix / ImatrixWithStats / Dwq) + `CalibrationCorpus` + `NoneCalibrator` truly no-op impl + 5 typed `CalibrationError` variants. 7 unit tests. P7 iter-3 k-quant codebook port: **Q4_K + Q5_K + Q6_K block layouts (`repr(C)` byte-for-byte match against `block_q*_K` in `ggml-common.h`) + `dequantize_row_q4_k` / `dequantize_row_q5_k` / `dequantize_row_q6_k` (pure-Rust ports of `ggml-quants.c:1467`/`:1669`/`:1877`)**. 20 unit tests covering block size invariants, byte round-trip, all-zero blocks, known-quant paths, sign preservation, multi-block decode, mis-aligned input rejection, and short output buffer rejection. Pending: ImatrixCalibrator + DwqCalibrator impls (P7 iter-2-bis); Q4_K/Q5_K/Q6_K **quantize** pass with `make_qkx2_quants`/`make_qkx3_quants` codebook search (P7 iter-3b); Layout A path migration (P7 iter-4). |
