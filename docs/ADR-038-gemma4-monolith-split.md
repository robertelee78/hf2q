...
- **Status**: 🚧 IN PROGRESS — Steps 1+2+3 SHIPPED + Step 4 G4-CFA-1/2/3/4/5/5b/5c/5d SHIPPED 2026-05-23 (latest hf2q `80a416c5`). CFA-5e IN-PROGRESS through 2 sub-iterations: step 1 (GELU activation fix; coherence improvement, surface symptom unchanged) + step 2 (m-dependency diagnostic pinpointing bug at m≥4 in tree-verify, ruling out lm_head as sole cause). Remaining root-cause work for CFA-5e narrowed to: tree_attention kernel at tree_seq_len≥4, Q/K head-norm + RoPE at positions≥1, KV cache offsets, or attention permute. CFA-6 (SOTA bench) still BLOCKED on CFA-5e. **Discovery arc**: ADR-038's loader-then-runtime-path-then-kernel investigation surfaced 4 dense-Gemma-4 production gaps in sequence: 5b loader, 5d projection dispatch, 5e/step1 wrong activation, 5e/step2 m≥4 coherence bug, 5f production prefill MoE gate. hf2q has never run dense Gemma 4 31B end-to-end before this session — every working code path is being validated for the first time. 9 commits this session.
- **Date**: 2026-05-22
...
| Step 4 G4-CFA-5 Layer B — target GGUF + ≥50 token generation | ⛔ BLOCKED on CFA-5e | — | With CFA-5b loader + CFA-5c persistent KV + CFA-5d ggml_dtype-aware dispatch + tokenizer.json fixture, Layer B (`g4_cfa5_redhatai_end_to_end_smoke_2026_05_23`) now generates 50 finite-valued tokens end-to-end. But all 50 are token 240017 ("額") and `mean_accept_rate=0.000` — degenerate finite-but-wrong verifier output on "The capital city of France is" (correct would be Paris-related). Test SKIPs cleanly via new G4-CFA-5e all-identical-tokens gate; CI green. Real ≥50-token coherence assertion stays in the test as a load-bearing post-gate, will activate once CFA-5e fixes the kernel correctness. |
| Step 4 G4-CFA-5e — verifier coherence bug surfaces only at m≥4 (NOT pure kernel correctness) | ⏳ IN-PROGRESS | hf2q `3bf4817c` (step 1: GELU activation) + `80a416c5` (step 2: m-dependency diagnostic) | **Step 1 SHIPPED**: `gemma4_tree_verify_full_layer_q` was using `silu_mul` (Qwen activation); Gemma 4 uses GELU. Switched to `fused_gelu_mul` matching production. Per-layer hidden values changed measurably (layer-0 max_abs 41.89→38.76 etc.) but prefill argmax unchanged (still 240017) — activation fix is correct but downstream of the bug. **Step 2 DIAGNOSTIC**: new `g4_cfa5e_lm_head_m_dependency_2026_05_23` test runs tree-verify at N∈{1,2,4,6,8,10}: N=1 produces plausible "S" (uses Q6_K NR2 m=1 fast path, production-validated); N=2 produces "la" via regular mv kernel (wrong but margin tight to 240017 at #3); **N≥4 consistently produces 240017** with widening margin. Also confirmed: lm_head ALONE is not the bug — at N=10 the dispatch path hits mm kernel (m>MM_ROUTING_THRESHOLD=8) and STILL returns 240017, so the bug is UPSTREAM of lm_head. **Remaining root-cause candidates** (in order of investigation priority): (a) tree_attention dk256/dk512 kernel correctness at tree_seq_len ≥ 4 with prefix_len=0; (b) Q/K head-norm + RoPE position alignment at sequence positions ≥ 1; (c) KV cache write/read offsets when prefill writes position 0..N-1 to F32 cache; (d) per-layer attention permute step (Q from [seq,nq,d] to [nq,seq,d]). Production `forward_decode` only ever runs m=1 per call so these multi-position paths have never been exercised end-to-end before. |
| Step 4 G4-CFA-5f — production `forward_prefill` MoE-gating gap on dense Gemma 4 31B | ⏳ TODO (separate-track; blocks CFA-5e baseline comparison) | — | NEW finding 2026-05-23: production `forward_prefill` (`src/serve/forward_prefill.rs:1572`) dispatches `fused_moe_routing_f32` UNCONDITIONALLY rather than gating on `num_experts > 0`. Dense Gemma 4 31B (`num_experts=0` per CFA-5b) trips on "fused_moe_routing_f32: num_experts and top_k must be > 0" at L0 T0. Also reads `pre_feedforward_layernorm_2` (a 1-element CFA-5b placeholder) without checking presence. **Implication**: hf2q has NEVER run dense Gemma 4 31B end-to-end through production prefill — CFA-5b was the first load + tree-verify was the first attempted run; both surface dense-vs-MoE assumptions baked deep in the production code path. Fix: wrap MoE-specific blocks (pre_ffw_norm_2 RMSNorm, router_combined_weight RMSNorm, router_proj, fused_moe_routing, expert dispatch) in `if num_experts > 0 { ... }` and add a dense branch that uses just `pre_feedforward_layernorm` + standard SwiGLU. Same pattern as CFA-5b's loader. Estimated ~150-200 LOC. CFA-5e baseline diagnostic blocked until this lands. |
