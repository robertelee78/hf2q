...
- **Status**: 🚧 IN PROGRESS — Steps 1 + 2 + 3 SHIPPED + Step 4 G4-CFA-1/2/3/4/5/5b/5c SHIPPED 2026-05-23 (latest: hf2q `225a83c6`); CFA-5 Layer B + CFA-6 now BLOCKED on **G4-CFA-5d** — verifier `forward_tree_verify_gpu_with_cache` produces NaN logits during prefill on real Gemma 4 31B Q4_K_M + valid 6-token prompt ("The capital city of France is"). Surfaced empirically post-CFA-5c once the `tokenizer.json` fixture unblocked the path past the prior NaN skip-gate; CFA-5c's drafter-NaN-attribution falsified — real tokens reproduce the same NaN. Layer B test now SKIPs cleanly via a new G4-CFA-5d skip-gate so CI stays green.
- **Date**: 2026-05-22
...
| Step 4 G4-CFA-5 Layer B — target GGUF + ≥50 token generation | ⛔ BLOCKED on CFA-5d | — | With CFA-5b loader + CFA-5c persistent KV both shipped + `tokenizer.json` symlinked into the GGUF dir, Layer B (`g4_cfa5_redhatai_end_to_end_smoke_2026_05_23`) progresses past prefill (prefix_len=6 for "The capital city of France is") but trips on `extract_top_k: row_logits[0] = NaN is not finite` at iter 0. Empirically traced at HEAD `225a83c6`: prefill returns `last_token=0` because the verifier's prefill logits ALL contain NaN (orchestrator argmax loop falls back to best_idx=0 since IEEE 754 `NaN > NEG_INFINITY` is false). The drafter NaN at extract_top_k is downstream. CFA-5c's drafter-NaN-attribution diagnosis FALSIFIED — real tokens reproduce the same NaN. Test now SKIPs cleanly via new G4-CFA-5d skip-gate (mirrors the CFA-5b/CFA-5c skip pattern). CI stays green. |
| Step 4 G4-CFA-5d — verifier NaN in `forward_tree_verify_gpu_with_cache` on real Gemma 4 31B | ⏳ TODO (NEW; blocks Layer B + CFA-6) | — | Diagnose + fix the verifier-side NaN that surfaces during prefill on dense Gemma 4 31B Q4_K_M with valid prompt tokens. Likely loci: (a) CFA-5b 1-element placeholder norms could be silently consumed somewhere despite the "mutually exclusive dense/MoE" claim; (b) Gemma 4 `attn_logit_softcapping` (config) not applied in tree-verify path while present in `forward_decode`; (c) RoPE `freq_factors` mask on global (dk512) layers wired wrong at full-model scale (G4-CFA-1 tiny-fixture test exercises kernel with synthetic weights only); (d) lm_head F16 path producing inf during F32→F16 casts on actual Q6_K → F16 lm_head dispatch route. Investigation strategy: (1) run base `forward_decode` on the same prompt to isolate tree-verify-specific vs loader-wide bug; (2) instrument prefill with per-layer hidden norm dumps to bisect which layer first produces NaN; (3) cross-reference qwen35 `forward_tree_verify_gpu` for any Gemma-4-specific dispatch path that the qwen35 reference doesn't hit. Estimated 1-3 day diagnostic effort. |
| Step 4 G4-CFA-6 — empirical SOTA bench on M5 Max (accept-length L≥1.5/k=1 + L≥2.0/k=3 primary; tokens/s>1.0× secondary) | ⛔ BLOCKED on CFA-5d | — | AC-4.10 exit gate (acceptance-length bands per RedHatAI README, not throughput ratio — see AC-4.10 §4). Cannot run meaningful bench until verifier produces finite logits. |
- **Author**: claude-flow
