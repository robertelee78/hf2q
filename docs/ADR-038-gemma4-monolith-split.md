...
| Step 4 G4-CFA-5e — verifier coherence bug — REVISED SCOPE: hf2q-wide vs llama.cpp baseline, NOT tree-verify-specific | ⏳ IN-PROGRESS (revised) | hf2q `3bf4817c` (step 1: GELU activation) + `80a416c5` (step 2: m-dependency diagnostic) + **5611242a** (step 4: CFA-5f shipped + llama.cpp baseline diagnostic) | **Step 4 BASELINE CONFIRMED 2026-05-23**: `llama-cli` (b9278) on same Gemma 4 31B Q4_K_M GGUF + same prompt "The capital city of France is" produces `"The user is providing a"` (chat-tuned model interprets raw text as user message). hf2q `forward_prefill` returns `last_token=0` (empty token) and hf2q tree-verify returns `236773 "S", 759 " la", 240017 "額"` saturated — **NEITHER matches llama.cpp**. This INVALIDATES the prior CFA-5e diagnosis that "CFA-5e is tree-verify-specific" — the bug affects BOTH paths, just manifesting as different wrong tokens. Coherence-P0 implication: **the production single-token path is ALSO broken vs peer**, the wrong-but-different output (token 0 vs llama's "The user...") suggests a shared upstream bug in loader / weight interpretation / model config (likely candidates: per-layer LayerType assignment for sliding-vs-global attention, rope_theta_global vs rope_theta_sliding values, sliding_window setting, final_logit_softcap, lm_head dtype dispatch). Prior steps 1-3 remain valid but address SYMPTOMS not root cause. **NEXT investigation priority** (multi-day, cannot complete in single session): (a) per-layer hidden-state diff between hf2q and llama.cpp at L0-L5 for same input token — bisect first divergence layer; (b) compare hf2q's `Gemma4Config::from_gguf` parse vs llama.cpp's `llm_load_hparams` for same GGUF — check rope_theta, sliding_window, layer pattern; (c) verify lm_head softcap / final RMSNorm interactions with Gemma 4 specifics. **Performance baseline (llama.cpp)**: 83 t/s prompt, 27.5 t/s generation on M5 Max. Bench parity bar for CFA-6 set. |
