...
- **Status**: 🚧 IN PROGRESS — Steps 1 + 2 + 3 SHIPPED + Step 4 G4-CFA-1/2/3/4/5 (drafter-load layer A) + CFA-5b (dense Gemma 4 loader path) + CFA-5c (persistent KV cache) SHIPPED 2026-05-23; CFA-6 is next
- **Date**: 2026-05-22
...
| Step 4 G4-CFA-5 Layer B — target GGUF + ≥50 token generation | ✅ SHIPPED (via CFA-5c) | (see CFA-5c) | Layer B test (`g4_cfa5_redhatai_end_to_end_smoke_2026_05_23`) runs to completion; NaN skip-gate removed. Test SKIPs (not fails) only when `tokenizer.json` is absent from GGUF directory — the ≥50-token AC requires a real tokenized prompt; the persistent-KV NaN bug is confirmed fixed by `g4_cfa5c_multi_iter_kv_continuity_2026_05_23` (3 iters, all-finite logits). |
| Step 4 G4-CFA-5c — persistent KV cache in `forward_tree_verify_gpu` | ✅ SHIPPED | this commit | `MlxModelWeights::forward_tree_verify_gpu_with_cache` new entry point accepts caller-owned `&mut Vec<(MlxBuffer, MlxBuffer)>` per-layer F32 KV cache (`alloc_tree_verify_kv_caches` helper handles per-layer head_dim variance: 256 sliding / 512 global). Old `forward_tree_verify_gpu` shim preserved for back-compat (allocates fresh cache, delegates). `Gemma4Eagle3Orchestrator` gains `kv_caches_f32` field; `prefill` seeds it once via `alloc_tree_verify_kv_caches`, `run_iteration` reuses across iters. Hard `ensure!` on double-prefill + uninitialized-cache guards. Two new tests: `g4_cfa5c_multi_iter_kv_continuity_2026_05_23` (3 iters, all-finite, hidden-state counterfactual proves cache is load-bearing), `g4_cfa5c_old_signature_back_compat_2026_05_23` (byte-identical to new sig). 36/36 g4_cfa* + 31/31 qwen35_tree_verify PASS. AC-G4-5c.1 through AC-G4-5c.7 all green. |
| Step 4 G4-CFA-6 — empirical SOTA bench on M5 Max (accept-length L≥1.5/k=1 + L≥2.0/k=3 primary; tokens/s>1.0× secondary) | ⏳ TODO | — | AC-4.10 exit gate (acceptance-length bands per RedHatAI README, not throughput ratio — see AC-4.10 §4). Blocked only on placing `tokenizer.json` alongside GGUF for end-to-end generation. |
- **Author**: claude-flow
