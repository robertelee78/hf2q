...
- **Status**: 🚧 IN PROGRESS — Steps 1+2+3 SHIPPED + Step 4 G4-CFA-1/2/3/4/5/5b/5c/5d SHIPPED 2026-05-23. CFA-5 Layer B + CFA-6 now BLOCKED on **G4-CFA-5e** — CFA-5d cleared the all-NaN bug (Q/K/V/O/MLP projections now use the correct ggml_dtype via new `apply_linear_projection_f32_qweight` helper) and Layer B generates 50 finite-valued tokens, but the verifier produces degenerate finite-but-wrong output (all 50 tokens identical = 240017 "額" instead of Paris-related token; mean_accept_rate=0.000). Likely root: Q4_K/Q5_K/Q6_K mv-kernel correctness at large-N shapes — Q-proj N=8192 vs prior-tested Qwen-router-weight N≤256 per `mlx-native/.../quantized_matmul_ggml.rs:487`. Layer B test now SKIPs cleanly via a new G4-CFA-5e all-identical-tokens gate so CI stays green.
- **Date**: 2026-05-22
...
| Step 4 G4-CFA-5e — Q4_K/Q5_K/Q6_K mv-kernel correctness at large-N projection shapes | ⏳ TODO (NEW; blocks Layer B + CFA-6) | — | Verifier produces deterministic-but-wrong logits (consistent argmax to "額" / 240017) even though all per-layer hidden states are finite and the all-NaN was cleared by CFA-5d. Strong signature of a SYSTEMATIC dequantization or matmul-orientation bug specific to large-N Q-family quantized projections. Per `mlx-native/.../quantized_matmul_ggml.rs:487` comment: Q4_K kernel has only been tested on Qwen "router weights with N ≤ 256" — Gemma 4 31B Q-proj has N=8192 (32×). Investigation strategy: (a) write a focused unit test that runs `apply_linear_projection_f32_qweight` on a real layer-0 Q-proj weight + a known input vector, compare output bit-by-bit to llama.cpp's `ggml_compute_forward_mul_mat` reference for the same Q6_K weight; (b) bisect mv vs mm dispatch by forcing `MM_ROUTING_THRESHOLD` to 0 (test at m=1 only via session.quantized_matmul_ggml directly); (c) inspect Q4_K super-block dequant kernel for row-major vs col-major orientation bug; (d) compare against production `forward_decode` running on the same model — if it produces "Paris", the kernel works at m=1 + production-path setup, narrowing the scope to tree-verify-specific dispatch (encoder vs session lifecycle, F16 shadow availability, etc.). Estimated 1-3 day kernel-debug effort. |
