...
- **Status**: 🚧 IN PROGRESS — Steps 1+2+3 SHIPPED + Step 4 G4-CFA-1/2/3/4/5/5b/5c/5d SHIPPED 2026-05-23. CFA-5 Layer B + CFA-6 BLOCKED on **G4-CFA-5e** (Q4_K/Q5_K/Q6_K mv-kernel correctness at large-N — verifier produces deterministic-but-wrong tokens). CFA-5e diagnosis blocked on **G4-CFA-5f** (production `forward_prefill` dispatches MoE routing kernel unconditionally, fails on dense `num_experts=0`). **Discovery arc**: ADR-038's loader-then-runtime-path-then-kernel investigation surfaced 3 dense-Gemma-4 production gaps in sequence (5b loader, 5d tree-verify dispatch, 5f production prefill MoE gate) — hf2q has never run dense Gemma 4 31B end-to-end before this session.
- **Date**: 2026-05-22
...
| Step 4 G4-CFA-5e — Q4_K/Q5_K/Q6_K mv-kernel correctness at large-N projection shapes | ⏳ TODO (NEW; blocks Layer B + CFA-6) | — | Verifier produces deterministic-but-wrong logits (consistent argmax to "額" / 240017) even though all per-layer hidden states are finite and the all-NaN was cleared by CFA-5d. Strong signature of a SYSTEMATIC dequantization or matmul-orientation bug specific to large-N Q-family quantized projections. Per `mlx-native/.../quantized_matmul_ggml.rs:487` comment: Q4_K kernel has only been tested on Qwen "router weights with N ≤ 256" — Gemma 4 31B Q-proj has N=8192 (32×). Investigation: tried to use production `forward_prefill` as a baseline comparison via new `g4_cfa5e_forward_prefill_baseline_2026_05_23` test — but discovered separate CFA-5f gap (see below). Remaining strategy: (a) focused unit test running `apply_linear_projection_f32_qweight` on real layer-0 Q-proj weight + known input, compare output to a Rust-side Q6_K CPU dequant + matmul reference (1-3 day effort to implement CPU ref); (b) bisect mv vs mm by forcing `MM_ROUTING_THRESHOLD = 0` via env override; (c) inspect Q4_K super-block dequant kernel for row-major vs col-major orientation bug; (d) once CFA-5f is fixed, use forward_prefill output as the baseline. |
