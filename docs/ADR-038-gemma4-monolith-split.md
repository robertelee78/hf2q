...
- **Status**: 🚧 IN PROGRESS — Steps 1 + 2 + 3 SHIPPED + Step 4 G4-CFA-1/2/3/4/5 (drafter-load layer A) + CFA-5b (dense Gemma 4 loader path) SHIPPED at hf2q `4d100919` (2026-05-23); CFA-5 Layer B + CFA-6 now BLOCKED on CFA-5c (persistent KV cache) only
- **Date**: 2026-05-22
...
| Step 4 G4-CFA-5 Layer B — target GGUF + ≥50 token generation | ⛔ BLOCKED on CFA-5c (loader gap closed by CFA-5b) | — | Loader path now succeeds end-to-end: target GGUF 60 layers in 24.64 s, drafter loaded, prefill ran prefix_len=32 in 4.76 s. Layer B then surfaces the **next blocker** — `Gemma4Model::forward_tree_verify_gpu` (gemma4/forward_gpu.rs:2088-2117) fresh-allocs per-layer F32 KV inside the call instead of accepting a caller-owned cache, so prefill's K/V is discarded across iterations, the verifier reads zeros at `[0, prefix_len)`, and `extract_top_k` trips on `row_logits[0] = NaN is not finite`. Test now SKIPs cleanly with a CFA-5c skip-gate (same shape as the pre-CFA-5b loader skip-gate this iter closed): explanatory eprintln points at CFA-5c, returns Ok so CI stays green. The ≥50-token assertion moves to CFA-5c's acceptance criteria. |
| Step 4 G4-CFA-5c — persistent KV cache in `forward_tree_verify_gpu` | ⏳ TODO (NEW; blocks CFA-5 Layer B + CFA-6) | — | Refactor `MlxModelWeights::forward_tree_verify_gpu` (gemma4/forward_gpu.rs:2088-2117) to accept caller-owned `&mut Vec<(MlxBuffer, MlxBuffer)>` per-layer F32 KV cache instead of fresh-alloc per call. Mirror the Qwen35 `HybridKvCache` pattern threaded through `Eagle3Orchestrator`. Current behavior (now empirically observed at `4d100919` Layer B iter 0): K/V discarded across iterations → verifier sees zeros at `[0, prefix_len)` → degenerate softmax → NaN logits → `extract_top_k` invariant fires. Required to unblock the AC-4.10 acceptance-length bar in CFA-6. Estimated ~200 LOC additive (new entry point that delegates from the old API for back-compat). |
| Step 4 G4-CFA-6 — empirical SOTA bench on M5 Max (accept-length L≥1.5/k=1 + L≥2.0/k=3 primary; tokens/s>1.0× secondary) | ⛔ BLOCKED on CFA-5c (CFA-5b loader gap closed) | — | AC-4.10 exit gate (acceptance-length bands per RedHatAI README, not throughput ratio — see AC-4.10 §4) |
- **Author**: claude-flow
