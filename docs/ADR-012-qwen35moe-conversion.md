...
**Status:** 🟡 **IN PROGRESS** — P0–P7 + P10 + P11 shipped 2026-04-24 (19 of 20 decisions complete, 1089 tests green as of audit pass). P8 functional-complete (real-model close hard-blocked on HF_TOKEN + disk + release build); P9 foundation shipped (NotReady shim + quality thresholds + drift gates), real activations hard-blocked on ADR-013 P12. Decision 17's real-model PPL/KL gate is the single remaining decision — same ADR-013-P12 hard block. P8 close path was triple-defect-fixed in the 2026-04-24 audit pass: real llama-cli format support (parsers + error msgs + flag set + sanitizer + quant alias + --with-vision wiring). Per mantra (`~/Documents/mantra.txt`, 2026-04-07): no deferred work, no stubs, no fallback, optimize for best outcomes. This ADR closes only when ADR-013 P12 unblocks Decision 17.
