...
| P8 — Arch-registry + smoke subcommand (Decisions 16, 20) | 🟡 in progress 2026-04-25 | — | `src/arch/{mod,registry,catalog,conformance,smoke,entries/{qwen35,qwen35moe}}.rs` landed (~1100 LOC); `hf2q smoke` Rust subcommand wired into `cli.rs` + `main.rs` with distinct exit codes (2 HF_TOKEN, 3 disk, 4 llama-cli, 5 hf2q binary, 6 HF repo). 32 unit tests + 8 integration tests in `tests/smoke_conformance.rs` green; uniform "unknown arch; known arches: qwen35, qwen35moe" dispatch error proven across 4 unregistered names. **Pending:** real-model transcripts under `tests/fixtures/smoke-transcripts/{qwen35,qwen35moe}-q4_0.txt` (requires HF download + llama-cli on hardware). |
...
| P11 — MTP tensor round-trip integrity gate (Decision 19) | ✅ shipped 2026-04-25 | — | `tests/convert_qwen35_mtp_roundtrip.rs` (3 tests, dense + MoE + placeholder-absent guard) drives `hf2q convert` end-to-end on a synthetic 4-layer model with `mtp_num_hidden_layers: 1` and proves every expected `nextn.{eh_proj,embed_tokens,enorm,hnorm}` tensor lands at `blk.{num_hidden_layers}.nextn.*` post-resolution.  **Chesterton's-fence fix shipped alongside:** `src/backends/gguf.rs` previously emitted unresolved placeholder names `blk.mtp{idx}.nextn.*` with comment "callers resolve later" — a literal stub.  Added `resolve_mtp_block_index(name, num_hidden_layers)` helper, wired into the production write path so the placeholder is now always resolved against `metadata.num_layers`.  Two new unit tests in `gguf::tests` pin the placeholder behavior + the resolver in isolation.  ADR-013 P14 cross-link entry landed in same change.  All 3 P11 tests green; existing P4 placeholder test left intact for backwards visibility. |
