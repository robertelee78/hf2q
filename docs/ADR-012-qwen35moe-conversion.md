...
| Commits on worktree `adr-012-p8-p11` | `ebec4a1` (P8 scaffold) · `0a1a7b7` (P11 + MTP stub fix + ADR-013 P14 cross-link) · `f33668f` (docs/arch-onboarding.md) · `13d619a` (P10 mmproj emitter + Layer C) · `6a3617c` (P10 Layer A + B) · `448477e` (P9 foundation) · `6c74599` (benchmarks) · `7d5d6eb` (P5 MoE wire-up fix) · `d90ac1d` (smoke Q4_0 runner + --local-dir) · `18cbaaa` (P10 sovereignty: single authoritative mmproj) · `31ace78` (DWQ NotReady CLI integration test) |
| Total tests | 942 unit + 40 integration (smoke_conformance × 12 + convert_qwen35_mtp_roundtrip × 3 + convert_vision_tower_integration × 3 + convert_vision_tower_adr005_roundtrip × 1 + convert_qwen35_real_activation_capture × 2 + quality_thresholds × 4 + calibration_eval_disjoint × 4 + preexisting × 11) = 982 green · 8 ignored |
| New tests added P8→P11 current state | ≈ 91 (P8 scaffold 46 + smoke_conformance 12 + MTP round-trip 3 + Layer A 3 + Layer B 1 + Layer C 8 + P9 foundation 4 + 4 + 4 + DWQ CLI 2 + activation_capture_real 4) |
...
**2026-04-24 silent wire-up finding (R2 still OPEN):** The `transform_linear_attn_tensor` and `transform_in_proj_qkvz` functions shipped in P2/P3 (`1a849e1` + `73a96e4`) were tested in isolation but never invoked from the convert pipeline. A `apply_qwen35_linear_attn_transforms_in_tensor_map` walker was added alongside this note; invoking it from `src/main.rs` exposes pre-existing incorrect shapes in the `tests/convert_qwen35*_integration.rs` synthetic fixtures (e.g. A_log shape [num_heads] where the spec requires [num_value_heads]). **The wire-up + fixture corrections are deferred to a dedicated follow-up commit that also adds positive-value regression tests for the V-head reorder invariants** (seed a Q<32,K<32,V>32 synthetic and verify the output bytes match a hand-authored expected permutation map). Until that commit lands, every Qwen3.5 GGUF produced by hf2q ships HF-grouped V-head layout — R2 is NOT yet mitigated in shipped output.
