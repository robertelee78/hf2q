...
**Status:** 🟡 **IN PROGRESS** — P0–P7 + P10 + P11 shipped 2026-04-24; **P9b two-pass wire-up shipped 2026-04-25** (all 7 sub-tasks 9b.1→9b.7 green). 19 of 20 decisions complete, 1114 tests green across the worktree. P8 functional-complete (real-model close hard-blocked on HF_TOKEN + disk + release build); P9 functional-complete (real `impl ActivationCapture` `28df83c` + real `run_dwq_activation_calibration` `076fd82` + convert-pipeline two-pass branch `c04e47d` + reachability tests `302c602` — convert pipeline can now run activation-aware DWQ end-to-end on a synthetic tiny qwen35; real-model DWQ artifacts blocked on the same HF_TOKEN+disk+release-build gate as P8). Decision 17's real-model PPL/KL gate is the single remaining decision; engineering complete, environment gate identical to P8. Per mantra (`~/Documents/mantra.txt`, 2026-04-07): no deferred work, no stubs, no fallback, optimize for best outcomes. **This ADR closes when the four real-model DWQ GGUFs are produced** — engineering side has no remaining blocker.
