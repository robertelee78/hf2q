...
| **Q4_K_M (FULL-PIPELINE 2026-05-20 POST-e500bbd9)** | **Gemma 0.00039% / Qwen 0.0019% overall** | After REVERTING b921616e (which was based on a stale-binary diagnostic artifact), Q6_K residual dropped 78% on both families. Final by type: **F32/F16 0% on Gemma, 0.0001% on Qwen** (ssm_a torch.exp 1-ULP); **Q4_K 0.00018% / 0.00021%** (sub-ppm, essentially byte-identical via make_qkx2_quants); **Q6_K 0.00737% / 0.00720%** (the inherent FP precision floor with .mul_add at sum/sumlx/suml2 matching canonical's clang fp-contract=on default — structurally similar across both families). Q5_0/Q8_0 on Gemma: 0/2.9GB and 0/3.9GB BYTE-IDENTICAL. hf2q convert wall time: Gemma 26B = 1m 30s, Qwen 35B = 3m 18s — **~6-10× faster than canonical's convert+quantize pipeline**. |
