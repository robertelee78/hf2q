...
| **Q4_K_M** | 0.053% overall bytes diff (pre-fix) → ~0.082% sampled Q4_K-only post-fix | **Q6_K tensors-in-Q4_K_M now BYTE-IDENTICAL** at b921616e (12 rows × 4 tensors verified, 0/16380 bytes diff; commit `60d458b3`). Remaining ~0.082% Q4_K residual comes from `make_qkx2_quants` per-site bisection-confirmed input-distribution-dependent rounding edge cases (commit `6ff990b8` + memory `project-make-qkx2-bisection-2026-05-20`). |
| **Q5_K_M** | Similar K-quant scatter (pre-fix) | 333/379 Q5_K (via `make_qkx2_quants`) + 64/64 Q6_K mismatches (pre-fix); **Q6_K side closed by b921616e**; Q5_K side at same `make_qkx2_quants` input-dependent ceiling as Q4_K. |
| **IQ4_NL** | Similar K-quant scatter | 71/391 IQ4_NL + 51/51 Q5_K + 1/1 Q6_K mismatches. Q6_K side closed by b921616e. |
