...
| **Q4_K_M (FULL-PIPELINE 2026-05-20)** | **Gemma 0.0015% / Qwen 0.008% overall** | Full hf2q convert vs current canonical SHA `e15384a5c`. By type: **F32/F16 0% on Gemma, 0.0001% on Qwen** (ssm_a torch.exp 1-ULP); **Q4_K 0.00018% / 0.00021%** (sub-ppm, essentially byte-identical via make_qkx2_quants); **Q6_K 0.034% / 0.033%** (the inherent FP precision floor — `make_qx_quants` input-distribution-dependent boundary cases, structurally similar across both families). hf2q convert wall time: Gemma 26B = 1m 30s, Qwen 35B = 3m 18s — **~6-10× faster than canonical's convert+quantize pipeline**. Earlier "broad sample byte-identical" findings were sampling bias — the 0.03% Q6_K residual is spread sparsely across all rows of all Q6_K tensors. |
