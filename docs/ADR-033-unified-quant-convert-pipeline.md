...
| **Q4_K_M (FULL-PIPELINE FINAL 2026-05-20 HEAD `6ed545c5`)** | **Gemma 0.000294% / Qwen 0.00175% overall** | After REVERTING b921616e AND a5c25adc (both based on misleading diagnostic + sampling-bias-prone synthetic tests), all `.mul_add` calls in make_qx_quants and make_qkx2_quants restored to canonical's clang `fp-contract=on` default fma posture. Final by type: **F32/F16 0% on Gemma, 0.0001% on Qwen** (ssm_a torch.exp 1-ULP); **Q4_K 0.0000013% / 0.0000011%** (essentially byte-identical via make_qkx2_quants — only 120 / 180 bytes across 9.3GB / 16.4GB); **Q6_K 0.00737% / 0.00720%** (inherent FP precision floor — 4,238/1,986,560 affected blocks on Qwen output.weight, each 1-9 bytes — input-distribution-dependent boundary cases in make_qx_quants iterative refinement). Q5_0/Q8_0 on Gemma: 0/2.9GB / 0/3.9GB BYTE-IDENTICAL. hf2q convert wall time: Gemma 26B = 1m 30s, Qwen 35B = 3m 18s — **~6-10× faster than canonical's convert+quantize pipeline**. **2026-05-20 LATE: Q6_K deep-dive (commit `15753080`)** — confirmed the residual is a per-sub-block 1-ULP boundary case at `sumlx² > best*suml2` comparison in `make_qx_quants` is-loop. NEON intrinsic exploration (per-chunk `vaddvq_f32` pairwise reduce + scalar accumulate) **bit-matches canonical exactly** for Gemma 4 blk.0.attn_v block 113 sub-block 14 (`scale=0x3b11a2a8` after fix vs `0x3b16f785` before), but **regresses 13 other sub-blocks** in the same tensor — clang's auto-vec picks different reduction patterns per call site based on register pressure, surrounding code, and inlining context. One Rust source-level pattern cannot match all canonical emit variants simultaneously without per-sub-block dispatch information we cannot infer. Diagnostic preserved in `src/quantize/ggml_quants/common.rs::tests::q6k_blk113_subblk14_byte_match` (`#[ignore]`'d). |
