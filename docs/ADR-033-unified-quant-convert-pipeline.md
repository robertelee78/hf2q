...
| **Q4_K_M (FFI CLOSURE 2026-05-20 commit `40ba4f50`)** | **Gemma 658/658 + Qwen 0/445 quantize-side BYTE-IDENTICAL ✅** | Opt-in `--ffi-canonical` dispatches per-tensor quantize through canonical `libggml-base.0.dylib`'s exported `quantize_q*` / `quantize_iq*` symbols. Pure-Rust kernels remain default. Codex review of prior dbcd2edc `.mul_add` restoration (3 HIGH findings: artifact-matching not source-matching, byte-identity to OLD canonical isn't legitimate §P1 closure when same-day regen differs) — REVERTED at 470ff537. FFI is the proper closure path: hf2q's pure-Rust kernels diverge from canonical at 1-ULP boundary cases due to clang auto-vectorized NEON reduction order vs Rust scalar `.mul_add`; no clean source-level Rust pattern matches all canonical emit variants across both Gemma and Qwen value distributions. FFI delegates to canonical's exact compiled emit. Verification: **Gemma 4 26B Q4_K_M vs FRESH canonical**: F32 0/392, Q4_K 0/192, Q5_0 0/32, Q6_K 0/14, Q8_0 0/28 — 658/658 byte-identical. **Qwen 3.5 35B Q4_K_M vs FRESH canonical**: F16 0/2, Q4_K 0/379, Q6_K 0/64 — all 445 quantize-side tensors byte-identical. Only F32 ssm_a tensors (28/308) still diff — 1-ULP `torch.exp` vs Rust libm `f32::exp` at pre-quantize storage (separate issue requiring PyTorch FFI for closure, tracked as Task #44). Architecture: process-singleton `OnceLock<CanonicalQuantInner>` (ggml-base's static init asserts on double dlopen — must load exactly once per process), rayon row-chunk parallelism per FFI call (16 chunks max), `PathMismatch` typed error if reload attempts a different path, no-fallback contract preserved (any FFI failure surfaces as typed `QuantizeError::CanonicalFfi`). |
