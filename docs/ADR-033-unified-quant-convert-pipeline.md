...
- **Llama 3 8B (NousResearch-Meta-Llama-3-8B) port in flight 2026-05-21** — refactored `llama3::build_metadata` to gemma4 signature (config, file_type, model_card, sampling, model_dir_basename) at commit `25bf6034`. Added `license_name` + `license_link` parsing to ModelCard (canonical `license: other` + `license_name: llama3` → `general.license.name` + `general.license.link`). Reordered `llama.*` arch KVs to canonical insertion order. Added missing KVs: `llama.attention.{key_length,value_length}`, `llama.vocab_size`, `llama.rope.dimension_count`. Added Llama 3-specific tokenizer branch matching canonical `_set_vocab_gpt2` order: model → `pre` right after model → tokens → token_type → merges → bos_token_id → eos_token_id → add_bos_token → add_sep_token (no scores, no add_space_prefix, no unk/pad). KV count: 35 = canonical 35 ✅. Tensor count: 291 = canonical 291 ✅. Final byte-cmp validation pending.
