...
After successful GGUF finalization, hf2q hashes the output and atomically publishes `<output>.receipt.json`. The receipt binds repo + exact revision, sorted source file hashes/sizes, converter package/version and compile-time git SHA when available, quant selector, output hash/size, DSpark exclusion status/count, and observed peak chunk-buffer bounds. A stale receipt is removed immediately before output mutation, and no success receipt is written on a partial or failing conversion.
