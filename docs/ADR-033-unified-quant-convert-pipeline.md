...
| **Q4_K_M (FFI CLOSURE FULL 2026-05-20 commits `40ba4f50` + `b1e0b0f7`)** | **Gemma 658/658 + Qwen 753/753 BYTE-IDENTICAL ✅✅** | Two opt-in FFI flags close all 1-ULP residuals between hf2q and canonical: **`--ffi-canonical`** dispatches per-tensor quantize through canonical `libggml-base.0.dylib`'s exported `quantize_q*` / `quantize_iq*` symbols (closes K-quant boundary cases). **`--ffi-torch-exp`** dispatches the `NegExp` bake operation (x → -exp(x), used for SSM A_log → ssm_a) through PyTorch's `Sleef_expf4_u10advsimd` from `libtorch_cpu.dylib` (closes the 28/308 ssm_a F32 tensors that differ by 1 ULP between libm.expf and torch.exp — empirically Sleef ≡ torch.exp for 1000/1000 uniform random f32 inputs). Pure-Rust kernels remain default; both FFI flags are opt-in. **Verification (with both flags)**: F16 0/2, F32 0/308, Q4_K 0/379, Q6_K 0/64 = all 753 Qwen tensors byte-identical to canonical regen. Gemma 4 26B with `--ffi-canonical`: 658/658 byte-identical. Codex audit of FFI implementation: NO BLOCKING FINDINGS. Architecture: `OnceLock<CanonicalQuantInner>` process-singleton (ggml-base's static init asserts on double dlopen), `OnceLock<TorchExpInner>` for sleef. Rayon row-chunk parallelism for canonical FFI. `PathMismatch` typed error on reload. No-fallback contract preserved (any FFI failure → typed error, never silent revert to pure-Rust kernel). |
