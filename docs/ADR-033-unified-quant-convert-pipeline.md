...
| **Q4_K_M** | 0.053% overall bytes diff | 461 diff-tensors; max per-tensor 5.47% on 1-D F32 ssm_a (1-ULP); most Q4_K tensors <0.1% bytes diff. K-quant `make_qx_quants` value-dependent FP edge cases. |
| **Q5_K_M** | Similar K-quant scatter | 333/379 Q5_K + 64/64 Q6_K mismatches, all sub-percent per tensor. |
| **IQ4_NL** | Similar K-quant scatter | 71/391 IQ4_NL + 51/51 Q5_K + 1/1 Q6_K mismatches. |
...
**Root cause proved 2026-05-20** (commit `66a3a2b8` + 4-variant C reproducer matrix at `/tmp/c_quant_repro/`): the K-quant residual on Qwen 3.5 is NOT simple FMA fusion (`-ffp-contract=on` level, which is what `f32::mul_add` exposes). It requires **`-ffp-contract=fast`** — cross-expression reassociation across the `sumlx += w*x[i]*l → suml2 += w*l*l → sumlx*sumlx > best*suml2` chain. Proof matrix on output.weight row 149 block 7 scales[12]: canonical lib **195**, canonical_mimic (full call chain) **195**, canonical_verbatim default **194**, canonical_verbatim `-ffp-contract=fast` **195**, hf2q Rust **194**. Rust stable has NO equivalent of `=fast` (only `f32::mul_add` maps to `=on`); nightly `core::intrinsics::fmuladd_f32` exists but is unstable. Diagnostic test added at `src/quantize/ggml_quants/q6_k.rs::tests::qwen35_lm_head_row149_q6k_block7_dump` (`#[ignore]`'d; requires `/tmp/c_quant_repro/row149_canonical_f32.bin`). This is the inherent FMA-rounding ceiling for K-quants until Rust exposes `=fast`-level reassociation, or until the K-quant kernels are vendored via FFI from clang-compiled `ggml-quants.c`. Sub-percent + functionally negligible; not blocking ADR-033 §P1 acceptance.
