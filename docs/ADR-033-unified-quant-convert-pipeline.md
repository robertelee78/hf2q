...
| **F32 (ssm_a only) — open pure-Rust gap at HEAD** | 28-30 / 308 tensors with 1-ULP diff | All `blk.X.ssm_a` 1-D tensors carry `-exp(A_log)` baked via `BakeOp::NegExp`. PyTorch's CPU `torch.exp` for f32 goes through SLEEF's `Sleef_expf4_u10` polynomial; Rust's `f32::exp()` (libm/Apple expf) bit-matches PyTorch only ~92% of the time on uniform random inputs in [-10, 5]. Verified one such divergence: input `-3.796875` → torch=`0x3cb7d5c0` (`2.244079e-02`), libm=`0xbfd5b7bc`. Apple Accelerate `vvexpf` matches torch on single-value tests but not across the full input space (different polynomial). **NEVER FFI** — closure path is a pure-Rust port of SLEEF's `expf4_u10` polynomial (BSL-1.0 licensed; coefficients are public, single function) implemented via `std::arch::aarch64::*` NEON intrinsics. Sleef-rs upstream exists (`sleef-rs` crate) but is currently a sys-bridge; an in-tree pure-Rust implementation of just the expf path is the target. Operator decision pending on prioritization vs the K-quant residual. |
