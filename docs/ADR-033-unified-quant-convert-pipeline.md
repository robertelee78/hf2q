...
| **Q4_K_M (FFI CLOSURE FULL 2026-05-20 commits `40ba4f50` + `b1e0b0f7` + `9ec98304`)** | **Gemma 658/658 + Qwen 753/753 BYTE-IDENTICAL ✅✅ — CODEX APPROVED ✅** | Two opt-in FFI flags close all 1-ULP residuals between hf2q and canonical: **`--ffi-canonical`** dispatches per-tensor quantize through canonical `libggml-base.0.dylib`'s exported `quantize_q*` / `quantize_iq*` symbols (closes K-quant boundary cases). **`--ffi-torch-exp`** dispatches the `NegExp` bake operation (x → -exp(x), used for SSM A_log → ssm_a) through PyTorch's `Sleef_expf4_u10advsimd` from `libtorch_cpu.dylib` (closes the 28/308 ssm_a F32 tensors that differ by 1 ULP between libm.expf and torch.exp — empirically Sleef ≡ torch.exp for 1000/1000 uniform random f32 inputs). Pure-Rust kernels remain default; both FFI flags are opt-in. **Verification (with both flags)**: F16 0/2, F32 0/308, Q4_K 0/379, Q6_K 0/64 = all 753 Qwen tensors byte-identical to canonical regen. Gemma 4 26B with `--ffi-canonical`: 658/658 byte-identical. **Codex audit FULL CLOSURE**: canonical_ffi at `40ba4f50` codex APPROVED (NO BLOCKING FINDINGS), torch_ffi at `9ec98304` codex APPROVED ("no blocking issues in the two reviewed areas. No findings."), orchestrator FFI wiring at `da9b9b1e` codex APPROVED ("FFI errors mapped to `QuantizeError::CanonicalFfi`...; no Rust-kernel fallback occurs. Payload sizing... matches FFI wrapper's expected `nrow * row_size`. `f16_rt` computed before branch, both paths see same input.") — verified end-to-end: float32x4_t AAPCS by-value FFI is ABI-correct, `try_init()` path-mismatch detection mirrors `canonical_ffi::load()`, tail handling only writes back populated lanes (no zero-padding corruption), orchestrator dispatch has no silent Rust-kernel fallback, no-fallback contract preserved end-to-end. Architecture: `OnceLock<(PathBuf, CanonicalQuantInner)>` process-singleton (ggml-base's static init asserts on double dlopen), `OnceLock<(PathBuf, TorchExpInner)>` for sleef (same path-mismatch semantics). Rayon row-chunk parallelism for canonical FFI. `PathMismatch` typed error on reload. No-fallback contract preserved (any FFI failure → typed error, never silent revert to pure-Rust kernel). |
...
| **F32 (ssm_a only)** | 28-30/308 tensors | All `blk.X.ssm_a` 1-D tensors with **1-ULP rounding diff** from PyTorch SIMD `torch.exp` vs Rust libm `f32::exp`. Verified: input -3.796875 → torch=`c0d5b7bc` vs libm=`bfd5b7bc`. Tested Apple Accelerate `vvexpf` (matches torch single-value) — full-tensor convert showed 30/30 ssm_a divergent at DIFFERENT positions; vvexpf doesn't bit-match torch across full input space. PyTorch likely uses SIMD intrinsics with its own polynomial. Closing this gap requires using PyTorch directly. |
