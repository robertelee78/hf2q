...
| **Q4_K_M (pure-Rust state at HEAD `488d480a`)** | **Gemma 4 26B: in-tree fixtures GREEN (0/658 byte_cmp tests); real-model 3 / 3,244,032 bytes diff (0.0001%) on `blk.0.attn_k` d-field. Qwen 3.5 35B: broad sample 0/21,888 closed.** | Pure-Rust port of `make_qkx2_quants` with plain `+= w * li_f` accumulators at L256-258 (no `.mul_add()`). The `dbcd2edc` re-introduction of `.mul_add()` was reverted by `470ff537`; the FFI workaround at `40ba4f50..6cb6f838` was reverted by `488d480a` (operator directive 2026-05-20: "we are a 100% PURE RUST repo... we port -- we NEVER ffi"). Remaining 3-byte Gemma d-field residual is an open pure-Rust source-level gap — candidate fixes: hand-write explicit `std::arch::aarch64::*` NEON intrinsics to match canonical's `-O3 -DNDEBUG` auto-vec reduction order at the relevant `sumlx*sumlx > best*suml2` comparison boundary. Full hf2q test suite at HEAD: 2770 passed / 0 failed / 33 ignored. |
...
| **F32 (ssm_a only) — CLOSED via pure-Rust SLEEF port** | bit-match torch on divergence point ✅ | All `blk.X.ssm_a` 1-D tensors carry `-exp(A_log)` baked via `BakeOp::NegExp`. PyTorch's CPU `torch.exp` for f32 goes through SLEEF's `xexpf` polynomial; Rust's `f32::exp()` (libm/Apple expf) bit-matches PyTorch only ~92% of inputs. **Closure shipped at module `src/convert/sleef_expf.rs`** — pure-Rust scalar port of SLEEF's `xexpf` (`sleefsimdsp.c:1314-1336`) using Cody-Waite ln(2) split + 6-term Horner-form polynomial via `f32::mul_add`. NO NEON intrinsics, NO `libloading`, NO FFI — per operator standing rule "we port -- we NEVER ffi" ([[feedback-we-port-never-ffi-2026-05-20]]). `BakeOp::NegExp` in `convert/arch/bake.rs:230` now calls `sleef_expf` instead of libm `x.exp()`. **Verified bit-identical to `torch.exp` for the documented divergence point**: input `-3.796875` → `0x3cb7d5c0` (== torch; libm produced `0x3cb7d5bf`, 1-ULP off). Tests: 5 unit tests pass (`matches_torch_on_known_divergence_point`, `differs_from_libm_at_divergence_point`, `matches_standard_reference_values`, `underflow_and_overflow_thresholds`, `sweep_shows_libm_divergence`). Full real-model byte-cmp on Qwen 3.5 ssm_a tensors pending operator-time verification. |
