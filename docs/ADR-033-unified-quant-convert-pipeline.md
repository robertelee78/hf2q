...
| **Q4_K_M (CLOSURE RESTORED 2026-05-20 commit `dbcd2edc`)** | **Gemma 658/658 BYTE-IDENTICAL ✅** | Restored `.mul_add()` at `make_qkx2_quants` L256-258 (`sum_l`/`sum_l2`/`sum_xl`) — reverting the c9c05d1f revert which had based the change on a verbatim C reproducer compiled in a different build cache state. The ORIGINAL canonical fixture at `tmp/byte-cmp/gemma4-llama-canonical-q4_k_m.gguf` (the reference hf2q achieved §P1 byte-identity against at commit 50fd89c2) was built with FMA-fused clang emit. Source of `ggml-quants.c` is byte-identical between c779f619 and e15384a5cb (git diff = 0 lines) but recompiling at e15384a5cb today produces different bytes — clang's effective fp-contract emit for these accumulator loops is build-cache-state dependent, not source-driven. Verification: F32 0/392, Q4_K 0/192, Q5_0 0/32, Q6_K 0/14, Q8_0 0/28 — all 658 tensors byte-identical. Test fixture regenerated against OLD canonical: `real_model_byte_cmp_blk0_attn_k` = 0/3244032 bytes diff. Full hf2q test suite: 3188 passed / 0 failed / 60 ignored. hf2q convert wall time: Gemma 26B = 1m 30s — **~6-10× faster than canonical's convert+quantize pipeline**. |
