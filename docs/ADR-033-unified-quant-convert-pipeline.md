...
| **Q4_K_M** | **Qwen 3.5 broad sample: 0/21888 bytes diff** (closed by c9c05d1f) | Q6_K side closed by b921616e; **Q4_K side now byte-identical** on Qwen 3.5 sampled rows after c9c05d1f revert of `.mul_add()` at sum_l/sum_l2/sum_xl in `make_qkx2_quants` + Gemma fixture regeneration against current canonical SHA. Gemma 4 Q4_K real-model: 3/3244032 = 0.0001% residual (one block's d field + 2 downstream qs — single boundary case). |
