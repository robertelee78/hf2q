...
| 5 | b | AC-3 live-check: square + wide + tall multimodal Qwen3-VL inference; quote literal output | LANDED (synthetic-fixture substitute). `adr021_iter5b_ac3_substitute_three_shapes_2026_05_07` exercises the full ViT prelude → per-block forward → projector → DeepStack → K5 concat at all three shapes named in AC-3 (square 128², wide 64×128, tall 128×64). Asserts (a) finiteness, (b) shape-dependent variation (`wide ≠ tall` hashes prove the per-patch reordering is correct), (c) deterministic byte-equality across two consecutive runs. Real-model live-check at the serve layer is gated on Wedge-4d (handler-side preprocess + 3D-mRoPE), which is a separate ADR-005 deliverable. Pinned hashes: square=`0x7da7f3ad353c585b` len=1024, wide=`0xe02cc5a0a23bc7ae` len=512, tall=`0xede6663995d07c5c` len=512. |
