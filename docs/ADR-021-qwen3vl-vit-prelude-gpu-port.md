...
| 11 | a | Peer-reference parity: build llama.cpp's `mtmd-cli`, run same image+prompt through both, byte-or-ULP compare ViT output AND first-token logits AND first 8 generated tokens. Pin in a new operator-gated test | PARTIAL — peer-reference text **CAPTURED** 2026-05-07. `llama-mtmd-cli` (homebrew binary against /opt/llama.cpp source-converted GGUF: bf16 text + f16 mmproj at `/tmp/llamacpp-qwen3vl-out/`) produces these literal outputs for the same 3 PNG fixtures hf2q's ViT already runs through:<br>`square 256×256 red:   "red"`<br>`wide   256×128 blue:  "Blue"`<br>`tall   128×256 green: "green"`<br>**hf2q's ViT image-token counts already match peer**: square=64, wide=32, tall=32. This is a strong signal the ViT prelude port is numerically correct (token grid math is identical). Stage A→B→C verified against the peer at the structural level. Pinning these output strings as the AC-3 target hf2q must produce once iter-228b lands the text-LM forward. Note: hf2q's converted GGUF emits arch=`qwen3_vl` (with underscore) which is NOT recognized by stock llama.cpp loader (expects `qwen3vl`); the peer-reference run used /opt/llama.cpp's own converter to sidestep this — the converter naming is a Wedge-4f deviation tracked under iter-11b cleanup. |
