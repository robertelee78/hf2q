...
| 30 | mlx-native `779aad4` + hf2q `445c0364` + `76d52174` | **H29-SPEED: V2-tile F16 mm-kernel LANDED (opt-in)**. Added `hf2q_mul_mm_tensor_v2_f16` — identical V2 64×128 tile + direct-device-B-read pattern, but with `half4x4` load from F16 shadow instead of per-call dequant. Rust dispatcher `dispatch_mm_v2_f16`. hf2q's `dispatch_qmatmul` F16 fast-path routes through this.<br>**Multi-context bench at byte-identical coherence**:<br>&nbsp;&nbsp;ctx=2K: 2583 → 2997 t/s = **+16.0%**<br>&nbsp;&nbsp;ctx=4K: 2722 → 2920 t/s = **+7.3%**<br>&nbsp;&nbsp;ctx=8K: 2560 → 2608 t/s = **+1.9%**<br>Gain shrinks with ctx (bandwidth-dominated at long ctx; F16 weight is 2 bytes/elem vs Q6_K ~0.7 bytes/elem packed → F16 reads MORE memory at long ctx). Peer ratio progression at 4K: V1 0.55× → V2 0.62× → V2+H29 **0.65× peer**.<br>H29 remains opt-in via `HF2Q_F16_SHADOW=1` — meaningful +7-16% at typical user-prompt sizes (2-4K) but diminishing returns at 8K+ argue against default-flip until a long-ctx compensation lever lands. Per `feedback_no_premature_mission_close` — improving, not closing. |
