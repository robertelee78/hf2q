...
| **H18** | **FFN_BODY 264 µs is fixed-overhead-bound, NOT kernel-throughput-bound. Skipping any subset of FFN_BODY's dispatches does not shrink wall time.** | iter-15 dual-skip experiment | **SKIP_DENSE_MLP + SKIP_MOE_EXPERTS (leaves only routing + post-FF norm 1 + WSum = 3 dispatches): 264 µs (no change). SKIP_DENSE_MLP + SKIP_MOE_EXPERTS + SKIP_ROUTING (leaves only norm 1 = 1 dispatch): 270 µs (no change).** | **✅ H18 CONFIRMED — FFN_BODY anchored by CB/driver overhead, not kernels** |
| **H19** | **Peer reaches 97 t/s at 1389 dispatches/tok (7.4 µs/dispatch) vs hf2q 75 t/s at 883 (15.1 µs/dispatch). Peer's per-dispatch wall is 2× faster — either smaller kernels, fewer barriers per dispatch, or different threadgroup geometry. The lever class is now Apple-Metal-internal kernel/threadgroup design, not graph structure.** | inferred from iter-9 ADR-029 + iter-15 | **not yet tested** | **STANDING — next direction (iter-16+ requires xctrace or per-kernel Metal Shader Profiler)** |
...
| 15 | this commit | **H17 FALSIFIED: HF2Q_B9_FORCE_SEQUENTIAL=1 → FFN_BODY +1.8% slower, not faster. Force-serializing concurrent dispatches HURTS. H18 CONFIRMED: even leaving ONLY 1 dispatch in FFN_BODY (skip dense+moe+routing) ⇒ FFN_BODY = 270 µs (vs 264 baseline, within noise). FFN_BODY is anchored by per-CB driver overhead, NOT kernel work. New direction H19: peer's 1389 vs hf2q's 883 dispatches/tok = peer's per-dispatch wall is 2× faster — Apple-Metal-internal kernel/threadgroup geometry difference; needs xctrace System Trace to drill in.** |
