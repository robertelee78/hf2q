...
| 27 | (this commit) | **Peer per-pipeline histogram at 4K prefill** (HF2Q_PEER_COUNT_PRINT=1 + HF2Q_PEER_PIPELINE_HIST=1 in `/opt/llama.cpp/ggml/src/ggml-metal/ggml-metal-device.m`):<br>&nbsp;&nbsp;**960 × kernel_mul_mm_f16_f32**  ← dominant dense MM, F16-weight × F32-input<br>&nbsp;&nbsp;928 × kernel_mul_mm_id_map0_ne20_8 (MoE pre-map, top_k=8)<br>&nbsp;&nbsp;464 × kernel_mul_mm_q8_0_f32 (Q8_0 dense MM — partial dense paths)<br>&nbsp;&nbsp;464 × kernel_mul_mm_id_q6_K_f32 (MoE gate_up_id, Q6_K weight)<br>&nbsp;&nbsp;160 × kernel_mul_mm_id_q5_1_f32 (MoE down_id Q5_1)<br>&nbsp;&nbsp;160 × kernel_mul_mm_id_iq4_nl_f32 (MoE down_id IQ4_NL)<br>&nbsp;&nbsp;144 × kernel_mul_mm_id_q8_0_f32 (MoE down_id Q8_0)<br>&nbsp;&nbsp;0 × kernel_mul_mm_q5_K_f32 (NEVER FIRED!)<br>**Surprise**: gemma4 attn weights are Q5_K on disk, but peer dispatches the F16 dense MM kernel — peer **pre-dequantizes Q5_K → F16 once at model load**, then uses fast F16-input matmul thereafter. hf2q runs the per-call Q5_K dequant inside `kernel_mul_mm_q5_K_tensor_f32`. The 480 attn-side matmul calls × dequant overhead is a candidate lever (H29). Concrete next test: profile hf2q's Q5_K mm vs an F16-pre-dequant equivalent.<br>**iter-27 standing**: V2 closed the tile-size lever (+7%); **F16-pre-dequant** is the new H29 candidate for closing more of the prefill gap. Need: load-time Q5_K → F16 conversion (~1 GB extra resident for gemma4 attn) + runtime use of kernel_mul_mm_f16_f32 (or kernel_mul_mm_bf16_f32) for attn dense MMs. Multi-day scope. |
