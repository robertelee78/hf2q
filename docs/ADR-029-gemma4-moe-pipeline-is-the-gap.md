...
| 30 | mlx-native `779aad4` + hf2q `445c0364` | **H29-SPEED: V2-tile F16 mm-kernel LANDED**. Added `hf2q_mul_mm_tensor_v2_f16` — identical V2 64×128 tile + direct-device-B-read pattern, but with `half4x4` load from F16 shadow instead of per-call dequant. Rust dispatcher `dispatch_mm_v2_f16` in `quantized_matmul_ggml.rs`. hf2q's `dispatch_qmatmul` F16 fast-path now routes through this instead of `dense_matmul_f16_f32_tensor` (which was V1-tile).<br>**Multi-trial bench, 4K prefill, gemma4-APEX-Q5_K_M** (3-trial median):<br>&nbsp;&nbsp;V2 default:  2744 t/s<br>&nbsp;&nbsp;V2 + H29:    **2903 t/s = +5.8%**<br>Peer @ 4K: 4452 t/s. Ratio progression:<br>&nbsp;&nbsp;V1 baseline:    0.55× peer<br>&nbsp;&nbsp;V2 (default-on): 0.62× peer<br>&nbsp;&nbsp;**V2+H29 (opt-in):  0.65× peer**<br>Byte-identical haiku at temp=0. Per `feedback_no_premature_mission_close` — improving, not closing. |
