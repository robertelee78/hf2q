...
| 21 (in progress) | this commit | H28 prefill gap **localization started**. `HF2Q_PROFILE_BUCKETS=1` at 4K-prefill shows: MoE 613 ms (35%), dense qmatmul 467 ms (27%), FA 339 ms (19%), other 145 ms (8%). Sum of GPU buckets = 782 ms (45%). **Residual = 966 ms (55%) — CPU+CB-sync overhead**. Peer hits ~1000 ms total at same shape (4341 t/s = 970 ms). If peer's GPU compute ≈ ours (~780 ms), peer's residual is ~200 ms — we have **~750 ms extra overhead per prefill**. Lever class: reduce CB commit count + per-dispatch CPU overhead in `forward_prefill_batched` (multiple `s.finish()` / `exec.begin()` pairs per layer). **NOT YET FIXED** — multi-day refactor scope. iter-21 continues. |
