...
| 29 | mlx-native `a459ee0` | **H29 COHERENCE RESTORED via linear-K Q6_K dequant**. The iter-28 dq_q6_K was a verbatim port of the matmul-internal dequant which scatters K-positions; for whole-tensor dequant we need linear-K order. Rewrote dq_q6_K to mirror the CPU reference at gguf/mod.rs:667-720. Bug found mid-debug: `(group & 1) == 0` vs correct `group < 2` for low/high nibble selection.<br>**Coherence verified**: BYTE-IDENTICAL haiku at temp=0 to V1/V2.<br>**Honest speed measurement** (3-trial median, 4K prefill):<br>&nbsp;&nbsp;V2 default:  2755 t/s<br>&nbsp;&nbsp;V2 + H29:    2782 t/s = **+1.0%**<br>**The +33% earlier was illusory** — broken math made the F16 matmul produce garbage quickly. Real H29 lever requires porting `dense_matmul_f16_f32_tensor` from V1 (32×64) to V2 (64×128) tile too. Current commit lands H29 CORRECTNESS; speed half awaits V2-tile F16 mm-kernel (next iter).<br>Per `feedback_no_premature_mission_close` — H29 lever direction proven but full gain awaits the V2-tile F16 port. |
