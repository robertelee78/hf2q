...
| 24 | (this commit) | **V2 per-bucket breakdown at 4K prefill (HF2Q_PROFILE_BUCKETS=1)** localizes where V2 helps vs not:<br>&nbsp;&nbsp;**QKV_MM** (85 calls): V1 204.6 ms → V2 138.7 ms = **−32%**<br>&nbsp;&nbsp;**O_MM** (30 calls): V1 109.7 ms → V2 110.7 ms = **unchanged** (perm021 kernel, separate from dispatch_mm)<br>&nbsp;&nbsp;**MLP_GUR_MM** (90 calls): V1 106.9 → V2 75.9 ms = **−29%**<br>&nbsp;&nbsp;**MLP_DN_MM** (30 calls): V1 45.4 → V2 35.9 ms = **−21%**<br>**Real composition of V2 prefill at 4K (1639 ms wall)**: MoE 612 ms (37%), dense qmatmul incl perm021 361 ms (22%), FA 339 ms (21%), other (norms/fusion) ~330 ms (20%).<br>**Standing levers**:<br>&nbsp;&nbsp;• **O_MM perm021 kernel** (110 ms): same 32×64 tile as V1 — port to 64×128 → expected +30% on this bucket (~33 ms wall = +2% prefill). Concrete next step (iter-25).<br>&nbsp;&nbsp;• **FA prefill** (339 ms = 21%): peer uses block-FA tile kernel at prefill (not flash_attn_vec). Different code path; investigate iter-26+.<br>&nbsp;&nbsp;• **MoE _id mm kernel** (612 ms = 37%): peer uses 32×64 tile too (confirmed iter-24 from `ops.cpp:2398`); not a tile-size lever. Per-expert dispatch overhead at gz=128 in BOTH engines; may need a different angle (e.g., separated active-expert dispatch).<br>Mission still NOT closed. |
