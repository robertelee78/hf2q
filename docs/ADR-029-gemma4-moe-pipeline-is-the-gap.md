...
| 28 | mlx-native `1ae8428` + hf2q `e58d35ea` | **H29 SCAFFOLDING + WIRING LANDED (coherence broken, math bug)**.<br>Components:<br>&nbsp;&nbsp;✓ `src/shaders/dequant_to_f16.metal` — whole-tensor dequant kernel, 7 type instantiations (Q4_0/Q8_0/Q5_1/IQ4_NL/Q4_K/Q5_K/Q6_K)<br>&nbsp;&nbsp;✓ `src/ops/dequant_to_f16.rs` — `dispatch_dequant_to_f16` + `materialize_f16_shadow`<br>&nbsp;&nbsp;✓ `MlxQWeight.f16_shadow: Option<MlxBuffer>` field<br>&nbsp;&nbsp;✓ Load-time pass in `MlxModelWeights::load_from_gguf` — gated on `HF2Q_F16_SHADOW=1` (default OFF), 0.43s to materialize 30 layers × 7 weights<br>&nbsp;&nbsp;✓ Fast-path routing in `dispatch_qmatmul` — m > 8 + shadow present → `dense_matmul_f16_f32_tensor`<br>**Speed proof**: V2 baseline 2667 t/s → H29 **3565 t/s = +33.7% prefill** at 4K (would be 0.80× peer if math worked — was 0.62× with V2 alone).<br>**BUT coherence is broken**: NaN logits, first-token=0 vs V2 baseline=138. Math bug somewhere in dequant kernel order OR F16 matmul weight layout. iter-29 task: write CPU-reference Q6_K dequant + element-wise comparison to localize. |
