...
| 25 | (this commit) | **iter-25 progress**: <br>(a) **V2 decode-regime safety verified**: dispatch_mm only fires at m > 8 (MM_ROUTING_THRESHOLD), so decode m=1 is unaffected. Multi-regime gate: V2 decode 98 t/s = identical to V1 decode 98 t/s. Clean prefill-only improvement with zero decode regression.<br>(b) **HF2Q_NO_FA=1 tested**: 4K prefill 2465 t/s vs V2+FA 2719 t/s = NO_FA REGRESSES −9%. Comment line 1027-1028 about peer's `-fa 0` win does NOT transfer to hf2q. FA path is our better path.<br>(c) **Peer's mul_mm_id source read** (ggml-metal.metal:9802): peer's tensor-branch mul_mm_id uses `matmul2d_descriptor(NR1=32, NR0=64, NK=32, ...)` — **same 32×64 tile as hf2q V1 MoE _id**. CONFIRMS MoE _id is geometry-matched. The 612 ms MoE bucket is not a tile-size lever; per-expert dispatch overhead at gz=128 in BOTH engines.<br>**Concrete next steps**:<br>&nbsp;&nbsp;1. **O_MM perm021 port to 64×128** (iter-26): 110 ms bucket, expected ~+2% prefill. Bounded multi-hour work (perm021 has gemma4-specific bf16 + permute logic; B-stage thread mapping needs rewrite for larger tile).<br>&nbsp;&nbsp;2. **FA prefill investigation** (iter-27+): peer's block-FA tile kernel vs hf2q `flash_attn_prefill_blk`. Separate code path. Multi-day.<br>&nbsp;&nbsp;3. **Consider V2 default-flip** (operator-decided): V2 is a clean +6-7% prefill win with no decode regression and byte-identical output. Per `feedback_no_premature_mission_close` not framing as "mission complete" — just an improvement shipped to users. |
