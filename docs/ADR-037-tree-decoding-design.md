...
- **Status**: 🚧 **IN PROGRESS** — Phases E1 + E3 + E4 + E5 + E6 (tree-mask v2 drafter + Qwen35 verifier dispatcher) SHIPPED at hf2q `02f13d99`. **193/193 eagle3+qwen35 tests PASS single-threaded**, including: max_depth=4 cross-branch drafter regression, byte-identity (0 ULP) Qwen35-verifier vs eagle3-drafter on chain mask. Phase E6 Qwen35 wrapper `dispatch_qwen35_tree_verify_attention` shipped via /cfa dual-mode session (Claude vs Codex parallel impls, queen-judged with Codex winning 18-pt margin on security/invariants — codex's impl guards `num_q_heads/num_kv_heads == 0` before GQA modulo, Claude's would runtime-panic). Remaining: HF2Q_SPEC_EAGLE3 env flag wiring, E2 drafter training (multi-week compute), E7 empirical validation, E8 final closure.
