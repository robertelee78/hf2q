...
| Qwen3.8 long decode | The same packed binary and model run a fixed OFF/AUTO/AUTO/OFF 512-token greedy comparison at 100K–120K prompt tokens. Output bytes must match, AUTO must improve mean decode throughput by at least 15%, each arm's two trials must remain within 5%, and AUTO must reduce mean curl wall time. Every trial binds curl time and shell phase time within two seconds of the response's total timer, requires exactly one unary SlotAware decode-complete event from the same decode clock, and runs inside the release gate's continuous fair-or-better thermal envelope. A benchmark-only summary is diagnostic evidence, not release authority. |
