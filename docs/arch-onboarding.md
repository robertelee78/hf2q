...
- Sibling matrices retain independent codec, byte geometry, stride, and
  capability metadata even when their shapes or activation operands match.
  Sharing gate/up metadata because both projections consume the same row is an
  implicit transform bug; fusion is allowed only after exact stored-format
