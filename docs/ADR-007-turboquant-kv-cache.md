...
| 10 | 2026-05-05 | F-4.4 needle: 12/12 PASS at 4K-32K | **SUBSTANTIALLY COMPLETE** | hf2q (commit pending). 16K + 32K needle trials harvested. **All 12 cells PASS** across {4K, 8K, 16K, 32K} × {pos 0.1, 0.5, 0.9}. The model correctly retrieves planted needles at every measured (length, position) cell, including positions outside the 1024-token sliding window — proving global attention layers carry long-range dependencies through the TQ-quantized KV cache. Combined with F-4.2/3 sub-linear memory scaling and predictable decode degradation, **F-4 is substantively complete** at production-relevant scales. 64K needle trials kicked off (iter-11 harvest); 128K + 262K extension is incremental and discretionary (~10-30 hr wall). Detail at `docs/adr007-pathC/F-4/needle_haystack_complete.md`. |
