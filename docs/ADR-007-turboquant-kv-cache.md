...
| 7 | 2026-05-05 | F-4.2/3 8K+32K bench + F-4.4 needle harness | PARTIAL | hf2q `405dedf` (bench) + iter-7 commit (needle harness). Iter-6 ran `bench-long-context.sh` at 8192/32768 targets. Decode t/s: 60.4 (8K) → 23.4 (32K). Peak RSS: 19.82 GB (8K) → 20.36 GB (32K) — KV cache delta ≈ 0.54 GB across 4× context. Iter-7 added `bench-needle-haystack.sh` for F-4.4 retrieval test. **4/4 PASS** so far at 4K (pos 0.1/0.5/0.9) + 8K (pos 0.1). Harness verdict logic patched to strip hf2q `[HF2Q_TQ_CODEBOOK_BITS]` log line that interleaves with stdout via `2>&1`. 64K bench + remaining 8K needle trials still running into iter-8. Detail at `docs/adr007-pathC/F-4/needle_haystack_iter-7.md`. |
