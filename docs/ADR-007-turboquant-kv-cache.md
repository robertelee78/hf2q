    ...
    - **C-4 strategic pivot (current gating step, 2026-04-22 late).** C-3 invalidates the C-2 prescription (`dense_kvs` was NOT going to be deleted after a spec-bug fix; there is no spec bug). The new strategic question: how to hit sourdough byte-parity given a physical 4-bit Lloyd-Max floor that amplifies to 0.32-0.55 SDPA nrmse under hf2q's Gemma contract. Four ordered prerequisites:
      ...
      3. **Revisit mixed-precision option C** from brain memory `8767af4a`: TQ only for sliding layers (errors bounded by the ~1024 window), dense F32 for the 5 global layers (where errors compound across full context). `dense_kvs` stays as a principled code path, not a fallback-to-remove.
         - **Progress log (autonomous /loop iter 2):**
           ...
           - **/loop iter 24 — `cfa-20260424-C4t3i24-native-higher-bit-sdpa-under-semantic-gate` (2026-04-24 late). REJECT. Four bit-widths measured; only 4-bit produces coherent results. Higher-bit native kernels have BUGS, not physics. Branches archived at `cfa/archive/cfa-20260424-C4t3i24-native-higher-bit-sdpa-under-semantic-gate/claude-archive`.**
             - **Bit-width matrix (Gate B under fixed-token replay, correct methodology):**
               | Bit | Gate A mean | Gate A p1 | Gate B rate | Gate C delta | Assessment |
               |---|---|---|---|---|---|
               | 4 | 0.9912 | 0.9702 | 5.3% | 1.55% | reproduces iter-23 exactly |
               | 5 | **0.7145** | **0.013** | 59.5% | 19.5% | **catastrophic regression** |
               | 6 | **0.7151** | **0.014** | 58.8% | 23.3% | nearly identical to 5-bit |
               | 8 | **-0.012** | **-0.076** | 99.3% | **611%** | **pipeline completely broken** |
             - **Critical diagnostic: higher-bit failures are BUGS, not physics.**
               - 4-bit baseline reproduces iter-23 exactly (cosine 0.9912, argmax 5.3%, PPL delta 1.55% under fixed-token-replay) — 4-bit path validated.
               - 5-bit and 6-bit Gate A values near-identical (mean 0.7145/0.7151, p1 0.013/0.014) — shared bug in HB native SDPA dispatch path.
               - 8-bit produces negative cosine (−0.012) and 611% PPL delta — pipeline fundamentally broken (brand new 256-centroid Lloyd-Max codebook + untested byte-packed path).
               - Iter-21 Track B measured 5-bit=69 bytes and 6-bit=93 bytes via Leg F DEQUANT path (coherent output); codebooks THEMSELVES work. The new `flash_attn_vec_tq_hb` native SDPA kernel is what's broken.
             - **Hypothesis: bit-packing/indexing bug in native HB SDPA kernel** (likely centroid table stride error, bit-extraction offset, or byte-pack/unpack asymmetry). Not a physics limit — this is the same class of bug as iter-20 Leg F which had reconstruction errors fixed in iter-21.
             - **Infrastructure preserved on branch:** native flash_attn_vec_tq_hb.metal + .rs dispatch, 8-bit Lloyd-Max codebook, HF2Q_TQ_CODEBOOK_BITS end-to-end wiring, iter24_audit.rs with bit-width matrix Gate A/B/C measurement and fixed-token-replay methodology.
             - **Primary iter-24 win:** Gate B methodology corrected (fixed-token replay, not pure-greedy cascade). 4-bit rate = 5.3% matches iter-19 exactly — confirming iter-23's 94.89% pure-greedy rate was indeed cascade artifact. Gate B is now a reliable quality metric across iters.
           - **/loop iter 25 target — `debug_native_hb_sdpa_kernel_iter_25` (scope stored in `patterns/iter-25-debug-native-hb-sdpa-kernel`):** Surgical debug of the native HB SDPA kernel bug. **Approach:** (a) Single-step unit test on known Gaussian input — TQ-encode at N-bit, TQ-decode via `flash_attn_vec_tq_hb` (native), compare output to dense `flash_attn_vec` on same input. Isolates the bug to encode or decode side. (b) Read `flash_attn_vec_tq_hb.metal` source; verify centroid table indexing, byte-pack bit-extraction, stride arithmetic against the 4-bit working reference (`flash_attn_vec_tq.metal`). (c) Inspect CODEBOOK_HB_5BIT/_6BIT/_8BIT constant definitions — confirm byte-count, memory alignment, value range. (d) Run at 5-bit first (smallest delta from working 4-bit); if 5-bit fixed, apply same pattern to 6 and 8. (e) After native kernel fixed: re-measure bit-width matrix under semantic gate. If 8-bit native cosine ≥ 0.999 → SHIPPABLE at 2× memory savings → ADR-007 CLOSES. If even 8-bit < 0.999 → we've eliminated the last plausible code-level axis; the remaining cosine gap (0.999 - 0.9995-extrapolated = 0.0005) would be hardware/numerical floor, and iter 26 would investigate accumulator dtypes / softmax precision. Budget 120-180 min (surgical kernel debug + 4 re-measurements).
