    ...
    - **C-4 strategic pivot (current gating step, 2026-04-22 late).** C-3 invalidates the C-2 prescription (`dense_kvs` was NOT going to be deleted after a spec-bug fix; there is no spec bug). The new strategic question: how to hit sourdough byte-parity given a physical 4-bit Lloyd-Max floor that amplifies to 0.32-0.55 SDPA nrmse under hf2q's Gemma contract. Four ordered prerequisites:
      ...
      3. **Revisit mixed-precision option C** from brain memory `8767af4a`: TQ only for sliding layers (errors bounded by the ~1024 window), dense F32 for the 5 global layers (where errors compound across full context). `dense_kvs` stays as a principled code path, not a fallback-to-remove.
         - **Progress log (autonomous /loop iter 2):**
           ...
           - **Meta-signal — 16 iters in, physics localization is converging but the closing fix remains elusive:** D1 SRHT (iter 13-14) VALIDATED. Per-block norm (iter 15-16) NEUTRAL (correctly plumbed but not load-bearing). Sqrt(HEAD_DIM) (iter 15-16) FALSIFIED (secretly compensated by decoder). 9 axes now eliminated: byte-exact F16, Rademacher-dominant, rotation-class, norm-extract-dominant, codebook-scale-swap, QJL residual, per-layer codebooks, calibration-fit, per-block-norm-structure. Remaining candidates: (i) per-layer-type TQ participation (iter 17 mixed-precision ablation); (ii) V-side sign convention; (iii) softmax precision / accumulator dtypes; (iv) something not yet identified. Coherent English fluency at byte 127 was NOT iter-15-specific; iter 14 and iter 16 both produce the same proximity signal.
