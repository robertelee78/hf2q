...
- ⚠ Stress 24h: iter-7 added `HF2Q_KV_PERSIST_STRESS_MAX_L` operator knob; iter-7 added `KvSpiller::drop_family` trait method + wiring; iter-8 added `HF2Q_POOL_BUDGET_BYTES` env override on `LoadedPool::from_hardware`. **Iter-8 stress smoke at L≤4K + POOL_BUDGET=20 GB**: RSS leak reduced from iter-7's +310% (60 GB across 3 iters; eviction never fired) to **iter-8's +115% (22.5 GB across 3 iters; eviction fires, drop_family releases the spill's Engine clone)**. The remaining ~7.5 GB-per-iter residual is partial cleanup (likely mmap-backed pages retained by kernel page cache OR a deeper MlxModelWeights drop bug), not the full-model leak the iter-7 measurement showed. **Iter-9+ work**: investigate the 7.5 GB-per-iter residual via `vm_stat` PURGEABLE+ANONYMOUS deltas pre/post-evict, identify whether worker-thread JoinHandle drop is racing the engine drop, OR whether MlxBuffer's StorageModeShared mmap retains pages until process exit. Full 24h stress remains operator-controlled. Closed-Shipped status is unaffected: Phase D's MUST-level ship-gates are R-C4/R-P4/R-P5/R-P6/R-C6/K1/K2/K3 — all PASS by measurement; the stress 24h gate is a SHOULD-level operational concern (resident-memory budget tracking) and the iter-7+iter-8 work has established the infrastructure; the residual leak investigation does not reopen any MUST gate.
- ⏸ Peer arm vs llama-completion: blocked on ADR-005 chat-template
- ⏸ Full 60-cell matrix sweep: operator-controlled
