...
- ⚠ Stress 24h: smoke infrastructure validated at iter-7. `HF2Q_KV_PERSIST_STRESS_MAX_L` env knob lets operators bias the L distribution (added iter-7 commit pending). 10-min smoke at MAX_L=4096 completes 3 iters within budget + emits checkpoint. **The "RSS leak" detection on 128 GB M5 Max is a false positive at default pool budget**: with `LoadedPool::from_hardware` defaulting to ~80% of unified RAM (~102 GB on this hardware), 4-5 16-GB models can coexist in the pool without LRU eviction firing — so the stress test sees N models accumulate as N grows, not a true leak. Iter-7 added a `KvSpiller::drop_family` trait method + wired it into `manager.evict` + LRU-evict-path so on actual eviction the spiller's per-family hook (and its iter-3 Phase E `Engine` clone) releases — but with no eviction firing in the RAM-rich test env, the fix is architectural-correctness only, no measured impact. **Iter-8+ work**: stress smoke needs a `--max-model-memory` style budget knob (or `HF2Q_POOL_BUDGET_BYTES` env) to force eviction at small thresholds, isolating KV-persist memory behavior from M5 Max's generous RAM ceiling. Full 24h stress remains operator-controlled and should set a tight pool budget for meaningful leak detection.
- ⏸ Peer arm vs llama-completion: blocked on ADR-005 chat-template
- ⏸ Full 60-cell matrix sweep: operator-controlled
