...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume research dossier. iter-1 at `9beb906`, iter-2 at `d17163b`, iter-2.5 at `70fca89`. **iter-3 LANDED across `a20606a` + `1006ab0` + `22e456a`** (Codex Phase-2b audit APPROVED-WITH-CAVEATS over 2 rounds). **iter-3.5 LANDED at `7ffb563`** — DenseKvBuffers dtype invariant (closes Codex round-1 LOW #4) + end-of-prefill snapshot (`MlxModelWeights::dense_kvs_snapshot_for_lcp` populated BEFORE decode mutates; LcpRegistry stores the snapshot, NOT the live buffer). The snapshot path lifts the iter-3 v1 sliding-ring wrap restriction — long-conversation prompts (`prompt_len + decode > sliding_window`) are now cacheable. ~50ms / ~5 GB GPU memcpy per resume-eligible request on Gemma 4 26B is the cost. R-C4 sourdough byte-identical + iter-3 K<N falsifier PASS post-snapshot. iters 4 (env-flip default-ON) / 5 (R-C4-LCP 5-fraction sweep) / 6 (R-P7 multi-turn-chat + /cfa fan-out) / 7 (sliding-window edge cases) PENDING. iter-4 default-ON gated on iter-5 + iter-6 passing.
