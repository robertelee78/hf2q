...
- ⚠ Stress 24h: 30-min reduced-duration smoke ran but didn't reach a checkpoint within the 30-min budget. The W2 stress test loop picks L randomly from `[512, 4096, 16384, 32768]`; a single L=32768 iter on Gemma 4 26B takes ~10 min for prefill alone (per B-F4 standing finding) + eviction overhead, so a single iter routinely exceeds the 5-min checkpoint cadence and blocks the entire 30-min budget. **Design issue documented; infrastructure validated**. Iter-7 follow-up: bias L distribution to shorter values (e.g. `[512, 1024, 4096]`) for the smoke variant, OR add `HF2Q_KV_PERSIST_STRESS_MAX_L=4096` env override. Full 24h stress remains operator-controlled with bias parameters tuned to the deployment's prefill-speed envelope.
- ⏸ Peer arm vs llama-completion: blocked on ADR-005 chat-template
- ⏸ Full 60-cell matrix sweep: operator-controlled
