...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume research dossier. ~1,060 LOC scope (Codex-revised, §10.7), 7 iters + iter-2.5 ownership refactor. iter-1 LcpRegistry standalone landed at `9beb906` (358 LOC src + 369 LOC tests; 13 unit tests). **iter-2 (engine probe + metrics, partial-prefill OFF) LANDED** — adds `probe_lcp_opportunity` helper, `KvCacheMetricsSink::record_lcp_probe` trait method, `KvSpillCounters::{lcp_lookups_total, lcp_detected_total}` (emitted as `hf2q_kv_lcp_lookups_total` / `hf2q_kv_lcp_detected_total` on `/metrics`), and probe + store at both non-streaming and streaming generate sites in `engine.rs::generate_once_with_soft_tokens` / `generate_stream_once`. Marker payload `()` (real KV pinning waits for iter-2.5 ownership refactor). 6 new unit tests (3 for `probe_lcp_opportunity`, 3 for counters). 2804/0/13 bin tests PASS. iters 2.5 / 3 / 4+ pending. iter-3 (env-gated `forward_prefill.rs` reset modification) requires Codex Phase-2b audit per `feedback_codex_review_catches_unified_memory_races`.
