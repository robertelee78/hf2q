...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume. iter-1 `9beb906`, iter-2 `d17163b`, iter-2.5 `70fca89`, iter-3 `a20606a`/`1006ab0`/`22e456a` (Codex APPROVED-WITH-CAVEATS), iter-3.5 `7ffb563` (dtype + decode-wrap snapshot), iter-3.5c + iter-5 `31d04b3` (prefill-wrap guard + KILL-criterion 5-K sweep PASS). **iter-3.5d + iter-6 LANDED at `04e6955`** — snapshot capacity headroom (global-layer buffers sized to sliding_window; multi-turn prompts grow within sw fit) + R-P7 multi-turn speedup bench (turn 2+ at 0.40-0.42× turn 0) + /cfa fan-out bench (workers 1-3 at 0.23× cold; aggregate 1.71× single-worker). Conservative pass thresholds all hold; dossier 0.10×/1.25× targets miss on small prompts where request overhead dominates. iters 4 (env-flip default-ON) / 7 (sliding-window edge cases) PENDING.
