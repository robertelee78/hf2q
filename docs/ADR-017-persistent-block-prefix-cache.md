...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume research dossier. iter-1 `9beb906`, iter-2 `d17163b`, iter-2.5 `70fca89`. **iter-3 LANDED across `a20606a` + `1006ab0` + `22e456a`** (Codex Phase-2b audit APPROVED-WITH-CAVEATS over 2 rounds). **iter-3.5 LANDED at `7ffb563`** — dtype invariant + end-of-prefill snapshot (lifts decode-wrap restriction). **iter-3.5c (sliding-layer prefill-wrap guard) + iter-5 (R-C4-LCP 5-K sweep KILL-criterion) LANDED at `31d04b3`** — 5/5 fractions byte-identical on Gemma 4 26B-DWQ; iter-5 sweep surfaced+fixed a real `.cloned()` Arc strong_count bug in `forward_prefill_with_soft_tokens_resume` that the single-resume iter-3 falsifier could not catch. iters 4 (env-flip default-ON) / 6 (R-P7 multi-turn-chat + /cfa fan-out bench) / 7 (sliding-window edge cases) PENDING. iter-4 default-ON gated on iter-6. iter-3.5c restriction (prompt_len ≤ sliding_window for sliding-layer models) lifts at iter-3.6 via mid-prefill snapshot.
