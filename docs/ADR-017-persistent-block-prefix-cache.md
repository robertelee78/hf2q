...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume research dossier. ~1,060 LOC scope (Codex-revised, §10.7), 7 iters + iter-2.5 ownership refactor. iter-1 LcpRegistry standalone landed at `9beb906` (358 LOC src + 369 LOC tests; 13 unit tests). **iter-2 (engine probe + metrics, partial-prefill OFF) LANDED at `d17163b`** (165 src + 85 tests; `probe_lcp_opportunity` helper + `KvCacheMetricsSink::record_lcp_probe` + `KvSpillCounters::{lcp_lookups_total, lcp_detected_total}` + `/metrics` lines; probe + store at both non-streaming + streaming generate sites; 6 new unit tests). **iter-2.5 (Strategy A ownership refactor `Option<Vec<DenseKvBuffers>>` → `Option<Vec<Arc<DenseKvBuffers>>>`) LANDED** — type change at `forward_mlx.rs:593`, read-path consumers UNCHANGED (auto-deref through `Arc::Deref`), three write sites updated (`forward_prefill.rs:1502` + `forward_prefill_batched.rs:2131` + `engine.rs::kv_restore_gemma` with `Arc::get_mut`). 2804/0/13 bin tests PASS. **R-C4 sourdough byte-identical PASS** on Gemma 4 26B-DWQ: baseline 3632 bytes == restored 3632 bytes; TTFT 287.3 ms cold → 0.5 ms restored (574× cache-hit speedup). iters 3 / 4+ pending. iter-3 (env-gated `forward_prefill.rs` reset modification) requires Codex Phase-2b audit per `feedback_codex_review_catches_unified_memory_races`.
