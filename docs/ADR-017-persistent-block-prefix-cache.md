...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume research dossier. ~1,060 LOC scope (Codex-revised, §10.7), 7 iters + iter-2.5 ownership refactor. iter-1 at `9beb906`, iter-2 at `d17163b`, iter-2.5 at `70fca89`. **iter-3 (LCP partial-prefill resume — env-gated `HF2Q_KV_LCP_RESUME=1` + `HF2Q_USE_DENSE=1`, default OFF) LANDED across `a20606a` + `1006ab0` + `22e456a`** — 705 LOC across `forward_prefill.rs`, `engine.rs`, `lcp_registry.rs`, `investigation_env.rs`, `tests/lcp_partial_prefill_byte_identity.rs`. **Codex Phase-2b audit verdict APPROVED-WITH-CAVEATS** across two rounds (round 1: 1 HIGH R12 sliding-ring wrap + 1 MED per-layer cap + 2 LOW; round 2: 1 HIGH wrap-counter + 1 LOW streaming-warn-parity; all resolved except dtype invariant deferred to v2). **R-C4 sourdough byte-identical PASS** + **iter-3 K<N partial-prefix byte-identity falsifier PASS** (Server A resume 144 bytes == Server B fresh-prefill 144 bytes BYTE-IDENTICAL on Gemma 4 26B-DWQ; engagement confirmed via `hf2q_kv_lcp_detected_total`). v1 sliding-ring wrap restriction: stores only when `prompt_len + physical_decode_writes <= sliding_window`. iters 4 (env-flip default-ON) / 5 (R-C4-LCP 5-fraction sweep) / 6 (R-P7 multi-turn-chat + /cfa fan-out) / 7 (sliding-window edge cases) PENDING. iter-4 default-ON gated on iter-5 + iter-6 passing.
