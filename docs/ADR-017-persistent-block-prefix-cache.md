...
- [`docs/research/adr017-phase-e-option-a-2026-05-05.md`](research/adr017-phase-e-option-a-2026-05-05.md) — Phase E option (a) LCP partial-prefill resume research dossier. ~1,060 LOC scope (Codex-revised, §10.7), 7 iters + iter-2.5 ownership refactor. iter-1 LcpRegistry standalone at `9beb906`. **iter-2 (engine probe + metrics, partial-prefill OFF) LANDED at `d17163b`**. **iter-2.5 (Strategy A ownership refactor) LANDED at `70fca89`** (R-C4 sourdough byte-identical PASS). **iter-3 (LCP partial-prefill resume — env-gated `HF2Q_KV_LCP_RESUME=1` default-OFF) IMPLEMENTATION LANDED at `a20606a`** — 623 LOC across 5 files; new `forward_prefill_with_soft_tokens_resume` entry point + `LcpRegistry::take_prefix` consuming variant + engine probe→take_prefix→install→resume wire-up gated on `HF2Q_KV_LCP_RESUME=1` + `HF2Q_USE_DENSE=1`. R-C4 sourdough byte-identical PASS at default flags (resume path defaults OFF; behavior byte-identical to pre-iter-3). **PENDING for iter-3 closure:** K<N partial-prefix byte-identity falsifier test (dossier §10.4 corrected) + Codex Phase-2b audit (dossier §4.3 mandate) before env-flip to default-ON. iters 4+ pending.
