...
- **Status:** **🎯 LANDED 2026-05-08, 3.94× MEMORY SAVINGS LIVE 2026-05-09, FULL VALIDATION + DOCS CLOSURE 2026-05-09 (iter-54)** — Phase A iter-1a..6b.3 + Phase B iter-7..19 + iter-23 sub-iter chain production (iter-29..36) + closure / validation / DRY consolidation iters (37..54) all LANDED. Live coherence on qwen36 35B-A3B-APEX-Q5_K_M PASS at byte-identical F32/TQ output (~129 tok/s). Cross-axis {F32, TQ-on} × {persist=unset, persist=/tmp/cache} 4-cell sweep BYTE-IDENTICAL. Long-context byte-identity proven 4K→8K→16K→**32K (45876-tok prefill)** = 4 orders of magnitude. **F32 K/V allocation DROPPED in TQ-only mode 2026-05-09 (iter-34); regression-pin proves f32_k_v_bytes==0 + total 340 MiB at qwen36 32K = 3.937× savings vs 1.34 GB F32-only baseline.** Cross-process replay correct end-to-end via QH35 codec v3 (iter-36) + iter-44 E2E test. Single-command full validation via `scripts/adr027-full-validation.sh` (iter-46 + iter-54 NULL_TEMPLATE override flowed). Remaining post-LANDED follow-up: needle-haystack at 32K with system-prompt-bearing chat template (separate operator-gated workstream — model-intrinsic chat-template + long-context EOS interaction documented at iter-40 + iter-51).
- **Status addendum — sub-iter 23d-γ (2026-08-03): TQ-only × LCP-resume × disk-persist made PRODUCTION-CORRECT end-to-end.** Three load-bearing defects found + fixed while wiring the production serve deployment (opencode agentic-coding backend):
...
- `/opt/hf2q/docs/adr/ADR-007-turboquant-kv-cache.md:1167-1442` — Path C Progress Log + frozen codec contract.
...
| 58 | 2026-05-09 | — | observed disambiguation | **iter-56+57 finding refinement via `--test-threads=1` reproduction.** Re-ran both peer-WIP test failures in isolation with `--test-threads=1` to disambiguate real precision regressions from test-isolation races: (1) **iter-57 dispatch_profile** confirmed RACE — 3/3 PASS at `cargo test --test dispatch_profile -- --test-threads=1` (was 1 FAIL in parallel run). Fix path validated: `[[test]] harness=false` OR per-binary `--test-threads=1` annotation. (2) **iter-56 test_quantized_matmul_id_ggml** confirmed REAL — 3/3 FAIL at `cargo test --test test_quantized_matmul_id_ggml -- --test-threads=1` (still 6/3 with strict `assert_eq!` against 1e-6 ULP F32 mantissa drift). Fix path validated: needs operator decision on tolerance (NRMSE / abs_diff bound vs strict equality). Both findings now have empirical disambiguation between "race vs real" axes. iter-58 did NOT modify (per "stay in ADR-020 lane" + the operator decision needed for iter-56 tolerance choice). |
| 57 | 2026-05-09 | — | observed (peer-WIP) | **mlx-native `dispatch_profile::per_dispatch_sampling_records_nonzero_ns` parallel-execution flake.** Background `cargo test --tests` job (b5nkur4p3) surfaced a third pre-existing peer-WIP test-stability issue. Failure: `cb_dump=[]` when `MLX_PROFILE_CB=1` was set. **3/3 PASS in isolation** (`cargo test --test dispatch_profile per_dispatch_sampling_records_nonzero_ns`) — fails ONLY when parallel with siblings in the same binary. Root cause: the test uses `OnceLock` + `unsafe set_var("MLX_PROFILE_CB", "1")` to set env vars; an `AtomicI8` cache (per the in-file comment "Must complete before any `kernel_profile::is_dispatch_enabled` call to avoid the AtomicI8 cache going cold-OFF") locks in OFF if a sibling test reads the flag before the OnceLock fires. Last touched by ADR-015 commits `9b343e0` + `3ff51b6` (pre-existing, not recent). Fix needs `[[test]] harness=false` OR `#[ctor]` priming OR drop the cache — peer-territory work. iter-57 did NOT modify (per "stay in ADR-020 lane" memory). Logged for operator awareness alongside iter-56's `test_quantized_matmul_id_ggml` 1e-6 ULP debt. |
| 56 | 2026-05-09 | — | LANDED (mlx-native fix) + observed | **mlx-native bench `#[ignore]` cleanup + observed 1e-6 ULP test debt in test_quantized_matmul_id_ggml.** mlx-native commit `293128a` marks all 4 `bench_sdpa_tq` perf benches `#[ignore]` so default `cargo test` doesn't flake on Metal CommandBufferError thermal-load failures (observed ~33% rate on `bench_tq_v1_vs_v2`, 840 dispatches/run; was 3 PASS / 1 FAIL across 3 reproducibility runs). File-top comment + per-test `#[ignore = "perf bench — run via …"]` annotation; operators access via `cargo test --test bench_sdpa_tq -- --ignored --nocapture`. Default `cargo test --test bench_sdpa_tq` now 0/0/4-ignored (was 3/1). **Adjacent finding (NOT FIXED — peer-ADR-022 territory):** `test_quantized_matmul_id_ggml::{test_q4_0_id_vs_norid, test_q4_0_id_vs_norid_4tok, test_q4_0_production_shape}` fail with **161 mismatches at max_err=1e-6** (single-ULP F32 precision drift between `id` and `norid` Q4_0 kernel paths). Tests use strict `assert_eq!` rather than NRMSE/abs_diff tolerance — likely needs operator decision on whether kernel was always this precise + recent peer change loosened it OR tolerance was always too-tight. Logged here for operator visibility; iter-56 did NOT modify the test (per "stay in ADR-020 lane" memory + not modifying peer-WIP unilaterally). |
| 55 | 2026-05-09 | B | LANDED | **ADR Status header refreshed to capture iter-37..54 closure work.** Top-of-ADR Status header from iter-37 referenced "iter-29..37" as the LANDED chain — stale by iter-54. Refresh: bumps the LANDED iter range to "iter-29..36 production + 37..54 closure / validation / DRY consolidation"; explicitly enumerates the 4 orders of magnitude of context-length byte-identity validation (4K→8K→16K→**32K = 45876-tok prefill**); cites the iter-44 E2E cross-process replay test by name; cites the `adr027-full-validation.sh` single-command entry point + iter-54 NULL_TEMPLATE flow-through; the "remaining follow-up" line specifies the needle-haystack at 32K gate as needing a "system-prompt-bearing chat template" per iter-40+iter-51 findings (more accurate than the previous "operator-gated workstream" wording). Docstring-only iter; no source change. The header is the FIRST thing future readers see — an accurate summary at the top saves them from grep-walking §10's iter log to understand where the work stands. |
| 54 | 2026-05-09 | B | LANDED | **`adr027-full-validation.sh` exposes NULL_TEMPLATE env override end-to-end.** Closes the 3rd consumer of the canonical-template consolidation chain (iter-52 long-context sweep + iter-53 bench-needle-haystack docs + iter-54 entry-point script). Updates: (1) docstring's `# Env overrides` section adds `NULL_TEMPLATE=...` with default pointing at `scripts/qwen35_raw_passthrough.jinja` (canonical 1-line passthrough Jinja, ADR-005 Phase 1) + comment block explaining when to override; (2) gate-5 invocation forwards `NULL_TEMPLATE="${NULL_TEMPLATE:-}"` to the long-context sub-script so operator's choice flows end-to-end (empty string preserves default canonical path resolution in the sub-script). Operators running the entry point can now override the null template via env without touching the long-context sub-script. Smoke validation: `SKIP_BUILD=1 SKIP_TESTS=1 LENGTHS=4096 bash scripts/adr027-full-validation.sh` PASSES with implicit default canonical NULL_TEMPLATE. Full canonical-template consolidation now consistent across THREE scripts. |
| 53 | 2026-05-09 | B | LANDED | **`bench-needle-haystack.sh` CHAT_TEMPLATE_FILE docstring enhanced with canonical recommendation + iter-51 caveat.** Sister of iter-52 — extends the canonical-template consolidation to operator-facing documentation in the bench-needle-haystack harness. The script's `CHAT_TEMPLATE_FILE` env doc previously only said "set to a null/passthrough template (e.g. one whose body is just `{{ messages[0].content }}`)" — now points operators at the canonical `scripts/qwen35_raw_passthrough.jinja` artifact + lists the other repo consumers (qwen35_tokenizer_parity.sh + adr027-long-context-sweep.sh) for cross-reference. PLUS adds the iter-51 caveat: under null template, the model treats prompt as text-completion (not instruction-following), so middle-position needles may PASS by chance while early/late positions FAIL with model echoing more haystack filler. Operators reading the docstring before running the bench now have the FULL context (canonical artifact + falsified hypothesis) without having to grep memory files. No code change. |
| 52 | 2026-05-09 | B | LANDED | **`adr027-long-context-sweep.sh` consolidated to use canonical `qwen35_raw_passthrough.jinja`.** Discovered during cross-script grep: the `scripts/qwen35_raw_passthrough.jinja` 1-line null template (ADR-005 Phase 1, used by `qwen35_tokenizer_parity.sh`) was the pre-existing canonical "no chat template wrapping" artifact. iter-42 inlined a duplicate copy into the long-context sweep harness's `$WORK_DIR`. iter-52 consolidates to the canonical file path with a `NULL_TEMPLATE` env override, plus a file-existence preflight + helpful error message pointing at the canonical path. Single source of truth — eliminates the ad-hoc duplicate. Smoke validation: 4K target → F32 vs TQ-on BYTE-IDENTICAL ✓ at 5742 prefill (canonical template path used). "No broken windows" / DRY consolidation iter — observed during ADR-027 follow-up close-out. |
| 51 | 2026-05-09 | B | LANDED (hypothesis FALSIFIED) | **null-template + MAX_TOKENS=1024 doesn't lift 4K needle-haystack FAILs.** Tested follow-up hypothesis to iter-40: maybe the recency-bias FAILs at 4K pos=0.1 / pos=0.9 (under null template) are budget exhaustion. FALSIFIED — same 1/3 pass rate at MAX_TOKENS=1024 as at 512. Failure mode is qualitatively DIFFERENT under null template: model treats prompt as text-completion (continues haystack filler) rather than instruction-following — never enters the answer phase. Proper fix would be a system-prompt-bearing chat template (operator scope, not /loop). **Standing recommendation captured in `project_iter40_qwen36_chat_template_long_context_eos_2026_05_09.md`:** for ADR-027 needle-haystack validation, the existing 1024-tok / MAX_TOKENS=512 / default chat-template configuration is the most reliable (3/3 PASS at TQ-on === 3/3 PASS at F32 baseline). Higher contexts exhibit model-intrinsic failure modes regardless of template choice — NOT TQ regressions. No code change. |
