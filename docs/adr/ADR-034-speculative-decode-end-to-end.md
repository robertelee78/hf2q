...
|---|---|---|---|
| **A — Qwen 3.6 27B dense MTP** | ✅ **1.26-1.37× SHIPPED; short-context absolute throughput peer-competitive; 64K parity unproven** | Auto-default K=1 BATCHED + Metropolis-Hastings stochastic acceptance (task #87 + #91 SHIPPED). **Empirical stability-validated bench of HEAD `e044131d` (recorded at `7da12c37`) 2026-05-22** (3-rep warmup-corrected paired): base 22.0 t/s code-gen / 21.9 essay; **code-gen `--temperature 0` greedy: 30.03 t/s @ 91.0% accept = 1.37× base** (was 30.0 at sub-iter H 1f130770); **essay `--temperature 0.5` MH: 27.60 t/s @ 77.8% accept = 1.26× base**; essay greedy: 26.57 t/s @ 68.4% (1.21×); code-gen MH 0.5: 28.70 t/s @ 84.1% (1.30×). All deltas vs sub-iter H baseline within ±0.2 t/s noise. Throughput matches reported MTPLX D=2 absolute (26.35 t/s @ 80.6% accept) under non-identical conditions (short ctx vs 64K, stock vs fine-tuned drafter). Fused-MLP family SHIPPED across all 5 dense quants (Q8_0/Q4_K/IQ4_NL/Q5_K/Q6_K — see "Iteration 2026-05-21 (cont. 29)" section). Determinism PASS 3/3 byte-identical (greedy AND seeded MH temp=0.5). 51/51 lib tests pass + parity_mtp_python_ref pass. **Length-sweep validation at HEAD `2912c295` 2026-05-22** (1-rep per length, Qwen 3.6 27B Q8_0 code-gen MTP K=1 greedy temp=0): 128 tok 30.0 @ 91% (vs base 20.7 = **1.45×**), 256 tok 30.0 @ 91% (vs 20.6 = **1.46×**), 512 tok 29.5 @ 90% (vs 20.8 = **1.42×**), 1024 tok 29.1 @ 93.8% (vs 21.0 = **1.39×**). Production winner SUSTAINS 1.39-1.46× speedup across 128→1024 output lengths; accept rate IMPROVES at longer outputs (91% → 93.8%); no degradation. **MH temperature sweep validation at HEAD `436a4b01` 2026-05-22** (3-rep essay, vs base 21.9): temp=0.3 26.27 @ 69.3% (1.20×), **temp=0.5 27.53 @ 77.8% (1.26× WINNER)**, temp=0.7 24.43 @ 58.0% (1.12×), temp=0.9 23.93 @ 54.2% (1.09×). Accept rate PEAKS at temp=0.5 (sharp drop to 58% at temp=0.7); the `--temperature 0.5` production recommendation is empirically validated as the MH accept-rate optimum. **Prompt-length sweep — SYNTHETIC filler** at HEAD `4a294830` 2026-05-22 (1-rep, MTP K=1 greedy code-gen, 64 tok gen, `"Context paragraph N..."` repetitive filler): short ~13 tok → MTP 29.1 @ 88.2% = 1.32×; med ~212 tok → 31.2 @ 96.9% = 1.42×; long ~812 tok → 30.1 @ **100% accept** = 1.38×; 2K ~2012 tok → 24.0 @ 61.5% = 1.14×; 4K ~4012 tok → 12.6 @ 82.9% = 0.63× (NET-NEG). **NATURAL prompt validation** at HEAD `0abca852` (3-rep, real code-context + Fibonacci, ~400 tok): Base 20.93, MTP 28.57 @ 85.3% = **1.36×** (matches production claim). **NATURAL long-prompt regression** at HEAD `0abca852` 2026-05-22 (1-rep, real Rust source code as context): 1K natural ~1572 tok → Base 21.3, MTP **19.7 @ 33.3% = 0.92× (REGRESSES)**; 2K natural ~2652 tok → Base 20.6, MTP **19.4 @ 34.0% = 0.94× (REGRESSES)**; 4K natural ~4970 tok → Base 14.7, MTP **12.8 @ 50.0% = 0.87× (REGRESSES)**. **CRITICAL caveat (REFINED)**: production claim 1.36× base is valid for SHORT natural prompts (~13-400 tok); longer natural prompts (1K+) show **REAL REGRESSION** to 0.87-0.94× of base. **Root-cause profiling at HEAD `cda4b6cc` 2026-05-22** (HF2Q_MTP_PROFILE=1, per-round breakdown): short ~24 tok prompt → mtp=4.08 ms, ver=60.30 ms, ITER=64.40 ms, accept=82.4%; long ~2.6K natural prompt → mtp=4.13 ms, ver=64.50 ms (only +7%), ITER=68.71 ms (only +6.7%), accept=**60.0%** (-22pp). **Per-round verifier cost grows only ~7% at 2.6K context; throughput drop is dominated by ACCEPT-RATE COLLAPSE, NOT verifier KV attention cost.** Math: at 82.4% accept ~1.82 tok/iter ÷ 64.4ms = 28.3 t/s (matches 28.9 observed); at 60.0% accept ~1.6 tok/iter ÷ 68.71ms = 23.3 t/s (matches 21.5 observed). **Implication**: task #89 batched-prefill kernel unification would NOT fix the long-context regression (it doesn't address drafter accept rate). The real fix is **drafter quality** at long context — needs either (a) custom fine-tuned drafter trained on long-context data, or (b) tree decoding (multiple branches per round to recover from drafter misses). The "64K context parity bench vs MTPLX" multi-week item should compare drafter accept rates at long context, not just throughput. **Production recommendation NOW BOUNDED**: `HF2Q_SPEC_DECODE=1` for short natural prompts (~≤500 tok) where it delivers 1.36× base; for longer natural prompts use base (no spec) — empirically faster at 1K+ context. **Three quick-win hypotheses for long-context regression EMPIRICALLY FALSIFIED**:
- **(1) MH temperature tuning** at HEAD `c5c51701` (3-rep + warmup + pinned fixture): MH at temps 0.5/0.7 does NOT recover — Base 17.40, greedy 14.85 (0.85×), MH 0.5 14.07 (0.81× WORST), MH 0.7 14.43 (0.83×). Prior 2-rep result suggesting MH 0.7 marginally helps was a 40% variance artifact.
- **(2) Per-round verifier kernel cost** at HEAD `1fb53d58` (HF2Q_MTP_PROFILE=1): ver_ms grows only +7% (60.30 → 64.50 ms) at 2.6K context — NOT the dominant factor. Task #89 batched-prefill kernel unification would NOT fix this.
- **(3) Hybrid K=0 spec** at HEAD `531784f6` (3-rep): K=0 at 88.9% accept rate (HIGHER than K=1's 77.8%) STILL regresses — Base 14.43, K=0 12.63 (0.88×). The MTP draft overhead (~10ms/iter) dominates even at 89% accept.

**Conclusion**: ALL three quick-win levers FALSIFIED. The long-context regression has only multi-week fix paths remaining: (a) drafter training (smaller/faster/long-context-aware MTP head), (b) tree decoding (multiple branches per round to recover accept rate above what the current single-draft-per-round can achieve), (c) DFlash with custom drafter (Cell B alternative path). The `--temperature` tuning, K-config, and kernel-optimization quick wins are all OFF THE TABLE. **Production recommendation (workload-dependent, see Mission status table at line 47): code-gen `--temperature 0`, essay `--temperature 0.5`**. | **Multi-week work that must NOT be punted** (per user directive 2026-05-21): (1) 64K context parity bench vs MTPLX; (2) custom fine-tuned drafter (training); (3) tree decoding (EAGLE-2/Medusa). Fused-MLP item from prior plan SHIPPED. Length-sweep up to 1024 tok VALIDATED at HEAD `2912c295`. |
...
| **B — Qwen 3.6 27B DFlash** | ⚠️ **WIRED, perf 0.77× of MTP production winner post-task #95, but coherence-degraded at long lengths — research-quality only** | Task #95 (compile-drafter lever) SHIPPED in 10 sub-iters A-H at HEAD `f23b9274`. Warmup-corrected 3-rep paired bench at HEAD `f23b9274`: DFlash BS=4 code-gen 23.17 t/s (was 18.4 pre-#95, +26% cumulative), BS=2 code-gen 21.63 (+28.8%), BS=4 essay 20.97 (+24.1%), BS=2 essay 20.73 (+25.6%). **Coherence-correction at HEAD `a8a34b5d` 2026-05-22**: previously documented as "WIRED + COHERENT" with a "byte-identical to single-token Qwen35 decode" invariant claim — that claim was empirically FALSE since task #78 Step 4 wiring. At HEAD `a8a34b5d`: 27B DFlash Fibonacci prompt 32 tok already diverges from base (`def fibonacci(n):` vs base typed `def fibonacci(n: int) -> list:`); 128 tok catastrophic garbling (`a, b = a, b = a, b = b, a + b = b, a + b, a + b`) + hits EOS + loops into `<\|im_end\|><\|im_start\|>user` chat markers + duplicates lines in thinking process. Root cause: same BF16-vs-F32 precision divergence ADR documents at "Output divergence note" for MTP K=1 BATCHED — DFlash verifier uses BF16 `flash_attn_prefill_resume`, base uses F32 `flash_attn_vec`; argmax flips compound across rounds. What's actually true: DFlash's accept-walk IS consistent with its own batched verifier's argmax (proof chain in `step_round_from_argmaxes` holds), but verifier kernel ≠ single-token decode. Codex /cfa caught the contradiction with the now-corrected doc claims at qwen35_orchestrator.rs:62-90, target.rs:52-62, spec_decode_cli.rs:273. Codex /cfa cleared cumulative task #95 work (Critical 0, Major 0, Minor 2 doc-only). **Real lever per peer-code read of MTPLX** (commit `e9358fde` 2026-05-21): MTPLX's 2.24× advantage is from `mx.compile`-compiled chained drafter (~17 commit boundaries eliminated), NOT fused projections kernels (peer code confirms they don't have one either). Task #89's vec-extension Steps 1-3b SHIPPED but EMPIRICALLY did not close the perf gap on its own — encoder fusion saves only ~1-2 ms (noise band); fa.ops1_4 at seq_len=2 cur_len>0 is COMPUTE-bound. | DFlash drafter forward cost remains structural + coherence-degraded output makes DFlash research-quality only. Closure needs drafter training (smaller/faster drafter) or tree decoding (EAGLE-2/Medusa). DFlash on Qwen35 stays opt-in via HF2Q_SPEC_DFLASH=1 (default OFF). Users should use `HF2Q_SPEC_DECODE=1 --temperature 0` (code-gen 1.37×) or `0.5` (essay 1.26×) for production. |
| **B — Qwen 3.5 35B-A3B DFlash** | ⚠️ **WIRED with 35B-A3B drafter (override HF2Q_DFLASH_DRAFTER_PATH); 0.31× of base / 0.43× of MTP K=1 BATCHED; coherence-degraded with duplication artifacts** | Empirical 3-rep paired bench at HEAD `334008e9` 2026-05-22 (canonical Qwen 3.5 35B-A3B Q4_K_M MTP GGUF + z-lab/Qwen3.6-35B-A3B-DFlash drafter, 128 tok code-gen, temp=0 greedy): DFlash BS=4 **42.7 t/s** (42.6/42.8/42.8). Reference: base autoregressive 136.1 t/s; MTP K=1 BATCHED forced 98.6 t/s; MTP K=0/auto 113.9 t/s @ 93.7% accept. Ratios: **0.31× of base; 0.43× of MTP K=1 BATCHED; 0.37× of MTP K=0**. Output stays closer to base structure than 27B DFlash but still shows duplication artifacts (e.g., duplicated `def fibonacci(n):` + duplicated `return [0]` lines). Same row-N kernel divergence pattern documented in §K=N CORRECTION. Drafter selection: user overrides default 27B path via `HF2Q_DFLASH_DRAFTER_PATH=/opt/hf2q/models/dflash-drafters/z-lab__Qwen3.6-35B-A3B-DFlash`. | Research-quality only — net-negative vs both base AND MTP K=0/K=1. Closure needs drafter training (smaller/faster 35B-A3B-tailored drafter) or tree decoding. |
...
> **Planned-test reality check**: ADR-034 plan text references several harness scripts that **were never actually created** at HEAD `d6cc7278`: `scripts/spec_bench.sh`, `scripts/adr034_gate.sh`, `scripts/mtp_parity.py` (planned at flat scripts/), `scripts/dflash_dump_wrapper.py`, plus the byte-identity test scripts `tests/spec_decode_byte_identity_qwen36.sh` + `tests/spec_decode_byte_identity_gemma4.sh` + `tests/spec_decode_byte_identity_synthetic.rs` (all planned but never built). **Actually-existing parity infrastructure (at corrected paths)**: `scripts/parity/mtp_parity.py` + `scripts/parity/dflash_parity.py` (both are SCAFFOLDS — print "NOT YET IMPLEMENTED" status until model downloads are available) + `tests/parity_mtp_python_ref.rs` + `tests/parity_dflash_python_ref.rs` (both are SCAFFOLDS — skip with explanatory message when .npy fixtures absent). Test paths are FLAT (`tests/parity_*.rs`), NOT in a `tests/parity/` subdirectory as some plan text states. Plan text mentioning the never-created scripts is HISTORICAL and should be read as planning, not as current deliverables.
...
| B — Qwen 3.6 27B DFlash | ⚠️ WIRED + 0.77× of MTP greedy (post-task #95), but coherence-degraded at long lengths (128-tok run garbles + EOS-loops; see Cell B row above) | OPT-IN only (HF2Q_SPEC_DFLASH=1; research-quality) |
| B — Qwen 3.5 35B-A3B DFlash | ⚠️ WIRED with 35B-A3B drafter (HF2Q_DFLASH_DRAFTER_PATH override) + 0.31× of base / 0.43× of MTP K=1, duplication artifacts | OPT-IN only (HF2Q_SPEC_DFLASH=1; research-quality) |
| C — Gemma 4 26B DFlash (Option A xlen) | ⚠️ WIRED + byte-identical to base for ~135 tokens then diverges via argmax flip + 1.62× over Option C, 0.40× of base generation | OPT-IN only (HF2Q_SPEC_DFLASH=1 + HF2Q_DFLASH_XLEN_SDPA=1 + HF2Q_FULL_F16_KV=1; research-quality) |
| C — Gemma 4 26B DFlash (Option C re-prefill) | ✅ WIRED + correct but diverges from base, 0.25× of base generation | OPT-IN only (HF2Q_SPEC_DFLASH=1, default — but Option A is preferred when F16 KV acceptable) |
| D — Gemma 4 -assistant | Deferred | N/A |
| (auxiliary) — N-gram speculation | ⚠️ WIRED but 0.15-0.17× of base on tested workloads at HEAD `6f1d1282` (Gemma 26B Q5_K_M, Fibonacci 19.6 t/s vs base 113.2; translate-verbatim 16.5 t/s vs base 111.6). NOT byte-identical to base (diverges from token 1; same batched-prefill verifier precision-flip phenomenon as DFlash + MTP). | OPT-IN only (HF2Q_SPEC_NGRAM=1; research-quality. ~80% accept rate required to beat baseline per code comment at mod.rs:1306; accept rate not measured via HF2Q_SPEC_NGRAM_PROFILE=1 in this iter — empirical perf alone is net-negative). |
...
**Cumulative DFlash perf** improvement vs the ADR-034 baseline (HEAD 00b9ac54): **+24% to +29%** across workload and block_size. GPU/CPU byte-identical output within the DFlash orchestrator preserved throughout (per-sub-iter gate `diff /tmp/iter_*_gpu.txt /tmp/iter_*_cpu.txt`). Note: this is NOT a claim of byte-identity to base autoregressive — DFlash output diverges from base (see Cell B 27B correction at line 37).
...
- GPU path vs CPU path of the same DFlash orchestrator: byte-identical at temp=0 greedy across all sub-iters (NOT a claim of byte-identity to base autoregressive — see Cell B 27B correction at line 37)

**What this proved**: the corrected ADR target diagnosis was right. MTPLX's 2.24× advantage isn't from a fused projections Metal kernel (peer code confirmed they don't have one either) — it's from `mx.compile`-compiled chained drafter. The Rust-side equivalent (in-encoder GPU kernel dispatches replacing CPU bridges + commit_and_wait boundaries) is now shipped and delivering +26% real perf at temp=0 greedy. (Note: the per-sub-iter byte-cmp gate during task #95 was `diff /tmp/iter_*_gpu.txt /tmp/iter_*_cpu.txt` — GPU path vs CPU path of the SAME DFlash orchestrator. It is NOT a claim that DFlash output is byte-identical to base autoregressive — see Cell B 27B row correction at line 37.)

DFlash on Qwen35 remains opt-in / research-quality at 23.17 tok/s post-#95 sub-iter H (was 18.4 pre-#95) — still below MTP K=1 BATCHED greedy code-gen production winner (30.0 tok/s) because the drafter forward cost is structural. The compile-drafter pattern is now PROVEN for the DFlash arch and delivered +26% cumulative perf with GPU/CPU byte-identical correctness (per-sub-iter gate). Output is NOT byte-identical to base autoregressive (see Cell B 27B row correction at line 37 — coherence-degraded at long lengths).
...
For Cell A 35B-A3B MoE MTP: the bottleneck is T_v(N)/T_v(1)=2.4× verifier overhead at MoE batch>1, not fused projections — needs batched-prefill kernel unification (task #89) so the K>=1 verifier kernels can amortize. **For Cell C Gemma DFlash: Option A (cross-length SDPA xlen path) IS WIRED and SHIPPED — see updated Cell C row in §START HERE table at line 38.** Empirical bench at HEAD `7da12c37`: Option A = 0.40× of base generation (vs Option C 0.25×), generated text byte-identical to base autoregressive for ~135 tokens then diverges via single-token argmax flip at temp=0 greedy (validated at HEAD `6a8a8f6f` 2026-05-22 at 256 tok). Closure to net-positive perf on Gemma still requires drafter training (smaller drafter) or tree decoding — DFlash drafter overhead exceeds Gemma's compact autoregressive cost regardless of verifier path.
...
200 tok --ignore-eos, test-coverage prompt):
...
**3-rep paired confirmation (temp=0.5 vs temp=0.6)**:

| Temp | tok/s mean | Accept | vs base 21.30 |
|-----:|-----------:|-------:|--------------:|
| 0.5  |    26.50   |  74.6% | **1.244x**    |
| 0.6  |    25.87   |  70.9% | 1.214x        |

**temp=0.5 is +2.4% throughput / +3.7pp accept over temp=0.6**. Coherence verified
at temp=0.5: full multi-paragraph essay with natural prose, no degeneration over
200 tokens.

**Recommended production sampler**: `SpecSampler::new(0.5, seed)` for K=1 BATCHED
on dense Qwen 3.6 27B Q8_0. CLI: `--temperature 0.5`.

Why temp=0.5 beats 0.6: lower temperature concentrates more probability mass on
the verifier's argmax, so MH accept_prob = min(1, p/q) approaches 1 more often
when the draft sampler also has temp=0.5 (drafter and verifier distributions are
more peaked + more aligned). At higher temp, both distributions spread, accept
rate drops.
...
1. **Per-position GDN state capture** (`gdn_capture.py:128-201` 80-line Metal
   ...
   **STATUS at HEAD `dddfc1b5`: PORTED — task #90 SHIPPED** the per-position GDN
   ...
   verifier row-N kernel divergence (see §K=N CORRECTION block at line ~95) —
...
└── spec_decode_byte_identity_*.sh (NEVER ACTUALLY CREATED at HEAD `2bd38d09` — `find /opt/hf2q -name 'spec_decode_byte_identity*'` returns nothing. Originally planned as G3 byte-identity gates, but the G3 gate itself was falsified at HEAD `04e8dc9d`; would need re-scope to accept-walk consistency before any implementation.)
docs/
...
- `tests/spec_decode_byte_identity_qwen36.sh` — **NEVER ACTUALLY CREATED** (verified at HEAD `2bd38d09`: file does not exist). Originally planned as G3 byte-identity gate for Qwen; would fail at temp=0 greedy past ~30 tok divergence window (Qwen35 DFlash) or ~14 tok (MTP K=1 BATCHED) if it had been built. Needs re-scope to accept-walk consistency before any implementation.
- `tests/spec_decode_byte_identity_gemma4.sh` — **NEVER ACTUALLY CREATED** (verified at HEAD `2bd38d09`: file does not exist). Originally planned as G3 byte-identity gate for Gemma 4 (DFlash path); would fail past ~135 tok divergence window (Option A) or from token 1 (Option C re-prefill or N-gram) if it had been built. Needs re-scope to accept-walk consistency before any implementation.
...
- Add `tests/spec_decode_real_qwen36_27b.rs` and `..._35b_a3b.rs` — **NEVER ACTUALLY CREATED at HEAD `2bd38d09`** (verified: `find /opt/hf2q -name 'spec_decode_real*'` returns nothing; the synthetic CI sibling `spec_decode_byte_identity_synthetic.rs` also does not exist). Originally planned as full end-to-end accept-walk-consistency + acceptance regression-pin (HISTORICAL framing was "byte-identity", re-scoped per G3 row at line 1501); if created, would need to use accept-walk-consistency assertions, not base byte-identity.
...
- `docs/adr/diary/ADR-012-qwen35moe-conversion.md:747-755` — MTP convert intent (superseded by this ADR's P2).
- `docs/adr/diary/ADR-013-qwen35-inference.md:780-801` — P14 "complete" claim (superseded by §1.2 audit).
- `docs/adr/diary/ADR-015-mlx-native-single-cb-decode.md` (iter61a-3) — B-W-1 heisenbug origin (R5).
- `docs/adr/diary/ADR-028-peer-parity-coherence-and-speed.md:1143,3354-3389` — MTP K=3 / Qwen3.6-27B-MTP GGUF inspection.
- `docs/adr/diary/ADR-029-gemma4-moe-pipeline-is-the-gap.md` — n-gram proposer (untouched).
- `docs/adr/diary/ADR-030-dflash-block-diffusion-spec-decode.md` — DFlash design (absorbed as Workstream B).
- `docs/adr/ADR-033-unified-quant-convert-pipeline.md` — convert-pipeline correctness methodology (G1 inherits its byte-cmp gate style).
