...
configuration axes. The earlier comparison to the peer's non-unified
`n_ctx / n_seq_max` path was incomplete; the peer's unified KV mode gives
...
|---|---:|---|
| Gemma 4 | 262,144 | 6,780/6,787 minimum prefix reuse; maximum cached TTFT 143.58 ms; the 24,200-token multi-slot suffix completed in 13.843 s (1,830.7 tok/s internal), versus 13.789 s / about 1,733 tok/s for the matched peer request shape. |
...
Correct, distinct outputs from all 4 slots (so KV isolation works), but **zero throughput gain — concurrency is slightly slower than serial.** The peer serving the same model on an RTX 6000 Pro *does* speed up under concurrency, which is what prompted the investigation.
...
- **Change:** drive the real fused path and report aggregate tok/s at N=1,2,4,8 for **both** models, **side-by-side with the peer server (`-np N`) and `mlx-lm`/`vllm-mlx`** on this M5 Max. **Must assert the fused MM / id_mm path was actually exercised** (route instrumentation), not merely wall-clock a still-mv path (codex §11).
...
**What peers do:** vLLM runs continuous batching **always-on** (no serial mode). The peer server enables continuous batching whenever `-np > 1`. Default-on is the industry norm.
...
| peer `llama-server -np 4` | 89 tok/s | **~200 tok/s** |

Peer setup = `/opt/gemma4/serve.sh` (`-fa auto -ctk q8_0 -ctv q8_0`, peer build b9360). The S2/S3 batched body closes the gap from **0.51× → 0.75×** of the peer's N=4 throughput; the peer remains ~1.33× faster than our batched body. Caveat: KV configs differ (peer q8_0 K/V vs our hybrid F16-K + TQ-HB-V); both flash-attention, greedy, same weights/hardware. Probe: `slot_aware_n4_batched_body_throughput_probe` (ours, `HF2Q_BENCH_N` streams) + `llama-server` driven by N concurrent `/completion` requests.

**M4 INVESTIGATION (2026-06-24) — the gap is batching-amortization SCALING, not per-token kernel speed. Two hypotheses tested + REFUTED, one root cause localized.** Scaling curve (batched body, same GGUF/hardware) vs the peer:

| N (concurrent) | hf2q batched | per-stream | peer | per-stream |
|----------------|--------------|------------|-----------|------------|
| 1 | **89.5 tok/s** | 89.5 | **89 tok/s** | 89 |
| 2 | 127.4 | 63.7 | — | — |
| 4 | 151.7 | 37.9 | ~200 | ~50 |

**At N=1 we are DEAD EVEN with the peer (89.5 vs 89)** — our per-token decode kernels are competitive. The entire gap opens from N=2 on: our per-stream throughput degrades 89→38 (2.4× contention) where the peer's degrades 89→50 (1.78×). So the lever is *amortization across concurrent slots*, not kernel speed.
...
- **M5 (cutover + peer bench) — PEER BENCH DONE** (vs the peer, same GGUF: us 162 / peer 200 @ N=4 post-M4); **default-on cutover decision GATED on M4 ship.**
- **Remaining throughput headroom (post-M4, ~162 vs peer 200):** the skip-ALL-attention ceiling is ~186.6 tok/s — i.e. even perfect attention batching caps at ~186; the ~186→200 residual is NON-attention scaling (MoE `_id` / dense `mul_mv` amortization at N=4) and is the next investigation lever. At N=1 we are dead-even with the peer (89.5 vs 89) — the entire gap is N>1 scaling, now being closed phase-by-phase (flash done; KV-encode ~4 tok/s + MoE/dense scaling remain).

**GAP ROOT-CAUSE + SOTA PLAN (2026-06-24) — research-validated, source-cited.** APPLES-TO-APPLES vs the peer (same gemma4-ara Q5_K_M GGUF, M5 Max, N×128 greedy, aggregate tok/s, peer `llama-server -np N`):

| N | hf2q (M4 batched flash) | peer | ratio |
|---|-------------------------|-----------|-------|
| 1 | 89.5 | 89 | **1.01×** |
| 4 | 163 | 199.6 | 0.82× |
| 8 | 189 | **242.6** | 0.78× |
...
| step (flags) | N=4 | N=8 | vs peer (200/243) |
|---|---|---|---|
...
- **Design APPROVED 2026-06-23** (m1-queen swarm; lead-reviewed vs scheduler/decode recon + the peer's `llama_ubatch` reference). Key design facts: `scheduler.step()` (`scheduler.rs:1266`) already batches all decoding handles correctly and has **zero callsites** — F1 is "drive the existing API." `forward_decode_slot_aware` (`forward_prefill.rs:3989`) is a swap-in/delegate wrapper around the **scalar** `forward_decode` → F2 is a genuinely new batched `forward_decode_batched` taking `[(token,seq_pos,slot)]` → `[N][vocab]`. Per-slot decode state (sampler/grammar/stop/logprob/reply) hoists into a new `ActiveSlot` struct in `engine.rs`.
...
**Re-measured the peer head-to-head under the new default (2026-06-25, fresh clean same-session, one engine at a time, idle, 200-tok gens, same prompt, aggregate tok/s):**

| N | hf2q (new default, batched) | peer `-np 8` | gap |
|---:|---:|---:|---|
| 1 | 74.6 | 88.4 | peer +18% |
| 4 | 146.3 | 218.4 | peer +49% |
| 8 | 163.0 | 266.8 | peer +64% |

hf2q's absolute numbers match §0.15's batched-body column (now shipping correctly + by default); the peer measured ~13% faster than §0.15 this session, so the **gap is real and if anything wider** than the §0.15 snapshot. The softcap fix was a *correctness* fix, not a throughput one — closing the speed gap is the next milestone (single-run N=1 is noisy; the N=1 gap reappearing vs §0.15's "~even" warrants a careful re-measure). **Coherence is now MET (byte-identical to serial over 600–800-tok concurrent generations, no cross-slot contamination); SPEED is the open gap.** Levers unchanged from §0.15: serve-layer overhead (~16%), batching-kernel efficiency (remaining per-slot loops in batched MoE/attention), and a fresh look at single-stream N=1.

#### 0.15 hf2q vs the peer head-to-head (2026-06-24) — MEASURED clean (idle, sequential, same GGUF/prompt/N, HTTP, 200-tok gens)
...
| N concurrent | hf2q (batched body) | peer (`-np 8`) | gap |
|---:|---:|---:|---|
| 1 | 82.4 | 83.9 | ~even |
| 4 | 149.3 | 197.1 | **peer +32%** |
| 8 | 166.2 | 235.8 | **peer +42%** |

**HONEST BOTTOM LINE: the "match/beat the peer" bar is NOT met — hf2q is ~even single-stream but ~30-42% SLOWER at concurrency.** This **corrects the stale §0.12 "N=8 202 > peer N=4 200" line** (prior-session, not a clean head-to-head). The peer also scales better (2.8× N=1→8 vs our 2.0×).
...
2. **Batching kernel efficiency ~18%:** in-process 198.8 vs peer 235.8 @ N=8 → the peer amortizes per-step better (our MoE routing + weighted-sum still have per-slot loops; attention/proj batching has headroom).
Caveats: (a) different KV quant (hf2q F16-K/TQ-HB-V 8-bit vs peer q8_0/q8_0) — a coherence/VRAM tradeoff, not pure speed; (b) coherence parity vs the peer NOT re-verified this session (throughput only); (c) the batched body still has the ~2.5% staggered determinism residual above.
...
| | hf2q decode | peer decode | hf2q prefill (TTFT) | peer prefill |
|---:|---:|---:|---:|---:|
| N=1 | **84–92 tok/s** | 83.9 tok/s | 335 ms | 156 ms |
| N=8 | **177–207 tok/s agg** | 198 tok/s agg | **2438 ms** | 322 ms |

- **DECODE IS AT PARITY** — hf2q ≥ the peer at N=1, ≈ the peer at N=8 (within run-to-run noise). The batched `[N,hidden]` decode body (now default) is competitive. The §0.15 "batching-kernel efficiency ~18%" lever was a **measurement artifact** of prefill bleeding into the decode number.
- **PREFILL IS THE ENTIRE GAP.** Two sub-causes, both measured:
  1. **No multi-sequence prefill** — the worker ADMIT loop (`engine.rs:5993`) calls `admit_gemma4_slot` → `prefill_seed` **once per request, sequentially**. `forward_prefill_batched.rs` batches *within* one sequence (per-layer, not per-token) but there is NO cross-slot prefill. 8 arriving requests ⇒ 8 sequential prefills ≈ 8 × ~300 ms = ~2.4 s, vs the peer's single batched multi-seq prefill (322 ms). This is the **7.6× N=8 prefill gap**.
  2. **Single-seq prefill ~2× slower per-token** — N=1 prefill 335 ms vs the peer's 156 ms for the same short prompt (~90 vs ~190 prompt tok/s). Independent of (1); helps every prefill.

**Direct prefill-compute measurement (`HF2Q_PREFILL_TIMING=1`, gated log in `prefill_seed`):** a 26-prompt-token request prefills in **~275 ms = ~94 prompt tok/s** (real compute, not HTTP/overhead — the cold first request is 456 ms, warm ~275 ms). The peer prefills the same at **~185 prompt tok/s** (its `timings.prompt_per_second`). Two confirmed root causes:
...
2. **Sequential across slots.** 8 slots × ~275 ms each = ~2.2 s (the N=8 TTFT), vs the peer's one batched multi-seq prefill (322 ms).

**Revised lever priority (measure-driven), tracked as `iter-G-prefill-batched`:** (i) **convert the slot-aware prefill to batched-per-layer** (writes all prompt tokens per layer in one `m=seq_len` pass into the per-slot scaffold) — ≈7× single-seq speedup, would make hf2q prefill *faster* than the peer per-token; (ii) **multi-sequence batched prefill** across pending slots (the 8× sequential → ~1 batched pass). Both must be byte-parity-validated against the per-token reference (mantra: prove correctness, like the §0.16 work). The decode path is NOT a priority — already at parity.
...
Phase F merged to `main` (PR #2). iter-G continues on `adr-040-iter-g-prefill`. Three parallel read-only investigations (our admit/prefill path, our batched-forward n_seqs capability, the peer's multi-seq mechanism) + a codex plan review + a determinism spike now ground the build.
...
**Significance:** greedy (temperature=0) decoding of any sufficiently long prompt on the **default** server is non-deterministic run-to-run — a direct violation of the "as coherent as the peer" bar. The per-token prefill path is the deterministic fallback. **Hypotheses TESTED (each a clean A/B), with results:**
...
#### 0.21 F6 PEER BENCHMARK vs the peer — the "as fast or faster than the peer" receipt (2026-06-25)

Head-to-head on the SAME model (`gemma4-ara-2pass-APEX-Q5_K_M.gguf`, which the peer loads with no arch error), SAME M5 Max, one engine at a time, clean/idle. Peer = `llama-batched-bench -npp 32 -ntg 128 -npl 1,8 -ngl 99`; hf2q = `HF2Q_CROSS_SLOT_ADMIT=1` prefill + `slot_aware_n4_batched_body_throughput_probe HF2Q_BENCH_N=8` decode.

| N=8 metric | hf2q | peer | peer advantage |
|---|---|---|---|
...
| Paged-KV / inflight-batched KV layout with per-slot KV separation | "Phase 4's pool is request-serial within each loaded model" | the peer's `src/llama-kv-cache.cpp` multi-seq semantics |
...
**Decision**: Mirror the peer's `-cb` (continuous batching) admission-during-decode loop semantics, NOT vLLM's full `Scheduler` class. ADR-005 line 1103 names both as candidates; this ADR picks the smaller-blast-radius option.

**Why**:
- The peer's `-cb` is mature, debugged against real workloads, and lives in the same comparator set ADR-005 is positioned against.
...
| **B3 (SHIPPED 2026-05-23)** | `InflightBatchedScheduler` real `step` FSM — SlotPhase enum {Queued, Prefilling, Decoding} + `advance_after_prefill`/`advance_after_decode` driver-callback APIs + DEFAULT_PREFILL_CHUNK_TOKENS=512 (mirrors the peer's `-ub` default) + 12 new FSM tests (30 total scheduler tests); iter-1.5 cfg(test) gate removed | **1 day landed** |
...
|---|---|---|---|---|---|
| — | ADR draft | `docs/adr/ADR-040-continuous-batching-reopen.md` | ~350 | — | ✅ landed |
...
The shape of this work is structural Walk, not optimization. ADR-005's comparator bar ("parity or better than ollama + the peer") gains a new axis once the reopen trigger fires: aggregate throughput under N concurrent. This ADR builds the infrastructure to measure that, then to ship it.
