...
- **Status addendum 3 — bounded live-append coordinate repair (2026-08-12):** A release-gate failure exposed two separate issues. The old long-resume falsifier compared the production resumed graph with a forced legacy per-token cold graph, whose greedy bytes are already known to differ at this prompt length; the control now uses the same production graph. Independently, bounded live append staged `min(K, cache_capacity - 1)` chronological history rows but capped its query-position vector at `sliding_window - 1`. The batched hybrid shader treats that vector as authoritative, so a long linear resume could omit the current suffix K/V and attend the wrong staged rows. Query positions now use the exact staged history length and short nonempty suffixes use this corrected bounded append instead of crossing graphs. The real-model falsifier requires two consecutive engagements (Q→P→R) and both resumed responses byte-match cold production controls. Explicit `HF2Q_SERVE_BATCHED_PREFILL=0` retains the legacy dense/per-token path and its eligibility requirements.
- **Status addendum 2 — "gemma-hybrid-lcp" long-resume + production hardening (2026-08-03):** LCP extended past `sliding_window` (1024) under the hybrid regime and three production defects fixed en route to opencode. (1) **Long-resume:** sliding layers allocate LINEAR buffers under `HF2Q_KV_LCP_LONG_RESUME=1` on BOTH prefill routes (batched store route + resume-execution route); hybrid encode writes slot=logical at prefill and decode (capacity-derived predicate in `gpu_full_attn.rs`); the hybrid `flash_attn_vec_hybrid` kernel's `mask_type=2 + sliding_window` composes byte-identically with a cold request through the same production graph (`gemma_hybrid_long_resume_byte_identity`, engagement-asserted). Snapshot headroom +4096/turn for multi-turn growth; `HF2Q_KV_LCP_RESUME_CAPACITY` is the registry budget knob (8g documented envelope ≈ 8-16K-token contexts). (2) **SerialFifo consume-gate 500:** turn N+1 with a longer prompt than turn N hard-errored on the non-batched route (`cached dense_kvs capacity < required`) — now drops the stale mount + fresh-allocs (the slot-aware scaffold bails are preserved verbatim). (3) **Pre-copy snapshot budget gate:** dual-leg snapshot bytes are estimated from shapes BEFORE the alloc+memcpy and skipped when over budget — a ~97K-token dual-leg snapshot is ~64 GB and previously swap-stormed the box before the registry could reject it. **Context-size envelope (measured):** batched prefill route builds O(n²) bf16 masks (~40 GB at ~100K) and the route OOMs past ~32K (`HF2Q_SERVE_BATCHED_PREFILL=0` forces the linear-memory non-batched route, wired as `BATCHED=0` in `scripts/serve_gemma4_opencode.sh`); gemma per-request dense F32 KV is ~4 GB/16K tokens — opencode-scale ~100K sessions exceed 128 GB RAM comfortably only on the qwen35 arch (fixed-size DeltaNet + TQ full-attn; qwen36 serve proven at 118K). **Two pre-existing divergences documented for follow-up (both reproduced at clean main, NOT introduced by the gemma-hybrid-lcp arc):** (a) batched-prefill sliding-layer output diverges from the non-batched parity reference at seq>sw on the 200-rep needle prompt; (b) `iter5_r_c4_lcp_5_fraction_sweep` fraction 0 (tiny K) diverges at byte 164 — dense-side, predates this arc.
- **Status addendum — "gemma-hybrid-lcp" (2026-08-03): Gemma 4 LCP partial-prefill resume shipped under the PRODUCTION hybrid regime (F16-K + TQ-HB V).** Three gaps closed: (1) the resume restored only dense K/V — under hybrid, decode reads `hybrid_kv`, so a dense-only restore left the decode cache zeroed over the prefix (silent corruption, same class as ADR-027 23d-γ); (2) the end-of-prefill snapshot was gated `HF2Q_USE_DENSE=1` — no snapshot under hybrid, ever; (3) the iter-344 batched prefill route (`forward_prefill_batched`, SerialFifo default) populated NO snapshots for ANY regime — the registry stayed empty in production regardless of env. Fixes: regime-aware per-layer payload `GemmaLcpLayerKv` (dense + hybrid legs; prefill reads dense, decode reads hybrid — both restored); snapshots populated in BOTH prefill routes; `effective_kv_lcp_resume` widened (resumable: gemma-dense, gemma-hybrid, qwen35-TQ per ADR-027 23d-γ; HB-encoded opt-out stays auto-disabled); regime-consistency install check (cross-substrate entry = clean miss). Gates: `gemma_hybrid_lcp_partial_prefix_byte_identity` two-server integration test (engagement-asserted) + live `K=516 of N=537` ENGAGED trace, byte-identical output vs cold control (non-streaming + streaming). Boundary: prompts > `sliding_window` (1024) still skip store/resume — LONG_RESUME remains dense-only; hybrid long-resume is follow-up work.
- **Status:** **Closed-Shipped via Phase E PromptCache replay (Closure iters 1→6 landed 2026-05-04; iters 7→11 landed 2026-05-05 close the stress 24h gate).** All MUST-level coherence + performance + corruption ship-gates PASS by measurement: R-C4 internal sourdough (3632-byte byte-identical baseline=restored, 624× speedup); R-P4 ≤ 0.20 at L=32K (ratio=0.000); **R-P5 ≤ 0.15 at L=32K cold-process resume (ratio=0.000, 43,000× cache-hit speedup, no_cache=585.5s, cache_hit=13.6ms)**; **R-P6 4-agent shared 4K aggregate ≤ 1.25× (1.00×, 25% margin)**; K1/K2/K3 kill-gates FALSIFIED; R-C6 hash-check corruption rejection (3-of-4 scenarios at unit level; 4th "delete intermediate block in chain" is intentionally non-fatal per design — chain-hash is informational, not validating). Phase D SPILL side (iters 1-4) + Phase E RESTORE-CONSUMER side (iters 5-6) close the original ADR-017 vision for full-equality prompt matching at production-observable behavior. **Stress 24h smoke ✅ at SPEC config (iter-11 + iter-12 2026-05-05):** 30-min/13-iter run at default `budget_mb=4096` + `rss_tol_pct=5` shows max RSS Δ=+0.1% (oscillates +0.0%/-0.0%/-0.0%/+0.1%/+0.1% across iters 3/5/7/9/11/13), cache stable at 4,194,436 KB = budget exactly (R-F5 working), lsof Δ+0. Extrapolated 24h growth rate (520 iters × 0.1% / 13 iters) = ~4% — still under the 5% spec tolerance even at the absolute worst-case growth rate observed. Previously this gate was ⚠ ("infrastructure validated; full 24h run is operator-controlled"); iters 7-9 surfaced + characterized a per-iter ~115% RSS leak; iter-10 root-caused it to two independent `Engine` clone holders (registry-side bindable + banner-only Engine clone) outliving pool eviction and fixed both (commit `0ccc98d`); iter-11 wired R-F5 budget eviction in `writer.rs::process_job` (was set_budget wired but `evict_lru_until_under_budget` had no production caller; commit `c2eeecd`) + 3-iter warmup before baseline_rss sample so the 5% rss_tol gate measures iter-to-iter linear growth rather than substrate ramp-up. Full 24h run remains operator-controlled but now passes on smoke. R-P5 + R-P6 ship-gates re-validated after iter-10/11 changes: R-P5 ratio=0.000 (44,500× cache-hit speedup, no_cache=619.5s, cache_hit=13.9ms — even faster than iter-5's 43,000×); R-P6 1.00× aggregate (64,931 ms total, agents 2-4 at 1.5-1.9 ms ttft via PromptCache replay). Pre-iter-5 Status: Primary gates: K1/K2/K3 kill-gates FALSIFIED + R-C4 internal + R-P4 ship-gate PASS at iter-4 (in-memory PromptCache replay path; ratio=0.000 on iter-4 evidence). **Closure iters 1→4 narrative (Code = Truth per project mantra):** iter-1 discovered R-P5 architectural gap via the new production test exposing zero-byte `cache_dir` after `pre_evict`; iter-2 added `/shutdown` HTTP endpoint + SIGTERM drain (commit `b2d0cda`); iter-3 closed a four-bug stack on the spill path — pool-key vs bare-repo lookup mismatch, A.3-stub `n_layers=1` + single-block range hardcodes never replaced by B-dense.1, `Weak<Engine>` engine_arc that could never upgrade in production, and a tokio nested-`block_on` panic — making the spill path actually write blocks to disk (commit `985be04`); iter-4 validated cross-process restoration plumbing (`post_admit` finds 44 metas, fingerprints match, restored=44/44) AND surfaced the next architectural gap: **forward_prefill resets `write_pos = 0` at every prefill (`forward_prefill.rs:446`) and re-runs the full prefill from token 0, overwriting the restored KV state.** The restored bytes are written into the buffer, then immediately discarded. The spill→restore plumbing is correct end-to-end; the request path doesn't use the cached state. **Phase E (LCP-based partial-prefill resume — explicitly deferred at `engine.rs:1442` "iter-97+ scope") is the missing layer for true R-P5/R-P6 cache-hit acceleration.** ADR-017's pre-iter-3 R-P4 "PASS at ratio=0.000" was iter-96 PromptCache full-equality replay (RAM, single-process), not actual cross-process KV restore — the iter-1 audit suspected this; iter-4 confirmed it from code. Spill side: 801 MB / 337 blocks written at L=512; restoration: 44/44 metas restored on cold-process restart with matching fingerprint. Phase E options: (a) implement iter-97+ LCP partial-prefill resume; (b) extend ADR-017 to also persist `PromptCache` for cross-process full-equality replay (smaller scope, ~150-300 LOC). User-facing iter-2 win independent of R-P5: Ctrl+C / `systemctl stop` / deploy graceful-shutdown now flushes KV cache to disk instead of silently discarding it. Operator-readiness P0/P1 round COMPLETE: P0-1 fsync(parent_dir), P0-3 *.tmp.<pid> orphan GC, P0-bench Weak<Engine>, P1-1 lock-step atomicity, P1-3 budget_bytes wiring, R-F1 HF2Q_KV_PERSIST=0 startup-disable, R-F6 cache --kv-namespace, R-F7 /metrics counters, F4 namespace fingerprint thread-through — all LANDED on origin/main. Remaining for closure: Phase E (LCP or PromptCache persistence — see iter-4 finding for design-options), R-P5/R-P6 re-bench post-Phase-E, full 32K bench, stress 24h (depends on Phase E for real cache-hit), R-C6 corruption injection (4 scenarios), peer arm rebench (post-ADR-005 chat-template fix), B-hybrid (post-ADR-013). Per-family read at [`docs/adr/ADR-017-per-family-status.md`](./ADR-017-per-family-status.md).
...
- [x] Gate A cosine ≥ 0.9998 on the TQ-active path for every in-scope family at every prefix length (R-C2).  *(Landed B-tq.1 — `tq_packed_v1_cosine_gate_a_satisfied_by_construction` proves cosine = 1.0 trivially via the D2 byte-exact rebuild: when restored bytes are byte-equal to pre-spill bytes, the dequantize function's outputs are element-wise byte-equal, hence cosine = 1.0 ≥ 0.9998.  Engine-side wiring of TQ-active inference vs the on-disk codec landed at B-tq.2 (`TqPackedSpill` engine hook) + B-tq.3 (codec_version=2 envelope w/ per-token norms + `MlxModelWeights::tq_v2_snapshot_block`/`tq_v2_restore_block`); the ADR-007 codec correctness contract is satisfied by Path C F-0.2 NRMSE 0.000247 + F-7 codec freeze LANDED 2026-05-05.)*
- [x] Storage delta documented at `docs/adr/diary/ADR-017-tq-storage-delta.md`: `bytes_on_disk_dense / bytes_on_disk_tq` ratio across 8K / 32K prefix lengths (informational, not a ship-gate; expected ratio ~3-4× per ADR-007 codec).  *(Landed B-tq.1 — measured: 4-bit envelope = **8.00×** vs dense F32 at 8K and 32K (sliding + global layers); 8-bit envelope = **4.00×**.  Higher than the in-runtime 3-4× number because the on-disk envelope strips runtime working memory.)*

**Phase B-tq.1 Status: GREEN (2026-05-05)** — on-disk envelope substrate landed at frozen codec_version=1.

**Phase B-tq.2 Status: GREEN-substrate (2026-05-05)** — `TqPackedSpill` engine-side family hook landed at `families/tq_packed.rs` post ADR-007 Path C runtime completion announcement.  Implements `KvCacheSpill::block_alignment` / `n_layers` / `snapshot_block` / `restore_block` / `model_fingerprint` via the B-tq.1 envelope codec.  Layout-mismatch guards on restore (head_dim drift, bits_per_coord drift, range-vs-n_tokens disagreement) reject mismatched payloads with `SpillErrorKind::CodecErr` to prevent silent corruption from a future engine upgrade that bumps shape without bumping `codec_version`.  11 unit tests cover round-trip parity, restore guards, fingerprint round-trip, block-count tracking.

**Phase B-tq.3 Status: GREEN-substrate (2026-05-06)** — engine wiring landed.  Three pieces ship together: (a) **codec_version=2 envelope** at `families/tq_packed.rs` carrying `bits × nkv × ntokens × hd / 8` indices PLUS a `nkv × ntokens` F32 LE per-token-per-head norms stream (`pack_tq_v2_payload` / `unpack_tq_v2_payload`).  v1 envelope was incomplete for the runtime path because `MlxKvCache.k_norms`/`v_norms` are per-token-per-head F32, not a single `scale: f64` — v2 fixes this without touching v1 (frozen-magic + frozen-codec-version regression tests for BOTH versions).  (b) **Raw-bytes capture/restore helpers** `capture_tq_v2_payload_from_buffers` / `restore_tq_v2_payload_into_buffers` that operate on the `[nkv, capacity, hd_packed]` U8 packed-buffer + `[nkv, capacity]` F32-LE-bytes norms layout the runtime uses, so they're unit-testable on a CPU host.  (c) **Engine-side hook** `MlxModelWeights::tq_v2_snapshot_block(layer, range, bits, flags, scale)` and `tq_v2_restore_block(layer, range, bits, k_payload, v_payload)` reading/writing through `MlxBuffer::as_slice` / `as_mut_slice` on `kv_caches[layer].{k_packed, k_norms, v_packed, v_norms}`.  `TqPackedSpill::insert_block` and `KvCacheSpill::restore_block` now magic-dispatch on the first 4 bytes — accept v1 (substrate path) AND v2 (engine-wired path), reject anything else with `CodecErr`.  11 new unit tests (45 cumulative across B-tq.1+2+3) cover v2 round-trip byte-exact, frozen v2 magic + codec_version, cross-version reject (v1 reader vs v2 payload + symmetric), capture→restore byte-identity on synthetic `[nkv, capacity, hd_packed]` buffers, capture-rejects-malformed-shape, restore-rejects-shape-drift (bits/head_dim/nkv/range), and spiller v1+v2 dispatch round-trip.

**Phase B-tq.4 Status: GREEN-substrate (2026-05-06)** — activation factory landed across 3 iterations on top of B-tq.3's engine-side methods.

* **iter-1** (commit `62bb8b5`, ~1,100 LOC) — substrate.  Five pieces:
  * `TqPackedSpillDescriptor` at `src/serve/api/tq_packed_descriptor.rs` (190 LOC) captures TQ-active runtime config from `MlxModelWeights` at engine spawn.  `bits_per_coord` reads `HF2Q_TQ_CODEBOOK_BITS` env (default 8 per ADR-007 §1234); `is_tq_active_mode()` reads `HF2Q_TQ_KV` env with permissive truthiness.  Pure-function parsers (`parse_tq_codebook_bits` / `parse_tq_active_mode`) are unit-tested directly to avoid env-var contention.
  * `TqPackedSpill::engine_arc: Arc<RwLock<Option<Engine>>>` field + bind methods (~140 LOC) mirror `Gemma4DenseSpill::engine_arc`.  Stores `Engine` value (not `Arc<Engine>` and not `Weak<Engine>`) for the same P0-bench-fix reason as the dense path.  `new_with_engine` / `set_engine_arc` / `clear_engine_arc` / `try_from_engine_arc` / `EngineBindable` impl all in place.
  * `Engine` worker bridge for TQ-packed snap/restore (~250 LOC).  Two new `Request` variants (`TqPackedKvSnapshot` / `TqPackedKvRestore`); worker-thread match arms call `MlxModelWeights::tq_v2_snapshot_block` / `tq_v2_restore_block` (shipped in commit `b7e975d`).  Public `Engine::tq_packed_v2_snapshot_block` / `tq_packed_v2_restore_block` send the requests via `try_send`-then-`blocking_send`.
  * K+V bundle codec (~150 LOC) — `pack_tq_v2_kv_bundle` / `unpack_tq_v2_kv_bundle` with frozen magic `b"TQK2"` (`0x32_4B_51_54`).  Bundles two `tq_packed_v2` envelopes into a single payload for the spiller's single-`Vec<u8>` API.  Bundle magic is FROZEN; v1 / v2 / bundle magics all distinct for the magic-dispatcher.
  * `TqPackedSpillFactory` + `EngineBindable` impl (~120 LOC) mirror `Gemma4DenseSpillFactory`.  Silent no-op on type mismatch per `FamilyHookFactory` contract.

  12 new unit tests (58 cumulative across B-tq.1+2+3+4-iter-1):  bundle round-trip byte-exact + SHA-stable; bundle frozen magic regression pin; bundle rejects malformed inputs; spill accepts bundle payload in memory; spill engine_arc starts unbound; factory rejects non-Engine Arc; `try_from_engine_arc` rejects non-Engine; `bind_engine` silent on type mismatch; descriptor pure-fn parser tests.  Full hf2q test suite: 2866/0/3 PASS.

* **iter-2** (commit `b346425`, ~80 LOC) — `cmd_serve` factory registration.  At `src/serve/mod.rs:3076`, the registration site now branches on `tq_packed_descriptor::is_tq_active_mode()`: `HF2Q_TQ_KV=1` ⇒ register `TqPackedSpillFactory`; otherwise register `Gemma4DenseSpillFactory` (existing B-dense.2 behavior).  **Single-registration policy** documented + enforced.  Decision rationale: `KvPersistRegistry::register_factory` uses `HashMap<FamilyKey, Arc<dyn FamilyHookFactory>>` so dual-registration would silently overwrite; the runtime is single-mode (env read once at engine spawn); future B-tq.5+ can lift FamilyKey shape if dual-mode swap-on-evict is ever needed.
...
**Why earlier B-tq.3 wording was wrong.**  The iter immediately preceding this one stated B-tq.4 was "gated on ADR-007 Path C runtime correctness work clearance."  Path C closed substantively on 2026-05-05 at 10 of 11 phases (`docs/adr/007-pathC/PATHC_CLOSURE.md`).  The codec-freeze contract (Path C F-7) is LANDED — that's the contract B-tq.4 codes against.  Path C F-0.2 NRMSE 0.000247 at Gemma 4 26B production shape proves codec correctness.  The remaining open Path C item is F-5 (MMLU + LongBench) — discretionary downstream-consumer-driven validation, NOT a B-tq.4 prerequisite.

**Actual B-tq.4 gates** are ADR-017 plumbing items, not ADR-007 ones:

1. `TqPackedSpillDescriptor` (~80 LOC) — captures TQ-active runtime config from `MlxModelWeights` at engine spawn (mirrors `KvSpillDescriptor::from_gemma_loaded_model` at `engine.rs:2190` for the dense path).
2. `TqPackedSpill::engine_arc: Arc<RwLock<Option<Engine>>>` field + bind methods (~120 LOC) — mirrors `Gemma4DenseSpill`'s pattern at `families/gemma4_dense.rs:255-460`.
3. Engine-bound `snapshot_block` / `restore_block` overrides on `TqPackedSpill` (~150 LOC) — upgrade `Weak<Engine>` → call `tq_v2_snapshot_block` / `tq_v2_restore_block` (already shipped this iter).
4. `TqPackedSpillFactory` (~100 LOC) — mirrors `Gemma4DenseSpillFactory` at `gemma4_dense.rs:1591-1692`.
5. `cmd_serve` factory registration (~30 LOC) — register `TqPackedSpillFactory` alongside `Gemma4DenseSpillFactory` at `mod.rs:3076`; runtime mode (`HF2Q_TQ_KV=1` vs dense default) picks via `FamilyKey`.
6. End-to-end integration test (~200 LOC) — `tests/kv_persist_tq_packed_roundtrip.rs`: spawn TQ-active Gemma 4 model → prompt → trigger eviction → restart → replay → assert byte-identical decode (R-C1).

Total: ~680 production + ~200 test ≈ 880 LOC.  The work is template-following (Gemma4Dense pattern is the spec); the unknowns are dual-mode vs exclusive factory registration and any TQ-active GGUF fixture sourcing for the integration test.
...
| **D.2** | D | Per-family ship-gate read; `docs/adr/ADR-017-per-family-status.md`; ADR-017 status flip Accepted → Closed-Shipped. | ~80 doc + measurement | D.1 |
...
| `git diff main..HEAD --stat` | OK | only `tests/kv_persist_harness.rs`, `tests/fixtures/synthetic_spiller.rs`, `docs/adr/diary/ADR-017-persistent-block-prefix-cache.md` changed |
...
| `docs/adr/diary/ADR-017-persistent-block-prefix-cache.md` (this section) | C.1 deliverable 8 | this subsection |
...
| `docs/adr/diary/ADR-017-persistent-block-prefix-cache.md` | this subsection | "Phase B-dense.2 LANDED" entry. |
...
| `docs/adr/diary/ADR-017-persistent-block-prefix-cache.md` | this subsection. | ~80 |
...
| `docs/adr/diary/ADR-017-persistent-block-prefix-cache.md` | this subsection | "Phase D LANDED (scaffolding)" entry. |
...
**Code = truth (per project mantra):** the comment at `engine.rs:1461-1468` documents the missing semantics (line drift from pre-iter-5 — was at `:1442`; verified at iter-13 mantra dive-deep, also flagged in Phase E.a dossier §10.8):
...
- ✅ **Stress 24h PASS at SPEC config (iter-11 2026-05-05): RSS Δ=0.0%, cache=budget, no leak.**
  ...
  **(2)** `startup_engine_for_banner` in `cmd_serve` cloned the canonical `Engine` for the banner print, then held the clone for the entire `rt.block_on` async block (server lifetime). This added a 3rd `Arc<EngineInner>` ref on top of (1).

  **Fix (commit `0ccc98d`):** `BlockPrefixCacheSpiller` now holds `Option<Arc<KvPersistRegistry>>` via `set_registry()` (wired in `cmd_serve` immediately after registry creation); `drop_family` calls `registry.unregister(repo, quant)` so both sides drop in lock-step. Banner-only Engine clone now drops immediately after the banner print.

  **Validation:**
  - **Before fix** (iter-9c, 5-iter wall=804s): RSS Δ=+115% (19.5 GB → 42 GB, linearly growing).
  - **After fix** (iter-10 long-run, 7-iter wall=1118s): RSS Δ stable across iters: t=307s/iter=2 Δ=+11.7%; t=834s/iter=5 Δ=+10.9% (decreased!); t=1118s/iter=7 Δ=+11.7%. Per-iter leak rate ≈ 0.
  - Residual +11.7% (2.2 GB) is amortized one-time working set (heap arena fragmentation + tokio runtime + writer/store + chat-template caches), NOT a per-iter leak. RSS bounded across 7 iters.

  Full 24h stress remains operator-controlled but should now PASS the spec's 5% RSS leak bar measured *iter-to-iter* (which is what "no leak at hour 24" reduces to once the per-iter-leak source is eliminated). Closed-Shipped status was already unaffected (R-C4/R-P4/R-P5/R-P6/R-C6/K1/K2/K3 all PASS); this iter promotes the SHOULD-level stress gate from ⚠ to ✅ at smoke level.
- ⏸ Peer arm vs llama-completion: blocked on ADR-005 chat-template
- ⏸ Full 60-cell matrix sweep: operator-controlled
...
This Qwen chunk-aligned counter note is separate from the 2026-08-12 Gemma
long-resume addendum above. Gemma's current Q→P→R falsifier requires two
live-prefix detections and an increasing second `cached_tokens` value so the
latest committed anchor, rather than merely an older common prefix, is proven
reusable.
