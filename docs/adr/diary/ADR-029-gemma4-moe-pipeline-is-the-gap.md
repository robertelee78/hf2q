...
- **Status**: 🏆 **iter-175 (2026-05-15) — MISSION COMPLETE.** Decode gap CLOSED + REVERSED via Step 1i (parallel SG-tournament top-K in MoE routing kernel) + Step 1j.2 (V3 batched softmax tree-reduce, byte-identity restored).  Same-session apples-to-apples thermal-fair bench: **tg200 1.055× / tg2000 1.050× / tg5000 1.052× = mean 1.052× peer-FA AHEAD** across all decode regimes.  Net trajectory: 0.924× (iter-100 reopen) → 1.052× = +12.8pp swing.  V3 default-on is BYTE-IDENTICAL with V2 baseline at 30+200-token greedy decode AND +10.96% measured speed.  Regression-protected by 8 parity tests + coherence_smoke + 200-token byte-identity verification.  The 2026-05-11 thesis "MoE pipeline IS the gap" CONFIRMED — gap localized to routing kernel's serial top-K phase, fixed by parallel SG-tournament.  Original iter-100 reopen status preserved below for historical context.
- **Date**: 2026-05-11 (original) → iter-9 rewrite → iter-18 short-ctx close → iter-19c long-ctx reopen → iter-21..73 prefill levers → iter-74 thermal-fair multi-regime re-measurement → iter-95 (merged-to-main) → iter-100 (regime miss — averaged decode reopens gap) → iter-100..174 lever ledger (28+ levers tested) → **iter-175 Step 1i (+11.5% breakthrough) + 1j.2 (byte-identity fix) — MISSION COMPLETE**.
- **Supersedes**: itself.  Original "MoE pipeline IS the gap" framing CONFIRMED CORRECT by Step 1i's MoE routing kernel localization.  Iter-100 reopen status SUPERSEDED by Step 1j.2 closure.
...
- `docs/adr/ADR-027-qwen35-tq-kv-cache-and-persist-family.md` (qwen3.6 TQ-KV path; gemma4 doesn't use this by default)
- `docs/adr/diary/ADR-028-peer-parity-coherence-and-speed.md` (prior 141-iter mission; ADR-029 corrects its iter-486/487 closure but otherwise builds on its empirical work)
