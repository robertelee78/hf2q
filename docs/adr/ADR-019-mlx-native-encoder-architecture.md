...
**Wall-time arc:** pp80 199 → 582 t/s = **2.92× speedup**; pp726 ~200 → 1968 t/s = **~10×**; decode 105 → 116 t/s = **1.07× peer parity** (4/4 fixtures byte-identical). 96% sync-count reduction. Source: `project_adr013_p21_stage4_canonical_2026_05_02.md` MEMORY note + `docs/adr/diary/ADR-013-qwen35-inference.md:2918-2932`.
...
**iter89e2-F LANDED 2026-05-02:**
- **Branch:** `adr019-phase2-iter89e2f-fa-layer-single-stage-fusion` (based off `8305bf9` to inherit iter89e2-E's `_into` extraction; merges to trunk together).
- **Commit:** `57fd82f` (single commit; +248/-28 = +220 net in `src/inference/models/qwen35/gpu_full_attn.rs` + 1 new helper script `scripts/adr019-phase2-parity.sh` for the 4-fixture parity harness).
- **What landed:** `build_gated_attn_layer` (`gpu_full_attn.rs:1854`) gains a unified Stage-A CB fast path. When all preconditions hold (production-arena prefill, `head_dim==256`, `cur_len==0`, dump_bisect off), opens ONE `CommandEncoder` per FA layer, encodes ops1-4 dispatches via the existing arena path, inserts `enc.memory_barrier()`, encodes `kv_cache_copy_seq_f32_dual` into the same encoder, inserts `enc.memory_barrier()`, calls iter89e2-E's `apply_flash_attn_prefill_seq_major_into(&mut enc, …)` to encode the FA prefill bridge into the same encoder, updates `slot.current_len[0]`, and issues a terminal `commit_labeled("layer.full_attn.stage_ab")`. Replaces 3 separate `commit_labeled` calls per FA layer (`layer.full_attn.ops1-4` + `layer.full_attn.kv_cache_write` + `fa.prefill_bridge`) with 1. ops6-7 remains its own CB in this iter to limit blast radius for parity debugging — iter89e2-G extends fusion to ops6-7.
- **What did NOT land:** ops6-7 fusion (deferred to iter89e2-G); cross-layer fusion (out of Phase 2 scope); legacy non-arena branches (preserved byte-exactly). Decode (`seq=1`) path (`apply_gated_attn_layer_decode_into`) is untouched.
- **Tests:** 2708 / 2708 PASS with `cargo test --release --test-threads=1` (no new failures vs `8305bf9` baseline). The iter89e2-E byte-exact unit test `flash_attn_prefill_into_byte_exact_parity_with_wrapper` stays green. (Full-suite parallel-thread run shows the same preexisting parallel-test contention flakiness on the iter89e2-E test that the test docstring already calls out — passes deterministically with `--test-threads=1`.)
- **AC-PA1 (4-fixture decode parity):** PASS — all 4 SHA256 byte-identical to baseline (HEAD = iter89e2-E without iter89e2-F fusion):
  - `qwen3.6-27b-dwq46`:                `d4beb0e4a8191a3ad3d98fd402853c45f0b99210da701060364cde9b2aeff406`
  - `qwen3.6-35b-a3b apex Q5_K`:        `4cfaa826b45d93d9152fe43dec1189e6cd732f7cba2ab54bb2b13662a2f1385c`
  - `qwen3.6-35b-a3b q4_0-flat`:        `4cfaa826b45d93d9152fe43dec1189e6cd732f7cba2ab54bb2b13662a2f1385c`
  - `gemma-4-26B-A4B-it-ara dwq`:       `dce2ad832f6bb760f8d13f44cca3a79f1f96a4d6277abd10bb7b8da2333ac396`
  Receipts: `/tmp/adr019_phase2_parity/<fixture>_{baseline,phase2-iter89e2f}.sha`. Apex/q4_0-flat collision is incidental (greedy generation produces identical first 32 tokens despite different quantization) — pre/post comparison per-fixture is byte-identical.
- **AC-PA2 (Heisenbug 5×):** PASS — 5 cold-cache trials on 35B-A3B q4_0-flat all produce SHA `4cfaa826b45d93d9...`. Receipts: `/tmp/adr019_phase2_heisenbug/trial_{1..5}.sha`. FA-path is not the iter58b chunk-prefill zone but the gate is exercised per ADR-019:599.
- **AC-PA4 (test suite):** PASS 2708/2708 (vs `8305bf9` baseline 2708/2708). No new failures.
- **AC-P3 (decode no-regression):** PASS — 35B-A3B q4_0-flat 5-trial trimmed median 129.3 → 129.1 t/s (-0.15 %, well within ±5 %). Trials: 129.5, 117.4 (cold-load outlier dropped), 129.1, 128.7, 130.3 t/s.
- **AC-P6 (cmd_buf_count delta):** PASS — at chunk-engaged pp4528 on 27B-dwq46 (16 FA layers × 2 CBs/layer eliminated), `cmd_buf_count` drops **274 → 242 (Δ = -32 CBs, exact prediction match)**. `barrier_count` rises 688 → 720 (+32, the new intra-stage memory_barriers between ops4↔kv_write↔bridge). `dispatch_count` unchanged at 1426 (same kernel dispatches, just fewer CBs containing them). `sync_count` unchanged at 9 (`commit_labeled` is non-blocking).
- **AC-P1 (chunk-engaged pp4528 wall):** PASS — 5-trial trimmed median **7918 ms → 7766 ms (Δ = -152 ms, -1.92 %)** on 27B-dwq46 pp4528 chunk-engaged prefill. Per-CB cost validation: **152 ms / 32 CBs = 4.75 ms/CB measured** vs Phase 0a.4's 1.48 ms/CB prediction (the 3.2× gap is the GPU-side dispatch latency component that Phase 0a.4's isolated-CB measurement excluded; iter89e2-F's measured cost includes per-CB end-to-end execution overhead, not just CPU encode+commit).
- **F1-F12 fence preservation:**
  - F1 (persistent compute encoder): ADOPTED. One persistent compute encoder per Stage-A CB, lazy-opened by the first dispatch. ops1-4 + kv_write + bridge dispatches share that encoder via `memory_barrier()` insertions. The legacy non-fused branches retain their existing one-encoder-per-CB lifecycle.
  - F2 (residency-rescission, iter58b): PRESERVED. `FaPrefillArena`'s 7 BF16 scratches + `FaProjectionsArena`'s 10 F32 scratches + persistent KV slot's `slot.k`/`slot.v` are all owned by `forward_gpu_impl` through the output-head terminal `commit_and_wait_labeled` (`forward_gpu.rs:1701/1738`). The fused Stage-A CB is wider than any legacy CB (4 ops in flight vs 1), but no buffer can drop between dispatch encode and GPU completion because the arenas + KV slot all outlive every prefill CB. iter58b race structurally unreachable.
  - F11 (zero-init alloc): PRESERVED. The fused branch adds exactly ONE per-call allocation (`out_seq`), matching the wrapper's per-call shape at `gpu_full_attn.rs:1411-1413` byte-for-byte. No ad-hoc allocations introduced; `decode_pool::pooled_alloc_buffer` not on the fused path.
- **R6 mitigation (dump_bisect compatibility):** the within-layer `dump_in_layer` call sites (`gpu_full_attn.rs:2159-2380`) read arena buffers via `download_f32 → as_slice`, which requires a committed CB. With the fused Stage-A CB, those buffers' producer encoder is not yet committed when the dumps would run. The fusion predicate `use_fused_stage_ab` includes `!super::dump_bisect::is_enabled()`; when `HF2Q_DUMP_LAYER_*` is set, the legacy 4-CB path runs unchanged, keeping bisection viable.
- **Recommended next step:** iter89e2-G (extend Stage-A fusion to include ops6-7). Per design doc §6: the ops6-7 encoder currently opens at `gpu_full_attn.rs:2436` and commits at `gpu_full_attn.rs:2503`; folding it into Stage-A drops another 10-16 CBs per chunk-engaged prefill (10 FA layers for 35B-A3B, 16 for 27B-dwq46), targeting another ~50 ms wall improvement at the same per-CB cost. Note: the existing ops6→ops7 RAW barrier at `gpu_full_attn.rs:2482` is preserved; the new boundary is bridge→ops6 which adds one more `enc.memory_barrier()`.
- **Notes / methodology issues caught:**
  - Single-thread test runs are required (`--test-threads=1`) for the iter89e2-E byte-exact parity test under full-suite parallel scheduling — the test's `download_f32` racing other tests' GPU work can leave the wrapper's CB unexecuted while `_into`'s CB completes, producing a false positive. This is preexisting iter89e2-E behavior, not iter89e2-F regression.
  - During the perf measurement phase, `git stash pop` after a parallel `Wedge-4f` worker landed on trunk caused an `Auto-merging` checkout to a different branch state. Recovery: re-stash with `--include-untracked`, `git checkout adr019-phase2-iter89e2f-…`, `git stash pop`. This is the same hf2q-side concurrent-session-fence class flagged in `feedback_concurrent_src_workers_need_worktree`. Verified-correct measurements are the ones reported above (iter89e2-F branch HEAD = `57fd82f`, baseline = `8305bf9` after stashing).

**iter89e2-G LANDED 2026-05-02:**
- **Branch:** `adr019-phase2-iter89e2g-ops67-fusion-extension` (based off `b11396b` to inherit iter89e2-F's Stage-A fusion; merges to trunk at end of Phase 2).
- **Commit:** `b35b86a` (single commit; +68/-5 = +63 net in `src/inference/models/qwen35/gpu_full_attn.rs`; only the ops6-7 region modified).
- **What landed:** extends iter89e2-F's Stage-A fusion to include ops6-7. iter89e2-F's terminal `commit_labeled("layer.full_attn.stage_ab")` is REPLACED by moving the Stage-A encoder out of the inner ops1-4 fusion block into a function-scope `Option<CommandEncoder>` (`fused_stage_a_enc`). At the ops6-7 encoder open site (`gpu_full_attn.rs:2436`), takes the deferred encoder when present, inserts a NEW intra-CB `memory_barrier()` between fa.prefill_bridge's final `permute_021_bf16_to_f32` write to `out_seq` and ops6's `sigmoid_gate_multiply` read of `attn_out`, encodes `sigmoid_gate_multiply` + `linear_projection`, then issues ONE terminal `commit_labeled("layer.full_attn.stage_a")` covering all 4 FA-layer ops. The label rename `stage_ab` → `stage_a` reflects the now-complete single-stage partition from design doc §2.2 FINAL RECOMMENDATION.
- **What did NOT land:** cross-layer fusion (out of Phase 2 scope); non-fused branches (decode path, dump_bisect, head_dim != 256, cur_len != 0, missing arenas) keep the legacy "layer.full_attn.ops6-7" label and arena-vs-pool encoder-open semantics — preserved byte-exactly.
- **Tests:** 3275 / 3275 PASS with `cargo test --release --test-threads=1` (no new failures vs `b11396b` iter89e2-F baseline; total grew from F's 2708 due to intervening iter89e2-E byte-exact parity tests + Wedge-4f convert-from-HF tests landing). 48 ignored (preexisting).
- **AC-PA1 (4-fixture decode parity):** PASS — all 4 SHA256 byte-identical to iter89e2-F's published values:
  - `qwen3.6-27b-dwq46`:                `d4beb0e4a8191a3ad3d98fd402853c45f0b99210da701060364cde9b2aeff406`
  - `qwen3.6-35b-a3b apex Q5_K`:        `4cfaa826b45d93d9152fe43dec1189e6cd732f7cba2ab54bb2b13662a2f1385c`
  - `qwen3.6-35b-a3b q4_0-flat`:        `4cfaa826b45d93d9152fe43dec1189e6cd732f7cba2ab54bb2b13662a2f1385c`
  - `gemma-4-26B-A4B-it-ara dwq`:       `dce2ad832f6bb760f8d13f44cca3a79f1f96a4d6277abd10bb7b8da2333ac396`
  Receipts: `/tmp/adr019_phase2_parity/<fixture>_phase2-iter89e2g.sha`.
- **AC-PA2 (Heisenbug 5×):** PASS — 5 cold-cache trials on 35B-A3B q4_0-flat all produce SHA `4cfaa826b45d93d9152fe43dec1189e6cd732f7cba2ab54bb2b13662a2f1385c`. Receipts: `/tmp/adr019_phase2_parity/heisenbug_trial{1..5}.text`. The new RAW barrier at fa.prefill_bridge → ops6 is correctly placed (no determinism loss vs the legacy 4-CB layout where the CB boundary enforced this edge implicitly).
- **AC-PA4 (test suite):** PASS 3275/3275.
- **AC-P3 (decode no-regression):** PASS — 35B-A3B q4_0-flat 5-trial trimmed median 128.8 t/s (iter89e2-F: 129.1 t/s, delta -0.23%, well within ±5%). Trials: 128.8, 128.9, 129.1, 128.8, 128.8 t/s.
- **AC-P6 (cmd_buf_count delta):** PASS — at chunk-engaged pp4096 on 27B-dwq46 (16 FA layers × 1 CB/layer eliminated), `cmd_buf_count` drops **338 → 322 (Δ = -16 CBs, exact prediction match)**. `barrier_count` rises 1008 → 1024 (+16, the new intra-stage memory_barriers between fa.prefill_bridge and ops6). `dispatch_count` unchanged at 2002 (same kernel dispatches). `sync_count` unchanged at 153 (commit_labeled is non-blocking). Cumulative Phase 2 vs pre-iter89e2-F baseline: 4 CBs/FA-layer × 16 layers = -64 CBs (iter89e2-F: -32 + iter89e2-G: -16 = -48 measured here on pp4096; the remaining -16 is the difference between F's pp4528 baseline and G's pp4096 baseline).
- **AC-P1 (chunk-engaged pp4096 wall):** trimmed mean **7489.7 → 7481.7 ms (-8 ms, -0.11 %)** on 27B-dwq46 chunk-engaged prefill. Per-trial spread: F 7476/7485/7488/7496/7556 (stdev 32 ms), G 7429/7463/7491/7491/7496 (stdev 28 ms). Per-CB cost cross-check: **-8 ms / -16 CBs = 0.50 ms/CB measured**, vs iter89e2-F's 4.75 ms/CB on pp4528. The smaller per-CB delta suggests workload-size-dependent amortization at pp4096; the structural CB-count win is exact (-16 predicted, -16 measured) but wall-time amortization varies. iter89e2-F's 4.75 ms/CB measurement was on a longer prompt (pp4528) where CB overhead dominates more vs GPU work.
- **F1-F12 fence preservation:**
  - F1 (persistent compute encoder): ADOPTED — one persistent compute encoder per Stage-A CB now covers all 4 FA-layer ops (previously 2 ops covered).
  - F2 (residency-rescission, iter58b): PRESERVED — `FaPrefillArena`'s 7 BF16 scratches + `FaProjectionsArena`'s 10 F32 scratches (including `gated_buf` for ops6) + persistent KV slot + caller-owned `out_seq` (the FA bridge output) + `out` (linear_proj output, on Rust stack) all outlive the deferred CB by design — `forward_gpu_impl` owns the arenas through the output-head terminal `commit_and_wait_labeled`. The wider in-flight CB window (4 ops in flight vs F's 3 vs E's 1) has zero F2 exposure: no buffer can drop between dispatch encode and GPU completion.
  - F11 (zero-init alloc): PRESERVED — no new ad-hoc allocations beyond iter89e2-F's existing `out_seq` alloc.
  - F12 (HF2Q_FORCE_SERIAL_DISPATCH probe): PRESERVED — single Stage-A CB is one more `get_or_create_encoder` call per FA layer, not fewer.
- **Phase 2 status: COMPLETE.** iter89e2-E + iter89e2-F + iter89e2-G all landed on the cumulative feature chain. The single-stage per-FA-layer partition from design doc §2.2 FINAL RECOMMENDATION is fully realized: 4 → 1 CB per FA layer × 10 FA layers (35B-A3B) or 16 FA layers (27B-dwq46) on production-arena chunk-engaged or autoregressive prefill.
- **Recommended next step:** Phase 3a (DN-path conservative — 30 DN layers × ops1-3 + qkv_split consolidation). DN-path is the iter58b-DANGEROUS class, so Phase 3a stays conservative (preserve existing chunk-prep / chunk-attn fences) and chases sync_count drop from chunk-engaged ~30 + 30 → 1 + 30 (chunk-pipeline sync points unchanged). See §Phase 3 design constraints below.
...
- ADR-013 P21 closure: `project_adr013_p21_stage4_canonical_2026_05_02.md` (MEMORY) + `docs/adr/diary/ADR-013-qwen35-inference.md:2912-3011` — sync_count 161→6, pp80 199→582 t/s.
...
- ADR-013: `/opt/hf2q/docs/adr/diary/ADR-013-qwen35-inference.md` — P19 H9 (1.32 ms commit floor); P21 stages 1-4; iter58b regression entry.
- ADR-015: `/opt/hf2q/docs/adr/diary/ADR-015-mlx-native-single-cb-decode.md` — iter88a-89e measurement chain.
