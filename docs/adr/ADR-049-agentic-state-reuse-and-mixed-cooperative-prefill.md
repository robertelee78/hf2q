...
- *Dense/stride state capture* (FreeToken-granularity anchors): deferred. An off-boundary edit degrades to cold, never to wrong. Instrument divergence positions (A.8) first; if misses cluster off-boundary, prefer adding another semantic oracle (e.g. verified tool-call opener, FreeToken's `--enable-special-token-ckpt` analog) before any stride-mode kernel; a stride mode would be its own ADR with its own byte budget. Note (corrected FreeToken characterization): FreeToken itself records only the deepest crossed 64-boundary per forward, not every boundary — dense-stride economics were never validated even there.
...
## Rejections (studied, decided, documented — with reopen conditions)

| FreeToken concept | Verdict | Reason | Reopen when |
|---|---|---|---|
| q\* CPU–GPU co-execution | Rejected | One memory controller: CPU matmul adds no aggregate bandwidth to a BW-bound decode and cannot be bit-identical to Metal kernels (coherence gate). FreeToken's own degenerate limit (B_H→B_P ⇒ q\*→m) is this hardware's whole regime. | Never on unified memory; the *method* (measured contended-pair bench → closed-form policy) is reusable for genuinely disjoint domains. |
| Global LRU expert cache + elastic expert↔KV pools | Deferred | Target models fit wired in 128 GiB; no capacity misses exist. SSD-as-PCIe analog is ~5–7 GB/s vs their 25–53 — worse, strengthening deferral. | Targeting a model > DRAM or small-RAM Macs. Seam: deepseek4 mmap/residency layer (residency.rs:75-120). |
| Elastic pool rebuild control plane | Deferred (honestly: NOT covered in hf2q's idiom today — scheduler high-water is monotonic by design, scheduler.rs:1051) | Not justified for this scope. | A KV-grant resize feature; crib FreeToken's *ordering* (fit-check before destructive free; rollback on failure), not its mechanism. |
| bf16 chunk-pipeline state capture | Rejected | hf2q's experimental FLA-style chunk path materializes per-chunk states like FreeToken's, but in bf16 — restore is not byte-identical (bf16 accumulation already failed the W-5b.3 walk-bar at pp4096). | Only with an exact-F32 side channel, under its own ADR. |
| Dense 64-token stride anchors | Deferred | See A.7. FreeToken itself only keeps the deepest crossed boundary per forward. | Divergence telemetry shows off-boundary edit clustering. |
...
- Rejected/deferred paths are recorded with reopen conditions, so future sessions do not re-litigate them from scratch.
