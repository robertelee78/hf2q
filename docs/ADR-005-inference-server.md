...
- [ ] Decode speed matches or exceeds llama.cpp on the same hardware (≥107 tok/s on M5 Max, Gemma 4 26B MoE, Q4_K_M), measured using the canonical benchmark harness. **Post-1bNEW.10+1bNEW.12: 37.06 tok/s median (was 37.03 post-1bNEW.3, was 36.78 post-1bNEW.1, was 23.76 pre-1bNEW.1). 1bNEW.10+1bNEW.12 added ~0 tok/s on decode as expected — both items primarily target prefill/TTFT, not decode throughput. Remaining gap: 69.94 tok/s → owed to 1bNEW.4 (fused RmsNorm, 4–8 ms), 1bNEW.6 (fused RoPE, 3–6 ms), and a follow-up item that fuses the sliding-layer (head_dim=256) prefill path (blocked on upstream candle fixes for the bd=256 F32 threadgroup memory and bd=256 BF16 NaN sawtooth issues documented in the Walk Exception Register).**
