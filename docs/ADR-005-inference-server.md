...
- [ ] Decode speed matches or exceeds llama.cpp on the same hardware (≥107 tok/s on M5 Max, Gemma 4 26B MoE, Q4_K_M), measured using the canonical benchmark harness. **Post-1bNEW.4: 44.55 tok/s median (was 37.06 post-1bNEW.10+1bNEW.12, was 37.03 post-1bNEW.3, was 36.78 post-1bNEW.1, was 23.76 pre-1bNEW.1). 1bNEW.4 delivered +7.49 tok/s (+20.2%) on decode — the `norm_dispatches_per_token` collapse from 3521 to 331 (−90.6%) translated directly into `total_dispatches` collapsing from 717,870 to 308,930 (−56.9%), which is where the wall-clock saving lives. Remaining gap: 62.45 tok/s → owed to 1bNEW.6 (fused RoPE, 3–6 ms budgeted, but treat as ceiling per Phase-1b measurement discipline), and a follow-up MoE fusion item that further collapses `moe_dispatches_per_layer` from 34 → ≤ 4 (the ADR End-gate acceptance threshold), plus the sliding-layer (head_dim=256) prefill fusion follow-up blocked on the two upstream candle bugs in the Walk Exception Register.**
...
- [ ] `metrics.txt` at End gate shows: `moe_to_vec2_count == 0`, `sampler_sync_count ≤ 1`, `norm_dispatches_per_token ≤ 331` (corrected from the original `≤ 330` estimate — see 1bNEW.4 item detail for the counter accounting), `moe_dispatches_per_layer ≤ 4`. **Post-1bNEW.4: `moe_to_vec2_count == 0` ✓; `sampler_sync_count == 0.26` ✓; `norm_dispatches_per_token == 331` (down from 3521; target met at the corrected number) ✓; `moe_dispatches_per_layer == 34` (down from 42 after 1bNEW.4's router `rms_norm_unit` dropped from 9 ops to 1; still well above the `≤ 4` stretch target, further fusion owned by a follow-up MoE reduction item).**
