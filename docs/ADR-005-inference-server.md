...
**1bNEW.17 — Quantized lm_head via QMatMul (ports llama.cpp `build_lm_head` — proposed 2026-04-10 from post-Walk re-spike)**
...
- [ ] **Walk-correctness end gate: hf2q's top-1 token at decode 1 matches llama.cpp's top-1 token** on the canonical bench prompt (currently: hf2q=`The`, llama.cpp=`To`; both in each other's top-2). This is the precise success metric for Walk on the correctness axis. 1bNEW.1 does not flip this (kernel-level reductions were not the gap; top-10 changed by ≤3e-5 across the loop→fused migration).
- [ ] Decode speed matches or exceeds llama.cpp on the same hardware (≥107 tok/s on M5 Max, Gemma 4 26B MoE, Q4_K_M), measured using the canonical benchmark harness. **Post-1bNEW.6: 48.71 tok/s median (was 44.55 post-1bNEW.4, was 37.06 post-1bNEW.10+1bNEW.12, was 37.03 post-1bNEW.3, was 36.78 post-1bNEW.1, was 23.76 pre-1bNEW.1). 1bNEW.6 delivered +4.16 tok/s (+9.3%) on decode — the `dispatches_per_token` collapse from 2432 to 2192 (−9.9%, exactly `(10 - 2) × 30 = 240` as predicted) translated directly into wall-clock saving of ~1.7 ms/token. Cumulative Walk progress: **+24.95 tok/s, +105% vs the 23.76 tok/s pre-1bNEW.1 baseline**. Remaining gap: 58.29 tok/s → owed to a follow-up MoE fusion item that further collapses `moe_dispatches_per_layer` from 34 → ≤ 4 (the ADR End-gate acceptance threshold), plus the sliding-layer (head_dim=256) prefill fusion follow-up blocked on the two upstream candle bugs in the Walk Exception Register, plus likely a `candle::Tensor`-allocation-rate item once the kernel fusions have been exhausted.**
...
- [ ] `metrics.txt` at End gate shows: `moe_to_vec2_count == 0`, `sampler_sync_count ≤ 1`, `norm_dispatches_per_token ≤ 331` (corrected from the original `≤ 330` estimate — see 1bNEW.4 item detail for the counter accounting), `moe_dispatches_per_layer ≤ 4`. **Post-1bNEW.6: `moe_to_vec2_count == 0` ✓; `sampler_sync_count == 0.26` ✓; `norm_dispatches_per_token == 331` (down from 3521; target met at the corrected number) ✓; `moe_dispatches_per_layer == 34` (down from 42; still well above the `≤ 4` stretch target, further fusion owned by a follow-up MoE reduction item); `dispatches_per_token == 2192.52` (down from the pre-1bNEW.1 baseline of 7513, −70.8%).**
