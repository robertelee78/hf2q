...
- **Correctness risk:** LOW — MEDIUM downgraded to LOW by Q4 spike measurement (2026-04-10). F32 prefill was the conservative choice from 1b.4. BF16 cast at the SDPA boundary only (decode stays F32) is narrower but still a numerical change; the spike confirmed it preserves argmax and top-5 order on both the 187-token bench prompt and a 638-token adversarial-recall prompt with a factual needle at position 0 and its query at position 630. Max |Δp post-softmax| = 1.12e-3 on the needle prompt (at the ε=1e-3 bar) and 1.63e-2 on the bench prompt (concentrated on the already-near-tied `The`/`To` pair; direction of drift is *toward* llama.cpp's ordering).
- **Validation plan:** (1) Compare post-softmax attention weights BF16 vs F32 manual on a 128-token prefill at ε=1e-3. (2) Compare next-token top-5 distribution. (3) Layer A token match at a 128-token prompt prefix. (4) Adversarial: 2048-token document with specific-fact recall, output must match. **Q4 spike (2026-04-10) pre-validated (2) and (4) at 638 tokens. (1) and (3) remain the commit gate.**
...
- [ ] Decode speed matches or exceeds llama.cpp on the same hardware (≥107 tok/s on M5 Max, Gemma 4 26B MoE, Q4_K_M), measured using the canonical benchmark harness. **Post-1bNEW.3: 37.03 tok/s median (was 36.78 post-1bNEW.1, was 23.76 pre-1bNEW.1). Remaining gap: 69.97 tok/s → owed to 1bNEW.4 (fused RmsNorm, 4–8 ms), 1bNEW.6 (fused RoPE, 3–6 ms), 1bNEW.10 (BF16 prefill SDPA), 1bNEW.11 (contingent vec SDPA port), 1bNEW.12 (extended warmup).**
...
- [ ] `metrics.txt` at End gate shows: `moe_to_vec2_count == 0`, `sampler_sync_count ≤ 1`, `norm_dispatches_per_token ≤ 330`, `moe_dispatches_per_layer ≤ 4`. **Post-1bNEW.3: `moe_to_vec2_count == 0` ✓; `sampler_sync_count == 0.26` (down from 1.01, per-token-average; the End-gate threshold was set before 1bNEW.3's batched drain model and should be reinterpreted as "average ≤ 1", which it is — the batched path drains once per 4 tokens so the average is structurally ≤ 0.25 plus a small first-token adjustment) ✓; `norm_dispatches_per_token == 3521` (unchanged, owned by 1bNEW.4); `moe_dispatches_per_layer == 42` (down from 104, well above the `≤ 4` stretch target — further fusion owned by a follow-up MoE reduction item).**
