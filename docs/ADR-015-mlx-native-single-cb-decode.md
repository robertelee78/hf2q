...
- **2026-04-28 — iter11 LANDED — D4-style cold-SoC bench refresh = 0.9342×; xctrace MST kernel-name attribution path FALSIFIED for CLI; output-head verified small (6.8% / 0.55 ms); residual lives in 40 × `layer.attn_moe_ffn` @ 191 µs/layer on M5 Max apex 35B-A3B-dwq46.**  CFA session `cfa-20260428-adr015-iter11`, review-only mode (Claude implementer; bench is sequential by GPU contention, dual-mode would contaminate).  Profile-only: NO kernel/orchestration code changes shipped.
  ...
  **D4 first-bullet bench refresh (paired cold-SoC, hf2q binary at worktree HEAD `d71d12e`, `scripts/bench-baseline.sh` n_gen=256 trials=3 prompt="Hello, my name is", `mcp-brain-server` `kill -STOP` for the entire window):** hf2q median **112.000 t/s** (per-trial 108.9 / 112.0 / 112.4) vs llama-bench median **119.890 t/s** (per-trial 120.06 / 119.89 / 119.50, σ ≈ 0.23) = **0.9342×**.  Recovery required to clear D4 = **+7.04%**.  Δpp vs cfc5358 stage-1 0.9456× = **−1.14pp** (within same-day llama-bench drift envelope per `project_end_gate_reality_check`); Δpp vs iter10 stage-2 0.8676× = **+6.66pp** (stage-2 was reverted, never on main).  Bench-summary metadata cites `hf2q_git_head: df9152c` because the parallel ADR-014 session pushed iter-39…iter-41 to main during the 16-min bench window; the binary itself was built at the iter11 worktree HEAD `d71d12e` (W-5b.27 reverts present, W-5b.26 reverts present, no decode-path code between `d71d12e` and `df9152c` — the parallel commits are calibrate/quantize/test-only).  Brain-server `kill -CONT` confirmed post-bench (PID 1205, R state, 100% CPU).
  ...
  **iter12 candidate — per-CB GPU attribution on the llama-side at the same per-layer granularity.**  The hf2q-side breakdown is now solid: 40 × 191 µs/layer = 7.64 ms.  We do NOT have the equivalent llama breakdown.  If llama's same per-layer body is, e.g., 7.0 ms (175 µs/layer), the 540 µs/token gap is then attributable as 40 × ~16 µs per-layer arithmetic delta — a tight kernel cost ceiling, not a scheduling lever.  llama.cpp's `ggml-metal-context.m` exposes per-CB MTL timestamp logging via `LLAMA_LOG=metal` + `GGML_METAL_DEBUG=1`; iter12 would harness that with the same `bench-baseline.sh` cold-SoC discipline, parse the per-CB timestamps, and produce the missing llama half of the per-layer attribution table.  This is profile-only (no llama edits) and unblocks any source-level kernel comparison with concrete µs/layer deltas instead of "kernels are byte-equivalent" claims that are exhausted (`project_metal_compiler_auto_optimizes_static_levers` 10×-falsified track record).  iter12 is gated on the parallel ADR-014 session settling so the worktree is not racing.

  **Confounder caveats.**  (1) Persistent harness overhead: cmux@~19% CPU + sibling-claude@~17% CPU were live throughout the bench (M5 Max Claude harness — not killable for our session).  Both paired between hf2q and llama on the same machine in the same trial window, so Δhf2q-llama attribution is preserved; absolute per-trial t/s may be inflated by 5-15% vs a no-harness baseline, but the 0.9342× ratio is well-founded.  (2) Trial 1 hf2q at 108.9 t/s is 3.1% below trials 2-3 (112.0 / 112.4); cold-page-fault first-token cost shows in trial-1 even after the 60-s thermal settle (model + KV cache fault in across the first decode).  Excluding trial 1 the 2-trial median is 112.2 t/s → ratio 0.9359× (verdict unchanged).  (3) `mcp-brain-server` was `kill -STOP` (T state) for the entire bench window; `kill -CONT` confirmed post-bench (PID 1205, R state, 100% CPU returned).

  **Bench environment audit per trial:** thermal `pmset -g therm` clean every trial (no thermal warning level recorded any trial); RAM 80 GB available pre-bench (vs 30 GB minimum); per-trial process audit recorded at `/tmp/adr015-iter11/bench/baseline-apex-iter11-20260428T223249Z.{hf2q,llama}.trial-{1,2,3}.process-audit`.  Trial-2/3 hf2q audit clean; trial-1 hf2q audit clean modulo cmux/claude harness.  All 3 llama trials clean modulo cmux/claude harness.
