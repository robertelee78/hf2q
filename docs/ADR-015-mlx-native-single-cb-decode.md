...
**Prefill side: OPEN. Cold-clean baseline 0.319× on apex q4_0-flat (3.13× slower than llama). iter62 ROOT-CAUSED iter60b regression via per-bucket attribution + Codex kernel cross-review; the implicit-pipeline-sync hypothesis is FALSIFIED. iter63a (port llama's NRB=128 tile shape) is the queued primary candidate.**

**iter60b — B-staging elimination — REGRESSED, hypothesis FALSIFIED at iter62, DO NOT MERGE:**
...
- **iter62 cold-bench (apex q4_0-flat pp=4096, 5-of-6 trimmed median, alternating with main, mcp-brain-server SIGSTOP'd, vm_stat 4.8M pages free):** main median **4726ms** vs iter60b median **5149ms** = **+423ms / +8.95% REGRESSION** (full-median including 1 cold-cache-affected outlier = +578ms / +12.2%, consistent with the prior +595ms claim). Token-id correctness PASS for both branches (first decoded id=11). The "+595ms" cited in earlier sessions was apparently from a methodology including the binary-swap shader-cache penalty per the iter62 caveat below.
- **iter62 per-bucket attribution (HF2Q_PROFILE_GPU_TS=1, paired A1 vs A2 trial):** the +423ms wall is concentrated in single-matmul-per-layer dispatches (`layer.ops1_3` +445ms across 30 DN layers, `layer.chunk_ops8_9` +105ms × 30 DN, `fa.ops1_4` +72ms × 10 FA, `fa.ops6_7` +35ms × 10 FA) — **NOT** in `layer.chunk_call` (-32ms = noise) which has the highest dispatch density. The implicit-pipeline-sync hypothesis would predict cost between matmul dispatches proportional to dispatch density; the observed signature is the OPPOSITE: cost scales with matmul SIZE, not dispatch COUNT. **Hypothesis FALSIFIED.**
- **iter62 Codex kernel cross-review (read-only of /opt/llama.cpp):** llama.cpp's `kernel_mul_mm_q4_0_f32` template under `#ifdef GGML_METAL_HAS_TENSOR` already stages only A in threadgroup memory and wraps B directly from device memory — the same approach iter60b takes. llama.cpp's tensor path is fast on M5 Max (169µs/dispatch vs hf2q 191µs). If "remove sb removes implicit-sync" were universal, llama would have the same regression. It does not. Independent corroboration of the bucket-attribution falsification.
- **iter62 actual mechanism candidates (carrying into iter63):** iter60b's diff is FAR MORE than just "remove sb" — it also flips `transpose_result` from false→true in the matmul2d_descriptor, reverses type-parameter order in `get_destination_cooperative_tensor<>`, replaces the partial-tile slow-path writeback entirely with a single column-major device store, and reads B device-direct without threadgroup re-use across simdgroups in a threadgroup. Combined with the unchanged NR1=32 tile shape (vs llama's NRB=128, a 4× tile-area gap per the iter59 dossier addendum), the per-tile B re-fetch from L2/DRAM may be ~4× higher than llama's combination of NRB=128 + device-direct B. See iter63 directions below.
- Branch: `cfa/iter60b-impl-20260430/claude` (mlx-native `136bf98` + hf2q `23f761e`) — kept as evidence; **DO NOT MERGE TO MAIN**.

**iter62 bench-methodology finding (NEW caveat for downstream):** the bench worker discovered a "first run after binary swap" Metal-shader-cache penalty: 2-3× wall-clock penalty for the first invocation of a freshly-built (or recently-unused) binary on M5 Max. All 5 outliers across 20 trials in the iter62 session (A3=11055, B1=11346, M3=4192, M4=4888, M8=6247) are explained by this pattern. Future bench harnesses must run 1 throwaway warmup per binary before the alternating cold-trial loop OR insert a longer pause between binary swaps to let the Metal driver shader cache settle. The iter61c "+595ms" iter60b regression number was within noise of the trimmed-median +423ms; both are real regressions of the same character.

**iter60c — function-constant bounds-check gating — NO_REGRESSION, MERGE_OPTIONAL:**
...
- **iter62 cold-bench (apex q4_0-flat pp=4096, 5-of-6 trimmed):** main 4594ms vs iter60c 4374ms = **−220ms (within noise floor; baseline trial-range alone 4183-4638 = 455ms)**. Decode parity: byte-identical 27B-dwq46 32-tok decode at temp=0.0 vs baseline. iter62 verdict carries earlier verdict forward: **MERGE_OPTIONAL** (no perf cost, no perf benefit, mirrors llama.cpp `FC_mul_mm_bc_*` pattern).
- Pre-merge gate REMAINING: 4-fixture decode no-regression (only 27B-dwq46 covered in iter62; needs apex Q5_K, q4_0-flat, gemma-26b-dwq).
- Branch: `cfa/iter60c-fc-gate-20260430/claude` (mlx-native `4d38baf` + hf2q `5799f98`).

**iter59 next-tier candidates — ALL THREE RESOLVED:**
- **#1 multi-token NSG kernel — IMPLEMENTED, NO_REGRESSION at long-prefill, WIN at short-prefill (iter62 short-fixture bench).** Hypothesis CONFIRMED empirically: kernel inner `for t in 0..n_tokens` loop produces byte-identical output to legacy 128-thread kernel for n_tokens ∈ {1,2,4,8,16,32} (6 new parity tests). Rust guard relaxed via `NSG_PREFILL_MAX_TOKENS=32` + `d_k%32==0` check. Routes decode (seq==1) and short-prefill autoregressive (seq_len≤32 non-chunk-eligible). Tests now 266/0/x. **iter62 short-prefill bench (seq_len=24, fixture `/tmp/cfa-iter62/short32-prompt.txt`):** wall-clock noisy (1677-6247 ms range due to Metal shader-cache warming), but the targeted bucket `layer.autoreg_ops5_9` drops from 30.5ms (baseline) to 13.7ms (mtnsg) = **2.2× speedup** on the predicted code path. Token-id parity PASS at seq_len=24 (first decoded id 198 on both branches). Branch: `cfa/iter59-mtnsg-impl-20260501/claude` (mlx-native `a3c7e94` + hf2q `00a7f62`) — **MERGE_OPTIONAL** (mechanism confirmed; pre-merge gate remaining: 4-fixture decode no-regression).
...
**Empirical bench summary table (apex q4_0-flat pp=4096, iter62 cold-bench, baseline = parent of each branch):**

| Branch | Cold trimmed median (ms) | Δ vs paired baseline | Verdict | Merge |
|---|---|---|---|---|
| iter60b (5-of-6 trimmed) | **5149** | **+423ms (+8.95%)** | **REGRESSED — implicit-pipeline-sync hypothesis FALSIFIED** | **DO NOT MERGE** |
| iter60c (5-of-6 trimmed) | 4374 | −220ms (within baseline noise 455ms) | NO_REGRESSION | OPTIONAL |
| iter59-mtnsg (long-prefill) | 5897 (prior session) | -24ms (-0.41%) | NO_REGRESSION at pp4096; **WIN at seq_len=24 short-prefill (autoreg_ops5_9 30.5→13.7 ms = 2.2×)** | OPTIONAL |

iter62 receipts: `/tmp/cfa-iter62/results/{A-VERDICT,B-60c-VERDICT,B-mtnsg-VERDICT}.md`; raw bench logs at `/tmp/cfa-iter62/results/{A-iter60b-rootcause,B-iter60c,B-mtnsg}/logs/`.
...
8. **iter62 root-cause investigation** — per-bucket GPU profile (HF2Q_PROFILE_GPU_TS=1) on iter60b vs parent at apex q4_0-flat pp4096; 6 alternating cold trials; Codex kernel cross-review of /opt/llama.cpp `mul_mm` template; verdict written to `/tmp/cfa-iter62/results/` with three branch verdicts and iter63a NRB=128 candidate queued. ADR doc commit on this branch (current).

### Repo HEADs at pause

- hf2q HEAD: this iter62 doc commit (current branch `adr017-iter17-2026-05-01`; parallel ADR-005, ADR-013, ADR-018 sessions all advance HEAD frequently — `git pull` before resuming)
- mlx-native HEAD: `0050329` (Release 0.6.3); no mlx-native change in iter62 (measurement-only iter)
- qwen35:: test count not re-counted in iter62 (no source changes in this branch beyond ADR doc)
- Both repos clean of iter62-related WIP; pre-existing stashes from older sessions retained per their original notes

### What's NEXT (in order)

**iter63a + iter63c POST-MORTEM (2026-05-01 ~22:30Z): the tile-shape-is-the-missing-piece hypothesis is REFUTED at two levels.**
- **iter63a** (mlx-native branch `cfa/iter63a-nr1-128-20260501/claude` HEAD `82b1c2f`, pushed): minimum-diff `NR1=32→128` on iter60b base. Parity 1/11 PASS. All 10 failures: M < NR1=128 produces `mm = 0` at in-bounds output positions where mv has valid non-zeros. **MPP `cT.store(tD.slice(r1, r0))` does NOT safely clamp partial M-axis tiles when cooperative-tensor shape (128×64) ≫ destination slice extent.**
- **iter63c** (mlx-native branch `cfa/iter63c-partial-tile-20260501/claude` HEAD `05abc9a`, pushed): re-introduced partial-tile slow-path writeback + bumped SHMEM_BYTES to 32768B. Parity 6/11 PASS. **Sharper diagnostic:** the bug is not just writeback clamping — `mm.run` itself drops simdgroup-row contributions (rows 32-63 owned by sgitg=1) when the live B-slice M-extent is < NR1=128. Three `sc` shape variants (column-major, row-major, flat-index) produced identical failures — no write-back mechanism can recover data the matmul itself did not produce.
...
4. **Parallel session is exploring `iter63-nr1-64`** (`/opt/mlx-native/.cfa-worktrees/iter63-nr1-64-claude` at `74b42c5`). May still drop sgitg=1 at M=33 per iter63c finding — independent verification needed; coordinate with whoever owns that session.
5. **Decide iter60c + iter59-mtnsg disposition.** Both NO_REGRESSION at iter62; iter60c mirrors llama.cpp `FC_*` pattern, iter59-mtnsg has confirmed 2.2× speedup on seq_len=24 short-prefill fixture. Pre-merge gate remaining: 4-fixture decode no-regression (apex Q5_K, q4_0-flat, gemma-26b-dwq beyond the iter62-tested 27B-dwq46). 4-fixture worker in flight; verdict pending.
6. **DO NOT pursue an "explicit threadgroup-barrier after each B-stride load" fix.** That treats the iter62-FALSIFIED implicit-pipeline-sync hypothesis.
7. **iter62 — Q4_K MoE matmul-id support**: handled by parallel `adr013-q4k-moe-kernels` session.
8. **Next-tier candidates** (queued from iter59 dossier index): full RoPE+QKV fusion (3-4 days, medium-high risk; only worth it if cumulative gain after the above batch leaves significant prefill gap).

**Critical methodological lesson from iter60b → iter62**: static analysis + parity tests + ADR coordinator hypothesis are ALL starting points. iter60b's "removed implicit pipeline-sync" hypothesis survived the +595ms regression measurement (which it actually predicted) but FALSIFIED at iter62 once per-bucket attribution showed the regression signature is matmul-SIZE-scaled, not dispatch-density-scaled. **Code+test==truth**: per-bucket GPU profile + peer-source kernel cross-review is the load-bearing evidence; the wall-time aggregate alone cannot distinguish "kernel slower" from "synchronization injected". Future similar iters must include per-bucket attribution before any retry design.

**iter62 methodological lesson (NEW)**: Metal shader-cache penalty 2-3× wall-clock on first invocation of a freshly-built (or recently-unused) binary. Bench harnesses must run 1 throwaway warmup per binary OR insert longer pause between binary swaps. iter62 trimmed-median methodology dropped 5 outliers across 20 trials cleanly attributable to this.
...
- **2026-05-01 (UTC ~22:30Z) — iter63a + iter63c — tile-shape hypothesis REFUTED at parity at two independent diff levels; iter63 GPU profiling kit becomes the load-bearing path.**
  ...
  **Method.** iter63a research worker (read-only) produced a complete design at `/tmp/cfa-iter63a/research/DESIGN.md` (42KB, 5 sections + 2 appendices) showing iter63a is a 10-LOC change (`NR1=32→128` at `mlx-native/src/shaders/quantized_matmul_mm_tensor.metal:189` + matching dispatcher constant in `quantized_matmul_ggml.rs:517` + drop SHMEM_BYTES 8192→4096) on top of iter60b's device-direct B base. K-loop body, descriptor, write-back math, and dispatch grid are all symbolically NR1-parameterized; no algorithmic changes needed. Pre-registered falsifiable hypothesis: `layer.ops1_3` recovers from 668ms (iter60b) to ≤230ms (parent baseline 223ms ± 5ms); wall-clock ≤4726ms. Falsifies if recovery <+200ms.

  **iter63a verdict — FALSIFIED at parity gate (1 PASS / 10 FAIL).** Branch `cfa/iter63a-nr1-128-20260501/claude` (mlx-native HEAD `82b1c2f`, pushed). The 10 failing tests in `cargo test --release --test test_quantized_matmul_mm` all share the property M < NR1=128: they produce `mm = 0` at in-bounds output positions where `mv` (the simdgroup-MMA reference path) has valid non-zero values. **MPP `cT.store(tD.slice(r1, r0))` does NOT safely clamp partial M-axis tiles when the cooperative-tensor shape (128×64) is much larger than the destination slice extent.** This empirically falsifies the design's Appendix B Q1 assumption. Bench was NOT run per the brief's STOP-at-parity-fail directive. Time elapsed ~7 min wall — extremely fast falsification because the parity test is byte-exact and the partial-tile failure is deterministic.

  **iter63c — fall-back attempt: re-instate partial-tile slow-path writeback (Codex review #2 H2 alternative); also FALSIFIED at parity (6 PASS / 5 FAIL).** Branch `cfa/iter63c-partial-tile-20260501/claude` (mlx-native HEAD `05abc9a`, +82/-24 LOC, pushed). Bumped SHMEM_BYTES to NR0×NR1×4=32768B (= M5 Max max_threadgroup_memory_length). Three slow-path variants tried (column-major sc, row-major sc, flat-index copy) — all produced IDENTICAL failure pattern. **Sharper diagnostic (the load-bearing finding):** the bug is NOT writeback clamping — `mm.run` itself drops simdgroup-row contributions when the live B-slice M-extent < NR1=128. All 5 remaining failures are at row indices in simdgroup-1's region (rows 32-63); data is missing from `sc` even after `cT.store(tC)`. No write-back mechanism can recover data the matmul itself did not produce. **NR1=128 with the M3+ MPP cooperative-tensor path requires M ≥ 128 for byte-exact output.** llama.cpp's geometry works only because their typical M (full-prompt prefill) is ≫ 128.

  **Production blast radius (load-bearing).** hf2q's chunk-prefill uses M=64 EXACTLY (`CHUNK_THRESHOLD` constant in `src/inference/models/qwen35/gpu_delta_net.rs:105`). The 3 `test_*_mm_matches_mv_prefill_shape` failures at M=64 are the production chunk shape exactly. **Shipping iter63a or iter63c would silently corrupt every prefill chunk.** DO NOT MERGE either branch.

  **Implication for iter62's matmul-size-scaled `layer.ops1_3` regression.** iter62 attribution showed iter60b's `layer.ops1_3` was +445ms vs main, scaling with matmul SIZE not dispatch density. iter63a + iter63c demonstrate the unchanged tile shape (NR1=32) is NOT the mechanism. The actual mechanism must be one of iter60b's OTHER changes (transpose_result toggle, type-param swap in `get_destination_cooperative_tensor<>`, partial-tile writeback removal, OR device-direct B reads). Localizing among these requires per-DISPATCH GPU counter measurement.

  **Standing rule reinforced.** `feedback_evidence_first_no_blind_kernel_rewrites` — even with iter62's per-bucket attribution as evidence, the tile-shape-is-the-missing-piece hypothesis was not load-bearing because the per-bucket attribution couldn't distinguish "tile shape" from "any other matmul-size-scaled change in iter60b's diff". Higher-resolution measurement (per-dispatch GPU cycle counters) is required to resolve.

  **Pivot — iter63 GPU profiling kit becomes the load-bearing path.** Research worker produced the full design + impl playbook at `/tmp/cfa-iter63/research/{PROFILING-KIT-DESIGN.md,PROFILING-KIT-PLAYBOOK.md}` (495 + 375 lines). Key findings: (a) `metal-rs 0.33` (existing dep at `Cargo.toml:33`) wraps every API needed — no FFI shim, no objc2-metal upgrade; (b) total ~430 LOC across mlx-native, 0 in hf2q; (c) env-gated by `MLX_PROFILE_DISPATCH=1` (mirrors existing `MLX_PROFILE_CB=1` pattern from `kernel_profile.rs`) and `MLX_METAL_CAPTURE=/path.gputrace` (mirrors llama.cpp's `GGML_METAL_CAPTURE_COMPUTE` at `ggml-metal-context.m:166`); (d) critical caveat: per-dispatch sampling with `with_barrier:YES` (mandatory under `MTLDispatchTypeConcurrent` at `encoder.rs:746-751`) **serializes the encoder**, so per-dispatch numbers are upper-bound (production-overlap masked); cross-validation: sum-of-per-dispatch gpu_ns per CB must match per-CB ground truth ≤5%; production decode runs always with both env vars unset. iter63 impl in flight on branch `cfa/iter63-profiling-kit-20260501/claude` worktree at `/tmp/cfa-iter63/impl/`.

  **Methodological lessons banked.**
  1. **Fast falsification at parity is cheap signal.** iter63a returned in 7 min, iter63c in 50 min. Both falsifications cost <1 hr of wall while preserving the iter63 profiling-kit research+impl in parallel.
  2. **iter62's per-bucket attribution was necessary but not sufficient.** "Cost scales with matmul SIZE" can't distinguish "tile too small" from "any other matmul-altering change". Higher-resolution counters needed.
  3. **MPP cooperative-tensor partial-tile semantics on M3+ Tensor Cores require M ≥ NR1.** This is a load-bearing constraint for any future tile-shape work; document in design docs.
  4. **The parallel-session `iter63-nr1-64` worktree** (`/opt/mlx-native/.cfa-worktrees/iter63-nr1-64-claude` at `74b42c5`) is exploring NR1=64; per iter63c's diagnostic, may still drop sgitg=1 at M=33. Independent verification needed.

  **STANDING COMPLIANCE.**
  - `feedback_evidence_first_no_blind_kernel_rewrites` — tile-shape change attempted only after iter62 per-bucket evidence; falsified at byte-exact parity test before any bench.
  - `feedback_correct_outcomes` — both iter63a and iter63c stopped at parity fail per brief's STOP rule; no shortcuts attempting "fix the kernel" beyond scope.
  - `feedback_no_shortcuts` — branches preserved as evidence (pushed to origin); not deleted.
  - `feedback_use_cfa_worktrees` — iter63a, iter63c, iter63 profiling-kit research, iter63 profiling-kit impl, 4-fixture decode all in their own worktrees.
  - `feedback_git_commit_pathspec_when_parallel` — parallel ADR-005 / ADR-013 sessions advanced HEAD during iter62→iter63 work; only `docs/ADR-015-mlx-native-single-cb-decode.md` modified by this iter; fences honored on `gpu_full_attn.rs`, `engine.rs`, `registry.rs`, `engine_qwen35.rs`, `mmproj.rs`, `vit_gpu.rs`, `handlers.rs`, `state.rs`, ADR-005 doc.

  **Receipts.**
  - iter63a: `/tmp/cfa-iter63a/research/DESIGN.md` (research, 42KB), `/tmp/cfa-iter63a/impl-bench/VERDICT.md`, `/tmp/cfa-iter63a/results/PARITY-FAIL.md`, `/tmp/cfa-iter63a/results/parity-mm.log`
  - iter63c: `/tmp/cfa-iter63c/impl-bench/VERDICT.md`, `/tmp/cfa-iter63c/results/PARITY-FAIL.md` (failure-pattern dossier with row-by-simdgroup table; 4 sc-shape variants tried), `/tmp/cfa-iter63c/results/parity-mm.log`
  - iter63 profiling kit: `/tmp/cfa-iter63/research/PROFILING-KIT-DESIGN.md` (495 lines), `/tmp/cfa-iter63/research/PROFILING-KIT-PLAYBOOK.md` (375 lines)

  Status: iter63a + iter63c CLOSED-FALSIFIED-AT-PARITY (branches preserved as evidence; DO NOT MERGE). iter63 profiling kit IS the next-iter load-bearing infrastructure (impl in flight). iter62's `layer.ops1_3` regression mechanism is now narrowed to one of iter60b's 4 non-tile changes; localization requires the kit.

- **2026-05-01 (UTC ~22:00Z) — iter62 — iter60b root-cause attribution: implicit-pipeline-sync hypothesis FALSIFIED at per-bucket profile + Codex kernel cross-review; iter63a (port llama's NRB=128 tile shape) queued as primary candidate.**

  **Method.** Single CFA dual-mode session (1 perf-engineer + 1 doc-prep researcher + 2 read-only Codex kernel cross-review passes), all in worktrees, hardware-bound so codex sandbox = read-only auxiliary only. **A — `HF2Q_PROFILE_GPU_TS=1` per-bucket GPU wall-clock attribution** on iter60b branch (`cfa/iter60b-impl-20260430/claude`, mlx-native `136bf98` + hf2q `23f761e`) vs the parent commit (mlx-native `0050329`); apex q4_0-flat pp=4096; 6 alternating cold trials (iter60b T1, baseline T2, iter60b T3, baseline T4, iter60b T5, baseline T6); 50-60s effective cooldown; mcp-brain-server SIGSTOP'd; vm_stat 4.8M pages free pre-bench. Bench prompt regenerated byte-identical to W-5b.10's `/tmp/walkbar-pp4096-prompt.txt` SHA `62e66013996f725c794d53fa9136f43c1b9eca0e`. Token-id correctness PASS (first decoded token id=11 across all 6 trials, both branches). **B — Codex read-only kernel cross-review** of `/opt/llama.cpp/ggml/src/ggml-metal/ggml-metal.metal` `kernel_mul_mm_q4_0_f32` template under `#ifdef GGML_METAL_HAS_TENSOR` vs `/opt/mlx-native/src/shaders/quantized_matmul_mm_tensor.metal` (current main + iter60b diff `git -C /opt/mlx-native diff main..136bf98`).

  **Bench result — wall-clock (apex q4_0-flat pp=4096, prefill ms for 4096 tokens, trimmed median dropping 1 outlier per branch attributable to Metal shader-cache):** iter60b 5149ms vs baseline 4726ms = **+423ms / +8.95% REGRESSION**. Full-median (including 1 cold-cache outlier) = +578ms / +12.2%, consistent with the prior +595ms claim from the iter60-bench session that motivated iter60b's DO NOT MERGE verdict.

  **Per-bucket attribution (paired clean adjacent trials A1 iter60b vs A2 baseline; ms summed across all per-layer invocations):**

  | Bucket | iter60b | baseline | Δ (ms) | Note |
  |---|---:|---:|---:|---|
  | layer.ops1_3 (×30 DN) | **668.1** | **222.9** | **+445.2** | DN pre-norm + qkv proj + z proj + ssm_conv (single-matmul-per-layer) |
  | layer.qkv_deinterleave (×30) | 27.3 | 29.2 | −1.9 | noise |
  | layer.chunk_call (×30 DN chunks) | 1075.6 | 1107.6 | −32.0 | **noise (slight win)** — highest dispatch density bucket |
  | layer.chunk_ops8_9 (×30 DN) | **178.3** | **73.1** | **+105.2** | post-chunk norm + output proj |
  | layer.linear_total (DN whole) | 2722.3 | 2264.9 | +457.4 | sum of DN-side buckets |
  | layer.full_total (×10 FA) | 739.3 | 653.2 | +86.1 | FA path total |
  | fa.ops1_4 (×10 FA) | **195.7** | **123.7** | **+72.0** | FA pre-norm + Q/K/V proj + RoPE |
  | fa.sdpa.kernel (flash_attn_prefill) | 223.7 | 233.0 | −9.3 | **FA kernel itself unaffected** |
  | fa.ops6_7 (×10 FA) | **78.1** | **43.4** | **+34.7** | FA output proj |
  | layer.post_attn_fused_norm (×40) | 106.0 | 115.9 | −9.9 | noise |
  | layer.ffn_dispatch (×40) | 855.0 | 911.5 | −56.5 | noise (slight win) |

  Cross-trial confirmation (A5 iter60b vs A4/A6 baselines): same per-bucket fingerprint — `layer.ops1_3` 646.1 ms (vs baseline 220.2 ms median, +426 ms), `layer.chunk_ops8_9` 177.7 ms (vs 77.2, +100 ms), `fa.ops1_4` 193.6 ms (vs 125.9, +68 ms), `fa.ops6_7` 80.1 ms (vs 49.5, +31 ms). The pattern reproduces.

  **Verdict — implicit-pipeline-sync hypothesis FALSIFIED.** The hypothesis predicts cost should appear *between* matmul dispatches proportional to dispatch density. Highest density is `layer.chunk_call` (6 sub-kernels per DN layer × 30 DN layers). Observed: `layer.chunk_call` is unchanged (−32 ms = noise). The single-matmul-per-layer dispatches are the only buckets with regression. Per-call cost shape: `layer.ops1_3` is +14.5 ms/layer × 30 DN; `layer.chunk_ops8_9` is +3.5 ms/layer × 30 DN. The qkv-proj inside `ops1_3` operates on a much larger M dimension than the output proj inside `chunk_ops8_9`. **Cost scales with matmul SIZE, not dispatch COUNT — the signature of a per-kernel-execution slowdown, not a sync-fence cost.** FA path is also affected (`fa.ops1_4` +72 ms, `fa.ops6_7` +35 ms) while `fa.sdpa_total` shows no regression — so the FA-layer regression is in the pre-/post-attention matmul dispatches (also `mul_mm`), reinforcing matmul-kernel regression vs sync regression.

  **Codex kernel cross-review (read-only, /opt/llama.cpp).** llama.cpp's `kernel_mul_mm_q4_0_f32` template under `#ifdef GGML_METAL_HAS_TENSOR` already stages only A in threadgroup memory (`sa`) and wraps B directly from device memory via `tensor<device float, ...>` — the same approach as iter60b. llama.cpp's tensor path is fast on M5 Max (169µs/dispatch vs hf2q 191µs). If "remove sb removes implicit-sync" were a universal mechanism, llama would exhibit the same regression. Independent corroboration of the bucket-attribution falsification.

  **iter60b actual diff is FAR MORE than `sb` removal.** Direct read of `git diff main..136bf98 -- src/shaders/quantized_matmul_mm_tensor.metal` shows iter60b also: (a) flips `transpose_result` from false→true in the `matmul2d_descriptor` call, (b) reverses type-parameter order in `get_destination_cooperative_tensor<>` from `<decltype(tA), decltype(tB), float>` to `<decltype(tB), decltype(tA), float>`, (c) replaces the partial-tile slow-path writeback (which handled `ne0/ne1 % NR0/NR1 != 0` via a threadgroup staging + simdgroup row-copy with M-bound check) with a single column-major device store via `tensor<device float, ...>(dstBatch, ..., {1, args.ne1})`, (d) reads B from device on every K-tile without threadgroup-tile re-use across simdgroups in the threadgroup. Combined with the unchanged NR1=32 tile shape (vs llama's NRB=128, a 4× tile-area gap per the iter59 dossier addendum), per-tile B re-fetch from L2/DRAM may be ~4× higher than llama's NRB=128 + device-direct B combination.

  **iter63 design (queued; do NOT pursue an "explicit-sync" replacement of iter60b — that addresses the falsified hypothesis):**

  - **iter63a HIGHEST PRIORITY** — port llama's NRB=128 tile shape into `quantized_matmul_mm_tensor.metal`. Expected: `layer.ops1_3` drops back to ≈220ms; iter60b's +423ms regression closes; the cast-elimination win remains.
  - **iter63b FALLBACK** — hybrid: keep iter60b's no-cast pipeline but reintroduce `tensor<threadgroup float, dextents<int32_t,2>>` (instead of `half`) as a B-tile cache. Trades cast-op savings for L2-prefetch benefit.
  - **iter63c FALLBACK** — re-instate the partial-tile slow-path writeback adapted to the new column-major orientation (~30 LOC).
  - **iter63d FALLBACK** — swap type-parameter order back in `get_destination_cooperative_tensor<>` (cheapest probe, 1-line change).

  **Bench-methodology finding (NEW caveat).** Discovered "first run after binary swap" Metal-shader-cache penalty: 2-3× wall-clock penalty for the first invocation of a freshly-built (or recently-unused) binary. All 5 outliers across 20 iter62 trials (A3=11055, B1=11346, M3=4192, M4=4888, M8=6247) are explained by this pattern. Future bench harnesses must run 1 throwaway warmup per binary OR longer pause between binary swaps. The "+595ms iter60b regression" cited in the iter60-bench session is consistent with iter62's full-median +578ms (including outlier); the trimmed +423ms is the more defensible number.

  **Companion bench results (carried by the same bench-worker session):**

  - **iter60c** (`cfa/iter60c-fc-gate-20260430/claude`, mlx-native `4d38baf` + hf2q `5799f98`) — re-bench at apex q4_0-flat pp=4096 trimmed-median wall iter60c=4374ms vs baseline=4594ms (Δ=−220 ms within baseline trial-range alone of 455ms; statistically NO_REGRESSION). 27B-dwq46 32-tok decode at temp=0.0 byte-identical between iter60c and baseline. **Verdict carries forward: MERGE_OPTIONAL** (mirrors llama.cpp `FC_*` pattern); pre-merge gate remaining is 4-fixture decode no-regression (only 27B-dwq46 covered in iter62).
  - **iter59-mtnsg** (`cfa/iter59-mtnsg-impl-20260501/claude`, mlx-native `a3c7e94` + hf2q `00a7f62`) — short-prefill bench constructed at `/tmp/cfa-iter62/short32-prompt.txt` (24 prompt tokens, in NSG_PREFILL_MAX_TOKENS=32 window). Wall-clock noisy due to Metal shader-cache (1677-6247 ms range) but per-bucket profile clean: target bucket `layer.autoreg_ops5_9` drops from 30.5ms (baseline) to 13.7ms (mtnsg) = **2.2× speedup** on the predicted code path (eliminating 128-thread `threadgroup_barrier` stalls in autoregressive prefill DN path for short non-chunk-eligible sequences). Token-id parity PASS at seq_len=24 (first decoded id 198 on both branches). **Verdict: MERGE_OPTIONAL** (mechanism win confirmed); pre-merge gate remaining is 4-fixture decode no-regression.

  **STANDING COMPLIANCE.**
  - `feedback_evidence_first_no_blind_kernel_rewrites` — per-bucket profile + kernel cross-review BEFORE any iter63 design; no source changes in this iter.
  - `feedback_dont_guess` — hypothesis was pre-registered at iter60b closure; iter62 falsifies it empirically.
  - `feedback_correct_outcomes` — measurement-only iter; no shortcuts.
  - `feedback_bench_process_audit` — mcp-brain-server SIGSTOP; pre-bench `vm_stat` 4.8M pages free.
  - `feedback_verify_baseline_determinism_before_perf_bench` — token-id correctness PASS for both branches (first decoded id=11 across all 6 trials).
  - `feedback_git_commit_pathspec_when_parallel` — doc-only commit: `docs/ADR-015-mlx-native-single-cb-decode.md` ONLY; ADR-005 + ADR-013 P21 file fences honored (`src/serve/api/engine.rs`, `src/serve/api/registry.rs`, `src/inference/models/qwen35/gpu_full_attn.rs`, `docs/ADR-005-inference-server.md` not touched).
  - `feedback_use_cfa_worktrees` — bench worker, doc worker, codex review all in their own worktrees / sandboxes.

  **Receipts:**
  - Bench logs: `/tmp/cfa-iter62/results/A-iter60b-rootcause/logs/A1-A6.log`, `/tmp/cfa-iter62/results/B-iter60c/logs/B1-B6.log`, `/tmp/cfa-iter62/results/B-mtnsg/logs/M1-M8.log`
  - Verdict markdowns: `/tmp/cfa-iter62/results/{A-VERDICT,B-60c-VERDICT,B-mtnsg-VERDICT}.md`
  - Codex findings: `/tmp/cfa-iter62/results/{CODEX-FINDING-1,CODEX-FINDING-2}.md`
  - Bench prompt regenerated: `/tmp/walkbar-pp4096-prompt.txt` SHA `62e66013996f725c794d53fa9136f43c1b9eca0e` (byte-identical to W-5b.10)
  - Saved binaries: `/tmp/cfa-iter62/baseline/hf2q-{iter60b,iter60c,mtnsg}-saved`, `hf2q-baseline-{iter60b,iter60c,mtnsg}-parent` (md5-verified pre/post bench)

  Status: iter62 CLOSED-MEASUREMENT-ONLY; iter63a opens as primary next-iter (NRB=128 tile shape port from llama.cpp); iter60c + iter59-mtnsg MERGE_OPTIONAL pending 4-fixture decode no-regression gate.

- **2026-04-30 — iter61a-3 PARTIAL — per-op bisection scaffold landed; FIRST DIVERGENCE PINNED + Heisenbug surfaced; fix requires iter61a-4.**  Worktree `fix/adr015-iter61a-3-bisection-scaffold` (branch from main `e20b269`). **S1 Scaffold landed:** new module `src/inference/models/qwen35/dump_bisect.rs` (290 LOC) gated by `HF2Q_DUMP_LAYER` env (`ALL` / `<N>` / unset = no-op) + `HF2Q_DUMP_RUN_ID=<id>` (defaults to PID). Writes full hidden buffers (not just last-token like the legacy `dump_layer_bin`) as raw little-endian f32 to `/tmp/hf2q-dump/<run_id>/step{step:04}_layer{NNN|___}_{op}.f32` plus a manifest. 4 unit tests pass (filter logic + GPU synthetic round-trip). Driver script `scripts/iter61a3-bisect.sh` runs two cold processes then byte-diffs. Wired call sites: `forward_gpu_impl` (prefill — embed, attn_out, ffn_out, layer_out per layer + output_norm), `forward_gpu_greedy` (decode — embed, ffn_residual, layer_out per layer + output_norm + argmax_logits), and within-layer dumps inside `build_gated_attn_layer` (x_norm, q/k/v/gate_flat, q/k_normed, q/k_rope, sdpa_out) via thread-local current-layer tag. **S2 Bisection result (FIRST PASS, layer-end dumps only, on hf2q+mlx-native v0.6.2 stack):** layers 0-2 (DeltaNet) byte-identical across cold runs; FIRST divergence at `step0000_layer003_attn_out` byte offset 20992 (= float 5248 = token 1, position 128 of post-wo_proj output, hidden_size=5120). Per-token analysis: token 0 IDENTICAL, tokens 1-10 fully diverge (mean abs diff 0.034, MAX abs diff 50.67 at token 9 — wildly different, not ULP noise). **S3 Mechanism — Heisenbug:** when within-FullAttn op-dumps were ADDED (each adds an empty-encoder `commit_and_wait` flush via `dump_bisect::dump`), the divergence MOVED from layer 3 to layer 63 (last layer). Adding sync barriers at every op pushes the divergence point later — **the bug is a RACE CONDITION, not kernel non-determinism.** Most consistent with **H-D-3 (Metal command-buffer ordering across cold-process schedules)** in the fused single-CB encoder path: an async `commit_labeled` somewhere lacks proper synchronization with the next consumer's read. H-D-1 (lazy pipeline compile) ruled out because the divergence pattern shifts with EVERY added barrier, not just first-call. H-D-2 (page-fault order) ruled out because `MlxDevice::alloc_buffer` (iter61a) and `MlxBufferPool::alloc_inner` (iter61a-2) both already zero-init fresh allocations. **S4 Fix — DEFERRED to iter61a-4:** the Heisenbug nature means the actual mis-synchronized encoder boundary is masked whenever we dump to observe it. Need a different probe (e.g. `MLX_PROFILE_CB=1` which forces sync mode globally — if that goes deterministic, confirms async-commit race). **S5 Validation:** 10/10 byte-identical gate NOT MET — left to iter61a-4. cargo test qwen35:: 166/166 PASS. cargo build --release PASS. **Files touched (worktree):** `src/inference/models/qwen35/{dump_bisect.rs (new, 290 LOC), forward_gpu.rs (+~80 LOC dump-call insertions), gpu_full_attn.rs (+~50 LOC within-FullAttn dumps), mod.rs (+1 LOC mod register)}`, `scripts/iter61a3-bisect.sh (new, 70 LOC)`. **Receipts:** scaffold-on bisection at `/tmp/hf2q-dump/run1,run2/manifest.txt`; first-pass DIFF report saved at `/private/tmp/.../tasks/banox50ay.output` showing layer 3 attn_out as first diverge; second-pass with within-FA dumps at `boh92fk5s.output` showing layer 63 only. **Hand-off to iter61a-4:** the scaffold + within-FullAttn dumps are reusable infra. Next investigator should (a) compare the L3 ffn_residual buffer first-divergence between cold runs, (b) probe with `HF2Q_PARTIAL_CHAIN_N=1` to disable chain mode (reduce CB count → fewer race surfaces), (c) try `HF2Q_LEGACY_PER_LAYER_CB=1` (legacy synchronous commit_and_wait per encoder — if THAT goes 10/10 identical, confirms the fused single-CB path's async commit boundary as the race site).

- **2026-04-30 — iter61a-4 LANDED — H-D-3 race ROOT-CAUSED + FIXED in FullAttn prefill ops6-7 encoder; 10/10 byte-identical greedy decode achieved across 27B-dwq46 + apex Q4_0.**  Worktree `fix/adr015-iter61a-4-fa-race` (branch from iter61a-3 HEAD `dbc6cd9`). **S1 Reproduction confirmed:** ran `scripts/iter61a3-bisect.sh` against the iter61a-3 HEAD's pre-existing dump pair at `/tmp/hf2q-dump/run{1,2}` — 369 file diffs, first-diverge at `step0000_layer003_attn_out` (matching iter61a-3's pin). **S2 Probe — pinpointed root cause via existing dump artefacts (no new probes needed):** all 10 within-FullAttn intermediates for layer 3 (`fa_x_norm`, `fa_q_flat`, `fa_k_flat`, `fa_v_flat`, `fa_q_normed`, `fa_k_normed`, `fa_q_rope`, `fa_k_rope`, `fa_gate_flat`, `fa_sdpa_out`) are byte-identical across cold runs; ONLY `attn_out` (post-wo_proj) differs. The transformation between `fa_sdpa_out` (identical) and `attn_out` (different) is the 2-dispatch ops6-7 encoder: `apply_sigmoid_gate_multiply` (writes `gated`) → `apply_linear_projection_f32_pooled` (reads `gated`) at `gpu_full_attn.rs:1659-1665`. **No `enc.memory_barrier()` between the two dispatches.** Probes A (HF2Q_LEGACY_PER_LAYER_CB) and B (encoder splitting) and C (alternative chains) WERE NOT NEEDED — the dump pair already pinned the race window to a single 2-dispatch encoder. **S3 Mechanism — RAW race within shared encoder:** Metal `MTLDispatchTypeConcurrent` permits the runtime to reorder the two dispatches within the same MTLCommandBuffer unless an explicit `memory_barrier()` enforces the producer→consumer edge. Even the encoder's terminal `commit_and_wait` (which DOES sync at the boundary) cannot save the race INSIDE the encoder. The legacy decode encoder (containing only these 2 dispatches) was deterministic by accident — no other parallel work to interleave; under prefill multi-token regime (seq=11 chat-template-wrapped "Hello" on 27B-dwq46) there is enough threadgroup pressure to expose the reordering. **This is the FullAttn-prefill twin** of (i) iter21's decode-path fix at `apply_gated_attn_layer_decode_into:1925` (commit `c46207d`) — same pattern, same edge, opposite forward path; (ii) iter58b's DeltaNet chunk-prefill residency-set-lifetime fix (commit `2565eab`). **Independent confirmation:** an iter61a-2 stash on main (`git stash@{0}` SHA `79198b8` titled "iter61a-2 wip restored 2026-04-30 by iter61a-4") had identified and fixed the same edge with identical diagnostics — never landed because of the iter61a-3 Heisenbug shift. **S3 Fix — single-line `enc.memory_barrier()` between Op 6 and Op 7 in `build_gated_attn_layer` ops6-7 encoder, with 30-line provenance comment.** No env-gating opt-out (per `feedback_no_shortcuts`). Locality-only; smallest-blast-radius. File: `src/inference/models/qwen35/gpu_full_attn.rs:1666` (~30 LOC including the audit-trail comment). **S4 Validation receipts (LIVE, 10-cold-run gates on M5 Max):** (a) **27B-dwq46 'Hello' T=0/top-k=1 max=8 → 10/10 IDENTICAL** md5 `47c7a4c919e626424a8c921cc5f5c5c5` (10 distinct cold processes, output text = `"Hello! How can I help you today"`); (b) **apex Q4_0 'Write one short paragraph...' T=0/top-k=1 max=32 → 10/10 IDENTICAL** md5 `77823464f64d5b5d1d5542e4091ab3c8` (output = `"Apple Silicon refers to the custom-designed system-on-chip (SoC) developed by Apple for its Mac computers, iPads, and other devices."`); (c) re-ran `scripts/iter61a3-bisect.sh` post-fix → **NO DIFFERENCES — runs are byte-identical (485/485 dump files match)**, falling from 369 diffs pre-fix to 0 diffs post-fix. **S4 Test gates:** cargo build --release PASS; `cargo test --release --bin hf2q "inference::models::qwen35"` 166/0/8 PASS; `cargo test --release --bin hf2q "backends::gguf"` 71/0/0 PASS; existing `forward_gpu_deterministic` regression test now serves as iter61a-4's regression coverage (additional in-binary regression test deferred — the 10-cold-run live gate is the load-bearing evidence). **S5 Perf delta on apex Q4_0 decode:** 3-trial cold-process bench (max-tokens=64): post-fix mean 2.95 t/s (excluding 1 swap-pressure outlier at 1.2 t/s) vs iter61a-3 baseline 2.87 t/s — within measurement noise. The added barrier is one MTLBarrier per FullAttn layer × 16 FullAttn layers per token; M5 Max barrier cost is sub-microsecond. **S5 Sourdough on apex:** chat-template render fails on the apex GGUF (orthogonal pre-existing issue documented in iter58b's gpu_delta_net.rs:1199-1210 comment — "apex GGUF lacks `tokenizer.chat_template` metadata so hf2q falls back to the hardcoded Gemma4 template"; iter61a-3 baseline binary FAILS the same gate identically); **NOT caused by this fix**. ADR-013's gguf-patch tool from 2026-04-30 P13 is the path to closure for that orthogonal blocker. **Files touched:** `src/inference/models/qwen35/gpu_full_attn.rs` (+31 LOC: 1 barrier + 30-line provenance comment); `scripts/iter61a3-bisect.sh` (1-line WORKTREE default updated to iter61a-4 path). **Receipts archived:** `/tmp/hf2q-dump/run{1,2}/` (post-fix bisect = 0 diffs); `/tmp/iter61a4-10x-27b/run{1..10}.out` (10/10 27B identical); `/tmp/iter61a4-10x-apex/run{1..10}.out` (10/10 apex identical). **Hand-off / unblocks:** ADR-013 P14 throughput-numeric bench is now precondition-met (deterministic decode required for stable speculative-decode acceptance-rate measurement). The spec_decode mechanism work at `origin/fix/adr013-mtp-shared-embed` (loader) + `origin/fix/adr013-spec-decode-prev-hidden` (runtime) is now ready to bench against this deterministic baseline. Status: iter61a-4 SHIPS — B-W-1 closure complete; greedy decode is byte-identical across cold processes for both production fixtures.

  Single Claude perf-engineer agent (debugging duty) attempted reproduction on base SHAs hf2q `6bce4e2` + mlx-native `3f21443` (both contain iter61a's two zero-init fixes). **Reproduction FAILED across exhaustive probe set:** (a) 5/5 cold-process runs of apex q4_0-flat pp4096 `--benchmark` mode → all PASS, prefill tok/s 207-231, zero CommandBufferError; (b) 5/5 cold-process runs of 27B dwq46 pp4096 → all PASS, prefill tok/s 36-52, zero CommandBufferError; (c) progressive prompt-size probe pp512 → pp1024 → pp2048 → pp4096 → pp8192 → pp16384 → pp32768 (8× the iter60a-reported wedge size, 30730 actual tokens) on apex q4_0-flat → all PASS at every size including under high memory pressure (~5GB free at pp32768); (d) concurrent-GPU contention test (27B + apex pp4096 in parallel processes on the same MTLDevice) → both ran cleanly. **All 5 hypothesis branches in the iter61b brief FALSIFIED:** (i) Metal CB timeout → falsified (pp32768 ran 137s with continuous CB submissions, no timeout); (ii) memory pressure → falsified (pp32768 with ~5GB free did not wedge); (iii) kernel deadlock at large K → falsified (same kernels execute correctly across pp512..pp32768); (iv) command queue overflow → falsified (commit_and_wait drains synchronously; pp32768 issued ~30000+ CBs with no overflow); (v) parallel-session contention → falsified (concurrent 27B+apex test ran cleanly). **Hypothesis (vi) INVERSE-CONFIRMED — root cause identified retrospectively:** B-W-2 was almost certainly a downstream symptom of B-W-1. Uninitialized GPU buffers from `MTLResourceOptions::StorageModeShared` returned bytes recycled from Metal's private heap; at long-prompt prefill scale, those garbage bytes propagated through scratch / KV-cache / DeltaNet conv_state buffers and produced NaN cascades, out-of-bounds index arithmetic, or invalid pointer dereferences that Metal surfaced as `MTLCommandBufferStatus::Error`. **iter61a's two zero-init commits closed both windows simultaneously:** commit `e613722` (hf2q HybridKvCache `reset_all_buffers`) + commit `3f21443` (mlx-native `MlxDevice::alloc_buffer` write_bytes). Once garbage-buffer reads are eliminated, the wedge cannot fire. The iter60a agent's empirical observation was valid for pre-iter61a state; on current HEAD with both fixes present, B-W-2 has no surface to reproduce. **AC validation (current HEAD):** AC1 cargo build --release clean (both repos) ✓; AC2 qwen35:: 255/0/8 ✓; AC3 cant_reproduce with empirical counter-evidence ✓ (10+ successful runs documented at `/tmp/iter61b/`); AC4 root cause identified ✓ (B-W-1↔B-W-2 cascade); AC5 5/5 apex pp4096 ✓; AC6 5/5 27B dwq46 pp4096 ✓; AC7 no new unsafe ✓ (zero code changes); AC8 decode no-regression moot ✓ (no code changes). **Pre-bench discipline followed:** mcp-brain-server + rust-analyzer suspended via `kill -STOP` throughout the bench window per `feedback_bench_process_audit`. **Iter61b closure:** docs-only commit recording finding + zero source changes. iter61a-2 (kernel-level determinism for the residual byte-identity gap) and iter61c (re-validate iter56/57/58b parity claims against deterministic baseline) remain open. Per `feedback_no_broken_windows` "fix all issues the moment they're discovered" — B-W-2 is closed by iter61a's defensive memory-hygiene work; no separate iter61b fix required. Empirical evidence archived at `/tmp/iter61b/{result.json,apex_pp4096_results.txt,27b_pp4096_results.txt}`. Status: iter61b CLOSED-CANT-REPRODUCE-SOLVED-BY-ITER61A.

- **2026-05-01 (UTC ~02:30Z) — iter61a PARTIAL — TWO defensive zero-init fixes shipped (mlx-native allocator + hf2q HybridKvCache); 10/10 byte-identical gate NOT met — kernel-level non-determinism remains, opens iter61a-2.**  Single Claude perf-engineer agent debugged B-W-1 (HEAD `1c7d4f0` non-deterministic at temp=0/top-k=1). Agent stalled at 10:38Z (stream watchdog 600s timeout) after identifying root cause + applying first fix. **Reproduction confirmed:** 4 cold runs at HEAD `1c7d4f0` produced 4 distinct md5s. Verified empirically. **Root cause identified (PARTIAL):** `MTLResourceOptions::StorageModeShared` does NOT guarantee zeroed pages on Apple Silicon — Metal's allocator recycles pages from recently-freed allocations within its private heap before the OS sees the free, so a fresh buffer can contain residual bytes from prior allocations in the same process. DeltaNet's `ssm_conv` reads `conv_state_in` on first prefill before decode populates it; `gated_delta_net` similarly reads `state_in` on first call; garbage propagates into logits → argmax → divergent generations across cold runs. **Fix #1 (agent's, hf2q):** explicit zero-init of HybridKvCache GPU buffers at construction via new `reset_all_buffers()` — 168 lines in `src/inference/models/qwen35/kv_cache.rs`, no `unsafe` (uses `MlxBuffer::as_mut_slice::<f32>` safe wrapper). Commit `e613722`. **Fix #2 (mine, mlx-native):** extended the fix to ALL Metal allocations at the source — `write_bytes(ptr, 0, byte_len)` in `MlxDevice::alloc_buffer` after the null check. 1 new `unsafe` block at FFI/Metal boundary (well-justified: just-allocated, exclusive access, byte_len verified). 27 lines in `/opt/mlx-native/src/device.rs`. Commit `3f21443`. Both fixes are real defensive bug fixes addressing genuine memory-hygiene issues. **Validation result: 10/10 BYTE-IDENTICAL GATE NOT MET.** With BOTH fixes built (cargo verified): 10 cold runs on 27B dwq46 "Hello" prompt produced 9 distinct body-hashes (runs 6+7 happened to match, 8 others all distinct). Some runs produce coherent output ("Hello! How can I help you today?"), others produce gibberish (markdown backticks repeating). HF2Q_USE_DENSE=1 probe (5 runs) also showed 4 distinct hashes — non-determinism is NOT MoE-routing-specific. **Kernel-level non-determinism remains the residual root cause.** Most likely candidates: (a) Apple Metal Tensor Cores (`mpp::tensor_ops::matmul2d` in quantized_matmul_mm_tensor.metal) tile-based reductions are timing-dependent across thread schedules — Apple acknowledges this; (b) simdgroup reductions (`simd_sum` in RMSNorm, L2-norm, attention output norms) producing tile-order-dependent FP-summation results since FP addition is non-associative; (c) MoE softmax_topk argsort tie-breaking when expert logits have near-equal values. **Build: cargo clean both repos, qwen35:: 255/255 tests pass, no decode regression visible.** **Iter61a partial closure:** ship the two defensive fixes (real bug fixes, retained); reopen as **iter61a-2** for kernel-level determinism investigation. iter61c (re-validate iter56/57/58b parity claims) DEFERRED until iter61a-2 establishes deterministic baseline. **Loop continues** per user directive ("we're in a loop until we're done", no pausing). Next: iter61b (GPU `CommandBufferError` wedge on long prompts — independent broken window), then iter61a-2 (kernel-level determinism). Status: iter61a CLOSED-PARTIAL with 2 commits shipped; iter61a-2 + iter61b open.

- **2026-04-30 (UTC ~23:30Z) — iter60a STRUCTURALLY FALSIFIED + TWO BROKEN WINDOWS surfaced — pause iter60b; fix bench infra first per `feedback_no_broken_windows` + mantra "No fallback. No stub. Pure excellence."**  Single Claude perf-engineer agent ran iter60a (B-stage vector-store fast path in `quantized_matmul_mm.metal`). **iter60a structural finding:** the brief targeted the non-tensor kernel `quantized_matmul_mm.metal`, but on M5 Max the production prefill path is `quantized_matmul_mm_tensor.metal` which **already implements** the 8-wide `float2x4` vector store at lines 285-287. The non-tensor kernel is fallback-only on pre-M3 hardware and **never dispatches on M5 Max** (verified at `/opt/mlx-native/src/ops/quantized_matmul_ggml.rs:466` `probe_tensor_mm` dispatcher gate). My earlier Explore agent (commit `cfdea4a`) read the wrong kernel as "PRIMARY"; the iter60a research mis-attributed the gap. **Implementation work was correct but unshippable for perf:** added `FC_mul_mm_bc_inp [[function_constant(250)]]` + 8-wide vector-store path at line 307 mirroring `llama.cpp/ggml/src/ggml-metal/ggml-metal.metal:9569`; 2 new parity tests proving FC=true vs FC=false byte-identity at K-aligned shapes + FC=false vs mv-reference correctness; 13/13 mm parity tests pass; 254/254 qwen35 tests pass; cargo build clean both repos; AC12 unsafe count 50→50 unchanged. Stashed at mlx-native `stash@{0}: iter60a-work-falsified` for potential fold-in if pre-M3 fallback ever matters. **Coherence preserved on both fixtures:** AC6 sourdough on apex q4_0-flat (the iter58b-broken fixture) PASSED; AC7 on 27B dwq46 PASSED. **ZERO commits shipped** — agent correctly honored "if <+1pp falsified revert" gate. **TWO BROKEN WINDOWS surfaced during iter60a validation** (these now block all further iter60 perf work per `feedback_no_broken_windows` and mantra: "No fallback. No stub. Just pure excellence."): (B-W-1) **HEAD `8a89f97` is NON-DETERMINISTIC at temp=0/top-k=1 NGEN=32** — agent verified empirically that the same "Hello" prompt produces different output bytes across cold-process runs. Greedy decode on a deterministic GPU MUST produce byte-identical output. If it doesn't: sampler tie-breaking bug, KV cache state leakage, kernel non-determinism, or harness state retention. **This invalidates every "byte-identical smoke PASSED" claim in iter56-iter59 changelog entries** — those gates ran against a non-deterministic baseline so any "pass" was pure noise (or coincidence). Per `feedback_verify_baseline_determinism_before_perf_bench` (already standing memory). (B-W-2) **GPU `CommandBufferError` wedges on long-prompt benches** — agent reported AC8 prefill bench couldn't run reliably on either HEAD `8a89f97` base OR iter60a binary; pre-existing mlx-native CommandBufferError on long-prompt workloads. Masked by short-prompt (32-tok) smoke tests all session. **iter61 plan (bench infra fix BEFORE any more iter60 perf work):** **iter61a** — root-cause non-determinism. Hypothesis tree: (a) sampler argmax tie-breaking with NaN/duplicate logits; (b) KV cache state retained across cold runs (file lock/mmap?); (c) kernel non-determinism (race condition? uninitialized scratch?); (d) Metal kernel-args buffer reuse without clear; (e) jinja chat-template randomness. Run differential: same prompt × N cold runs → diff byte-by-byte → bisect to first diverging token → trace back to source. **iter61b** — root-cause GPU wedge. Hypothesis tree: (i) Metal CB timeout at long prompts; (ii) memory pressure with apex 23GB Q4_0 + scratches; (iii) specific kernel deadlock at K=2816 N=...; (iv) command queue overflow. Capture stderr from wedge, gdb attach if available, check Metal logs in Console.app. **iter61c** — re-validate prior parity claims against fixed deterministic baseline. iter56 (fused gated_delta_net), iter57 (MoeQ encoder fusion), iter58b (DN state writeback) all claimed byte-identical smoke; need to re-run on a known-deterministic base. **iter60b queued AFTER iter61c**. Mantra compliance: this iter ships ZERO code (iter60a falsified, no commits); pivots to fixing the foundation before more perf claims. iter60a stash retained for possible future relevance. Status: iter60a CLOSED-FALSIFIED-STRUCTURAL; iter61a opens as next concrete iter.

- **2026-04-30 (UTC ~22:00Z) — iter60a research-only — peer-repo + goalie deep research identifies TWO concrete matmul kernel gaps; iter60a candidate fix is `FC_mul_mm_bc_inp` vector-store path; iter60b is tensor-variant f32→half cast deferral.**  Per user directive "deep research with goalie, and of peer repos", launched parallel research: goalie `reasoning_agentic_research` (4 agents: researcher / fact_checker / synthesizer / critic, 90% consensus) + Explore agent reading mlx-native + llama.cpp matmul kernels side-by-side. **Three Q4_0 matmul kernels compared:** (1) llama.cpp `kernel_mul_mm_q4_0_f32` at `/opt/llama.cpp/ggml/src/ggml-metal/ggml-metal.metal:9440` (non-tensor, simdgroup_half8x8 + simdgroup_multiply_accumulate); (2) mlx-native simdgroup `kernel_mul_mm_q4_0_f32` at `/opt/mlx-native/src/shaders/quantized_matmul_mm.metal:203`; (3) mlx-native tensor `kernel_mul_mm_q4_0_tensor_f32` at `/opt/mlx-native/src/shaders/quantized_matmul_mm_tensor.metal:171` (mpp::tensor_ops::matmul2d for M3+ hardware tensor cores). **Common config:** all three use 128 threads (4 simdgroups × 32), 64×32 output tile, K-dim NK=32, 8KB threadgroup mem, dispatch shape `(ceil(M/32), ceil(N/64), 1)`. **Identified gaps explaining hf2q 191µs/dispatch vs llama 169µs/dispatch (+13% slower per kernel × ~3000 dispatches = significant prefill time):** (PRIMARY) **B-stage vector store missing in mlx-native** — llama.cpp at line 9569 uses `*(threadgroup S1_2x4 *)(sb + 64*ib + 8*ly) = (S1_2x4)(*((device T1_2x4 *) y))` (8-wide vector store, 4-8× throughput) gated by `FC_mul_mm_bc_inp=false` for aligned shapes; mlx-native non-tensor at lines 287-305 ALWAYS uses per-element loop with no vector path compiled despite line 289-291 comment claiming support. For 32×32 tile (1024 elements) at K=2816 (~35 K-iters/dispatch), per-element store latency compounds across the entire prefill. (SECONDARY) **f32→half cast overhead in tensor variant** — `quantized_matmul_mm_tensor.metal:285-287` casts B from f32 to half at staging via `(half2x4)` cast of `(float2x4)`, forcing 1024 extra fp32→fp16 instructions per K-iteration; non-tensor stores f32 directly and defers precision loss to the simdgroup MMA's internal half representation. May saturate memory bandwidth before tensor cores' compute throughput is realized. (TERTIARY) **Function constants** — llama.cpp uses `FC_mul_mm_bc_inp` + `FC_mul_mm_bc_out` to gate bounds-check vs aligned paths at compile time; mlx-native always bounds-checks (quantized_matmul_mm.metal:24 comment "We always enable the bounds-checked input / output paths"). Minor branch overhead but missed compile-time elimination opportunity. **iter60+ implementation order (per user "test test test — no more regressions"):** **iter60a** — implement `FC_mul_mm_bc_inp=false` vector-store fast path in `quantized_matmul_mm.metal` (conditional on K divisible by NK=32 + N divisible by 32). Estimated +5-10pp prefill if Apple's vector store throughput claim holds. PRE-MERGE GATES MUST INCLUDE: sourdough on apex q4_0-flat (the iter58b-broken fixture), 200+ token coherence check vs llama-completion, full 4-fixture decode no-regression bench, 16-prompt byte-identical smoke. **iter60b** — defer f32→half cast in tensor variant by staging B as f32, casting only at matmul2d() call. Estimated +2-5pp on tensor variant (smaller fixtures use it). **iter60c** — gate bounds-check via function constants matching llama.cpp's pattern. Estimated +0.5-1pp. **Methodology compliance:** evidence-based per `feedback_evidence_first_no_blind_kernel_rewrites` — TWO independent research agents converged on B-stage vector store as primary suspect with file:line citations from both peer repos. iter60a is the smallest-first fix matching the strongest evidence. NO code change this iter — research-only. Status: iter60a queued; awaiting user approval to launch implementation.

- **2026-04-30 (UTC ~21:30Z) — iter58b-fix · CFA dual-mode (claude winner) — iter58b's "Metal serial-queue ordering preserves correctness without a CPU-side wait" claim FALSIFIED on the chunk-prefill correctness side; restored `enc.commit_and_wait()` at `gpu_delta_net.rs:1177`; iter58b's structural cleanup (caller-provided buffers, `Result<()>`, no `unsafe`, no `as_slice` CPU bridge) preserved verbatim.**

  **What iter58b got right.** GPU-side execution ordering on a Metal serial queue: CB1 begins before CB2 begins; CB1 completes before CB2 completes. The single `MlxDevice::queue` (`/opt/mlx-native/src/device.rs:21-37`) and the absence of a parallel-encoder pool make this watertight at the queue layer.

  **What iter58b's claim missed — H1 (residency-set lifecycle race).** The wrapper-internal scratches (`q_expanded`, `k_expanded`, `q_bf16`, `k_bf16`, `v_bf16`, `g_log_decay`, `o_bf16`, plus the chunk-pipeline's own internal scratches at `/opt/mlx-native/src/ops/chunk_gated_delta_rule.rs:399-543`) are allocated via `MlxDevice::alloc_buffer` and AUTO-REGISTERED with the device-level `MTLResidencySet` (`/opt/mlx-native/src/buffer.rs:146-167` `with_residency`). When the wrapper returns and they drop, `MlxBufferStorage::Drop` (`buffer.rs:68-77`) calls `set.remove_allocation(&inner)` — **deferred** (marked pending; NO `[set commit]` until next encoder commit boundary, per `residency.rs:177-187` and `223-240`).

  iter58b's `enc.commit()` (no host wait) returned to the caller before CB1 had executed. The wrapper-scope scratches dropped, staging ~16 deferred `removeAllocation:` calls. The caller's downstream ops 8+9 encoder then issued `enc.commit_and_wait()`, whose `flush_residency_pending()` (`encoder.rs:1426`) ran BEFORE submitting CB2 — committing the staged removes. **By the time CB1 actually executed on the GPU, its still-bound buffers had been removed from the residency set.** Under the apex 23 GB chunk-prefill working-set memory pressure, Metal is free to demote those pages mid-dispatch, producing garbage tensor values. Empirically observed: degenerate residual stream when the chunk path engaged (`seq_len > 64 && seq_len % 64 == 0 && d_k == 128`).

  **Pre-iter58b `commit_and_wait` worked because** the host wait blocked until CB1 had completed; only THEN did the wrapper return, the scratches drop, and the deferred `removeAllocation:` stage. The next encoder's commit then flushed those removes — but CB1 had long since finished reading them. No race.

  H2 (queue pool) and H3 (CB-to-CB fence transitivity) were both falsified during the investigation (single serial queue; submission-order guarantees hold for both `commit` and `commit_and_wait`). The breakable invariant was the residency-lifecycle one, not GPU-execution ordering.

  **The fix.** `enc.commit()` → `enc.commit_and_wait()` at `gpu_delta_net.rs:1177`, plus a 50-line in-source comment block documenting the mechanism. Per iter58b's own re-measurement (b6f416b commit body), perf cost is ~71µs / ~3650ms = **0.06% wall** — well below the +1pp gate. iter58b's structural improvements (caller-provided `output_buf` + `final_state`, `Result<()>` return, deletion of the `as_slice` CPU bridge and the `copy_nonoverlapping` unsafe) are PRESERVED verbatim. The wait restoration is the minimal correctness fix; no other code paths regress.

  **Methodology lesson.** This is the second time on ADR-013 / ADR-015 that a Metal-ordering-style optimization has been falsified by a downstream invariant the optimizer didn't model: iter21 (`c46207d`, 2026-04-29) caught a missing `enc.memory_barrier()` between Op 6 and Op 7 in a single-CB encoder where the runtime was free to reorder under `MTLDispatchTypeConcurrent`. Today's iter58b-fix is the cross-CB analog: a deferred residency-set state change interleaving with a not-yet-executed CB. Standing pin (already ratified): perf changes that touch synchronization invariants must run a same-day deterministic byte-parity gate at full memory pressure (apex 23 GB GGUF, full chunk-prefill seq_len), not just smoke-fixture parity. **iter58b's published AC matrix at base `02be0ce` shipped 16/16 byte-identical smoke PASS** — but the fixture didn't put enough pressure on the residency hint for the demotion to fire. The post-ship regression on apex chunk-prefill is the discovered ground-truth invariant.

  **iter58b's perf decision stays valid.** The structural cleanup is a real ergonomic + safety win (1 unsafe block deleted, 1 CPU memcpy removed from the hot path, 47 lines of unsafe-bridge code gone). The +0.06pp perf claim was already falsified by iter58b itself; nothing about the cold-clean-baseline correction (0.3190× true ratio) changes.

  **iter59+ unblocked.** All future kernel-fusion / matmul-optimization work in iter59 (`kernel_swiglu_f32`, `kernel_rms_norm_mul_f32_4`, `kernel_l2_norm_f32_f32_4`, etc.) is unaffected; this fix only restores the chunk-prefill correctness invariant.

  Raw artifacts: branch `cfa/adr013-iter58b-fix/claude` in `/opt/hf2q/.cfa-worktrees/adr013-iter58b-fix-claude/`. Mechanism doc: `/tmp/cfa-adr013-iter58b-fix/claude-mechanism.md`. Test receipts: `/tmp/cfa-adr013-iter58b-fix/claude-test-receipts.md`. Status: iter58b PARTIAL stays SHIPPED for the structural cleanup; the `commit` → `commit_and_wait` half is FALSIFIED and corrected.

- **2026-04-30 (UTC ~20:30Z) — iter59 paper analysis (system too contaminated for new MST capture) — kernel fusion identified as next lever, +1.7-2.4pp combined est; remaining ~0.66 ratio gap lives in mlx-native matmul kernel.**  System audit at iter59 fire showed `rustc` PID 33637 at +618% CPU + Xcode strings at 97% + 4 concurrent claude sessions — cold-clean MST capture impossible right now. Pivoted to paper analysis using existing iter57 MST registry (`/tmp/iter57-mst/aggregate-prefill.txt`, lines 1100-1140 hf2q registry vs 1145-1180 llama registry) interpreted with cold-clean baseline understanding (true prefill ratio 0.3190× not 0.22×). **Per-kernel fusion candidates by registry comparison:** (1) **SwiGLU fusion** — llama ships `kernel_swiglu_f32` (1 fused gate*silu*up); hf2q ships separate `silu_mul_f32 (23)` + `sigmoid_mul_f32 (32)` (2 dispatches). Per FFN call saves 1 dispatch; for 35B-A3B-MoE 64 layers × 16 chunks = 1024 saves at ~50-100µs/dispatch ≈ 50-100ms saved on 3635ms prefill = **+0.4-0.9pp ratio**. (2) **RMSNorm+mul fusion** — llama ships `kernel_rms_norm_mul_f32_4`; hf2q ships separate `rms_norm_f32 (10)` + `scalar_mul_f32 (16)`. Per layer norm fires multiple times (pre-attn, post-attn, q-norm, k-norm); 4 norms × 64 layers × 16 chunks ≈ 4096 dispatches at ~30µs each ≈ 120ms saved = **+1pp ratio**. (3) **L2-norm vec4** — llama ships `kernel_l2_norm_f32_f32_4` (vec4); hf2q ships scalar `l2_norm_f32 (15)`. 2 dispatches per DN layer (q_l2 + k_l2) × 30 DN layers × 16 chunks = 960 dispatches; vec4 ~2-3× faster on memory-bound = **+0.3-0.5pp ratio**. (4) **Binary op fusion** — llama ships `kernel_bin_fuse_f32_f32_f32` (12 variants of a+b*c, a*b+c, etc.); hf2q ships individual ops. Coverage TBD per workload. **Combined estimated upside:** +1.7-2.4pp ≈ cold 0.3190× → 0.336-0.343×. **Residual ~0.66 ratio gap** to ≥1.00× target lives in **matmul kernel itself** — iter57 MST showed hf2q `kernel_mul_mm_q4_0_tensor_f32` runs at 191µs/dispatch vs llama `kernel_mul_mm_q4_0_f32` at 169µs/dispatch (+13% slower per matmul). The "tensor" variant should be faster via Apple Tensor Cores but isn't helping — possibly wrong tile size, wrong path, or tensor cores not engaging on this shape. **iter59+ implementation order:** iter59a RMSNorm+mul fusion (+1pp, mlx-native + hf2q dispatch swap), iter59b SwiGLU fusion (+0.4-0.9pp), iter59c L2-norm vec4 (+0.3-0.5pp), iter59d binary op fusion (TBD), iter60 matmul kernel optimization (mlx-native scope, the BIG residual lever). After all kernel fusion exhausted, expect ~0.34× cold prefill ratio; closing the remaining 3× gap to ≥1.00× requires structural matmul work (tile size sweep, possibly proper MLX SDK integration vs current hand-rolled tensor-cores variant). Status: iter59 OPEN; first concrete sub-iter is iter59a RMSNorm+mul fusion. Note: this iter ships ZERO code — paper analysis only; updates planning & quantitative estimates so future iter implementations have evidence-based upside calibration.

- **2026-04-30 (UTC ~20:00Z) — iter58b PARTIAL — DN state GPU writeback refactor LANDS as architectural cleanup; perf lever falsified at +1pp gate; CRITICAL re-baseline finding: TRUE cold-clean prefill ratio is 0.3190×, not 0.22× (prior measurements were CPU-contaminated).**  Single Claude perf-engineer agent in main repo, commit `b6f416b`. **Refactor:** `apply_gated_delta_net_chunk` (gpu_delta_net.rs:801) signature changed from `Result<(MlxBuffer, MlxBuffer)>` to `Result<()>` taking caller-provided `output_buf` + `final_state` as `&MlxBuffer` input parameters. Caller (`build_delta_net_layer` chunk-prefill branch) threads outer-scope `attn_out_buf` + caller-owned `state_out` ping-pong directly into the wrapper; deletes the 47-line `as_slice::<f32>()` + `std::ptr::copy_nonoverlapping` block including 1 `unsafe` block + 1 hot-path CPU memcpy. Wrapper terminal `commit_and_wait` downgraded to `commit` (no host wait); Metal serial-queue ordering preserves downstream correctness for ops 8+9. Three unit tests updated (2-line sync helper for parity tests that CPU-download wrapper outputs; production callers don't). 242 lines changed in `src/inference/models/qwen35/gpu_delta_net.rs`. **AC matrix (cold-SoC paired, M5 Max, mcp-brain-server SIGSTOP'd):** AC1 build PASS, AC2 241/0/8 tests PASS, AC3 16/16 byte-identical smoke vs base `02be0ce` PASS, AC5 27B dwq46 decode **1.0686×** (≥1.05× threshold PASS, -0.69pp vs iter57 within noise), AC6 apex Q5_K decode **1.1124×** (≥1.07× PASS, -0.37pp within noise), AC8 q4_0-flat decode **1.0594×** (≥1.04× PASS, -0.26pp within noise). **AC4 perf lever FALSIFIED:** absolute ratio **0.3190×** clears the ≥0.2313× target threshold — BUT this reflects RE-BASELINE not iter58b contribution. Same-day Δpp vs HEAD-pre-iter58b in 5-trial cold-SoC paired interleaved bench: **+0.02pp** (1127.8 vs 1127.1 t/s = +0.06% wall = noise). **Mechanism finding:** 30 `commit_and_wait` removals × 71µs/wait (per iter58c calibration) = ~2ms saved on ~3650ms prefill = 0.06% wall. iter57 blocker analysis's +5-10pp estimate was based on a faulty mental model attributing the gap to encoder-wait elimination; the actual mechanism (each wait ≈71µs) puts ALL within-layer wait-removal levers below the +1pp threshold. **CRITICAL methodology finding:** initial bench under contaminated SoC (`bfs` at +349% CPU, `diskimagesiod` at +130% CPU) showed iter58b 1086 vs base 971 t/s = +12% (paired Δpp +5.05pp) — FALSE SIGNAL. Cold-SoC clean re-bench falsified that signal. **The "0.22× prefill ratio" all prior iter57+iter58c work was chasing was CPU-CONTAMINATED.** True cold-clean ratio at HEAD is **0.3190×** (hf2q 1127 t/s vs llama-bench -p 4096 -ub 4096 -r 5 = 3534.94 ± 92.07 t/s). Methodology drift correction = **+9.77pp** vs published 0.2213× baseline. Hf2q is **3.13× slower** than llama at prefill, not 4.47×. **Decision:** SHIP iter58b commit `b6f416b` — refactor is structurally correct, parity-clean, decode no-regression, eliminates 1 unsafe + 1 CPU memcpy from hot path, aligns with production ping-pong contract. Falsify the perf claim ONLY (not the structural change). **iter58 redirect:** with true cold-clean baseline at 0.3190×, the gap to ≥1.00× is +0.68 ratio (~213% improvement needed). Next steps: (i) re-do MST per-encoder GPU-time attribution on same-day v3 binary at HEAD `b6f416b` to re-identify the actual remaining cost class (iter57 attribution was based on contaminated baseline + correct-by-accident encoder count); (ii) iter58a FA K/V GPU copy at `gpu_full_attn.rs:1265-1287` may or may not still be the lever — re-attribute first; (iii) cross-layer encoder coalescence (the original iter57 thesis but properly measured) could be revisited. Standing matrix updated below. Raw: `b6f416b`, `/tmp/iter58b/result.json`, `/tmp/iter58b/bench-pp4096-paired/`. Status: iter58b ships as architectural cleanup; iter58 redirect re-attributes from cold-clean baseline.
