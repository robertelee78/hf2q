...
- **2026-04-28 — iter13 FALSIFIED — audit's primary `encode_dual_buffer` prescription architecturally blocked by iter10 single-chain dependency; separable unretained-refs sub-lever lands as default-OFF mlx-native infrastructure but is bench-falsified at current scratch coverage (smoke parity divergence + −10.8pp wall regression at NGEN=8).** CFA session `cfa-20260428-adr015-iter13` (worktree `cfa-20260428-adr015-iter13-claude` from base `7f2e54e`). This is the first code-shipping iter on ADR-015 since iter10's chain-encoder falsification; per the iter12 audit ranking, candidate #1 was "wire `encode_dual_buffer` into `Qwen35Model::forward_gpu` with `commandBufferWithUnretainedReferences` + scratch-lift" with expected +5-7% wall.
  ...
  **A. PHASE 0 FALSIFICATION — the audit's central premise is structurally wrong.** `/tmp/cfa-20260428-adr015-iter12/audit/findings.md` line 32-35 prescribed: "switch decode to dual-buffer pattern using mlx-native's existing `ComputeGraph::encode_dual_buffer` API … encode layers 0-19 on a worker thread, layers 20-39 on the main thread; commit() both without wait; single terminal wait_until_completed." Direct code read at HEAD falsifies the drop-in characterization on three points:
  ...
  2. **Zero production callers in mlx-native or hf2q.** `git -C /opt/mlx-native grep -n encode_dual_buffer`: definition at `:298`, only callers are `GraphSession::finish_optimized` at `:1801` and `GraphSession::finish_optimized_with_timing` at `:1877`, both invoked only from BERT test fixtures. `git -C /opt/hf2q grep -n GraphExecutor` shows 50+ uses in `src/inference/models/bert/bert_gpu.rs` and **zero** in `src/inference/models/qwen35/*.rs` — qwen35 forward_gpu uses 41 distinct `device.command_encoder()` calls, not a captured-graph session.
  ...
  **B. UNRETAINED-REFS GATE LANDS in mlx-native (default-OFF, sourdough-safe).** mlx-native commit `64e69de` (pushed to `origin/main` at 2026-04-28T16:36-07:00) adds a `MLX_UNRETAINED_REFS=1` env-cached gate. `/opt/mlx-native/src/encoder.rs`:
  - new `unretained_refs_enabled()` `OnceLock`-cached helper (~20 LOC + 23 LOC docstring) reading `MLX_UNRETAINED_REFS`.
  - `CommandEncoder::new_with_residency` at line 430 switches to `queue.new_command_buffer_with_unretained_references()` when the gate is on; `queue.new_command_buffer()` (existing default) when off. metal-rs 0.33 already exposes `CommandQueueRef::new_command_buffer_with_unretained_references` at `~/.cargo/registry/src/index.crates.io-.../metal-0.33.0/src/commandqueue.rs:37-39` — no objc msg_send shim needed.
  - extended docstring (lines 416-461) with the caller contract: every Metal buffer bound to a dispatch must outlive the CB; transient scratches must be backed by the per-decode-token `MlxBufferPool` `in_use` ARC list (`buffer_pool.rs:60-63`) or hoisted to a caller scope outliving the terminal commit.
  ...
  **Two falsification signals:**
  ...
  1. **Token-level parity divergence is byte-deterministic.** Gate-OFF stable median produces ` якобы ( )`; gate-ON 4-trial-stable median produces ` якобы!!!!!!!`. The byte-stable corruption (NOT random ARC-zero crash) signature points to **deterministic scratch over-write**: an unpooled `device.alloc_buffer` in a helper function returns its scratch buffer, the helper exits, ARC drops to 0 under unretained refs, Metal reclaims the slot, the bucket allocator hands the *same* slot to a subsequent allocation, the original dispatch reads stale-overwritten contents, and because bucket order is deterministic the corruption is bit-stable across runs.

  2. **Wall regresses -10.8pp at NGEN=8.** Gate-OFF stable median 131.2 t/s vs gate-ON median 117.0 t/s = **0.892×**. NGEN=8 is too short for production attribution (load + first-decode-token cost dominates), but the regression is directionally clear: the unretained-refs lever does NOT pay off at current pool coverage.
  ...
  **G. ARTIFACTS.** Phase 0 falsification + scratch inventory: `/tmp/cfa-20260428-adr015-iter13/claude/scratch-inventory.md` (5 KB). Parity smoke evidence: `/tmp/cfa-20260428-adr015-iter13/claude/parity-smoke-evidence.md`. iter13 ADR-entry draft: `/tmp/cfa-20260428-adr015-iter13/claude/iter13-adr-entry-draft.md`. mlx-native commit: `64e69de` (encoder.rs gate + smoke test, pushed `origin/main`). hf2q commit: this commit on branch `cfa/cfa-20260428-adr015-iter13/claude`.
