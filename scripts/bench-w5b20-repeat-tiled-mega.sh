#!/bin/bash
# Wave 5b.20 — repeat_tiled GPU kernel folded INTO the chunk wrapper's
# mega-encoder (vs the W-5b.4 CPU triple-loop legacy under
# HF2Q_GQA_EXPAND_LEGACY=1). Bench at PP4106.
#
# Compares the new in-encoder GPU `dispatch_repeat_tiled_f32` path (default)
...
#
# Reuses the W-5b.16/17/18/19 walk-bar prompt + measurement protocol so wall-
# clock numbers are directly comparable across iters.
...
echo "=== Wave 5b.20 repeat_tiled in-encoder GPU wire-up ($TRIALS trials × 2 hf2q paths × $TRIALS llama) ==="
...
# (1) Same-day llama baseline (drift gate <= 10% vs W-5b.19's 6,695 ms).
...
for t in $(seq 1 "$TRIALS"); do
  /usr/bin/time -p /opt/homebrew/bin/llama-completion \
    ...
    -f "$PROMPT" > "/tmp/w5b20/llama-T${t}.log" 2>&1
  prefill_ms=$(grep -oE 'prompt eval time = +[0-9.]+ ms' "/tmp/w5b20/llama-T${t}.log" | sed -E 's/.*= +([0-9.]+) ms/\1/' | head -1)
  printf '[llama T%d] prompt_eval=%sms\n' "$t" "${prefill_ms:-???}"
done
echo

# (2) hf2q NEW path — default behaviour, repeat_tiled GPU kernel inside mega-encoder.
echo "--- hf2q chunk path NEW (in-encoder repeat_tiled, $TRIALS cold trials) ---"
for t in $(seq 1 "$TRIALS"); do
  run_one "[T${t}] W-5b.20 NEW (mega_encoder)" "/tmp/w5b20/new-T${t}.log" \
    HF2Q_PROFILE_W5B8=1 HF2Q_PROFILE_W5B17=1 \
    HF2Q_QWEN36_AUTOREG=1 HF2Q_UNSAFE_EXPERIMENTS=1 HF2Q_CHUNK_SCAN_PREFILL=1
done
echo

# (3) hf2q LEGACY path — HF2Q_GQA_EXPAND_LEGACY=1 fires the W-5b.4 CPU memcpy.
echo "--- hf2q chunk path LEGACY (CPU memcpy, $TRIALS cold trials) ---"
for t in $(seq 1 "$TRIALS"); do
  run_one "[T${t}] W-5b.20 LEGACY (cpu_memcpy)" "/tmp/w5b20/legacy-T${t}.log" \
    HF2Q_PROFILE_W5B8=1 HF2Q_PROFILE_W5B17=1 \
    HF2Q_QWEN36_AUTOREG=1 HF2Q_UNSAFE_EXPERIMENTS=1 HF2Q_CHUNK_SCAN_PREFILL=1 \
