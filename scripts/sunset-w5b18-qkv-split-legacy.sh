#!/bin/bash
# Wave 5b.19 sub-task: HF2Q_QKV_SPLIT_LEGACY=1 sunset parity audit.
#
# Per W-5b.18 closure plan: 5 cold model loads × 3 cold prefills × 2 paths
...
#
# Outputs: /tmp/w5b19-sunset/{new,legacy}-loadN-trialT.log
#
...
echo "=== Wave 5b.19 sunset parity audit (HF2Q_QKV_SPLIT_LEGACY=1 removal pre-flight) ==="
...
for L in $(seq 1 "$LOADS"); do
  for T in $(seq 1 "$TRIALS_PER_LOAD"); do
    run_one "[L${L}T${T}] QKVSPLIT new" "/tmp/w5b19-sunset/new-L${L}-T${T}.log" \
      HF2Q_QWEN36_AUTOREG=1 HF2Q_UNSAFE_EXPERIMENTS=1 HF2Q_CHUNK_SCAN_PREFILL=1
    tok=$(grep -oE 'first decoded token: [0-9]+' "/tmp/w5b19-sunset/new-L${L}-T${T}.log" | sed -E 's/.*: ([0-9]+)/\1/' | head -1)
    if [ "$tok" = "11" ]; then PASS_NEW=$((PASS_NEW+1)); else FAIL_NEW=$((FAIL_NEW+1)); fi

    run_one "[L${L}T${T}] QKVSPLIT legacy" "/tmp/w5b19-sunset/legacy-L${L}-T${T}.log" \
      HF2Q_QWEN36_AUTOREG=1 HF2Q_UNSAFE_EXPERIMENTS=1 HF2Q_CHUNK_SCAN_PREFILL=1 \
      HF2Q_QKV_SPLIT_LEGACY=1
    tok=$(grep -oE 'first decoded token: [0-9]+' "/tmp/w5b19-sunset/legacy-L${L}-T${T}.log" | sed -E 's/.*: ([0-9]+)/\1/' | head -1)
