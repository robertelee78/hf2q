...
if grep -Eq 'seed[[:space:]]*:' \
    "$script_dir/qwen38_physical_multislot_gate.sh"; then
    fail "physical-width runner turns greedy proof requests into sampling"
fi
grep -Fq 'write_cache_buster_request' \
    "$script_dir/qwen38_physical_multislot_gate.sh" || \
    fail "physical-width runner does not replace terminal caches before scalar replay"
# shellcheck disable=SC2016
grep -Fq 'cache-buster-responses/lane-$index.json' \
    "$script_dir/qwen38_physical_multislot_gate.sh" || \
    fail "physical-width runner does not validate every cache-replacement request"
...
grep -Fq 'HF2Q_Q5K_CANONICAL_Q4X4="$Q5K_CANONICAL_Q4X4"' \
    "$script_dir/qwen38_physical_multislot_gate.sh" || \
    fail "physical-width runner does not pass the canonical Q5_K route"
grep -Fq 'dense_q5k_canonical_q4x4=(true|false)' \
    "$script_dir/qwen38_physical_multislot_gate.sh" || \
    fail "physical-width runner does not reopen the frozen Q5_K route"
grep -Fq 'frozen_qwen_policy_log_count:1' \
    "$script_dir/qwen38_physical_multislot_gate.sh" || \
    fail "physical-width receipt omits the observed frozen-policy count"
