...
VERIFY_CANDIDATE="$ROOT_DIR/scripts/verify_standalone_candidate.sh"
...
checksum = "76ce4c8d5773c72554a98020aadd330e566792dd27f843451ac5dd567bb6b5dd"
LOCK
...
awk '{gsub(/76ce4c8d5773c72554a98020aadd330e566792dd27f843451ac5dd567bb6b5dd/, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")}1' \
...
grep -qF '/usr/bin/tar -xzf "$crate" -C "$packed_build_root"' \
  "$STANDALONE_WORKFLOW" || fail "standalone workflow does not unpack the sealed crate externally"
grep -qF 'cargo package --locked --no-verify' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow performs ambient package verification before isolated build"
grep -qF 'test -z "$(find "$cargo_home" -mindepth 1 -print -quit)"' \
  "$STANDALONE_WORKFLOW" || fail "standalone workflow does not prove Cargo home starts empty"
grep -qF 'cargo_env=(env)' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not construct a controlled Cargo environment"
grep -qF 'RUSTUP_TOOLCHAIN|RUSTC|RUSTDOC|RUSTFLAGS|RUSTDOCFLAGS|' \
  "$STANDALONE_WORKFLOW" || fail "standalone workflow does not clear Rust toolchain overrides"
grep -qF 'CARGO_BUILD_*|CARGO_PROFILE_*|CARGO_TARGET_*' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not clear Cargo build/profile/target overrides"
grep -qF 'cargo_env+=(-u "$variable_name")' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not unset discovered override variables"
grep -qF 'check-build-env "$cargo_home" "$cargo_target"' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not verify the controlled Cargo environment"
grep -qF 'trusted_verifier="$checkout_root/scripts/verify_release_dependency_provenance.sh"' \
  "$STANDALONE_WORKFLOW" || fail "standalone workflow does not use the trusted checkout verifier"
if grep -qF '"$package_root/scripts/verify_release_dependency_provenance.sh"' \
  "$STANDALONE_WORKFLOW"; then
  fail "standalone workflow invokes the verifier from the package being attested"
fi
grep -qF 'check-build-root "$package_root" "$checkout_root"' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not reject inherited Cargo configuration before resolution"
[[ $(grep -cF 'check-build-root' "$STANDALONE_WORKFLOW") -ge 3 ]] || \
  fail "standalone workflow does not recheck build roots before and after Cargo build"
grep -qF 'bash "$trusted_verifier" capture' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not capture dependency evidence from the packed crate"
grep -qF 'cp -R "$provenance_stage/." "$candidate_root/dependency-provenance/"' \
  "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not package dependency provenance with the candidate"
if grep -qF 'package_root="$GITHUB_WORKSPACE/target/package/' "$STANDALONE_WORKFLOW"; then
  fail "standalone workflow still builds the hardware binary under checkout ancestry"
fi

grep -qF 'verify_release_dependency_provenance.sh" verify' "$VERIFY_CANDIDATE" || \
  fail "standalone candidate verifier does not independently verify dependency evidence"
grep -qF 'scripts/verify_standalone_candidate.sh \' "$RELEASE_WORKFLOW" || \
  fail "release workflow does not invoke the standalone candidate verifier"
...
grep -qF '"$script_directory/../Cargo.lock"' "$VERIFY_CANDIDATE" || \
  fail "standalone candidate lock comparison depends on the caller working directory"
grep -qF 'PACKED_BUILD_ROOT=%s' "$STANDALONE_WORKFLOW" || \
  fail "standalone workflow does not export the external packed build root"
grep -qF '/private/var/tmp/hf2q-packed-build.*)' "$STANDALONE_WORKFLOW" || \
...
awk '
  ...
  in_packed_bash_n && /scripts\/verify_standalone_candidate\.sh/ {
