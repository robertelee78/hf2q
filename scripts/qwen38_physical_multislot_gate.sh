...
case "$DECODE_MVN:$DECODE_MV_EXT:$Q5K_CANONICAL_Q4X4" in
    ...
    *)
        echo "physical-width dense routing must use boolean MVN/MV_EXT and canonical Q5_K q4x4=1" >&2
        exit 2
        ;;
esac
...
assert_frozen_q5k_policy() {
    ...
    ' "$log_path" || {
        echo "physical-width server did not freeze canonical Q5_K q4x4 exactly once" >&2
        return 1
    }
}
...
run_width() {
    ...
    local pid buster_start_file="$width_dir/cache-buster-start" buster_failed=0
    ...
    mkdir -p "$width_dir/requests" "$width_dir/responses" \
        "$width_dir/scalar-responses" "$width_dir/cache-buster-requests" \
        "$width_dir/cache-buster-responses"
    if [[ -n "$(lsof -nP -iTCP:"$PORT" -sTCP:LISTEN 2>/dev/null || true)" ]]; then
    ...
    server_pid=$!
    wait_ready "$log_path"
    assert_frozen_q5k_policy "$log_path"
    ...
    # Replace every slot's terminal-response cache before scalar replay. A
    # sampling parameter would also bypass replay, but it would make the target
    # ineligible for the ordinary greedy physical batch this gate is proving.
    # One concurrent, distinct deterministic request per configured slot
    # guarantees that none of the measured lane identities survives in any
    # slot. The scalar requests below therefore execute the model while keeping
    # the exact same greedy semantics as their concurrent counterparts.
    for ((index = 1; index <= width; index++)); do
        printf -v lane '\\x%02x' "$((64 + index))"
        printf -v lane '%b' "$lane"
        request="$width_dir/cache-buster-requests/lane-$index.json"
        response="$width_dir/cache-buster-responses/lane-$index.json"
        write_cache_buster_request "$request" "$loaded_model_id" "$lane"
        (
            while [[ ! -e "$buster_start_file" ]]; do sleep 0.01; done
            curl --fail --silent --show-error \
                --max-time "$REQUEST_TIMEOUT_SECONDS" \
                --header 'Content-Type: application/json' \
                --data-binary "@$request" \
    ...
    : >"$buster_start_file"
    for pid in "${buster_pids[@]}"; do
        if ! wait "$pid"; then
            buster_failed=1
        fi
    done
    (( buster_failed == 0 )) || {
        echo "one or more width-$width cache-replacement requests failed" >&2
        return 1
    }
    for ((index = 1; index <= width; index++)); do
        qwen38_physical_validate_response \
            "$width_dir/cache-buster-responses/lane-$index.json" "$loaded_model_id"
    done
    ...
    jq -n \
        ...
        --argjson metrics "$metrics_json" '{
          schema:2,verdict:"pass",width:$width,model_id:$model_id,
...
jq -n \
    ...
    --argjson widths "$width_summaries" '{
      schema:2,verdict:"pass",gate:"qwen38-physical-multislot",
