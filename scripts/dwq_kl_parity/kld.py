...
# load_eval_tokens shim — PR #1146 (kld.py) imports this from
# mlx_lm.utils but it's added by a separate downstream PR not yet
# merged.  Substitute a wrapper around mlx_lm.quant.utils.load_data
# (the same calibration-v5 corpus mlx_lm.dwq uses) that matches the
# expected signature.
def load_eval_tokens(tokenizer, data_path, num_samples, sequence_length, seed=None):
    """Shim for PR #1146's expected signature.  `data_path` arg is
    accepted but ignored — we always use the canonical calibration-v5
    corpus that mlx_lm.dwq uses (gist 9e207a95... by tristandruyen),
    which `mlx_lm.quant.utils.load_data` downloads on first call.
    `seed` is honored by setting mx's RNG seed before sampling."""
    import mlx.core as mx
    from mlx_lm.quant.utils import load_data
    if seed is not None:
        mx.random.seed(int(seed))
    _ = data_path  # ignored — calibration-v5 is hard-coded in load_data
    return load_data(tokenizer, num_samples, sequence_length)
...
def evaluate_kld(args, cache: BaselineCache) -> dict:
    ...
    for batch_idx, batch in enumerate(iter_batches(tokens, batch_size)):
        ...
        kl = kl_from_cached_batch(model_logprobs, cache.load_batch(batch_idx))
        mx.eval(kl)
        all_kl.append(kl)
...
def _stderr(values: np.ndarray) -> float:
    ...
    return math.sqrt(max(variance, 0.0)) / math.sqrt(count)


def _summarize_kl(all_kl) -> dict:
    values = all_kl[0] if len(all_kl) == 1 else mx.concatenate(all_kl)
    np_values = np.array(values)
    sequence_means = np.mean(np_values, axis=-1)
    mean = float(np.mean(sequence_means))
    sequence_stderr = _stderr(sequence_means)
    return {
        "max_kl_per_token": float(np.max(sequence_means)),
        "mean_kl_per_token": mean,
        "median_kl_per_token": float(np.median(sequence_means)),
        "p95_kl_per_token": float(np.percentile(sequence_means, 95)),
        "stderr": sequence_stderr,
        "total_positions": int(np_values.size),
    }
