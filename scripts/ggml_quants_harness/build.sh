#!/usr/bin/env bash
# Build the ADR-033 P0 reference-fixture generator against the peer's
# pre-built dylibs at /opt/llama.cpp/build/bin/.
#
# Peer pin: data/llama_cpp_pin.txt
...
[ -f "$LLAMA/build/bin/libggml.dylib" ] || { echo "missing $LLAMA/build/bin/libggml.dylib — build the peer first"; exit 1; }
