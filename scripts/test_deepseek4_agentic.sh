...
MODEL=${MODEL:-DeepSeek-V4-Flash-0731-Q2_K_S}
...
cleanup() {
  rm -f "$request_file" "$first_file" "$second_file" "$stream_file" \
...
assert_tool_path() {
  local response=$1
  local actual
  actual=$(jq -r '.choices[0].message.tool_calls[0].function.arguments | fromjson | .path // empty' "$response")
  if [[ "$actual" != "$EXPECTED_PATH" ]]; then
    echo "agentic gate failed: expected read_file path $EXPECTED_PATH, got ${actual:-<missing>}" >&2
...
if ! jq -e --arg expected "$EXPECTED_PATH" 'fromjson? | .path == $expected' \
...
jq --slurpfile base "$request_file" --slurpfile prior "$second_file" \
...
if [[ "$continuation_content" != "$SENTINEL" ]]; then
  echo "agentic gate failed: tool-result continuation did not consume the result correctly" >&2
