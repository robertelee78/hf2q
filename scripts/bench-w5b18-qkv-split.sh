#!/bin/bash
# Wave 5b.18 — QKV-split GPU kernel wire-up bench at PP4106.
#
# Compares the new mlx-native `dispatch_qkv_split_f32` GPU path (default)
# against the legacy CPU download → triple-loop split → 3× upload path
# (HF2Q_QKV_SPLIT_LEGACY=1) at PP4106 with W-5b.8/W-5b.17 instrumentation
...
#
# Reuses the W-5b.16/17 walk-bar prompt + measurement protocol so wall-clock
...
echo "=== Wave 5b.18 qkv_split GPU wire-up ($TRIALS trials × 2 hf2q paths × $TRIALS llama) ==="
...
for t in $(seq 1 "$TRIALS"); do
  /usr/bin/time -p /opt/homebrew/bin/llama-completion \
    ...
    -f "$PROMPT" > "/tmp/w5b18/llama-T${t}.log" 2>&1
  prefill_ms=$(grep -oE 'prompt eval time = +[0-9.]+ ms' "/tmp/w5b18/llama-T${t}.log" | sed -E 's/.*= +([0-9.]+) ms/\1/' | head -1)
  printf '[llama T%d] prompt_eval=%sms\n' "$t" "${prefill_ms:-???}"
done
echo

# (2) hf2q NEW path — default behaviour, qkv_split_f32 GPU kernel.
echo "--- hf2q chunk path NEW (qkv_split GPU kernel, $TRIALS cold trials) ---"
for t in $(seq 1 "$TRIALS"); do
  run_one "[T${t}] W-5b.18 NEW (gpu_split)" "/tmp/w5b18/new-T${t}.log" \
    HF2Q_PROFILE_W5B8=1 HF2Q_PROFILE_W5B17=1 \
    HF2Q_QWEN36_AUTOREG=1 HF2Q_UNSAFE_EXPERIMENTS=1 HF2Q_CHUNK_SCAN_PREFILL=1
done
echo

# (3) hf2q LEGACY path — HF2Q_QKV_SPLIT_LEGACY=1 fires the CPU round-trip.
echo "--- hf2q chunk path LEGACY (CPU round-trip, $TRIALS cold trials) ---"
for t in $(seq 1 "$TRIALS"); do
  run_one "[T${t}] W-5b.18 LEGACY (cpu_loop)" "/tmp/w5b18/legacy-T${t}.log" \
    HF2Q_PROFILE_W5B8=1 HF2Q_PROFILE_W5B17=1 \
    HF2Q_QWEN36_AUTOREG=1 HF2Q_UNSAFE_EXPERIMENTS=1 HF2Q_CHUNK_SCAN_PREFILL=1 \
    HF2Q_QKV_SPLIT_LEGACY=1
done
echo

echo "Post-bench: $(vm_stat | awk '/Pages free/ {printf "free=%s pages\n", $3}')"
echo "Logs in /tmp/w5b18/"
