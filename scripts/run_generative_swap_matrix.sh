...
for command in awk cargo find footprint git grep jq lsof mv ps sed shasum sort stat vm_stat vmmap; do
...
hf2q_generative_swap_reject_cargo_configuration "$ROOT_DIR"
...
# load. The server's retained load-time SHA-256 is joined to every completion;
# re-hashing the same 107 GB artifact here would add I/O without adding an
# independent execution fact. An inode/size/timestamp snapshot proves each
# qualified input stayed stable throughout the run.
artifact_snapshots=()
...
if [[ -n "${HF2Q_GENERATIVE_SWAP_BINARY:-}" ]]; then
    ...
    (cd "$ROOT_DIR" \
      && GIT_COMMIT_SHA="$source_commit" cargo build --release --locked --bin hf2q)
