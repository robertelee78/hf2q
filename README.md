...
| **Status** | hf2q 0.1.7 is the release line described by this checkout and resolves published, checksum-pinned `mlx-native 0.10.9`. Public availability is authoritative only when the `v0.1.7` tag, GitHub artifact, and crates.io bytes match the exact main-branch release SHA. Support is family- and scheduler-specific; see `docs/shipping-contract.md`. |
...
projections through the family-neutral `mlx-native 0.10.9` schedule primitive.
...
gates from a clean hf2q package resolving published `mlx-native 0.10.9`:
