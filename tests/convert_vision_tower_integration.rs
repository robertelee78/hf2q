...
fn build_tiny_vit_safetensors() -> Vec<u8> {
    ...
    for layer in 0..2 {
        ...
        if layer == 1 {
            ...
            push_f16_zeros(&mut tensors, &format!("{prefix}.linear_attn.in_proj_a.weight"), vec![8, h]);
            push_f16_zeros(&mut tensors, &format!("{prefix}.linear_attn.in_proj_b.weight"), vec![8, h]);
