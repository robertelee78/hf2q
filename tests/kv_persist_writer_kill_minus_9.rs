...
use hf2q::serve::kv_persist::{
    ...
    format::{
        self, compute_model_fingerprint, BlockHash, EnvelopeHeader, ModelFingerprint,
...
fn child_body(cache_root: &std::path::Path) -> ! {
    ...
    for s in 0u32..10 {
        ...
        let _ = handle.enqueue_blocking(WriteJob {
            header,
            body,
            completion_tx: None,
        });
...
fn kill_minus_9_mid_write_leaves_committed_blocks_and_no_partial_named_files() {
    let nanos = std::time::SystemTime::now()
        ...
        .as_nanos();
    let cache_root =
        std::env::temp_dir().join(format!("hf2q-kv-kill9-{}-{nanos}", std::process::id()));
    ...
    if !kv_dir.exists() {
        eprintln!("[kv-kill9] kill before any write; skip");
    ...
    for fanout_ent in std::fs::read_dir(&kv_dir).unwrap().filter_map(|e| e.ok()) {
        ...
        for ent in std::fs::read_dir(fanout_ent.path())
            .unwrap()
            .filter_map(|e| e.ok())
        {
            let p = ent.path();
    ...
    if final_files.is_empty() {
        eprintln!(
            "[kv-kill9] kill before any rename committed ({} tmp); skip",
            tmp_files.len()
        );
        let _ = std::fs::remove_dir_all(&cache_root);
        return;
    ...
    for p in &final_files {
        let (h, body) = format::read_envelope_body(p)
            .unwrap_or_else(|e| panic!("partial-named-final {p:?}: {e}"));
        assert_eq!(h.format_version, CURRENT_FORMAT_VERSION.0);
        assert!(!body.is_empty());
    }
    eprintln!(
        "[kv-kill9] PASS — {} committed, {} tmp orphans, 0 partial-named-final",
        final_files.len(),
        tmp_files.len()
    );
    let _ = std::fs::remove_dir_all(&cache_root);
