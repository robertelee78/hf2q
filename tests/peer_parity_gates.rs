//! ADR-014 P10 iter-1 + iter-2a + iter-2b + iter-2c + iter-3 — peer-parity
//! benchmark harness.
//!
...
//!
//! iter-2c renamed the driver to `measure_ppl_qwen35`
...
//!
//! iter-3 adds `scripts/fetch_wikitext2.sh` and teaches the corpus
//! loader to auto-pick `wikitext2-full.tokens` when present and valid,
//! falling back to the committed smoke fixture otherwise.
//!
...
const SMOKE_CORPUS_TOKENS: usize = 512;
const FULL_CORPUS_MIN_TOKENS: usize = 280_000;
const FULL_CORPUS_MIN_BYTES: usize = 1_048_576;
...
/// Resolves the PPL corpus fixture directory relative to
/// `CARGO_MANIFEST_DIR`. This avoids hard-coding `/opt/hf2q` so the
/// harness works from any worktree.
fn ppl_corpus_dir() -> std::path::PathBuf {
...
/// (`tests/fixtures/ppl-corpus/wikitext2-smoke.tokens`). The fixture
/// is committed to the repo (2 KB deterministic ramp — see the
/// sibling README).
fn smoke_corpus_path() -> std::path::PathBuf {
    ppl_corpus_dir().join("wikitext2-smoke.tokens")
}

/// Resolves the iter-3 full-corpus path. This file is generated by
/// `scripts/fetch_wikitext2.sh`, is intentionally gitignored, and is
/// the corpus P11 uses for parity-grade PPL.
fn full_corpus_path() -> std::path::PathBuf {
    ppl_corpus_dir().join("wikitext2-full.tokens")
}

fn read_u32_tokens(path: &std::path::Path) -> Option<Vec<u32>> {
    let bytes = match std::fs::read(path) {
        Ok(bytes) => bytes,
        Err(err) => {
            eprintln!(
                "PPL corpus loader: failed to read {}: {err}",
                path.display()
            );
            return None;
        }
    ...
    if bytes.len() % 4 != 0 {
        eprintln!(
            "PPL corpus loader: {} has {} bytes, not divisible by 4",
            path.display(),
            bytes.len()
        );
        return None;
...
/// if the file is missing or has the wrong byte length.
///
...
fn load_smoke_corpus_tokens() -> Option<Vec<u32>> {
    ...
    let tokens = read_u32_tokens(&path)?;
    if tokens.len() != SMOKE_CORPUS_TOKENS {
        eprintln!(
            "PPL corpus loader: smoke fixture {} has {} tokens, expected {}",
            path.display(),
            tokens.len(),
            SMOKE_CORPUS_TOKENS
        );
...
fn load_full_corpus_tokens(path: &std::path::Path) -> Option<Vec<u32>> {
    let tokens = read_u32_tokens(path)?;
    let byte_len = tokens.len().saturating_mul(4);
    if tokens.len() < FULL_CORPUS_MIN_TOKENS || byte_len < FULL_CORPUS_MIN_BYTES {
        eprintln!(
            "PPL corpus loader: full corpus {} failed validation ({} tokens, {} bytes); falling back to smoke",
            path.display(),
            tokens.len(),
            byte_len
        );
...
/// Prefer the generated full wikitext-2 corpus when it is present and
/// valid; otherwise fall back to the checked-in smoke fixture. Returns
/// both the selected path (for peer `llama-perplexity`) and the token
/// vector (for hf2q-side PPL). The selected path is logged to stderr,
/// and every filesystem/format failure returns `None` rather than
/// panicking.
fn load_corpus_tokens() -> Option<CorpusSelection> {
    let full_path = full_corpus_path();
    if full_path.is_file() {
        if let Some(tokens) = load_full_corpus_tokens(&full_path) {
            eprintln!(
                "PPL corpus loader: selected full corpus {} ({} tokens)",
                full_path.display(),
                tokens.len()
            );
            return Some(CorpusSelection {
                path: full_path,
                tokens,
            });
        }
    ...
    let smoke_path = smoke_corpus_path();
    let tokens = load_smoke_corpus_tokens()?;
    eprintln!(
        "PPL corpus loader: selected smoke corpus {} ({} tokens)",
        smoke_path.display(),
        tokens.len()
    );
...
/// PPL via `run_llama_perplexity`; iter-2b wired the hf2q-side PPL
/// via the dense-named driver for cells 0-3; iter-2c wires the
/// hf2q-side PPL via the renamed
...
/// iter-3 teaches the corpus loader to prefer the generated full
/// wikitext-2 test split and fall back to the checked-in smoke
/// fixture when the full corpus is absent. Wall + RSS stay sentinel
/// for both sides this iter — real-model wiring lands in P11.
///
/// Cell-routing rules (post-iter-2c):
/// - All 8 cells call [`measure_ppl_qwen35`] with the cell's hf2q
///   model path and the selected PPL corpus. With sentinel paths the
...
fn run_cell(cell: GateCell) -> CellResult {
    let corpus_selection = load_corpus_tokens();
    ...
    let (peer_metrics, peer_ppl) = if let Some(selection) = corpus_selection.as_ref() {
        llama_cpp_runner::run_llama_perplexity(&model_path, &selection.path)
    } else {
        // Corpus missing or malformed (should never happen for the
        // checked-in smoke fixture) but stay safe rather than panic.
        // The verdict will surface the state via the peer sentinel.
        (RunMetrics::missing_binary("PPL corpus"), None)
    };
    ...
    let hf2q_ppl: Option<f32> = match corpus_selection {
        Some(selection) => match measure_ppl_qwen35(&model_path, &selection.tokens, None) {
            Ok(ppl) => Some(ppl),
            Err(_) => None,
        },
        None => None,
    ...
    let hf2q_metrics = RunMetrics::missing_binary(
        "hf2q (iter-2c: PPL via variant-agnostic driver; wall+RSS pending P11)",
    );
...
fn subprocess_wrapper_handles_missing_python_module() {
    ...
    assert!(
        m.is_missing_binary(),
        "missing python must surface sentinel; got {m:?}"
    );
    assert!(!ok);
    assert_eq!(m.exit_code, -1);
...
fn gate_cells_match_decision_15_verbatim() {
    ...
    let expected: [(&str, BackendKind, &str, PeerId, f64, f64, f64); 8] = [
        (
            ...
            BackendKind::Gguf,
            "None (q4_k_m)",
            PeerId::LlamaCppUncalibratedQ4KM,
            1.10,
            1.10,
            1.02,
        ),
        (
            "27B dense",
            BackendKind::Gguf,
            "Imatrix (imatrix-q4_k_m)",
            PeerId::LlamaCppImatrixQ4KM,
            1.10,
            1.10,
            1.02,
        ),
...
fn markdown_table_renders_ppl_columns() {
    ...
    assert_eq!(
        cells[12], "—",
        "hf2q PPL must render as em-dash; got `{}`",
        cells[12]
    );
    assert_eq!(
        cells[13], "5.4200",
        "peer PPL must render to 4 dp; got `{}`",
        cells[13]
    );
    assert_eq!(
        cells[14], "—",
        "PPL ratio must em-dash when hf2q missing; got `{}`",
        cells[14]
    );
}
...
fn markdown_table_renders_full_ppl_row_when_both_measured() {
    ...
    let pass_md = emit_markdown_table(
        std::slice::from_ref(&pass_result),
        "M5 Max, 128 GB",
        "iter2a02",
    );
