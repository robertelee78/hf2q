...
// **ADR-017 §B-tq.5** — shared subprocess driver lives at
// `tests/common/serve_driver.rs`.  `phase_d_driver` mod below is now
// a thin wrapper that re-exports it + adds the gemma4-specific
// peer-arm helpers (`run_llama_completion_peer` /
// `resolve_llama_completion_bin`).
mod common;
...
//
// **ADR-017 §B-tq.5**: the generic subprocess driver moved to
// `tests/common/serve_driver.rs` so `tests/kv_persist_tq_packed_roundtrip.rs`
// can drive its B-tq.4 acceptance test.  The `phase_d_driver` mod
// below is now a thin wrapper that re-exports the common driver
// surface + adds the gemma4-specific peer-arm helpers
// (`run_llama_completion_peer` + `resolve_llama_completion_bin`).
// =========================================================================

mod phase_d_driver {
    //! Backward-compat wrapper for the generic
    //! `crate::common::serve_driver` (B-tq.5 extraction).  Existing
    //! call sites (`phase_d_driver::ServerGuard`,
    //! `phase_d_driver::spawn_hf2q_serve_with_kv_persist`, etc.) keep
    //! working through the `pub use` re-export.
    #![allow(dead_code)] // each consumer uses a subset
    pub use crate::common::serve_driver::*;
